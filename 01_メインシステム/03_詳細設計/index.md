...
| [DD12_利用量計測・課金.md](DD12_利用量計測・課金.md) | 利用量計測・課金 | FR-120〜127 / FR-139 | §6 SCR-015 プロジェクト視点 / SCR-026 オーナー視点 / 課金部は契約 WS 配置(基本設計 §4.4.3 / §5.10 が正本)|
...
データ保持期間は **システム固定値**（NFR-702、UI 上の変更不可）。SCR-028 アカウント設定にデータ保持期間設定 UI は存在しない（[01_画面設計.md §5.20b](../02_基本設計/01_画面設計.md) 参照。SCR-016 は v1.5 で SCR-028 に統合・廃止）。

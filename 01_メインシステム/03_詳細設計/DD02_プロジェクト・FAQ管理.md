...
| 元章 | 元タイトル | 概要 |
|---|---|---|
| §6 SCR-004〜006 | 画面詳細設計 | プロジェクト一覧 / FAQ 一覧 / FAQ 編集（正本は基本設計） |
...
SCR-004 プロジェクト一覧 / SCR-005 FAQ 一覧 / SCR-006 FAQ 編集の画面詳細は [../02_基本設計/01_画面設計.md](../02_基本設計/01_画面設計.md) を正本とする。メッセージ ID は [../02_基本設計/06_メッセージ一覧.md](../02_基本設計/06_メッセージ一覧.md) を参照。
...
CSV ファイル（UTF-8、BOM 許容）→ R2 ステージング → Queue 投入 → consumer が分割 INSERT。1 リクエスト最大 1000 件。**JSON 形式は受理しない**（不正形式は E-INPUT-CSV-INVALID）。UI は SCR-006-M1 FAQ CSV インポートモーダル（`POST /faqs/import`）。詳細は [DD13_バッチ・非同期処理.md](DD13_バッチ・非同期処理.md) を参照。

**文字コード事前検証**（クライアント側）: SCR-006-M1 のドラッグ&ドロップ / ファイル選択の時点で、ファイル先頭バイト（BOM）および本文のバイト列から文字コードを判定し、**UTF-8（BOM 許容）以外はアップロードせず即エラー**（E-INPUT-CSV-INVALID / MSG-SCR-006-M1-ERR-005、検出した文字コード名を併記）。これによりサーバ側の文字化け INSERT を未然に防ぐ。

**CSV 列定義**（ヘッダ行必須）: `FAQ ID, 質問, 回答, カテゴリ`。`status`（状態）列は受理しない。

**テンプレート生成**（`GET /faqs/import/template`）: ヘッダ行のみ（`FAQ ID,質問,回答,カテゴリ`）を `text/csv; charset=utf-8`（BOM 付与）でストリーム返却。動的データなし（静的文字列）、Queue 不経由の同期レスポンス。

**FAQ ID 行判定ロジック**（consumer 側、行単位）:

1. `FAQ ID` 空欄 → 新規 INSERT（`status='draft'`、`version=1`）。
2. `FAQ ID` が当該契約（`contract_owner_user_id` 境界）内の既存 `faqs.id` と一致 → UPDATE（`status` は維持、`faq_revisions` に改訂行を追記、`version` をインクリメント）。
3. `FAQ ID` が当該契約内に存在しない → 当該行を失敗（E-INPUT-CSV-FAQID-NOTFOUND）。他行は継続（部分成功）。
4. 質問・回答の欠落 / 文字数超過 → 当該行を失敗（E-INPUT-CSV-INVALID 包含）。カテゴリは任意。

失敗行は「行番号 + エラー理由」の構造化リストとしてジョブ結果（`GET /faqs/import/jobs/{jobId}` 等のポーリング応答）に含め、SCR-006-M1 進捗パネル内のスクロール領域に画面表示する（エラー CSV のダウンロードは行わない）。
...
`POST /projects` 実行時は SCR-004-M1(新規作成モード)から `name` / `allowedDomains` 等の基本情報のみを受け取る(`initialAdmins` フィールドは受け取らない)。オーナーは作成時に自動で当該プロジェクトの管理者となる(`project_users(role='admin', valid=1)` 自動 INSERT)。他者をプロジェクト管理者として招待する操作は、プロジェクト作成後に SCR-009-M1 経由(`POST /projects/{id}/members` + `role='admin'`)で行う。
...
5. `UPDATE projects SET status='deleted', valid=0, deleted_at=now(), updated_at=now() WHERE id=?` + 関連テーブル(`allowed_domains` / `faqs` / `inquiries` / `question_logs` / `project_quota_limits`)を `valid=0, updated_at=now()` に伝播(`usage_metering` は法定保管 7 年のため論理削除せず保持)
6. 監査ログ `project.logical_delete`(`metadata` に `{ projectId, logicallyDeletedUserIds: [...] }`、`retention_class=general`)を記録

オーナーの `users` 行は本処理の論理削除対象外(`contract_owners` 行を持つユーザーは常に `valid=1` 維持)。当該プロジェクトに対するオーナー自身の `project_users` admin 行は他のメンバー行と同様に `valid=0` に論理削除される。論理削除データは `updated_at < now() - 90d AND valid=0` の物理削除バッチ([DD13_バッチ・非同期処理.md §3.12](DD13_バッチ・非同期処理.md))で 90 日後に物理削除される。
...
// (2) SCR-019 着地時(POST /auth/contact-verifications/{token})
...
- 詳細なトークン発行ロジックは [DD07_トークン発行・検証.md §3.3b](DD07_トークン発行・検証.md) を参照

### 3.12 90 日後物理削除バッチ(概要)

詳細は [DD13_バッチ・非同期処理.md §3.12](DD13_バッチ・非同期処理.md) を正本とする。日次バッチで `valid=0 AND updated_at < now() - 90d` の行を物理削除する。`users` 行が物理 DELETE されると `ON DELETE CASCADE` で関連 `contract_owners` / `project_users` / `sessions` / `access_tokens` / `terms_agreements` 等が連鎖物理削除される。`projects` 行が物理 DELETE されると `ON DELETE CASCADE` で関連 `faqs` / `inquiries` / `chat_*` / `project_quota_limits` 等が連鎖物理削除される。`billing_subscriptions` / `usage_metering` は物理削除バッチの対象外(電子帳簿保存法 7 年保持、永久維持。`usage_metering` はプロジェクト単位行も含め 7 年保持)。匿名化モード(`contract_owners.data_deletion_mode='anonymize'`)は物理削除前に匿名化処理を実施する。

### 3.13 関連する横断設計

- 認可: [DD08_認可ヘルパ.md](DD08_認可ヘルパ.md) の `requireProjectRole` でオーナー境界 + プロジェクトロール(`admin` / `member`)を検証
...
| 関連 DD | [DD03_AI回答パイプライン.md](DD03_AI回答パイプライン.md) / [DD08_認可ヘルパ.md](DD08_認可ヘルパ.md) / [DD09_監査ログ書込・完全性検証.md](DD09_監査ログ書込・完全性検証.md) / [DD12_ウィジェット配信.md](DD12_ウィジェット配信.md) |

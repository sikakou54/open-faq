...
CSV ファイル（UTF-8、BOM 許容）→ R2 ステージング → Queue 投入 → consumer が分割 INSERT。1 リクエスト最大 1000 件。**JSON 形式は受理しない**（不正形式は E-INPUT-CSV-INVALID）。UI は SCR-012-M1 FAQ CSV インポートモーダル（`POST /faqs/import`）。詳細は [DD14_バッチ・非同期処理.md](DD14_バッチ・非同期処理.md) を参照。

**文字コード事前検証**（クライアント側）: SCR-012-M1 のドラッグ&ドロップ / ファイル選択の時点で、ファイル先頭バイト（BOM）および本文のバイト列から文字コードを判定し、**UTF-8（BOM 許容）以外はアップロードせず即エラー**（E-INPUT-CSV-INVALID / MSG-SCR-012-M1-ERR-005、検出した文字コード名を併記）。これによりサーバ側の文字化け INSERT を未然に防ぐ。
...
失敗行は「行番号 + エラー理由」の構造化リストとしてジョブ結果（`GET /faqs/import/jobs/{jobId}` 等のポーリング応答）に含め、SCR-012-M1 進捗パネル内のスクロール領域に画面表示する（エラー CSV のダウンロードは行わない）。`conflict` パラメータは廃止（旧: スキップ / 上書き / 別件追加の管理者選択）。
...
`DELETE /projects/{id}` 実行時は **SCR-042 プロジェクト設定のオーナー専有 DangerSectionのみ** から起動可能。以下のトランザクション境界で論理削除する:
...
5. `UPDATE projects SET status='deleted', valid=0, deleted_at=now(), updated_at=now() WHERE id=?` + 関連テーブル(`allowed_domains` / `faqs` / `inquiries` / `end_users` / `chat_rooms` / `question_logs` / `project_quota_limits`)を `valid=0, updated_at=now()` に伝播(`usage_metering` は法定保管 7 年のため論理削除せず保持)
...
SCR-010-M1で初期値を入力した時、またはSCR-042で `projects.contact_email` を変更した時に、TPL-PROJECT_CONTACT_VERIFY確認メールを送信する。受信者はSCR-032へ着地し、`POST /auth/contact-verifications/{token}` で確認完了する。

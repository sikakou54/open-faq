...
| データエクスポート・復元 | `data.export.request`, `data.export.complete`, `data.restore.execute` | operator_high_priv (restore のみ) / general (他)。※ 契約全データエクスポート(`data.export.*`)は MVP 対象外・将来対応 |

...
export async function requireProjectRole(c: Context, projectId: string, minRole: ProjectRole) {
  ...
  if (project.owner_account_id !== principal.ownerAccountId) throw new HTTPException(404); // 404 偽装
...
- `projectRoles`: `Map<projectId, 'admin' | 'member'>`。**v1.10**: オーナー / メンバー共通で `account_project_grants WHERE valid=1` を 1 クエリで取得して構築する(従来の「オーナーは空 Map」分岐を撤回)。認可ヘルパは `isOwner=true` のときに `projectRoles` の中身を参照せず bypass する優先順位を維持(認可上の挙動は変わらない)。`projectRoles` は通知宛先解決と画面表示の事実情報として参照可能
...
| 境界値 | v1.7 でアカウント削除はアカウントレベル物理削除に統一されたため、現存メンバーは常に 1 件以上の割当を保持。割当 0 件はテスト境界としては「データ不整合時のフェイルセーフ動作」(`/auth/login` への強制ログアウト)を検証する |

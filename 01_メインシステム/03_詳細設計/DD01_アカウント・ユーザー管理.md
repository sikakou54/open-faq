...
router.beforeEach(async (to) => {
  ...
  if (scope === 'projects') {
    ...
    if (!me.is_owner && !grant) {
      // メンバーは FR-015d 不変条件により grants.length >= 1。0 件はデータ不整合
...
メンバーは FR-015d の不変条件により grants.length >= 1 を満たすため、5. 「grants が 0 件のケース」は構造的に発生しない。データ不整合等で grants.length === 0 を検出した場合のフェイルセーフは Sentry へ重大度 high を通知し `/auth/login` へ強制ログアウトする(運営者 IF #2 強制ログアウト発火を期待)。

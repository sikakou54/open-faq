...
利用者側アカウントは **オーナー**(契約あたり 1 アカウント固定、全権、MVP では譲渡不可、`accounts.owner_account_id = accounts.id` で自己参照、**プロジェクト作成時に `account_project_grants(role='admin', valid=1)` 行を自動取得**する明示的なプロジェクト管理者保持者となる。認可上は `isOwner=true` bypass を優先)と **メンバー**(0..N、`owner_account_id` でオーナーに紐付き、プロジェクトごとに `account_project_grants.role` = `admin`(プロジェクト管理者)/ `member`(メンバー)のいずれかを持つ)に分かれる。オーナー専有機能(プロジェクト作成・編集・削除、課金、退会、規約再同意)はプロジェクト別ロールでは付与できない。**メンバーアカウント削除はプロジェクト単位の SCR-017-M1「アカウントを削除」(`DELETE /projects/{id}/members/{accountId}`)で行う論理削除(`accounts.valid=0`)**(基本設計 FR-019 / FR-019a / FR-019b 参照)。論理削除データは `updated_at` 基準で 90 日経過後に物理削除バッチ(DD14)で物理削除される。論理削除済 `valid=0` 行はログイン拒否されるため割当 0 件状態は構造的に発生しない。プロジェクト削除起因で `valid=1` 割当が 0 件となるメンバーは同一トランザクションで `accounts.valid=0` に論理削除する。
...
- オーナーもプロジェクト作成時に `account_project_grants(role='admin', valid=1)` 行を自動取得し、明示的なプロジェクト管理者として grants 行を保持する。認可フロー先頭の `isOwner=true` bypass は維持し、grants 行は通知宛先解決と画面表示の事実情報として参照する
...
router.beforeEach(async (to) => {
  ...
  if (scope === 'projects') {
    ...
    if (!me.is_owner && !grant) {
      // アカウント削除は論理削除(valid=0、grants も valid=0 連動)。`me.grants` は `WHERE valid=1` で絞り込まれるため、メンバー存続中は grants.length >= 1。0 件はデータ不整合
...
アカウント削除はアカウントレベル論理削除のため、現存するメンバー(`accounts.valid=1`)は `account_project_grants.valid=1` を 1 件以上満たし、「grants が 0 件のケース」は構造的に発生しない。データ不整合等で `WHERE valid=1` 条件下で grants.length === 0 を検出した場合のフェイルセーフは Sentry へ重大度 high を通知し `/auth/login` へ強制ログアウトする(運営者 IF #2 強制ログアウト発火を期待)。

**認証ミドルウェアの追加チェック**: 認証成功直後に `accounts.valid=1` を確認し、`valid=0` の場合は 401 でログイン拒否(セッション失効扱い)。これにより論理削除済みアカウントのセッションが残存しても再アクセスを防止する。

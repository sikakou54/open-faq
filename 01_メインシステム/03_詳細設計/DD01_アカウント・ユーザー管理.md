...
利用者側アカウントは **オーナー**(契約あたり 1 アカウント固定、全権、MVP では譲渡不可、`accounts.owner_account_id = accounts.id` で自己参照、**v1.10 でプロジェクト作成時に `account_project_grants(role='admin', valid=1)` 行を自動取得**する明示的なプロジェクト管理者保持者となる。認可上は `isOwner=true` bypass を優先)と **メンバー**(0..N、`owner_account_id` でオーナーに紐付き、プロジェクトごとに `account_project_grants.role` = `admin`(プロジェクト管理者)/ `member`(メンバー)のいずれかを持つ)に分かれる。オーナー専有機能(プロジェクト作成・編集・削除、課金、退会、規約再同意)はプロジェクト別ロールでは付与できない。**メンバーアカウント削除は v1.7 でプロジェクト単位の SCR-017-M1「アカウントを削除」(`DELETE /projects/{id}/members/{accountId}`)に統一、v1.10 で論理削除(`accounts.valid=0`)に再統一**(基本設計 FR-019 / FR-019a / FR-019b 参照)。論理削除データは `updated_at` 基準で 90 日経過後に物理削除バッチ(DD14)で物理削除される。FR-015d 不変条件は v1.7 で撤廃された(論理削除済 `valid=0` 行はログイン拒否されるため割当 0 件状態は構造的に発生しない)。プロジェクト削除起因で `valid=1` 割当が 0 件となるメンバーは同一トランザクションで `accounts.valid=0` に論理削除する。
...
- **v1.10**: オーナーもプロジェクト作成時に `account_project_grants(role='admin', valid=1)` 行を自動取得し、明示的なプロジェクト管理者として grants 行を保持する(従来の「行を持たない暗黙の admin」設計を撤回)。認可フロー先頭の `isOwner=true` bypass は維持し、grants 行は通知宛先解決と画面表示の事実情報として参照する
...
router.beforeEach(async (to) => {
  ...
  if (scope === 'projects') {
    ...
    if (!me.is_owner && !grant) {
      // v1.10: アカウント削除は論理削除化(valid=0、grants も valid=0 連動)。`me.grants` は `WHERE valid=1` で絞り込まれるため、メンバー存続中は grants.length >= 1。0 件はデータ不整合
...
v1.7 / v1.10 でアカウント削除がアカウントレベル削除(v1.7 物理→ v1.10 論理)に統一されたため、現存するメンバー(`accounts.valid=1`)は `account_project_grants.valid=1` を 1 件以上満たし、「grants が 0 件のケース」は構造的に発生しない。データ不整合等で `WHERE valid=1` 条件下で grants.length === 0 を検出した場合のフェイルセーフは Sentry へ重大度 high を通知し `/auth/login` へ強制ログアウトする(運営者 IF #2 強制ログアウト発火を期待)。

**v1.10 認証ミドルウェアの追加チェック**: 認証成功直後に `accounts.valid=1` を確認し、`valid=0` の場合は 401 でログイン拒否(セッション失効扱い)。これにより論理削除済みアカウントのセッションが残存しても再アクセスを防止する。
...
- 各ステップ CTA の遷移先 URL は §3.9 の規約に従う:
  ...
  - ステップ 2: 最新作成プロジェクトの `/projects/{projectId}/members` → 招待モーダル(SCR-017-M1。v1.7 で `/owner/members` から変更、SCR-017 はプロジェクト WS 単一視点)

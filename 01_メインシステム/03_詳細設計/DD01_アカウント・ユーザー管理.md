...
router.beforeEach(async (to) => {
  ...
  if (scope === 'projects') {
    ...
    if (!me.is_owner && !grant) {
      // v1.7 ではアカウント削除がアカウントレベル物理削除のため、メンバー存続中は grants.length >= 1。0 件はデータ不整合
...
v1.7 でアカウント削除がアカウントレベル物理削除に統一されたため、現存するメンバーは grants.length >= 1 を満たし、5. 「grants が 0 件のケース」は構造的に発生しない。データ不整合等で grants.length === 0 を検出した場合のフェイルセーフは Sentry へ重大度 high を通知し `/auth/login` へ強制ログアウトする(運営者 IF #2 強制ログアウト発火を期待)。

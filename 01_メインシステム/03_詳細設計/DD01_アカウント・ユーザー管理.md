...
| テーブル | `accounts` `sessions` `access_tokens` `account_permissions` `account_project_grants` | アカウント基盤 |
...
ユーザー種別、5 種権限フラグ（`faq:manage` / `chat:respond` / `users:manage` / `project:manage` / `logs:view`）、画面別権限マトリクス、認証フロー、Principal 構築は [../02_基本設計/04_権限設計.md](../02_基本設計/04_権限設計.md) および [../02_基本設計/08_認証・認可設計.md](../02_基本設計/08_認証・認可設計.md) を正本とする。
...
- メンバー権限は `account_permissions` テーブルの 5 種フラグで管理
- プロジェクト割当は `account_project_grants` テーブル

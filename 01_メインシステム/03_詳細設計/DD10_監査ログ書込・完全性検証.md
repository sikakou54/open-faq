...
`general` 1 年 / `operator_high_priv` 5 年 / `billing` 7 年。`tombstone` フラグで本文を物理削除しつつチェーン継続性を維持。tombstone 化のバッチ実装は [DD14_バッチ・非同期処理.md](DD14_バッチ・非同期処理.md) §14.1.8 を参照。
...
| 関連 DD | [DD09_認可ヘルパ.md](DD09_認可ヘルパ.md) / [DD11_暗号化・鍵管理.md](DD11_暗号化・鍵管理.md) / [DD14_バッチ・非同期処理.md](DD14_バッチ・非同期処理.md) |
...
|---|---|---|---|
| TBD-DD10-001 | 月次フル検証の X 万行で Y 分の実測値 | 中 | 未確定（SRE） |
| TBD-DD10-002 | 年次総合検証の実測値 | 中 | 未確定（SRE） |
| TBD-DD10-003 | 既存データの segment_key バックフィル手順 | 中 | 未確定（開発リード） |

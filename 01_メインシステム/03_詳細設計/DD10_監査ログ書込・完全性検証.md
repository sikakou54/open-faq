...
- **不正検知への影響緩和**: GDPR 強化モード時はレート制限の判定キーを `ip_masked` → `account_id (またはセッション ID)` に切替（不正検知バッチで `ip_mask_mode='gdpr_enhanced'` 契約には `account_id` キーを優先）。
...
| 論理削除 / 物理削除(v1.10) | `account.logical_delete`, `project.logical_delete`, `account_project_grants.logical_delete`, `account.hard_delete_by_batch`, `project.hard_delete_by_batch`, `project.created_with_owner_admin`(プロジェクト作成時のオーナー admin 行自動 INSERT) | general |

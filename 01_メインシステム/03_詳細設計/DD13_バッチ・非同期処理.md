...
詳細は [DD11_利用量計測・課金.md](DD11_利用量計測・課金.md) §3.4 を参照。

#### 3.1.2 月次確定（JST 02:00、月初 1 日）

詳細は [DD11_利用量計測・課金.md](DD11_利用量計測・課金.md) §3.5 を参照。

#### 3.1.3 無料枠超過・支払方法未登録判定（JST 00:00 daily）

支払方法ゲート(FR-136)のバックストップ判定。**支払方法が未登録**かつ、いずれかのプロジェクトが質問数無料枠を超過している場合、当該プロジェクトのウィジェット新規質問受付を停止する。FAQの無料枠超過は停止フラグや 429 の対象にせず、利用量記録と課金案内だけを継続する。
...
#### 3.1.6 監査ログ完全性検証（JST 03:00 daily / 日次全件再計算）

メイン側 `audit_logs` は API 操作ログ専用であり、ハッシュチェーン構造を持たない(`prev_hash` / `current_hash` 列は持たない)。本処理は **運営者側 `audit_logs`(ハッシュチェーン正本)に対してのみ実行** される。詳細は運営者側設計 §7 + [DD09_監査ログ書込・完全性検証.md](DD09_監査ログ書込・完全性検証.md) §3.6 を参照。
...
export async function retentionCleanup(env: Env) {
  ...
  await env.DB.batch([
    ...
    // バッチで論理削除済 users をまとめて物理削除する前段として、別途 withdrawal-finalize 処理(SCR-014 動線)で
...
3. `users` 行を物理 DELETE(`ON DELETE CASCADE` で関連 `contract_owners` / `project_users` / `sessions` / `access_tokens` / `terms_agreements` 等が連鎖物理削除)
4. `projects` 行を物理 DELETE(`ON DELETE CASCADE` で関連 `faqs` / `inquiries` 等が連鎖物理削除)
...
async function hardDeleteLogicalDeletedUsers(env, now) {
  ...
  // 3a. users が valid=1 で残ったまま contract_owners 単独が valid=0 のケースへのフェイルセーフ
...
| `export-queue` | データエクスポート(契約全データエクスポートは **MVP 対象外・将来対応**。MVP では FAQ CSV エクスポートのみが本キューを利用、または同期処理)| data/export | consumers/export.ts | 3 | 2 回 | 同上 |
...
| 関連 DD | [DD05_inquiry_code採番・未解決質問.md](DD05_inquiry_code採番・未解決質問.md) / [DD06_通知ロジック.md](DD06_通知ロジック.md) / [DD09_監査ログ書込・完全性検証.md](DD09_監査ログ書込・完全性検証.md) / [DD11_利用量計測・課金.md](DD11_利用量計測・課金.md) |

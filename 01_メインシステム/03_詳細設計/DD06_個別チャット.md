...
- **デフォルト担当者設定(SCR-034, `PATCH /projects/{id}` `defaultAssigneeUserId`)**: オーナー / 該当 PJ の `admin` のみ。`null`(未設定に戻す)を許容し、その場合は次回以降の新規作成でオーナーにフォールバックする。

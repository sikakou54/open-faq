...
| 関連機能ID | FR-150〜156（ウィジェット設定 / 配信）/ FR-083 / FR-084（個別チャット再入室）/ AC-015 / AC-025 / AC-033 |
| 作成日 | 2026-05-17 |
| 版数 | v1.0 |
| ステータス | 承認済 |
...
| 種別 | ID | 名称 |
|---|---|---|
| 機能 | FR-150〜156 | ウィジェット設定 / 配信 / 許可ドメイン / 公開鍵 |
| 機能 | FR-083 / FR-084 | 個別チャット再入室（SCR-027） |
| 機能 | FR-194 | レート制限契約別 |
| 画面 | SCR-014 | ウィジェット表示 |
| 画面 | SCR-027 | 個別チャット再入室 |
...
| §6 SCR-014 / SCR-027 | 画面詳細設計 | ウィジェット表示 / 個別チャット再入室（正本は基本設計） |
...
`POST /widget/v1/ask` の処理冒頭で、以下 **2 つの契機のいずれか** を満たす場合、**AI 推論・FTS5 検索を実行せず** 即時に停止応答する(KV `usage-limit:<owner>` 30s TTL を参照):

| 契機 | 条件 | `reason` |
|---|---|---|
| 上限件数到達(FR-122)| 当月質問数 ≥ `owner_quota_overrides.question_monthly_limit`(オーナー設定)| `question_monthly_limit_reached` |
| 支払方法ゲート(FR-136)| 支払方法未登録 かつ 当月質問数 ≥ 無料枠(質問 1,000)| `payment_method_required` |
...
| 契約状態 | **`contract_status` は変更しない**(`active` のまま、ウィジェット質問受付のみ停止。契約サスペンション 423 とは別)|
| 復帰 | 上限件数到達: 翌月リセット or オーナーの上限引き上げ(SCR-015-M1)。支払方法ゲート: **支払方法登録(Setup Intent 成功)で即時再開** |
...
### 3.8 SCR-027 個別チャット再入室フロー

エンドユーザーがメール内リンク（再入室トークン）を踏むと SCR-027 に遷移し、トークン検証後に既存チャット部屋へ復帰する。再入室トークン仕様は [DD08_トークン発行・検証.md](DD08_トークン発行・検証.md) §3.4 を参照。

### 3.9 関連する横断設計

- AI 回答: [DD03_AI回答パイプライン.md](DD03_AI回答パイプライン.md)
- しきい値: [DD04_AIしきい値3階層適用.md](DD04_AIしきい値3階層適用.md)
- 個別チャット: [DD06_個別チャット.md](DD06_個別チャット.md)
...
| 関連 DD | [DD03_AI回答パイプライン.md](DD03_AI回答パイプライン.md) / [DD04_AIしきい値3階層適用.md](DD04_AIしきい値3階層適用.md) / [DD06_個別チャット.md](DD06_個別チャット.md) / [DD08_トークン発行・検証.md](DD08_トークン発行・検証.md) / [DD12_利用量計測・課金.md](DD12_利用量計測・課金.md) |

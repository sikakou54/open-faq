...
| 関連機能ID | FR-150〜156, FR-156a〜d（ウィジェット設定 / 配信 / エンドユーザー向けチャット UI）/ FR-070〜077（未解決質問）/ AC-015 / AC-025 / AC-033 |
| 作成日 | 2026-05-17 |
| 版数 | v1.6 |
| ステータス | 承認済 |
| 更新履歴 | v1.6(2026-06-14): 丸型ランチャーバッジからチャット UI を開閉する仕様を追加 / v1.5(2026-06-14): 未解決返信から問い合わせID表示を削除 / v1.4(2026-06-14): エンドユーザー向けヘッダーからメニューと規約導線を削除 |
...
| 機能 | FR-070〜077 | 未解決質問登録 / 管理用問い合わせID採番 |
| 機能 | FR-194 | レート制限契約別 |
| 画面 | SCR-007 | ウィジェット表示 |
...
| §6 SCR-007 | 画面詳細設計 | ウィジェット表示（正本は基本設計） |
...
sequenceDiagram
  ...
  W-->>E: 右下に丸型ランチャーバッジを表示
  E->>W: バッジをクリック / Enter / Space
  W-->>E: チャット UI を展開
  E->>W: 質問入力 → 送信
  ...
  alt 回答可能
    ...
    W-->>E: 回答不能メッセージ + 連絡先表示（問い合わせ ID は描画しない）
  end
```
...
ウィジェット質問レートは 50/min（警告）/ 60/min（拒否、429 `RATE_LIMITED` + Retry-After 付）。同時並列はオーナー 10 / グローバル 200。詳細は [DD11_利用量計測・課金.md](DD11_利用量計測・課金.md) §3.1 参照。
...
| 復帰 | 上限件数到達: 翌月リセット or オーナー / 当該プロジェクト管理者の上限引き上げ(SCR-021-M1)。支払方法ゲート: **支払方法登録(Setup Intent 成功)で即時再開**(契約単位の支払方法登録で全プロジェクトのゲートが解除)|

連絡先メール表示は確認済み(`projects.contact_email_verified_at IS NOT NULL`)のもののみ。停止判定・通知ロジックの正本は上限件数到達 = [DD11_利用量計測・課金.md §3.8](DD11_利用量計測・課金.md)、支払方法ゲート = [DD13_バッチ・非同期処理.md §3.1.3](DD13_バッチ・非同期処理.md)。
...
| 現在状態 | 契機 | 次状態 | UI 処理 |
|---|---|---|---|
| `collapsed`(初期状態) | 丸型ランチャーバッジのクリック / Enter / Space | `expanded` | 右下固定の直径56pxバッジを非表示にし、チャット UI を表示 |
| `expanded` | ヘッダーの閉じるボタン押下 | `collapsed` | チャット UI を非表示にし、丸型ランチャーバッジを再表示。会話履歴、入力内容、`normal` / `limited` の受付状態をメモリ上で保持 |
...
| `normal` | `/ask` が `unanswered` | `normal` | `inquiries`を管理用問い合わせID付きで登録し、`MSG-WIDGET-UNANSWERED` と確認済み連絡先メールを会話履歴へ追加。`inquiryCode` は描画せず、FAQ質問入力は継続 |
| `normal` | `/ask` が制限 429 | `limited` | `MSG-WIDGET-QUESTION-LIMIT` または `MSG-WIDGET-QUESTION-LIMIT-NOEMAIL` をシステム返信として追加し、入力・送信を無効化 |

ウィジェットヘッダーは全状態でウィジェットタイトル、受付状態、閉じるボタンのみを表示する。ハンバーガーメニューその他の操作メニュー、および SCR-010 利用規約・SCR-020 プライバシーポリシーへの導線は表示しない。閉じるボタンには `aria-label="FAQチャットを閉じる"`、丸型ランチャーバッジには `aria-label="FAQチャットを開く"` を設定する。
...
| 関連 DD | [DD03_AI回答パイプライン.md](DD03_AI回答パイプライン.md) / [DD04_AIしきい値3階層適用.md](DD04_AIしきい値3階層適用.md) / [DD11_利用量計測・課金.md](DD11_利用量計測・課金.md) |
...
| UI | 初期表示では丸型ランチャーバッジのみが表示され、クリック / Enter / Space でチャット UI が展開すること |
| UI | 閉じるボタンで丸型ランチャーバッジ表示へ戻り、再展開時に会話履歴、入力内容、受付状態が保持されること |
| UI | 通常 / 未解決 / 制限中の全状態でヘッダーに閉じるボタンがあり、ハンバーガーメニュー、利用規約リンク、プライバシーポリシーリンクが存在しないこと |
| UI | 未解決返信に `inquiryCode` または問い合わせIDのラベル・値が表示されないこと |

...
export async function processOpenInquiryRetention(env: Env) {
  ...
  const stuck = await env.DB.prepare(`
    ...
    WHERE case_status = 'open'
      AND created_at <= ?1
      AND valid = 1
  `).bind(cutoff).all();  -- v1.11: deleted_at 列廃止 → valid=1 で論理削除済みを除外
...
v1.11 でメイン側 `audit_logs` は API 操作ログ専用となり、ハッシュチェーン構造を持たない(`prev_hash` / `current_hash` 列を削除)。本処理は **運営者側 `audit_logs`(ハッシュチェーン正本)に対してのみ実行** される。詳細は運営者側設計 §7 + [DD10_監査ログ書込・完全性検証.md](DD10_監査ログ書込・完全性検証.md) §3.6 を参照。

#### 3.1.8 audit_logs 保持期間超過削除バッチ（JST 04:00、v1.11 で tombstone 方式から物理 DELETE 方式に変更)

v1.11 でメイン側 `audit_logs` は「API 操作ログ専用」と役割明確化し、ハッシュチェーン正本は運営者側に一本化された。これに伴い、メイン側ではチェーン破断を防ぐための tombstone 方式が不要となったため、保持期間超過後は物理 DELETE に変更する。

```ts
export async function purgeExpiredAuditLogs(env: Env) {
  // 一般 (1 年) / 課金 (7 年) / 運営者高権限 (5 年) 経過の audit_logs を物理 DELETE
  // v1.11: 旧 tombstone バッチを廃止し、本処理に置換
...
> **注**: 運営者側 `audit_logs`(ハッシュチェーン正本)では従来通り tombstone 方式を維持する(運営者側設計 §7 参照)。本処理はメイン側 D1 限定。
...
export async function retentionCleanup(env: Env) {
  ...
  await env.DB.batch([
    ...
    env.DB.prepare(`DELETE FROM inbox_messages WHERE created_at < ?1`)
      .bind(new Date(now - 365 * 86400000).toISOString()),
    // 退会確定: deleted_pending → deleted (30 日経過、v1.10 で valid=0 も同時セット、v1.11 で deleted_at 列廃止)
    env.DB.prepare(`
      UPDATE accounts SET contract_status = 'deleted', valid = 0, updated_at = ?1

...
| 種別 | ID | 名称 |
|---|---|---|
| cron | 9 ジョブ | 月次集計 / 月次確定 / 無料枠超過・支払方法未登録判定 / 保持期間処理 等 |
...
| §14.1.3 | 無料枠超過・支払方法未登録判定（JST 00:00 daily） | 支払方法ゲート(FR-136)、§7.10.4 で実装済み |
...
#### 3.1.3 無料枠超過・支払方法未登録判定（JST 00:00 daily）

支払方法ゲート(FR-136)のバックストップ判定。判定は **プロジェクト単位**。**支払方法が未登録**(`billing_subscriptions` に Stripe 支払方法が紐づかない契約)かつ、配下の **いずれかのプロジェクト** が当月の月次無料枠(質問数、`project_quota_limits.free_quota` or デフォルト)を超過している場合、当該プロジェクトについて **ウィジェットの新規質問受付を停止するフラグ**(KV `usage-limit:<owner>:<projectId>` の `paymentGateStopped=true` 相当 / 30s TTL)を設定し、`NOTIF-PAYMENT_METHOD_REQUIRED`(`critical`、オーナー + 当該プロジェクト管理者宛、`{project_name}` 含む)を送信する。**契約状態(`contract_status`)は変更しない**(`active` のまま、管理画面は全機能利用可)。以降 当該プロジェクトの `widget/v1/ask` は 429 を返す(DD13 §3.7a)。リアルタイム判定(質問到達時の同期チェック)で取りこぼしたプロジェクトを日次で拾う冪等処理とし、既にフラグ設定済みのプロジェクトは重複通知しない。FAQ / 個別チャットの無料枠超過は新規作成拒否(429)で同期処理され本バッチ対象外。支払方法登録によるフラグ解除(全プロジェクトのウィジェット質問受付再開)は契約単位の支払方法登録で行い、メイン側の Setup Intent 成功ハンドラで即時実行する(本バッチ対象外)。詳細は基本設計 [../02_基本設計/10_課金・請求設計.md §5.1 / §8.2](../02_基本設計/10_課金・請求設計.md) を参照。
...
全削除対象テーブル(`users` / `contract_owners` / `project_users` / `projects` / `allowed_domains` / `faqs` / `question_logs` / `inquiries` / `end_users` / `chat_rooms` / `project_quota_limits`)の `valid` カラムによる論理削除データを、`updated_at` 基準で 90 日経過後に物理削除する。`billing_subscriptions` / `usage_metering` は除外(電子帳簿保存法 7 年保持、永久維持)。`project_quota_limits` は単独で論理削除された行も本バッチで物理削除し、`projects` 物理削除時は CASCADE で連鎖物理削除される。

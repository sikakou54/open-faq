...
支払方法ゲート(FR-136)のバックストップ判定。**支払方法が未登録**(`billing_subscriptions` に Stripe 支払方法が紐づかない)かつ当月の月次無料枠(質問数 / FAQ 件数 / 個別チャット部屋数のいずれか)を超過しているオーナーを `contract_owners.contract_status='suspended'`(`reason=quota_exceeded_no_payment_method`)へ遷移させ、`NOTIF-PAYMENT_METHOD_REQUIRED`(`critical`)を送信する。ウィジェット応答は以降 423 となる。リアルタイム判定(質問到達時 / FAQ・チャット作成時の同期チェック)で取りこぼした契約を日次で拾う冪等処理とし、既に `suspended` の契約は対象外。支払方法登録による解除はメイン側の Setup Intent 成功ハンドラで即時実行する(本バッチ対象外)。詳細は基本設計 [../02_基本設計/10_課金・請求設計.md §5.1 / §5.2](../02_基本設計/10_課金・請求設計.md) を参照。

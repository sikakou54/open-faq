...
全削除対象テーブル(`users` / `contract_owners` / `project_users` / `projects` / `allowed_domains` / `project_ip_allowlist` / `faqs` / `question_logs` / `inquiries` / `end_users` / `chat_rooms`)の `valid` カラムによる論理削除データを、`updated_at` 基準で 90 日経過後に物理削除する。`billing_subscriptions` は除外(電子帳簿保存法 7 年保持、永久維持)。

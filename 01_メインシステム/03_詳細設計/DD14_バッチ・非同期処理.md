...
export async function retentionCleanup(env: Env) {
  ...
  await env.DB.batch([
    ...
    env.DB.prepare(`DELETE FROM inbox_messages WHERE created_at < ?1`)
      .bind(new Date(now - 365 * 86400000).toISOString()),
    // 退会確定: deleted_pending → deleted (30 日経過、valid=0 を同時セット)
    env.DB.prepare(`
...
全削除対象テーブル(`accounts` / `account_project_grants` / `projects` / `allowed_domains` / `project_ip_allowlist` / `faqs` / `question_logs` / `inquiries` / `inquiry_contacts` / `chat_rooms`)の `valid` カラムによる論理削除データを、`updated_at` 基準で 90 日経過後に物理削除する。`billing_subscriptions` は除外(電子帳簿保存法 7 年保持、永久維持)。

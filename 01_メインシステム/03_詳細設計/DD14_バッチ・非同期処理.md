...
支払方法ゲート(FR-136)のバックストップ判定。**支払方法が未登録**(`billing_subscriptions` に Stripe 支払方法が紐づかない)かつ当月の月次無料枠(質問数)を超過しているオーナーについて、**ウィジェットの新規質問受付を停止するフラグ**(KV `usage-limit:<owner>` の `paymentGateStopped=true` 相当 / 30s TTL)を設定し、`NOTIF-PAYMENT_METHOD_REQUIRED`(`critical`)を送信する。**契約状態(`contract_status`)は変更しない**(`active` のまま、管理画面は全機能利用可)。以降 `widget/v1/ask` は 429 を返す(DD13 §3.7a)。リアルタイム判定(質問到達時の同期チェック)で取りこぼした契約を日次で拾う冪等処理とし、既にフラグ設定済みの契約は重複通知しない。FAQ / 個別チャットの無料枠超過は新規作成拒否(429)で同期処理され本バッチ対象外。支払方法登録によるフラグ解除(ウィジェット質問受付再開)はメイン側の Setup Intent 成功ハンドラで即時実行する(本バッチ対象外)。詳細は基本設計 [../02_基本設計/10_課金・請求設計.md §5.1 / §8.2](../02_基本設計/10_課金・請求設計.md) を参照。

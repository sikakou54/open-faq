...
| 関連機能ID | FR-140〜149（通知 13 種 / Resend / Webhook）/ AC-021 |
| 作成日 | 2026-05-17 |
| 版数 | v1.0 |
| ステータス | 承認済 |
...
| 種別 | ID | 名称 |
|---|---|---|
| 機能 | FR-140〜149 | 通知 13 種 / メール送信 / Webhook 受信 / サプレッション |
...
指数バックオフ: 60s, 120s, 240s, ...（最大 600s）。3 回失敗で DLQ。詳細は [DD13_バッチ・非同期処理.md](DD13_バッチ・非同期処理.md) §14.3 を参照。
...
通知重要度は `low` / `normal` / `high` / `critical` の 4 区分（共有概念）。`critical` はメール送信が必須となるイベント(`project_users.role='admin' AND valid=1` を 1 件でも保持する全ユーザー = オーナー自身も admin 行を保持するため自動的に網羅)に予約。`critical` はシステム判定で**強制送信**される。

**critical 通知の宛先解決クエリ**: オーナーが自動 admin 行を持つため、`project_users.role='admin' AND valid=1` の EXISTS 単独条件で宛先を網羅できる。`SELECT DISTINCT user_id` でメンバーが複数 PJ で admin を保持する場合の二重カウントを防ぐ。

プロジェクト関連通知(`NOTIF-CHAT_REPLY_TO_STAFF` / `NOTIF-CHAT_HOLD_CHECK` 等の `normal` / `low`)は **常時 ON 固定** とし、配信時点で `project_users` を `WHERE valid=1` 条件で動的スキャンして当該プロジェクトの全メンバー(オーナー + admin + member)に即時配信する。画面設定と `project_notification_settings` テーブルは持たない。契約単位の `normal` / `high` 通知(課金・契約・運営お知らせ等)は契約 WS 側で扱う。個人通知オプトアウト・メンバー選択型 UI は MVP では提供しない。

質問数上限アラート(`usage.question_limit.threshold`)はSCR-021-M1で選択された25/50/80/90/100%への当月初回到達時に、オーナーと当該プロジェクトの有効な`admin`へメールだけを送信する。宛先は`project_id`で限定し、ユーザーIDと正規化メールアドレスで重複排除する。冪等キーは`question-limit-alert:{projectId}:{yyyyMM}:{thresholdPercent}`とし、画面上に送信先の表示・変更項目は設けない。
...
| 関連 DD | [DD10_暗号化・鍵管理.md](DD10_暗号化・鍵管理.md) / [DD13_バッチ・非同期処理.md](DD13_バッチ・非同期処理.md) / [DD09_監査ログ書込・完全性検証.md](DD09_監査ログ書込・完全性検証.md) |

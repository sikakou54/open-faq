...
| 関連機能ID | NFR-401（プライバシー保護）/ WCAG 2.1 AA 準拠 / i18n 基盤 |
| 作成日 | 2026-05-17 |
| 版数 | v1.0 |
| ステータス | 承認済 |
...
| 元章 | 元タイトル | 概要 |
|---|---|---|
| §13.4 | アクセシビリティ（WCAG 2.1 AA） | 実装規約 |
...
- **IP 許可リスト 403 画面（運営者ログイン前、`operator_ip_allowlist`）** / **契約停止 401 画面**:
  ...
  - **FAQ ウィジェット内の IP 許可リスト 403(プロジェクト単位、`project_ip_allowlist`、FR-179 / FR-330)**: 一般エンドユーザー向けにはリストの存在自体を秘匿するため「現在ご利用いただけません」のみを表示する。`role="alert"` は付与しつつ、原因詳細・連絡先表示は行わない(誤設定通報経路は当該プロジェクトの管理者連絡先(`projects.contact_email`)からのみとする)

...
| API | `/me/notification-preferences` | 通知設定 |
...
通知重要度は `low` / `normal` / `high` / `critical` の 4 区分（共有概念）。`critical` はメール送信が必須となるイベント（オーナー + 同一オーナースコープで `account_project_grants.role='admin'` を 1 件でも保持する全プロジェクト管理者へ配信）に予約。`critical` メールは通知設定（オプトアウト）の影響を受けず**強制送信**される。

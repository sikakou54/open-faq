...
プロジェクト関連通知(`NOTIF-CHAT_REPLY_TO_STAFF` / `NOTIF-CHAT_HOLD_CHECK` 等の `normal` / `low`)は **常時 ON 固定** とし、配信時点で `project_users` を `WHERE valid=1` 条件で動的スキャンして当該プロジェクトの全メンバー(オーナー + admin + member)に即時配信する(画面設定 SCR-030 / `project_notification_settings` テーブルは持たない)。契約単位の `normal` / `high` 通知(課金・契約・運営お知らせ等)は契約 WS 側で扱う。個人通知オプトアウト・メンバー選択型 UI は MVP では提供しない。

質問数上限アラート(`usage.question_limit.threshold`)はSCR-036-M1で選択された25/50/80/90/100%への当月初回到達時に、オーナーと当該プロジェクトの有効な`admin`へメールだけを送信する。宛先は`project_id`で限定し、ユーザーIDと正規化メールアドレスで重複排除する。冪等キーは`question-limit-alert:{projectId}:{yyyyMM}:{thresholdPercent}`とし、画面上に送信先の表示・変更項目は設けない。

...
通知重要度は `low` / `normal` / `high` / `critical` の 4 区分（共有概念）。`critical` はメール送信が必須となるイベント(`account_project_grants.role='admin' AND valid=1` を 1 件でも保持する全アカウント = オーナー自身も admin 行を保持するため自動的に網羅)に予約。`critical` はシステム判定で**強制送信**される。

**critical 通知の宛先解決クエリ**: オーナーが自動 admin 行を持つため、`account_project_grants.role='admin' AND valid=1` の EXISTS 単独条件で宛先を網羅できる。`SELECT DISTINCT account_id` でメンバーが複数 PJ で admin を保持する場合の二重カウントを防ぐ。

プロジェクト関連通知(`NOTIF-CHAT_REPLY_TO_STAFF` / `NOTIF-CHAT_HOLD_CHECK` 等の `normal` / `low`)は **常時 ON 固定** とし、配信時点で `account_project_grants` を `WHERE valid=1` 条件で動的スキャンして当該プロジェクトの全メンバー(オーナー + admin + member)に即時配信する(画面設定 SCR-030 / `project_notification_settings` テーブルは持たない)。契約単位の `normal` / `high` 通知(課金・契約・運営お知らせ等)は契約 WS 側で扱う。個人通知オプトアウト・メンバー選択型 UI は MVP では提供しない。

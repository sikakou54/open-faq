...
通知重要度は `low` / `normal` / `high` / `critical` の 4 区分（共有概念）。`critical` はメール送信が必須となるイベント（オーナー + 同一オーナースコープで `users:manage` フラグを保持する全メンバーへ配信）に予約。`critical` メールは通知設定（オプトアウト）の影響を受けず**強制送信**される。

...
| 関連機能ID | FR-070〜077（未解決質問一覧 / 詳細 / 状況管理 open/closed）/ FR-072（inquiry_code 採番）/ AC-010 / AC-012 / AC-013 |
| 作成日 | 2026-05-17 |
| 版数 | v1.0 |
| ステータス | 承認済 |
...
| 種別 | ID | 名称 |
|---|---|---|
| 機能 | FR-070〜077 | 未解決質問一覧 / 詳細 / 状況管理（2 値: open / closed、手動操作のみ） |
| 機能 | FR-072 | inquiry_code 採番（`INQ-YYYYMMDD-XXXXXXXX`） |
| 画面 | SCR-005 | FAQ 一覧（未解決質問は派生表示） |
...
| §6 SCR-005 | FAQ 一覧 | 未解決質問派生表示（正本は基本設計） |
...
未解決質問一覧 / 詳細 / 状況管理は [../02_基本設計/01_画面設計.md](../02_基本設計/01_画面設計.md) を正本とする。`inquiries.status` の 2 値（`open` / `closed`）と SCR-005 詳細画面からの手動操作のみで更新する仕様は [../02_基本設計/03_テーブル設計.md](../02_基本設計/03_テーブル設計.md) §4.5.2 を正本とする。

### 3.3 未解決質問の保持期間通知（cron JST 09:00）

`status=open` の未解決質問が保持期間に近づいた場合に、管理者ユーザーへ滞留通知を送信する。`closed` は対象外。状況は本処理で変更しない。詳細は [DD13_バッチ・非同期処理.md](DD13_バッチ・非同期処理.md) §14.1.4 を参照。
...
| 関連 DD | [DD06_通知ロジック.md](DD06_通知ロジック.md) / [DD13_バッチ・非同期処理.md](DD13_バッチ・非同期処理.md) |

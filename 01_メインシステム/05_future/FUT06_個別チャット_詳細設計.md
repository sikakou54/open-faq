...
本書は [FUT06_個別チャット.md](FUT06_個別チャット.md) の詳細設計を定義する。
...
| §6 SCR-027 / SCR-034 | 画面詳細設計 | 個別チャット再入室 / デフォルト担当者設定 |
...
| §10.5.4 | 再入室トークン | 30 日 access_tokens、ブラウザへ直接返却 |
...
| SCR-027 個別チャット再入室 | エンドユーザー | チャットID、期限、再開ボタン、期限切れ案内 | ウィジェットメニューまたは再入室URLから遷移 |
...
| `access_tokens(purpose='reentry')` | 30日有効の再入室トークンをHMACで保持 |
| `projects.default_assignee_user_id` | 新規部屋のデフォルト担当者 |
...
- **担当管理者の論理削除時の付け替え**: 担当者であった管理者ユーザーがアカウント論理削除(`users.valid=0`)される場合、当該管理者が担当する全部屋の `assignee_user_id` を `resolveInitialAssignee` と同じ優先順位(デフォルト担当者→オーナー)で付け替えてから `valid=0` に更新する。これにより NOT NULL 制約と「未割当を作らない」不変条件を維持する。デフォルト担当者(`projects.default_assignee_user_id`)が当該削除ユーザーを指していた場合も同フローで `NULL` 化(以後オーナーへフォールバック)する。
...
| 種別 | 参照先 |
|---|---|
| Future要件 | [FUT06_個別チャット_要件.md](FUT06_個別チャット_要件.md) |
| Future概要 | [FUT06_個別チャット.md](FUT06_個別チャット.md) |

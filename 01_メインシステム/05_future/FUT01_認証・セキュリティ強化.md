...
|---|---|---|
| 利用者側 admin / operator MFA | SCR-028 個人設定での MFA 登録・解除・リカバリ、SCR-041 契約設定での契約単位強制状態、未設定ユーザーのログイン制御 | FUT-REQ-SEC-002 |
...
| 画面 | SCR-028 個人設定(MFA 登録・解除・回復コード)、SCR-041 契約設定(MFA 強制ポリシー)、ログイン画面(MFA 入力)、招待受諾フロー(契約単位強制下) |

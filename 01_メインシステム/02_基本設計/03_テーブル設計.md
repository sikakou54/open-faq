...
**v1.10 で `valid` カラムによる論理削除を全削除対象テーブルに導入**(対象テーブルは §9 削除モード表を正本)。`valid INTEGER NOT NULL DEFAULT 1 CHECK (valid IN (0,1))` を追加し、論理削除は `UPDATE … SET valid=0` で表現する。論理削除データは `updated_at` 基準で 90 日経過後に物理削除バッチで物理削除する([DD14_バッチ・非同期処理.md](../03_詳細設計/DD14_バッチ・非同期処理.md) を正本)。`deleted_at` カラムは追加せず、論理削除日時は既存 `updated_at`(`valid` が 0 に更新された時刻)で代用する。例外テーブル: `audit_logs`(tombstone 方式と整合)、`sessions`(`revoked_at` で代用)、法定保管 7 年テーブル(`billing_invoices` / `usage_metering` / `withdrawal_requests` / `terms_versions` / `terms_agreements`)、全契約横断テーブル(`email_suppression_list`)、ログ系(`notification_logs` / `error_logs` / `inbox_messages`)、トークン / 履歴系(`access_tokens` / `faq_revisions` / `faq_search_fts` / `chat_messages` / `inquiry_status_history` / `question_log_faq_refs`)。
...
| 主キー | `id`(TEXT, ULID) |
| 設計判断 | オーナー行は `owner_account_id = id` で自己参照。**v1.10 でオーナー(`is_owner=1`)も `account_project_grants` に `role='admin'` 行を保持する**(プロジェクト作成時に自動 INSERT。認可上は `isOwner=true` bypass を優先する)。論理削除は `valid` カラムで表現(`valid=0` = 論理削除済、`valid=1` = 有効)。論理削除日時は `updated_at` で代用(`deleted_at` 列は MVP 前期に契約レベルの退会日時用に追加されたものを継続利用するが、v1.10 で新規追加した `valid` カラムとは独立)|
...
| `audit_logs` | (`valid` なし、tombstone 方式)| 削除しない | (tombstone 方式、匿名化対象外)|

...
**割当ゼロ状態の発生条件(v1.7 で FR-015d 不変条件を撤廃)**: v1.7 ではメンバー削除を **アカウント全体の物理削除**(`DELETE /projects/{id}/members/{accountId}`)に統一し、`account_project_grants` 行のみの単独削除(離脱)操作は提供しない。このため、メンバー行が `account_project_grants` に 0 行残った状態は構造的に発生しない。プロジェクト削除起因で割当が 0 件になるメンバーは同一トランザクション内で `accounts` 行ごと物理削除する(下記「プロジェクト削除時の孤立メンバー処理」)処理は引き続き有効。最後の admin が削除された場合のプロジェクト存続はオーナーの暗黙 admin 継承(FR-019a)で担保するため、DB スキーマ上の不変条件は不要。
...
オーナーは対象外(`is_owner=1` は常に存続)。v1.7 でメンバー削除はアカウント全体物理削除に統一されたため、孤立メンバー cleanup が走るのはプロジェクト削除経路のみとなる(他 PJ にも割当があるメンバーは当該 PJ 行のみ削除 + 他 PJ 行は維持)。詳細は [DD02_プロジェクト・FAQ管理.md](../03_詳細設計/DD02_プロジェクト・FAQ管理.md) を参照。

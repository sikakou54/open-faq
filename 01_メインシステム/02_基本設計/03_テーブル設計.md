...
| 22 | `audit_logs` | 監査ログ(メイン側、API 操作ログ専用)| メイン管理者向けの API 操作ログ。**ハッシュチェーン正本は運営者側 `audit_logs`**(IF #8 で片方向同期)| `id` | あり(`owner_account_id` カラム)| NFR-602a/b/d | retention_class 別(1 / 7 / 5 年)| 操作日時 | `general` / `billing` / `operator_high_priv` |
...
| 28a | `announcement_audiences` | お知らせ配信対象 M:N | お知らせの配信先 owner 限定指定(行 0 件 = 全契約配信)| `(announcement_id, owner_account_id)` | (限定)| - | 親お知らせに従属 | - | `operator_high_priv` |
...
**オーナー(`accounts.is_owner=1`)も `account_project_grants` に行を持つ**(プロジェクト作成時に `account_project_grants(account_id=オーナー, project_id=新PJ, role='admin', valid=1)` を自動 INSERT)。認可フロー先頭の `isOwner=true` bypass は維持し、`account_project_grants` 行は通知宛先解決(critical 通知)と画面表示(プロジェクト管理者一覧)の事実情報として参照する。個別権限フラグ方式(`account_permissions` 別テーブル + `permission_kind` 5 値)は、運用パターンが固定的なため UI / 認可ロジック双方の複雑さに見合わず採用しない。
...
| 主キー | `id`(TEXT, ULID) |
| 設計判断 | オーナー行は `owner_account_id = id` で自己参照。**オーナー(`is_owner=1`)も `account_project_grants` に `role='admin'` 行を保持する**(プロジェクト作成時に自動 INSERT。認可上は `isOwner=true` bypass を優先する)。論理削除は `valid` カラムで表現(`valid=0` = 論理削除済、`valid=1` = 有効)。`deleted_at` 列は持たず、論理削除日時は `updated_at` で代用する。退会日時の証跡は `withdrawal_requests.applied_at` で代替する。`email_key_version` は鍵ローテーション dual-decrypt 期間中の鍵バージョン参照用 |
...
-- 同一オーナー配下で valid=1 のメール重複のみ禁止(論理削除済みの valid=0 行は重複許容、同メールでの再招待で新規 accounts 行を作成可能にする)
...
| `contract_status` | `deleted` | 削除済み(オーナー退会完了時は `valid=0` も同時セット) |
| `valid` | `1` | 有効 |
...
オーナー(`accounts.is_owner=1`)とメンバー(`is_owner=0`)の両方に対し、プロジェクトごとに `admin`(プロジェクト管理者) / `member`(メンバー)のロールを付与する。**オーナーも本テーブルに行を持つ**(プロジェクト作成時に `(account_id=オーナー, project_id=新PJ, role='admin', valid=1)` 行を自動 INSERT)。認可フロー先頭の `isOwner=true` bypass は維持し、本テーブルは通知宛先解決と画面表示に使う。

**割当ゼロ状態の発生条件**: メンバー削除は **アカウント論理削除**(`UPDATE accounts SET valid=0`)で行い、`account_project_grants` 行のみの単独削除(離脱)操作は提供しない。論理削除に伴い `account_project_grants` 行も `valid=0` に同時 UPDATE する。`valid=1` 行が 0 行残った状態は構造的に発生しない(アカウント論理削除と同時に発生するため)。プロジェクト削除起因で `valid=1` 割当が 0 件になるメンバーは同一トランザクション内で `accounts.valid=0` に論理削除する(下記「プロジェクト削除時の孤立メンバー処理」)。最後の admin の有無に関しては、オーナーが常に admin 行を保持するため「プロジェクトに admin 行が 0 件になる」状態は構造的に発生しない(`E-AUTHZ-LAST-ADMIN-PROTECTED` の判定はオーナー行除外、メンバー由来の admin のみで判定)。
...
CREATE TABLE account_project_grants (
  ...
  granted_by  TEXT REFERENCES accounts(id) ON DELETE SET NULL,  -- 招待者削除でも整合維持(監査履歴は audit_logs で補完)
  updated_at  TEXT NOT NULL,
  PRIMARY KEY (account_id, project_id)
);
-- 同一 (account_id, project_id) ペアで valid=1 行は 1 件のみ、再招待時は valid=0 行を残したまま valid=1 行を上書き(UPSERT)で復活
...
CREATE TABLE faqs (
  ...
  valid                             INTEGER NOT NULL DEFAULT 1 CHECK (valid IN (0,1)),
  created_by                        TEXT REFERENCES accounts(id) ON DELETE SET NULL,  -- 作成者削除でも整合維持
  updated_by                        TEXT REFERENCES accounts(id) ON DELETE SET NULL,  -- 同上
  created_at                        TEXT NOT NULL,
  updated_at                        TEXT NOT NULL
);
...
CREATE TABLE question_logs (
  id                     TEXT PRIMARY KEY,
  owner_account_id       TEXT NOT NULL REFERENCES accounts(id) ON DELETE CASCADE,  -- ON DELETE 明示
...
CREATE TABLE inquiry_status_history (
  ...
  reason       TEXT,
  changed_by   TEXT REFERENCES accounts(id) ON DELETE SET NULL,  -- 操作者削除でも履歴は残す
...
CREATE TABLE notification_logs (
  id                    TEXT PRIMARY KEY,
  owner_account_id      TEXT NOT NULL REFERENCES accounts(id) ON DELETE CASCADE,        -- FK 明示
...
CREATE TABLE ai_threshold_persistent_cache (
  id                      TEXT PRIMARY KEY,
  owner_account_id        TEXT REFERENCES accounts(id) ON DELETE CASCADE,  -- FK 明示
...
CREATE TABLE service_announcements (
  ...
  retracted_at                TEXT,
  created_by                  TEXT REFERENCES accounts(id) ON DELETE SET NULL,  -- 作成者削除でも履歴は残す
  created_at                  TEXT NOT NULL
);
...
**設定**: `accounts.data_deletion_mode`(`physical` / `anonymize`、初期値 `physical`)で保持する独立カラム。
...
| `announcement_audiences` | (`valid` なし、配信関係 M:N)| `service_announcements` / `accounts` 物理削除時に ON DELETE CASCADE で連鎖物理削除 | (匿名化対象外)|

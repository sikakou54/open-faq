...
| 権限設計書 | `account_permissions` / `account_project_grants` | [05_権限設計書.md](05_権限設計書.md) |
...
| 2 | `account_permissions` | メンバー権限 | メンバー権限フラグ(`faq:manage` 等) | `(account_id, permission_kind)` | (`accounts` 経由) | FR-333 | 紐づくアカウントに従属 | - | `general` |
| 3 | `account_project_grants` | メンバープロジェクト割当 | メンバーのプロジェクトアクセス | `(account_id, project_id)` | (`accounts` 経由) | FR-030-035 | 同上 | - | `general` |
...
採用方式は **案 A: 別テーブル方式**(`account_permissions`)。`(account_id, permission_kind)` 複合 PK + `granted_at` / `granted_by` 保持で行レベル監査を維持。検討時の案 B(bitmask)/ 案 C(JSON 配列)は却下。
...
| 主キー | `id`(TEXT, ULID) |
| 設計判断 | オーナー行は `owner_account_id = id` で自己参照。**`is_owner=1` 行は `account_permissions` / `account_project_grants` に行を持たない(オーナーは全権)**|
...
### 3.2 `account_permissions`(メンバー権限)
...
CREATE TABLE account_permissions (
  ...
  permission_kind TEXT NOT NULL
                  CHECK (permission_kind IN ('faq:manage','chat:respond','users:manage','project:manage','logs:view')),
...
#### コード値: `permission_kind` 5 値

詳細は [05_権限設計書.md §3](05_権限設計書.md) を参照。

### 3.3 `account_project_grants`(メンバープロジェクト割当)
...
erDiagram
  ...
  accounts ||--o{ account_permissions : "0..5(メンバーのみ)"
  accounts ||--o{ account_project_grants : "0..N(メンバーのみ)"
...
**ガード**: AI / Batch による `draft → published` 直接遷移を 3 層で禁止(API / Domain / DB)。AI 下書き(`draft`)生成は許容。
...
## 10. データ保持の調整制約(§11.4.1 から移管)

- **短縮は可**
- **長期化は SCR-093 経由(運営者承認必須)**── 認証・認可設計書 09 §6 / 運営者 権限設計書 05 §7 参照

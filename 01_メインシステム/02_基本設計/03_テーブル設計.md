...
2. 削除後に「他プロジェクト割当が 0 件かつ `is_owner=0`」となったメンバー `accounts` 行を抽出して `DELETE FROM accounts WHERE id IN (...)`

オーナーは対象外(`is_owner=1` は常に存続)。詳細は [DD02_プロジェクト・FAQ管理.md](../03_詳細設計/DD02_プロジェクト・FAQ管理.md) を参照。

...
メール送信を含む全 14 テンプレートを以下に索引する。詳細(件名 + 本文)は §4 を参照。
...
| 13 | TPL-SERVICE_ANNOUNCEMENT | NOTIF-SERVICE_ANNOUNCEMENT | normal/high/critical | admin(範囲指定可)| 運営者選択 | 運営者入力 URL |
...
**発火条件**: SCR-009-M1 メンバー招待モーダルの「招待メールを送信する」/「招待メールを再送する」ボタン押下時、`access_tokens.purpose='activation'` 発行時。
...
**着地ページ**: SCR-018 メンバーアカウント有効化(01_画面設計.md §5.25)。

**備考**: 再送時は **旧 `access_tokens` を失効させ新トークンを発行** したうえで本テンプレートで再送信(`MSG-SCR-009-M1-TOAST-002` と整合)。本文は新規招待時と同一で、差出人による文脈は本文中の「招待操作者」フィールドで補足する。氏名は招待時点で未登録のため、宛先表示は `{recipient_email}` を用いる。

---

### 4.4 TPL-PROJECT_ADMIN_INVITE(プロジェクト管理者招待)

**発火条件**: SCR-009-M1 メンバー招待モーダルで `role='admin'` を指定して招待を実行した時、招待対象メールアドレスへ送信(FR-015 / FR-015e / FR-030a)。
...
**備考**: 既存アカウントがある場合(FR-021c 名前解決)は `{activation_url}` がログイン URL になり、ログイン後に当該プロジェクトに `project_users.role='admin'` 行が追加される(SCR-018 はバイパス、氏名は既存値を保持)。

---

### 4.5 TPL-PROJECT_CONTACT_VERIFY(プロジェクト連絡先メール確認)

**発火条件**: SCR-004-M1 プロジェクト設定モーダルで連絡先メールアドレスが新規入力 / 変更された時、ウィジェット上での表示前確認用に送信。
...
**着地ページ**: **SCR-019 プロジェクト連絡先メール確認完了**(01_画面設計.md §5.26、共通領域、未認証可)。トークン検証成功時に `projects.contact_email_verified_at=now()` をセットし、結果(完了 / 期限切れ / 既使用)を表示する。
...
### 4.7 TPL-DELETION_REMINDER(退会猶予期間案内)

**発火条件**: SCR-014 退会申請後、退会発効日(申請当月末)の 7 日前 / 当日 / 完全削除 3 日前の 3 回送信。
...
### 4.8 TPL-BILLING_INVOICE_ISSUED(月次請求確定通知)
...
| `{billing_screen_url}` | SCR-023 料金・請求 deeplink |
...
### 4.9 TPL-BILLING_PAYMENT_FAILED(支払い失敗通知)
...
| `{payment_method_url}` | SCR-023 料金・請求の支払い方法変更 URL |
...
### 4.10 TPL-BILLING_SUSPENSION(サスペンション開始/解除)
...
### 4.11 TPL-PAYMENT_METHOD_REQUIRED(無料枠超過・支払方法登録要請通知)
...
| `{billing_setup_url}` | 料金・請求 URL(SCR-023)|
...
### 4.12 TPL-TERMS_REVISION(利用規約改定告知)
...
### 4.13 TPL-SERVICE_ANNOUNCEMENT(運営からのお知らせ)
...
### 4.14 TPL-SYSTEM_NOTICE(サブ契機ごとの定型文 = メタテンプレート)
...
| `{action_url}` | 対応画面 URL(上限系は SCR-021、請求系は SCR-023、個人設定は SCR-017)|

...
各フィールドはオプション(指定したものだけ更新)。状況は `open` ↔ `closed` の双方向遷移を許容(再オープン制限なし)。FAQ 下書き保存・FAQ 公開・個別チャットの操作は本フィールドを変更しない(連動ロジックなし)。状態変更時は `inquiry_status_history` に `reason='manual'` で記録する。

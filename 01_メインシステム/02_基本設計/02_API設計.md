...
| メンバー管理 | `/projects/{id}/members`(一覧・招待), `/projects/{id}/members/{userId}`(ロール変更・割当解除), `/owner/members`(契約横断一覧), `/owner/members/{userId}`(契約削除), `/members/{id}/resend-invitation` | プロジェクト操作はオーナー / 該当PJ管理者、契約操作はオーナー専有 | SCR-017, SCR-017-M1, SCR-039 |
| プロジェクト管理 | `/projects`, `/projects/{id}` | オーナー専有(ただし参照と `defaultAssigneeUserId` は該当PJ管理者にも許可)| SCR-010, SCR-010-M1, SCR-034, SCR-042 |
| FAQ 管理 | `/projects/{id}/faqs`, `/projects/{id}/faqs/{faqId}`, `/projects/{id}/faqs/import`, `/projects/{id}/faqs/import/template`, `/projects/{id}/faqs/export` | オーナー / 該当 PJ の `member`+ | SCR-012 / SCR-012-M1 |
| ウィジェット | `/widget/bootstrap`, `/widget/ask`, `/widget/feedback` | end_user(公開キー + セッション)| ウィジェット |
| 未解決質問 | `/inquiries`, `/inquiries/{id}` | オーナー / 該当 PJ の `member`+ | SCR-011 |
| チャット | `/chat/rooms`, `/chat/rooms/{id}`, `/chat/rooms/{id}/messages` | admin / end_user | SCR-013(一覧)/ SCR-033(部屋)|
| 通知 | `/webhooks/resend` | 署名検証のみ | (Resend Webhook) |
| 利用量・課金 | `/usage`, `/owner/projects/usage`, `/billing/summary`, `/billing/subscription`, `/billing/invoices` | プロジェクト利用量は該当PJの `member`+、契約利用量・課金はオーナー専有 | SCR-036, SCR-037, SCR-038 |
| プロジェクトの質問数上限・アラート | `/projects/{id}/quota-limits`(GET), `/projects/{id}/quota-limits/questions`(PATCH)| 閲覧: オーナー / 該当 PJ の `member`+、変更: オーナー / 該当 PJ の `admin`(再認証必須)| SCR-036 / SCR-036-M1 |
| 操作履歴 | `/owner/activity` | オーナー専有 | SCR-040 |
...
| 利用規約 | `/terms/current`, `/terms/agree` | 認証不要(閲覧)/ admin(同意)| SCR-018, SCR-025, SCR-031 |
| プライバシーポリシー | `/privacy/current`, `/privacy/agree` | 認証不要(閲覧)/ admin(同意)| SCR-035, SCR-025, SCR-031 |
...
| `X-Chat-Token: <token>` | 個別チャット再入室 | `/widget/sessions/:token` |
...
| 月間質問数(プロジェクト単位)| 80%(通知)| 100%(設定上限件数到達でウィジェット質問受付停止。無料枠超過分は事後課金、**無料枠超過自体は拒否しない**)| プロジェクト別上書き可(`project_quota_limits`、SCR-036 / SCR-093)| 125% で追加制限 | FR-122 |
...
#### 5.2.5a `DELETE /owner/members/{userId}`(契約メンバーのアカウント論理削除)
...
| 必要権限 | **オーナー専有** |
| 関連画面 | `PATCH` / `DELETE`: SCR-042 プロジェクト設定 |
...
プロジェクト関連通知を常時ON固定にしたため、本API 2種を廃止。プロジェクト情報の参照・編集はSCR-042プロジェクト設定と `GET/PATCH /projects/{id}` に集約する。
...
FAQ の状態(`draft` / `published` / `hidden`)は `status` フィールドで指定し、`PATCH /faqs/{id}` の保存操作で相互に自由遷移できる(専用の公開 API・状態遷移ガードは持たない)。SCR-012 の「保存」ボタンは「状態」ラジオで選択した値をそのまま本 API に渡す。`status='published'` への変更は管理者(admin)が編集画面で内容を確認したうえで明示選択する操作であり、これが FR-045「FAQ の公開前に管理者が内容を確認できること」を満たす。`published` 設定時は `publishedAt` をサーバ側で記録する。

#### 5.4.4 `POST /faqs/import`

関連画面: SCR-012-M1 FAQ CSV インポートモーダル。

リクエスト: `multipart/form-data { file: <CSV> }`(`conflict` パラメータは廃止)
...
#### 5.4.4a `GET /faqs/import/template`

関連画面: SCR-012-M1「テンプレートをダウンロード」。
...
関連画面: SCR-012 FAQ 管理一覧「CSV をエクスポート」。一覧フィルタ適用結果を **CSV(UTF-8、`text/csv`)** で返す(JSON 形式は提供しない)。
...
#### 5.5.4 `POST /widget/v1/chat-rooms`
...
レスポンス(201): `{ "roomId": "room_...", "chatCode": "CHAT-20260513-XYZ123" }`

部屋作成時は担当者を自動割当する(FR-086b): 当該プロジェクトの `projects.default_assignee_user_id` が設定されていればその管理者を、未設定または当該管理者が当該プロジェクトに割当を持たない場合はオーナー(`contract_owners.user_id`)を `chat_rooms.assignee_user_id` にセットする。`assignee_user_id` は NOT NULL のため、担当者未割当の部屋は作成されない(FR-086a)。

#### 5.5.5 `POST /widget/v1/chat-rooms/{id}/messages`
...
PATCH は状況の手動切替のみを更新可能(担当者概念は未解決質問にはない。担当者の指定は個別チャットのみの概念 — FR-086a)。
```json
{
  "status": "open" | "closed"
}
...
各フィールドはオプション(指定したものだけ更新)。状況は `open` ↔ `closed` の双方向遷移を許容(再オープン制限なし)。FAQ 下書き保存・FAQ 公開・個別チャットの操作は本フィールドを変更しない(連動ロジックなし)。状態変更履歴の永続化は行わない(FR-077 廃止)。

### 5.7 チャット API

#### 5.7.1 `GET /chat-rooms/{id}/messages` / `POST /chat-rooms/{id}/messages`

#### 5.7.2 `POST /chat-rooms` / `GET /chat-rooms/{id}`

チャット部屋は `chat_code` を発行し、未解決質問 ID を受け付けない。部屋の close / reopen API は提供しない。

#### 5.7.3 `PATCH /chat-rooms/{id}`

担当管理者ユーザーの変更を行う(FR-086a)。担当者の指定は個別チャットのみの概念であり、未解決質問にはこの概念を持たない。`assigneeUserId` は **当該プロジェクトに割当のある管理者の `users.id` 必須**(`null` は不可。担当者は常に 1 名)。プロジェクト割当のある `member` 以上の誰でも変更可。当該プロジェクトに割当のない `users.id` を指定した場合は 400(E-INPUT-ASSIGNEE)。
```json
{
  "assigneeUserId": "usr_..."
}
...
#### 5.7.4 `PATCH /projects/{id}`(`defaultAssigneeUserId`)

プロジェクトのデフォルト担当者を設定 / 変更する(FR-086c)。SCR-034 自動割り当てから呼び出す。**操作はオーナー / 該当 PJ のプロジェクト管理者のみ**(メンバーは 403 / E-AUTHZ-FORBIDDEN)。`defaultAssigneeUserId` は当該プロジェクトに割当のある管理者の `users.id`、または未設定に戻す場合は `null`(未設定時は新規チャット作成時にオーナーへフォールバック)。
```json
{
  "defaultAssigneeUserId": "usr_..." | null
}
```
...
| 機能ID | F-120 |
| 関連画面 | `viewMode=owner`: SCR-037、`viewMode=project`: SCR-036 / SCR-015 |
...
```json
{
  ...
  "chatRooms": { "used": 28, "freeQuota": 30 }
}
```

`viewMode=owner` のレスポンスは各課金対象を `{ "used": <全プロジェクト SUM> }` のみとする(`limit` / `freeQuota` / `percentage` は含めない。上限はプロジェクト単位のため契約全体の単一上限は存在しない)。プロジェクト別の上限・無料枠・消化率は §5.8.3 `GET /projects/{id}/quota-limits` を用いる。

#### 5.8.1a `GET /owner/projects/usage?period=current_month`

| 機能ID | F-120 |
| 関連画面 | SCR-037 契約利用状況 |
| 必要権限 | オーナー専有 |

プロジェクト別の質問数、FAQ件数、チャット部屋数、AI利用コスト、前期間比較、各プロジェクトの質問数上限に対する利用率を返す。FAQ・チャットの上限に対する利用率は返さない。
...
#### 5.8.3 `GET /projects/{id}/quota-limits`

| 機能ID | F-LIMIT-001 |
| 関連画面 | SCR-036 |
| 必要権限 | オーナー / 該当プロジェクトの `admin` または `member`(閲覧)|

当該プロジェクトの質問数について、利用状況・月次上限件数・アラート設定を返す。SCR-036 の表示には `used` / `limitEnabled` / `limit` / `percentage` / `yenEquivalent` だけを使用し、アラート設定・設定元は表示しない。SCR-036-M1 は `limitEnabled` / `limit` / `alertThresholds` を使用する。FAQ 件数・個別チャット部屋数は扱わないためレスポンスに含めない。質問数の無料利用枠は課金判定用の内部データとして保持するが、本APIのレスポンスには含めない。
...
上限OFF時は `limitEnabled=false`、`limit=null`、`percentage=null`、`yenEquivalent=null` を返す。SCR-036 は「今月の利用上限」を「OFF」と表示し、「利用量」には割合・件数比・ProgressBar・状態バッジを描画しない。

#### 5.8.4 `PATCH /projects/{id}/quota-limits/questions`
...
当該プロジェクトの質問数について、月次上限のON / OFF・月次上限件数・アラート閾値を更新する(SCR-036-M1)。`project_quota_limits.resource_kind='question_monthly_limit'` かつ `source='owner'` の 1 行だけを upsert する。質問数の `free_quota` と FAQ 件数・個別チャット部屋数の行は更新しない。
...
リクエスト: `{ "version": "..." }`。`terms_agreements` に `(user_id, doc_type='privacy_policy', terms_version)` で 1 行記録(冪等: 既存同意があれば 200)。SCR-025 / SCR-031 では利用規約・プライバシーポリシー双方の同意が必須条件であり、改定再同意では改定された文書の agree のみを呼び出す。
...
`kind` は `question_monthly_limit` / `faq_monthly_limit` / `chat_room_monthly_limit`。質問数は `limit` と `freeQuota`、FAQ・チャットは運営者が管理する `freeQuota` だけを受け付ける。FAQ・チャットの `limit` は常に NULL とし、指定時は 422。質問数の優先順位は「プロジェクト設定 > 運営者上書き > デフォルト推奨値」、FAQ・チャット無料枠は「運営者上書き > デフォルト値」。

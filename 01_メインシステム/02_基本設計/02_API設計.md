...
`unanswered` では質問ログと未解決質問を同一トランザクションで作成する。ウィジェットは回答できなかった旨、`inquiryCode`、確認済みプロジェクト連絡先メールを表示し、FAQ質問入力は継続可能とする。連絡先未設定時は `contactEmail=null` を返す。冪等性は `questionLogId` を基準に担保する。

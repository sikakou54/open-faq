...
| プロジェクト管理 | `/projects`, `/projects/{id}` | **オーナー専有**(ただし `PATCH /projects/{id}` の `defaultAssigneeUserId` のみオーナー / プロジェクト管理者)| SCR-010, SCR-010-M1, SCR-034 |
...
| 利用量・課金 | `/usage`, `/billing/summary`, `/billing/subscription`, `/billing/invoices` | ダッシュボード閲覧: オーナー / 該当 PJ の `member`+、課金操作(支払方法・請求・解約): オーナー専有 | SCR-015 |
| プロジェクト上限・無料枠 | `/projects/{id}/quota-limits`(GET / PATCH)| 閲覧: オーナー / 該当 PJ の `member`+、変更: オーナー / 該当 PJ の `admin`(再認証必須)| SCR-036 / SCR-036-M1 |
...
| 必要権限 | **オーナー専有** |
| 関連画面 | `PATCH`: SCR-010-M1 編集モード / `DELETE`: **SCR-015 プロジェクト情報パネル(オーナー専有)のみ** |
...
プロジェクトのデフォルト担当者を設定 / 変更する(FR-086c)。SCR-034 チャット管理から呼び出す。**操作はオーナー / 該当 PJ のプロジェクト管理者のみ**(メンバーは 403 / E-AUTHZ-FORBIDDEN)。`defaultAssigneeUserId` は当該プロジェクトに割当のある管理者の `users.id`、または未設定に戻す場合は `null`(未設定時は新規チャット作成時にオーナーへフォールバック)。
```json
{
...
| 機能ID | F-120 |
| 関連画面 | SCR-015 / SCR-026 |
| 必要権限 | `viewMode=owner` はオーナー専有(契約全体のサマリ = 全プロジェクトの SUM)。`viewMode=project` はオーナー / 該当プロジェクトの `admin` または `member`(当該プロジェクトの利用状況)|

`viewMode` はヘッダーのダッシュボード切替アイコンと連動する。オーナーは `owner` / `project` を切替可能。プロジェクト管理者・メンバーは `project` 固定とし、`projectId` はヘッダーで選択中のプロジェクトを指定する。月次上限件数・無料枠はプロジェクト単位のため、`viewMode=owner` の `limit` / `freeQuota` は表示しない(契約全体は `used` の SUM のみ)。
...
```json
{
  ...
  "faqs": { "used": 85, "limit": 100, "freeQuota": 100, "percentage": 85 },
  "chatRooms": { "used": 28, "limit": 30, "freeQuota": 30, "percentage": 93 }
}
```
...
当該プロジェクトの課金対象ごとの利用状況・月次上限件数・無料枠・アラート設定を返す。`source` は適用中の設定の出所(`owner` = プロジェクト設定 / `operator` = 運営者上書き / `default` = デフォルト推奨値)。
...
```json
{
  "projectId": "prj_...",
  "period": "2026-05-01T00:00:00Z",
  "questions": { "used": 750, "limit": 5000, "freeQuota": 1000, "percentage": 15, "alertEnabled": true, "alertFrequency": "staged", "yenEquivalent": 2500, "source": "owner" },
  "faqs": { "used": 85, "limit": 300, "freeQuota": 100, "percentage": 28, "alertEnabled": true, "alertFrequency": "staged", "yenEquivalent": 1500, "source": "owner" },
  "chatRooms": { "used": 28, "limit": 100, "freeQuota": 30, "percentage": 28, "alertEnabled": true, "alertFrequency": "staged", "yenEquivalent": 3000, "source": "default" }
}
```

#### 5.8.4 `PATCH /projects/{id}/quota-limits`
...
当該プロジェクトの課金対象ごとの月次上限件数・月次無料枠・アラート設定を一括更新する(SCR-036-M1)。`project_quota_limits`(`source='owner'`)に保存し、KV `usage-limit:<contract_owner_user_id>:<projectId>`(30s TTL)をミラー更新する。旧 `PATCH /billing/monthly-usage-limits`(契約単位)を置き換える。各上限件数の MVP 単価に基づく円相当はサーバ側で算出してレスポンスに含める(表示用、判定には用いない)。

リクエスト: `{ "questionLimit": 5000, "faqLimit": 300, "chatRoomLimit": 100, "questionFreeQuota": 1000, "faqFreeQuota": 100, "chatRoomFreeQuota": 30, "alertEnabled": true, "alertFrequency": "staged" }`
レスポンス(200): `{ "projectId": "prj_...", "questionLimit": 5000, "faqLimit": 300, "chatRoomLimit": 100, "questionFreeQuota": 1000, "faqFreeQuota": 100, "chatRoomFreeQuota": 30, "alertEnabled": true, "alertFrequency": "staged", "yenEquivalent": { "question": 2500, "faq": 1500, "chatRoom": 3000 }, "updatedAt": "..." }`

バリデーション: 各上限件数は 1 件刻み、課金対象ごとの最小件数・最大件数(KV `usage-limit:min` / `usage-limit:max`)の範囲内。各無料枠は任意(省略時はデフォルト無料枠)で **当該課金対象の上限件数以下**。`alertFrequency` は `first` / `staged`。範囲外・無料枠超過は 422(E-INPUT-*)。当該プロジェクトに割当のないユーザーは 403(`E-AUTHZ-FORBIDDEN`)。
...
`kind` は `question_monthly_limit` / `faq_monthly_limit` / `chat_room_monthly_limit`。優先順位は「プロジェクト設定(`source='owner'`)> 運営者上書き(`source='operator'`)> デフォルト推奨値」(本 IF は `source='operator'` 行を作成・更新する)。

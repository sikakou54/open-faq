...
| チャット | `/chat/rooms`, `/chat/rooms/{id}`, `/chat/rooms/{id}/messages` | admin / end_user | SCR-013 |
...
レスポンス(201): `{ "inquiryId": "inq_...", "inquiryCode": "INQ-20260513-XYZ123", "faqRegistrationStatus": "unregistered" }`
...
#### 5.6.1 `GET /inquiries?status=unregistered&projectId=...&cursor=...`

#### 5.6.2 `GET /inquiries/{id}` / `PATCH /inquiries/{id}`

PATCH は担当者のみ更新可能。FAQ 登録状況は FAQ API から自動更新し、直接指定を拒否する。
```json
{ "assigneeUserId": "..." }
```

FAQ 下書きの初回保存時は `faq_registration_status=drafted`、FAQ 公開成功時は `registered` へ FAQ 保存処理と同一トランザクションで更新する。

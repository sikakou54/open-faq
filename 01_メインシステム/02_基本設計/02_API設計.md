...
- 対象: 利用者向け管理 API(認証 / プロジェクト / FAQ / 未解決質問 / ウィジェット設定 / 利用量・課金 / ユーザー管理 / お知らせ / 規約)、エンドユーザー向けウィジェット API、AI 推論抽象化 IF、メール配信抽象化 IF、顧客管理システムとの連携 IF #1〜#12(送信側)、外部 Webhook(Resend 等)
...
| メール確認 | `/auth/email-verifications/{token}` | トークン認証 | SCR-013 |
| メンバー管理 | `/projects/{id}/members`(一覧・招待), `/projects/{id}/members/{userId}`(ロール変更・割当解除), `/owner/members`(契約横断一覧), `/owner/members/{userId}`(契約削除), `/members/{id}/resend-invitation` | プロジェクト操作はオーナー / 該当PJ管理者、契約操作はオーナー専有 | SCR-009, SCR-009-M1, SCR-024 |
| プロジェクト管理 | `/projects`, `/projects/{id}` | オーナー専有(参照は該当PJ管理者にも許可)| SCR-004, SCR-004-M1, SCR-027 |
| FAQ 管理 | `/projects/{id}/faqs`, `/projects/{id}/faqs/{faqId}`, `/projects/{id}/faqs/import`, `/projects/{id}/faqs/import/template`, `/projects/{id}/faqs/export` | オーナー / 該当 PJ の `member`+ | SCR-006 / SCR-006-M1 |
| ウィジェット | `/widget/bootstrap`, `/widget/ask`, `/widget/feedback` | end_user(公開キー + セッション)| ウィジェット |
| 未解決質問 | `/inquiries`, `/inquiries/{id}` | オーナー / 該当 PJ の `member`+ | SCR-005 |
| 通知 | `/webhooks/resend` | 署名検証のみ | (Resend Webhook) |
| 利用量・課金 | `/usage`, `/owner/projects/usage`, `/billing/summary`, `/billing/subscription`, `/billing/invoices` | プロジェクト利用量は該当PJの `member`+、契約利用量・課金はオーナー専有 | SCR-021, SCR-022, SCR-023 |
| プロジェクトの質問数上限・アラート | `/projects/{id}/quota-limits`(GET), `/projects/{id}/quota-limits/questions`(PATCH)| 閲覧: オーナー / 該当 PJ の `member`+、変更: オーナー / 該当 PJ の `admin`(再認証必須)| SCR-021 / SCR-021-M1 |
| 操作履歴 | `/owner/activity` | オーナー専有 | SCR-025 |
| データ管理 | `/data/export` | admin / オーナー専有 | SCR-014 |
| お知らせ受信箱 | `/me/announcements`, `/me/announcements/{id}`, `/me/announcements/{id}/read` | admin 限定 | SCR-011, SCR-012 |
| 利用規約 | `/terms/current`, `/terms/agree` | 認証不要(閲覧)/ admin(同意)| SCR-010, SCR-015, SCR-018 |
| プライバシーポリシー | `/privacy/current`, `/privacy/agree` | 認証不要(閲覧)/ admin(同意)| SCR-020, SCR-015, SCR-018 |
...
`valid` カラムを持つテーブル(`users` / `contract_owners` / `project_users` / `projects` / `allowed_domains` / `faqs` / `question_logs` / `inquiries` / `billing_subscriptions`)に対する全 GET 系 API(一覧 / 詳細)のクエリは、原則として `WHERE <table>.valid=1` フィルタを追加する。論理削除済み(`valid=0`)の行は通常 API 経由では返却しない。
...
| 月間質問数(プロジェクト単位)| 80%(通知)| 100%(設定上限件数到達でウィジェット質問受付停止。無料枠超過分は事後課金、**無料枠超過自体は拒否しない**)| プロジェクト別上書き可(`project_quota_limits`、SCR-021 / SCR-093)| 125% で追加制限 | FR-122 |
...
詳細は [10_課金・請求設計.md §8](10_課金・請求設計.md) の選択閾値アラート / 100%停止 / 125%運営者通知を参照。
...
| 機能ID | F-016 |
| 認証 | 未認証可(トークン認証のみ)|
| 関連画面 | SCR-018 メンバーアカウント有効化(初期表示時)|

トークン形式 = `activation` purpose の `access_tokens.token_hash` 検証。検証成功時は招待情報(プロジェクト名 / 付与ロール / 招待元オーナー名 / 招待者メールアドレス)を返す。SCR-018 の招待情報パネル表示に使用する。Turnstile 不要(プレビューは情報取得のみで状態変更なし)。
...
| 関連画面 | SCR-018 メンバーアカウント有効化(「登録を完了する」押下時)|
...
#### 5.2.1 `GET /projects/{id}/members`

| 機能ID | F-015 |
| 必要権限 | オーナー / 当該プロジェクト管理者 |
| 関連画面 | SCR-009 |
...
`items[]` はURLで指定したプロジェクトの割当だけを返す。最終ログイン日時と他プロジェクトの割当は返さない。

#### 5.2.2 `GET /owner/members`

| 必要権限 | オーナー専有 |
| 関連画面 | SCR-024 契約メンバー |

契約内の全メンバーを横断表示する。レスポンスは `id`, `name`, `email`, `status`, `projectCount`, `projectGrants[]` を含む。認証監査用の最終ログイン日時は返さない。

#### 5.2.3 `POST /projects/{id}/members`(プロジェクト単発招待 / SCR-009-M1 招待モード正本)

| 認証 | Cookie + CSRF + 再認証必須 |
| 必要権限 | オーナー / 該当プロジェクトの `admin` ロール保持メンバー |
| 関連画面 | SCR-009-M1(プロジェクト管理者の招待操作)|
...
#### 5.2.4 `PATCH /projects/{id}/members/{userId}`(プロジェクト単位のロール変更 / SCR-009-M1 編集モード正本)

| 認証 | Cookie + CSRF + 再認証必須 |
| 必要権限 | オーナー / 該当プロジェクトの `admin` ロール保持メンバー |
| 関連画面 | SCR-009-M1(編集モードの「変更を保存」)|
...
#### 5.2.5 `DELETE /projects/{id}/members/{userId}`(プロジェクト割当解除)

| 認証 | Cookie + CSRF + 再認証必須 |
| 必要権限 | オーナー / 該当プロジェクトの `admin` ロール保持メンバー |
| 関連画面 | SCR-009-M1「プロジェクトから外す」(L1) |

当該プロジェクトの `project_users` 行だけを `valid=0` にする。`users`、他プロジェクトの `project_users`、セッション、招待トークンには影響させない。対象が自分自身またはオーナーの場合は拒否する。対象の他プロジェクト有効割当が0件の場合は `E-BIZ-MEMBER-NO-GRANT` で拒否する。

レスポンス(204): No Content
エラー: 403 `E-AUTHZ-OWNER-PROTECTED`、403 `E-AUTHZ-SELF-MUTATION`、403 `PROJECT_ACCESS_DENIED`、404 `NOT_FOUND`、409 `E-BIZ-MEMBER-NO-GRANT`

#### 5.2.6 `DELETE /owner/members/{userId}`(契約メンバーのアカウント論理削除)

| 認証 | Cookie + CSRF + 再認証必須(L3) |
| 必要権限 | オーナー専有 |
| 関連画面 | SCR-024「契約から削除」 |

対象ユーザーの `users.valid=0`、全 `project_users.valid=0`、全セッション失効、未使用招待トークン失効を同一トランザクションで実行する。オーナー自身は対象外。監査ログに対象プロジェクト一覧と件数を記録する。

レスポンス(204): No Content
エラー: 403 `E-AUTHZ-OWNER-ONLY`、403 `E-AUTHZ-OWNER-PROTECTED`、404 `NOT_FOUND`、409 `E-BIZ-ACCOUNT-INACTIVE`

#### 5.2.7 `POST /members/{id}/resend-invitation`(招待メール再送)

| 認証 | Cookie + CSRF + 再認証必須 |
| 必要権限 | オーナー / 該当プロジェクトの `admin` 保持メンバー |
| 関連画面 | SCR-009-M1(招待再送)|
...
- 他者をプロジェクト管理者として招待する操作は、プロジェクト作成後に当該プロジェクトの SCR-009 / SCR-009-M1(プロジェクト WS)から `POST /projects/{id}/members` 経由で行う。
...
| 必要権限 | **オーナー専有** |
| 関連画面 | `PATCH` / `DELETE`: SCR-027 プロジェクト設定 |
...
4. 関連テーブル(`allowed_domains` / `faqs` / `inquiries` / `question_logs`)の対象行を `valid=0, updated_at=now()` に伝播。匿名化モード(`contract_owners.data_deletion_mode='anonymize'`)の場合の即時匿名化は本処理では行わず、90 日後の物理削除バッチ時に実施する
...
FAQ の状態(`draft` / `published` / `hidden`)は `status` フィールドで指定し、`PATCH /faqs/{id}` の保存操作で相互に自由遷移できる(専用の公開 API・状態遷移ガードは持たない)。SCR-006 の「保存」ボタンは「状態」ラジオで選択した値をそのまま本 API に渡す。`status='published'` への変更は管理者(admin)が編集画面で内容を確認したうえで明示選択する操作であり、これが FR-045「FAQ の公開前に管理者が内容を確認できること」を満たす。`published` 設定時は `publishedAt` をサーバ側で記録する。

#### 5.4.3 `POST /faqs/import`

関連画面: SCR-006-M1 FAQ CSV インポートモーダル。

リクエスト: `multipart/form-data { file: <CSV> }`
受理形式は **CSV のみ**(UTF-8、BOM 許容、ヘッダ行必須、1 ファイル最大 1000 件)。**JSON 形式は受理しない**(不正形式は 415 / 400、E-INPUT-CSV-INVALID)。
CSV 列構成は `FAQ ID, 質問, 回答, カテゴリ`。各行の `FAQ ID` で **空欄 = 新規(`status=draft`)/ 既存 ID 一致 = 上書き(状態維持・`faq_revisions` に改訂追記)/ 当該契約に存在しない ID = 当該行を失敗**(行単位エラー、E-INPUT-CSV-FAQID-NOTFOUND)と判定する。`status` 列は受理しない。
レスポンス(202): `{ "jobId": "job_...", "status": "processing" }`

#### 5.4.4 `GET /faqs/import/template`

関連画面: SCR-006-M1「テンプレートをダウンロード」。
ヘッダ行のみの CSV テンプレート(`FAQ ID,質問,回答,カテゴリ`)を `text/csv; charset=utf-8`(UTF-8 BOM 付与可)で返す(200)。`Content-Disposition: attachment; filename="faq_import_template.csv"`。認可は `POST /faqs/import` と同一(オーナー / 該当 PJ の `member`+)。

#### 5.4.5 `GET /faqs/export`

関連画面: SCR-006 FAQ 管理一覧「CSV をエクスポート」。一覧フィルタ適用結果を **CSV(UTF-8、`text/csv`)** で返す(JSON 形式は提供しない)。
...
`unanswered` では質問ログと未解決質問を同一トランザクションで作成する。`inquiryCode` は管理用識別子としてレスポンス互換性のため返却するが、ウィジェットには描画しない。ウィジェットは回答できなかった旨と確認済みプロジェクト連絡先メールを表示し、FAQ質問入力は継続可能とする。連絡先未設定時は `contactEmail=null` を返す。冪等性は `questionLogId` を基準に担保する。
...
レスポンス(201): `{ "inquiryId": "inq_...", "inquiryCode": "INQ-20260513-XYZ123", "status": "open" }`

未解決質問登録ではチャット部屋を作成しない。

### 5.6 未解決質問 API

#### 5.6.1 `GET /inquiries?status=open&projectId=...&cursor=...`

`status` クエリは `open` / `closed` のいずれか(または未指定で両方)。
...
各フィールドはオプション(指定したものだけ更新)。状況は `open` ↔ `closed` の双方向遷移を許容(再オープン制限なし)。FAQ 下書き保存・FAQ 公開は本フィールドを変更しない(連動ロジックなし)。状態変更履歴の永続化は行わない。

### 5.7 利用量・課金 API

#### 5.7.1 `GET /usage?period=current_month&viewMode=owner|project&projectId={id}`

| 機能ID | F-120 |
| 関連画面 | `viewMode=owner`: SCR-022、`viewMode=project`: SCR-021 / SCR-008 |
| 必要権限 | `viewMode=owner` はオーナー専有(契約全体のサマリ = 全プロジェクトの SUM)。`viewMode=project` はオーナー / 該当プロジェクトの `admin` または `member`(当該プロジェクトの利用状況)|

`viewMode` は現在の管理範囲と連動する。オーナーは契約 / プロジェクトを切替可能。プロジェクト管理者・メンバーは `project` 固定とする。上限・無料利用枠はプロジェクト単位のため、`viewMode=owner` の `limit` / `freeQuota` は返さない。

レスポンス(200、`viewMode=project`):
```json
{
...
`viewMode=owner` のレスポンスは各課金対象を `{ "used": <全プロジェクト SUM> }` のみとする(`limit` / `freeQuota` / `percentage` は含めない。上限はプロジェクト単位のため契約全体の単一上限は存在しない)。プロジェクト別の上限・無料枠・消化率は §5.7.3 `GET /projects/{id}/quota-limits` を用いる。

#### 5.7.1a `GET /owner/projects/usage?period=current_month`

| 機能ID | F-120 |
| 関連画面 | SCR-022 契約利用状況 |
| 必要権限 | オーナー専有 |

プロジェクト別の質問数、FAQ件数、AI利用コスト、前期間比較、各プロジェクトの質問数上限に対する利用率を返す。

#### 5.7.2 `GET /billing/invoices?limit=6`
...
#### 5.7.3 `GET /projects/{id}/quota-limits`

| 機能ID | F-LIMIT-001 |
| 関連画面 | SCR-021 |
| 必要権限 | オーナー / 該当プロジェクトの `admin` または `member`(閲覧)|

当該プロジェクトの質問数について、利用状況・月次上限件数・アラート設定を返す。SCR-021 の表示には `used` / `limitEnabled` / `limit` / `percentage` / `yenEquivalent` だけを使用し、アラート設定・設定元は表示しない。SCR-021-M1 は `limitEnabled` / `limit` / `alertThresholds` を使用する。FAQ 件数は扱わないためレスポンスに含めない。質問数の無料利用枠は課金判定用の内部データとして保持するが、本APIのレスポンスには含めない。
...
`yenEquivalent` は上限件数をすべて利用した場合の最大課金額の参考値で、`billableCount = max(0, limit - 適用中の質問数無料利用枠)`、`yenEquivalent = billableCount × 質問単価` により算出する。画面では「5,000件 - 1,000件(無料枠) = 4,000件 (¥2,000 / 月)」の形式で表示する。

上限OFF時は `limitEnabled=false`、`limit=null`、`percentage=null`、`yenEquivalent=null` を返す。SCR-021 は「今月の利用上限」を「OFF」と表示し、「利用量」には割合・件数比・ProgressBar・状態バッジを描画しない。

#### 5.7.4 `PATCH /projects/{id}/quota-limits/questions`

| 認証 | 再認証必須 |
| 機能ID | F-LIMIT-002 / FR-121 / FR-125 / FR-127 |
| 関連画面 | SCR-021-M1 |
| 必要権限 | オーナー / 該当プロジェクトの `admin`(member は `E-AUTHZ-FORBIDDEN`)|

当該プロジェクトの質問数について、月次上限のON / OFF・月次上限件数・アラート閾値を更新する(SCR-021-M1)。`project_quota_limits.resource_kind='question_monthly_limit'` かつ `source='owner'` の 1 行だけを upsert する。質問数の `free_quota` と FAQ 件数の行は更新しない。

リクエスト例(ON): `{ "limitEnabled": true, "limit": 5000, "alertThresholds": [25, 50, 80, 90, 100] }`
リクエスト例(OFF): `{ "limitEnabled": false, "limit": null, "alertThresholds": [] }`
レスポンス(200): `{ "projectId": "prj_...", "limitEnabled": true, "limit": 5000, "alertThresholds": [25, 50, 80, 90, 100], "yenEquivalent": 2000, "source": "owner", "updatedAt": "..." }`

バリデーション: `limitEnabled=true` の場合、`limit` は必須かつ 1 件刻みで最小件数・最大件数の範囲内。`alertThresholds` は `25` / `50` / `80` / `90` / `100` の重複しない配列とし、サーバ側で昇順へ正規化する。空配列はアラート通知なし。`limitEnabled=false` の場合は `limit=null`、`alertThresholds=[]` とする。`freeQuota` / `alertEnabled` / `alertFrequency` を指定した場合は未サポート項目として 422。当該プロジェクトに割当のないユーザーは 403。

### 5.8 操作履歴 API

#### 5.8.1 `GET /owner/activity`

| 機能ID | F-AUDIT-READ-001 |
| 関連画面 | SCR-025 操作履歴 |
| 必要権限 | オーナー専有 |

クエリ: `from`, `to`, `actorUserId`, `action`, `projectId`, `result`, `cursor`。

レスポンスは `occurredAt`, `actor`, `action`, `targetType`, `targetLabel`, `project`, `result` を含む。トークン、認証秘密、本文PII、運営者専用内部メタデータは除外する。取得操作自体も `audit.read` として記録する。
...
利用規約とプライバシーポリシーは個別の文書として扱い、閲覧・同意ともに文書ごとに分離する(`terms_versions.doc_type` / `terms_agreements.doc_type` = `terms_of_service` / `privacy_policy`)。

#### 5.10.1 `GET /terms/current`(利用規約 最新版)

| 認証要否 | 不要 |
| 関連画面 | SCR-010 |

レスポンス(200): `{ "docType": "terms_of_service", "version": "...", "effectiveDate": "...", "bodyHtml": "..." }`(最新版のみ)

#### 5.10.1a `GET /privacy/current`(プライバシーポリシー 最新版)

| 認証要否 | 不要 |
| 関連画面 | SCR-020 |

レスポンス(200): `{ "docType": "privacy_policy", "version": "...", "effectiveDate": "...", "bodyHtml": "..." }`(最新版のみ)

#### 5.10.2 `POST /terms/agree`(利用規約 同意)

| 関連画面 | SCR-015 / SCR-018 |
| 関連テーブル | `terms_agreements`(`doc_type='terms_of_service'`)|

リクエスト: `{ "version": "..." }`。`terms_agreements` に `(user_id, doc_type='terms_of_service', terms_version)` で 1 行記録(冪等: 既存同意があれば 200)。

#### 5.10.2a `POST /privacy/agree`(プライバシーポリシー 同意)

| 関連画面 | SCR-015 / SCR-018 |
| 関連テーブル | `terms_agreements`(`doc_type='privacy_policy'`)|

リクエスト: `{ "version": "..." }`。`terms_agreements` に `(user_id, doc_type='privacy_policy', terms_version)` で 1 行記録(冪等: 既存同意があれば 200)。SCR-015 / SCR-018 では利用規約・プライバシーポリシー双方の同意が必須条件であり、改定再同意では改定された文書の agree のみを呼び出す。
...
動作:
1. JWT + Idempotency-Key 検証
2. `owner_quota_overrides` テーブル更新(レート種別のみ。契約単位)
3. KV キー `ratelimit:{contractOwnerUserId}:{kind}` 更新(即時反映)

> 本 IF はレート制限(契約単位)専用。月次上限件数の運営者上書きはプロジェクト単位のため §5.13.4a を用いる。

#### 5.13.4a IF #5(月次上限件数上書き受信 / プロジェクト単位)

```
POST /internal/admin-integration/v1/usage-limit/override
```
...
```json
{
  ...
  "projectId": "prj_...",
  "overrideId": "...",
  "limits": [{ "kind": "question_monthly_limit", "limit": 8000, "freeQuota": 2000, "validUntil": "..." }],
  "reason": "...",
  "operatorAccountId": "..."
}
```
...
動作:
1. JWT + Idempotency-Key 検証(4-eyes 承認済みであること)
2. `project_quota_limits` テーブル更新(`source='operator'`、`project_id` で対象プロジェクトを特定、`reason` / `valid_until` / `created_by` を記録)
3. KV キー `usage-limit:{contractOwnerUserId}:{projectId}` 更新(30s TTL、即時反映)

`kind` は `question_monthly_limit` / `faq_monthly_limit`。質問数は `limit` と `freeQuota`、FAQは運営者が管理する `freeQuota` だけを受け付ける。FAQの `limit` は常に NULL とし、指定時は 422。質問数の優先順位は「プロジェクト設定 > 運営者上書き > デフォルト推奨値」、FAQ無料枠は「運営者上書き > デフォルト値」。
...
詳細は運営者側 [03_API設計書.md §5.7](../../02_運営者システム/個別設計書群/03_API設計書.md) を参照。

...
| 利用量・課金 | `/usage`, `/billing/summary`, `/billing/subscription`, `/billing/invoices`, `/billing/monthly-usage-limits` | ダッシュボード閲覧: オーナー / 該当 PJ の `member`+、課金操作: オーナー専有 | SCR-015 |
...
#### 5.8.3 `PATCH /billing/monthly-usage-limits`

| 認証 | 再認証必須 |
| 機能ID | FR-121 / FR-125 / FR-127 |

課金対象ごとの月次上限件数を一括更新する(SCR-015-M1)。`owner_quota_overrides`(`resource_kind` = `question_monthly_limit` / `faq_monthly_limit` / `chat_room_monthly_limit`)に保存。各上限件数の MVP 単価に基づく円相当はサーバ側で算出してレスポンスに含める(表示用、判定には用いない)。

リクエスト: `{ "questionLimit": 5000, "faqLimit": 300, "chatRoomLimit": 100 }`
レスポンス(200): `{ "questionLimit": 5000, "faqLimit": 300, "chatRoomLimit": 100, "yenEquivalent": { "question": 2500, "faq": 1500, "chatRoom": 3000 }, "updatedAt": "..." }`

バリデーション: 各上限件数は 1 件刻み、課金対象ごとの最小件数・最大件数(KV `usage-limit:min` / `usage-limit:max`)の範囲内。範囲外は 422(E-INPUT-*)。

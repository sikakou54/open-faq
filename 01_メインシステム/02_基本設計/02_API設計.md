...
| メンバー管理 | `/projects/{id}/members`(一覧・招待), `/projects/{id}/members/{userId}`(ロール変更・割当解除), `/members/{id}/resend-invitation` | オーナー / 該当PJ管理者 | SCR-009, SCR-009-M1 |
| プロジェクト管理 | `/projects`, `/projects/{id}` | オーナー専有(参照は該当PJ管理者にも許可)| SCR-004, SCR-004-M1 |
...
| 利用量・課金 | `/usage`, `/owner/projects/usage`, `/billing/summary`, `/billing/subscription`, `/billing/invoices` | プロジェクト利用量は該当PJの `member`+、契約利用量・課金はオーナー専有 | SCR-016, SCR-021, SCR-022 |
...
| データ管理 | `/data/export`(契約全データエクスポート、**MVP 対象外・将来対応**)| admin / オーナー専有 | -(UI 導線なし)|
...
#### 5.2.2 `POST /projects/{id}/members`(プロジェクト単発招待 / SCR-009-M1 招待モード正本)
...
#### 5.2.3 `PATCH /projects/{id}/members/{userId}`(プロジェクト単位のロール変更 / SCR-009-M1 編集モード正本)
...
#### 5.2.4 `DELETE /projects/{id}/members/{userId}`(プロジェクト割当解除)

| 認証 | Cookie + CSRF + 再認証必須 |
| 必要権限 | オーナー / 該当プロジェクトの `admin` ロール保持メンバー |
| 関連画面 | SCR-009-M1「プロジェクトから外す」(L1) |

対象プロジェクトの `project_users.valid=0` を更新する。対象が自分自身またはオーナーの場合は拒否する。

- 対象ユーザーに他プロジェクトの有効割当が残る場合: `users`、他プロジェクトの割当、セッション、招待トークンは維持する。
- 他プロジェクトの有効割当が0件になる場合: 同一トランザクションで `users.valid=0`、全セッション失効、未使用招待トークン失効を実行する。

レスポンス(200): `{ "userId": "...", "projectId": "...", "accountDeactivated": true|false }`
エラー: 403 `E-AUTHZ-OWNER-PROTECTED`、403 `E-AUTHZ-SELF-MUTATION`、403 `PROJECT_ACCESS_DENIED`、404 `NOT_FOUND`

#### 5.2.5 `POST /members/{id}/resend-invitation`(招待メール再送)
...
| 必要権限 | **オーナー専有** |
| 関連画面 | `PATCH` / `DELETE`: SCR-004-M1 プロジェクト作成・編集モーダル(編集モード)|
...
#### 5.7.1 `GET /usage?period=current_month&viewMode=project&projectId={id}`

| 機能ID | F-120 |
| 関連画面 | SCR-021 / SCR-008 |
| 必要権限 | オーナー / 該当プロジェクトの `admin` または `member` |

プロジェクト単位の利用量を返す。契約全体の利用状況は §5.7.2 `GET /owner/projects/usage` を使用する。
...
#### 5.7.2 `GET /owner/projects/usage?period=current_month`

| 機能ID | F-120 |
| 関連画面 | SCR-016 利用状況 |
| 必要権限 | オーナー専有 |

契約全体サマリーとプロジェクト別の質問数、質問数上限に対する利用率、公開FAQ件数、最終更新日時を返す。
...
#### 5.7.3 `GET /billing/summary?period=current_month`
...
#### 5.7.4 `GET /billing/invoices?limit=6`
...
#### 5.7.5 `GET /projects/{id}/quota-limits`
...
#### 5.7.6 `PATCH /projects/{id}/quota-limits/questions`
...
#### 5.8.1 `GET /me/announcements?cursor=...`
...
#### 5.8.2 `POST /me/announcements/{id}/read`

レスポンス(200): `{ "id": "...", "readAt": "..." }`

#### 5.8.3 `GET /me/announcements/unread-summary`
...
#### 5.9.1 `GET /terms/current`(利用規約 最新版)

| 認証要否 | 不要 |
| 関連画面 | SCR-010 / SCR-015 |

レスポンス(200): `{ "docType": "terms_of_service", "version": "...", "effectiveDate": "...", "bodyHtml": "...", "diffSummary": "..." }`(最新版のみ。`diffSummary` は `terms_versions.diff_summary`。SCR-015 の「主な変更点」表示に用いる。NULL の場合は省略)

#### 5.9.1a `GET /privacy/current`(プライバシーポリシー 最新版)

| 認証要否 | 不要 |
| 関連画面 | SCR-020 / SCR-015 |

レスポンス(200): `{ "docType": "privacy_policy", "version": "...", "effectiveDate": "...", "bodyHtml": "...", "diffSummary": "..." }`(最新版のみ。`diffSummary` は `terms_versions.diff_summary`。SCR-015 の「主な変更点」表示に用いる。NULL の場合は省略)

#### 5.9.2 `POST /terms/agree`(利用規約 同意)
...
#### 5.9.2a `POST /privacy/agree`(プライバシーポリシー 同意)
...
#### 5.9.3 `POST /withdrawal-requests`(退会申請)
...
### 5.10 AI 推論 IF(`AnswerProvider` 抽象化)
...
### 5.11 メール配信 IF(`EmailProvider` 抽象化)
...
### 5.12 連携 IF(受信側、メイン側エンドポイント)

#### 5.12.1 IF #1 契約停止イベント受信(顧管 → メ)
...
> 本 IF はレート制限(契約単位)専用。月次上限件数の運営者上書きはプロジェクト単位のため §5.8.4a を用いる。

#### 5.12.4a IF #5(月次上限件数上書き受信 / プロジェクト単位)
...
#### 5.12.7 IF #8 監視メトリクス取得(送信側、メ → 顧管)
...
#### 5.12.9 IF #10 課金 Webhook 受信(顧管 → メ)
...
### 5.13 外部 Webhook(Resend 等)

#### 5.13.1 `POST /webhooks/resend`

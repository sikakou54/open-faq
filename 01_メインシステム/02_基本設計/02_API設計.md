...
プロジェクト別の質問数、FAQ件数、チャット部屋数、AI利用コスト、前期間比較、各プロジェクトの質問数上限消化率を返す。FAQ・チャットの上限消化率は返さない。
...
上限OFF時は `limitEnabled=false`、`limit=null`、`percentage=null`、`yenEquivalent=null` を返す。SCR-036 は「今月の利用上限」を「OFF」、「上限消化率」を「対象外」と表示し、ProgressBarを描画しない。

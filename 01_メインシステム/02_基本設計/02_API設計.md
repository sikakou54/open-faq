...
- 認証ミドルウェアは `accounts.valid=0` 行へのログインを 401 で拒否する(セッション内 `accountId` の `valid` を確認、`valid=0` ならセッションも失効扱い)
...
3. `UPDATE sessions SET revoked_at=now() WHERE account_id=? AND revoked_at IS NULL`(全セッション失効)

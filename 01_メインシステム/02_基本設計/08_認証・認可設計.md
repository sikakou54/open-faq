...
- `payment_failure_grace_expired` / `quota_exceeded_no_payment_method`: 既存セッション継続(課金 / エクスポート / 退会のみ)
...
- `quota_exceeded_no_payment_method`(無料枠超過 + 支払方法未登録、支払方法ゲート FR-136): オーナーが課金画面から支払方法を登録(Setup Intent 成功)した時点でメイン側が即時 `active` へ復帰させる。

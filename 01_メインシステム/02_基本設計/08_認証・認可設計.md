...
ロールベース(契約ロール `users.role` = `admin` / `service_operator` / `end_user` + プロジェクト別ロール `project_users.role` = `admin` / `member` + オーナー判定 = `contract_owners` 行存在)
...
| ユーザー種別 | DB 値 | 概要 |
|---|---|---|
| オーナー | `users.role='admin'` AND `contract_owners.user_id = users.id` 行が存在 | 契約あたり 1 オーナー固定(`contract_owners.user_id PRIMARY KEY` で DB レベル担保)。全権 + オーナー専有機能 |
| プロジェクト管理者 / メンバー | `users.role='admin'` AND `contract_owners` 行を持たない AND `project_users` 行を保持 | プロジェクト別ロール(`project_users.role` = `admin`/`member`)で操作範囲が決まる |
| エンドユーザー | `users.role='end_user'` | 自分の問い合わせ ID に紐づくチャットのみ |
| サービス運営者 | `users.role='service_operator'` | 本 SaaS 提供者(運営者側、メイン側 D1 には格納しない論理値)|

`users.role` 値域: `admin` / `service_operator` / `end_user` の 3 値固定(CHECK 制約は `admin` / `end_user` のみ、`service_operator` は別 D1 管理)

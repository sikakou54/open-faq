...
| 観点 | 仕様 |
|---|---|
| 設定権限 | オーナー / `project:manage` フラグ保持 + 該当 `account_project_grants` あり |
...
4. **アカウント状態確認**(`accounts.status='active'`)── `pending_activation` / `pending_verification` なら 403
...
| メンバー招待 / 編集 / 招待再送 / 削除 | SCR-017-M1 | `POST /members/*`, `DELETE /members/{id}` |

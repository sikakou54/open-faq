...
| プロジェクト単位 IP 許可リスト(FAQ ウィジェット向け、FR-179 / FR-330)| `project_ip_allowlist` テーブル + `PATCH /projects/{id}/ip-allowlist` API + SCR-010-M1 UI で実装。**管理画面 API は本設定の評価対象外**。詳細は [02_API設計.md §5.5.0](02_API設計.md) を正本とする | (本書内 §11 参照)|
...
3. `accounts.status='active'` + 当該プロジェクト分の `account_project_grants` 行を反映(他プロジェクトに参加させる場合は、対象プロジェクトの WS で個別に招待操作を行い、名前解決により既存アカウントに行が追加される)
4. SCR-001 へ
...
| メンバーアカウント | `accounts.role='admin'` AND `accounts.is_owner=0` AND `accounts.owner_account_id` に所属オーナーの id | プロジェクト別ロール(`account_project_grants.role` = `admin`/`member`)で操作範囲が決まる |
...
- プロジェクトの作成・編集・削除(プロジェクト設定全般: 名称・許可ドメイン・IP 許可リスト・ウィジェット鍵ローテーション 等)
- 課金(SCR-015 の月次予算変更 / 支払い方法変更)
- 退会(SCR-024)
- 規約再同意(SCR-025 の同意 / 不同意)
- メンバーアカウント物理削除(`DELETE /projects/{id}/members/{accountId}`、v1.7 で再定義)── オーナー操作時は全メンバー削除可、プロジェクト管理者(オーナー以外)操作時は当該プロジェクトの member のみ削除可(他 admin は member 在籍中削除不可)
...
### 5.6 プロジェクト単位 IP 許可リスト(FR-179 / FR-330)

FAQ ウィジェット(エンドユーザーアクセス)に対する IP 制限はプロジェクト単位で `project_ip_allowlist` に保持する。本書 §6.2 認可判定の順序では扱わず、**ウィジェット系エンドポイントの前段ガード**(ドメイン検証 / レート制限と同列)として独立評価する。詳細は [02_API設計.md §5.5.0](02_API設計.md) を正本とする。
...
Principal {
  ...
  projectRoles:   Map<string /*projectId*/, 'admin' | 'member'>   // v1.10: オーナーも account_project_grants を持つが認可は isOwner=true bypass を優先
}
```

#### 認可ミドルウェア実装

- **v1.10**: オーナーもプロジェクト作成時に自動で `account_project_grants(role='admin', valid=1)` 行を保持するため、`accounts` と `account_project_grants` を 1 クエリで結合する処理はオーナー / メンバー共通(従来「オーナーは grants を引かない」の特殊分岐を廃止)。ただし認可上は `isOwner=true` 判定が成立した時点で `projectRoles` の中身を参照せず bypass で全プロジェクト許可する(認可優先順位)
- `projectRoles` は通知宛先解決と画面表示の事実情報として参照される(空 Map ではなく、オーナー自身の admin 行も含む)
- 取得クエリには `WHERE account_project_grants.valid=1` フィルタを必須付与(論理削除済み行は除外)
- 認可判定ヘルパ:
...
2. **アカウント論理削除確認**(v1.10、`accounts.valid=1`)── `valid=0` なら 401(セッション失効扱い、ログイン画面へリダイレクト)
3. **規約再同意状態確認**(`terms_agreements` 最新版)── 未同意 → SCR-025 へリダイレクト
4. **契約状態確認**(`accounts.contract_status`、オーナー行)── `suspended` で許可機能外なら 403、`deleted_pending` / `deleted` なら 403
5. **アカウント状態確認**(`accounts.status='active'`)── `pending_activation` / `pending_verification` なら 403
...
7. **オーナー専有機能判定**(`requireOwner`、`is_owner=0` でプロジェクト作成 / 課金 / 退会 / 規約再同意 等にアクセス)── 違反 → 403(`E-AUTHZ-OWNER-ONLY`)
...
### 6.3 `critical` 通知の宛先解決(共有概念正本、v1.10 簡素化)

`critical` 重要度の通知 = メール強制送信。送信先は **「`account_project_grants.role='admin' AND valid=1` を 1 件でも保持するアカウント」**(v1.10 でオーナー自身もプロジェクト作成時に admin 行を自動取得するため、本クエリ単独でオーナー + プロジェクト管理者を網羅できる)。
...
> **v1.10 重要**: 従来は `WHERE a.is_owner=1 OR EXISTS (...admin row)` の OR 条件でオーナーとプロジェクト管理者を UNION していたが、v1.10 でオーナー自身が admin 行を持つようになったため、admin 行の EXISTS 条件単独で十分。`DISTINCT` でメンバーが複数プロジェクトで admin を持つ場合の二重カウントを防ぐ。オーナー側も grants 行が 1 件以上あるため `DISTINCT` 対象に含まれる(二重カウント無し)。

部分インデックス `idx_account_project_grants_role_admin`(`WHERE role = 'admin' AND valid = 1`)で高速化。詳細は [06_メッセージ一覧.md §2.4](06_メッセージ一覧.md) も参照。
...
| メンバー招待 / ロール変更 / アカウント論理削除(v1.10)/ 招待再送 | SCR-017-M1(プロジェクト WS)| `POST /projects/{id}/members`, `PATCH /projects/{id}/members/{accountId}`, `DELETE /projects/{id}/members/{accountId}`, `POST /members/{id}/resend-invitation` |
| 月次予算上限変更 | SCR-015 | `PATCH /billing/monthly-budget-limit` |
| プロジェクト削除(論理削除、v1.10)| **SCR-026 のみ**(v1.10 で SCR-010 から動線移管)| `DELETE /projects/{id}` |
...
| 2026-05-19 | 1.10 | **オーナー固定化 + 論理削除統一**。① §6.1 Principal 構築の「オーナーは `account_project_grants` を引かない」分岐を撤回し、オーナー / メンバー共通で 1 クエリ取得(`WHERE valid=1` フィルタ必須)。認可上は `isOwner=true` bypass を優先する優先順位は維持。② §6.2 認可判定の順序にステップ 2「アカウント論理削除確認(`accounts.valid=1`)」を新設(`valid=0` 行への API アクセスは 401)。③ §6.3 critical 通知の宛先解決クエリを簡素化(オーナー UNION を撤去し、`account_project_grants.role='admin' AND valid=1` の EXISTS 条件単独で十分。オーナーも v1.10 で admin 行を保持するため網羅される)。④ §6.4 重要操作の再認証表で「プロジェクト削除」の関連画面を SCR-010 → SCR-026 に変更、「メンバー削除」を物理削除 → 論理削除に書き換え | claude |

...
5. 既定の管理範囲に応じて SCR-026 契約概要または SCR-015 概要へ
...
- 料金・請求(SCR-038)と契約設定(SCR-041)── 質問数上限・アラート変更はプロジェクト単位(SCR-036-M1)で該当PJ管理者にも付与する
- 退会(SCR-024)
...
| 課金情報変更 | SCR-038 | `PATCH /billing/payment-method` |
| メンバー招待 / ロール変更 / プロジェクト割当解除 / 招待再送 | SCR-017-M1 | `POST /projects/{id}/members`, `PATCH /projects/{id}/members/{userId}`, `DELETE /projects/{id}/members/{userId}`, `POST /members/{id}/resend-invitation` |
| 契約メンバーのアカウント論理削除 | SCR-039 | `DELETE /owner/members/{userId}` |
| プロジェクトの質問数上限・アラート変更 | SCR-036-M1(オーナー / 当該 PJ 管理者)| `PATCH /projects/{id}/quota-limits/questions` |
| プロジェクト削除(論理削除)| SCR-042 プロジェクト設定 | `DELETE /projects/{id}` |

...
| 対象テーブル | カラム | 説明 |
|---|---|---|
| `users` | `email_key_version` | オーナー / メンバーのメールアドレス暗号化バージョン |
...
**ローテーション完了の判定**:`SELECT COUNT(*) FROM users WHERE email_key_version < N` が 0 件、かつ `SELECT COUNT(*) FROM inquiry_contacts WHERE email_key_version < N` が 0 件であること。

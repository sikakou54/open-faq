...
- **最小権限原則**(オーナー専有 / プロジェクト別ロール `admin` / `member`(`project_users.role`)の二段階)
...
|---|---|---|---|---|
| エンドユーザーメール | AES-256-GCM | オーナー派生鍵(`deriveOwnerKey()`)| `end_users.email_encrypted` + `email_hmac`(HMAC で検索用)| 不可 |
| 利用者メール | AES-256-GCM | オーナー派生鍵 | `users.email_encrypted` + `email_hmac` | 不可 |
...
| `end_users` | DELETE | `email_encrypted` → `anon_<sha256>`、`display_name` → 「匿名」 |
...
- 設定管理: `contract_owners.data_deletion_mode` は契約締結時に既定 `physical`。MVP では UI 上の変更操作を提供せず、変更が必要な場合はサポート窓口経由の運営者対応とする(SCR-028 個人設定にデータ保持・削除モード設定 UI は持たない)
...
重複生成対策(R-012): 同一 `(contract_owner_user_id, event_kind, dedup_key)` で 60 分以内の複数発火は 1 件に集約
...
| 対象 | 制御方法 | 関連設計書 |
|---|---|---|
| 利用者管理画面 | セッション + プロジェクト別ロール(`project_users.role`)+ オーナー境界 | [09_認証認可設計書.md](09_認証認可設計書.md) |
...
- ハッシュチェーン実装: 各行に `prev_hash`(直前行の `current_hash`) と `current_hash`(`SHA-256(id, contract_owner_user_id, actor_user_id, action, target_type, target_id, payload, created_at, prev_hash)`)
...
| **API キー** | 動的生成・管理者操作で再発行 | ウィジェット公開キー(`pk_live_<32-char base62>`)| 無期限。失効は再発行(ローテーション)でのみ。再発行時に旧キーを 24 時間猶予で失効(FR-193 / FR-194)|
...
| API キーローテーション猶予 | 24 時間(FR-193)、SCR-014 に「旧キー使用中」バッジ表示 |
...
dual-decrypt 期間中に「どのレコードがどの世代の鍵で暗号化されているか」を明示するため、PII 暗号化列を持つテーブルに `email_key_version INTEGER NOT NULL DEFAULT 1` を保持する。

| 対象テーブル | カラム | 説明 |
|---|---|---|
| `users` | `email_key_version` | 管理者(オーナー / プロジェクト管理者 / メンバー)のメールアドレス暗号化バージョン |
| `end_users` | `email_key_version` | エンドユーザー(ウィジェット利用者)のメールアドレス暗号化バージョン |
...
**ローテーション完了の判定**:`SELECT COUNT(*) FROM users WHERE email_key_version < N` が 0 件、かつ `SELECT COUNT(*) FROM end_users WHERE email_key_version < N` が 0 件であること。

**例外**:`contract_owners.contract_status='deleted'` または `users.valid=0` の論理削除済み行は、90 日経過後の物理削除バッチに任せ、再暗号化対象外とする(復元時の dual-decrypt 整合性を維持するため、ローテーション期間は 60 日 + 物理削除遅延 90 日を考慮した運用窓設計とする)。

### 8.3 ウィジェット API キー(`projects` / `project_legacy_keys`、§8.3.4 から移管)

- 形式: `pk_live_<32-char base62>`
- 有効期限: なし(無期限)。失効は再発行(ローテーション)でのみ行う
- ローテーション: 旧キー 24 時間併存
...
| 観点 | 対策 | 関連設計書 |
|---|---|---|
| **オーナー間データ漏洩防止** | 全テーブルに `contract_owner_user_id` + API 層でオーナー境界判定(**404 偽装**)+ 自動テスト(基本設計書 §14.3、月次 50 ケース)| [04_テーブル定義書.md](04_テーブル定義書.md) / [09_認証認可設計書.md](09_認証認可設計書.md) |
| **管理者権限の濫用防止** | 監査ログ全件 + プロジェクト別ロール(`admin` / `member`)の最小化 + オーナー専有機能の `project_users.role` 付与不可(プロジェクト作成・編集・削除・課金・退会・規約再同意はオーナー専有。メンバーアカウント物理削除はオーナーおよび当該プロジェクト管理者だが、プロジェクト管理者は member 在籍中は他 admin を削除不可)| [05_権限設計書.md](05_権限設計書.md) / §7 |
...
| イベント | アクション | 復帰方法 |
|---|---|---|
| ハードバウンス | 即時サプレス、全契約横断共有 | SCR-041 契約設定からサポート申請 → 運営者承認(IF #5)→ 復帰 |

### 11.11 契約操作履歴の参照安全性

SCR-040 / `GET /owner/activity` は自契約のオーナーに限定し、認証秘密、トークン、本文PII、内部スタックトレースを返さない。対象名は画面識別に必要な最小限に限定し、操作履歴の取得自体も `audit.read` として監査ログへ記録する。

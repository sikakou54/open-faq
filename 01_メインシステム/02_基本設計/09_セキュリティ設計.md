...
- 設定管理: `accounts.data_deletion_mode` は契約締結時に既定 `physical`。MVP では UI 上の変更操作を提供せず、変更が必要な場合はサポート窓口経由の運営者対応とする(SCR-016 にデータ保持・削除モード設定 UI は持たない)
...
| **管理者権限の濫用防止** | 監査ログ全件 + プロジェクト別ロール(`admin` / `member`)の最小化 + オーナー専有機能の `account_project_grants.role` 付与不可(プロジェクト作成・編集・削除・課金・退会・規約再同意・アカウント完全削除はオーナー専有)| [05_権限設計書.md](05_権限設計書.md) / §7 |

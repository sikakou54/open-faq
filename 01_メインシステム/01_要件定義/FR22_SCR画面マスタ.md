...
| SCR-010-M1 | プロジェクト設定モーダル(新規作成 / 編集) | オーナー専有 | FR-030〜035, FR-033a, FR-033c, FR-179, FR-330 | P0 | メイン |
...
| ~~SCR-016~~ | (廃止) 旧「設定(退会・課金画面入口)」── 連絡先 / 退会導線は SCR-028 アカウント設定に統合 | — | — | — | — |
| SCR-017 | ユーザー管理(プロジェクト WS のみ・当該プロジェクト範囲。v1.7 で契約 WS 視点は廃止) | 管理者ユーザー(オーナー / プロジェクト管理者) | FR-017, FR-019, FR-019a, FR-019b, FR-021a〜c | P0 | メイン |
...
| ~~SCR-030~~ | (廃止)v2.0 で SCR-030 を廃止し、プロジェクト情報(プロジェクト名・許可ドメイン・IP 許可リスト)は SCR-015 プロジェクトホームの「プロジェクト情報」パネルに統合。通知設定は機能ごと撤廃(常時 ON、画面設定なし)| — | — | — | — |
...
- 利用者管理コンソールは「契約ワークスペース」(オーナー専有)と「プロジェクトワークスペース」(オーナー / プロジェクト管理者 / メンバー)の 2 軸で構成し、各 SCR はいずれかのワークスペースに配置される(SCR-001/002/003/018/021/022/023/027/028/029 は共通領域として両ワークスペース外で扱う)。配置の正本は基本設計 01_画面設計.md §4 とする。

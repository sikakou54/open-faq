...
1. **DDL マイグレーション(forward-only)**:
   - 11 テーブル(`accounts` / `account_project_grants` / `projects` / `allowed_domains` / `project_ip_allowlist` / `faqs` / `question_logs` / `inquiries` / `end_users` / `chat_rooms` / `billing_subscriptions`)に `valid INTEGER NOT NULL DEFAULT 1 CHECK (valid IN (0,1))` カラムを追加(v1.10 当時の表現。v1.13 で `inquiry_contacts` → `end_users` に置換)
...
**前提**: MVP 未リリースのためデータ移行不要。新規 DDL のみ。`users.role='end_user'` 行 + `inquiry_contacts` の二重管理を解消し、エンドユーザーを独立 `end_users` テーブルに集約する。新スキーマは [../02_基本設計/03_テーブル設計.md](../02_基本設計/03_テーブル設計.md) §3.1 / §3.5 / §3.15 / §3.17 / §3.20 / §3.24 を正本とする。
...
| M13-3 | 新規 DDL を適用(基本設計 03_テーブル設計.md §3.17 を正本):<br>・`end_users`(エンドユーザー、プロジェクト単位スコープ、`(project_id, email_hmac) WHERE valid=1` 部分 UNIQUE、`users` と独立)| CREATE TABLE | 低 |

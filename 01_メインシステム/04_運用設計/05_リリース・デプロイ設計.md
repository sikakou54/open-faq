...
> **注記**: 本セクション(M1〜M10、§5a / §5b)は **v1.10 リファクタ時点の手順記録**(履歴)である。**MVP 未リリースのため、その後の v1.11 でユーザー情報管理を 3 表分割(`users` / `contract_owners` / `project_users`)へ再リファクタリングしたため、本セクション内の `accounts` / `account_project_grants` を直接参照する SQL や手順は新スキーマでは適用できない**。新規環境への適用手順は §5c「v1.11 ユーザーマスタ 3 表分割マイグレーション」を参照する。本セクションは設計判断の文脈保持(なぜ valid カラムを全テーブルに付けたか、なぜ削除動線が SCR-026 集約になったか等)のため残置する。なお、その後の改訂で**プロジェクト削除動線は SCR-026 から SCR-015 プロジェクトホームのプロジェクト情報パネル(オーナー専有 DangerSection)へ移設**された(一覧画面の「操作」列撤去 + 行内リンク遷移への統一に伴う。FR-030 / 基本設計 §5.10 が現行の正本)。本履歴中の「SCR-026 にプロジェクト削除動線を追加」は v1.10 時点の記録であり現行仕様ではない。

本変更を本番反映する際の手順(ゼロダウンタイム前提):

1. **DDL マイグレーション(forward-only)**:
   - 10 テーブル(`accounts` / `account_project_grants` / `projects` / `allowed_domains` / `faqs` / `question_logs` / `inquiries` / `end_users` / `chat_rooms` / `billing_subscriptions`)に `valid INTEGER NOT NULL DEFAULT 1 CHECK (valid IN (0,1))` カラムを追加(v1.10 当時の表現。v1.13 で `inquiry_contacts` → `end_users` に置換。プロジェクト単位 IP 許可リスト機能 `project_ip_allowlist` は撤去済み)
...
| M6 | `end_users.email_key_version` を ADD COLUMN(v1.13 で `inquiry_contacts` 改名・吸収済み)| ALTER TABLE | 低 |
...
- **個別チャット**: `chat_rooms` は部屋状態・リマインド状態を持たず、投稿・既読・通知・再入室のみを扱う
...
| M11-3 | 9 テーブルの FK 書換 + カラムリネーム(契約境界系 → `contract_owners(user_id)`、操作者系 → `users(id)`)<br>・契約境界(CASCADE):`projects.contract_owner_user_id` / `faqs.contract_owner_user_id` / `inquiries.contract_owner_user_id` / `question_logs.contract_owner_user_id` / `billing_subscriptions.contract_owner_user_id` / `billing_invoices.contract_owner_user_id` / `usage_metering.contract_owner_user_id` / `owner_quota_overrides.contract_owner_user_id` / `audit_logs.contract_owner_user_id` / `notification_logs.contract_owner_user_id` / `inbox_messages.contract_owner_user_id` / `withdrawal_requests.contract_owner_user_id` / `ai_threshold_persistent_cache.contract_owner_user_id` / `error_logs.contract_owner_user_id`<br>・操作者(SET NULL):`faqs.created_by` / `faqs.updated_by` / `projects.default_assignee_user_id` / `chat_messages.actor_id` / `sessions.user_id` / `access_tokens.user_id` / `terms_agreements.user_id` / `project_users.granted_by` / `audit_logs.actor_user_id` / `error_logs.user_id`<br>・操作者(NO ACTION):`chat_rooms.assignee_user_id`(NOT NULL のため SET NULL 不可。担当管理者の論理削除時はアプリ層でデフォルト担当者→オーナーへ付け替え、DD01/DD06 §3.4a) | テーブル再作成 | **高**(参照先広範) |
...
### 5c.1a マイグレーション M12: 月次上限件数・無料枠のプロジェクト単位化(forward-only)

レート制限は契約単位(`owner_quota_overrides`)に残し、月次上限件数・無料枠をプロジェクト単位(`project_quota_limits`)へ移行する。

| ステップ | 内容 | 方式 | リスク |
|---|---|---|---|
| M12-1 | `project_quota_limits` を新規作成(基本設計 03_テーブル設計.md §3.36)。FK `project_id`→`projects(id)` CASCADE / `contract_owner_user_id`→`contract_owners(user_id)` CASCADE | CREATE TABLE | 低 |
...
| M12-3 | **`usage_metering` 履歴バックフィル**: 当月(オープン月)は §DD12 §3.4 の (owner, project) 単位再集計で再生成。確定済み過去月は契約代表プロジェクト(当該契約の最古 `projects.id`)に `project_id` を割当てて一意制約を満たす(請求は契約キーで確定済みのため過去 `billing_invoices` は不変)| データ移行 | **高**(唯一の難所。staging で件数照合)|
| M12-4 | 既存 `owner_quota_overrides` の月次上限件数行(`*_monthly_limit`)を、各オーナー配下の全プロジェクトの `project_quota_limits`(`source='owner'`、初期値 = 旧契約値)へ展開 INSERT | データ移行 | 中 |
| M12-5 | `owner_quota_overrides.resource_kind` の CHECK から 3 つの `*_monthly_limit` を撤去(レート 5 種別のみに縮小)+ 旧月次上限行を削除 | DDL + DELETE | 中 |
| M12-6 | KV 再構築: `usage-limit:<owner>:<projectId>` を `project_quota_limits` から生成、`usage-limit:default:<kind>` / `usage-limit:free-default:<kind>` を運営者定義値で初期化 | スクリプト | 低 |
| M12-7 | アプリ層: `PATCH /billing/monthly-usage-limits` を `GET/PATCH /projects/{id}/quota-limits` に置換、ウィジェット停止・支払方法ゲート判定を `usage-limit:<owner>:<projectId>` 参照に変更(DD13 §3.7a / DD14 §3.1.3)| アプリ改修 | 中 |
| M12-8 | `bash 99_script/check-spec-sync.sh` で SC-001〜SC-011 グリーン確認(SC-004 に SCR-036 / SCR-036-M1 が含まれること)| スクリプト | — |
...
`deriveOwnerKey(masterKey, ownerId)` の引数 `ownerId` セマンティクスを「`contract_owners.user_id`(= 旧 `accounts.id` と物理値同じ ULID)」に再定義。DD11 §10.9.2 の HKDF info 文字列 `owner:<id>` の中身は不変。MVP 未リリースのためバックフィル不要。鍵ローテーション単位は引き続き「契約 1 つ = `contract_owners` 1 行」。
...
- 12 テーブル論理削除リスト([CLAUDE.md](../../CLAUDE.md) 同期ルール §)も同期して v1.10 の 11 テーブル(`accounts` / `account_project_grants` / …)に戻すこと

### 5c.6 想定リスクと緩和策

| リスク | 影響 | 緩和策 |
|---|---|---|
| `project_users.contract_owner_user_id` と `projects.contract_owner_user_id` の不整合 | 高(認可境界クエリの結果が齟齬する)| アプリ層 INSERT で必ず両者一致を担保、レビューチェックリスト化、将来トリガで強制検討 |
...
## 5d. v1.13 エンドユーザー独立テーブル化マイグレーション(MVP 未リリース、DDL 切替のみ)

**前提**: MVP 未リリースのためデータ移行不要。新規 DDL のみ。`users.role='end_user'` 行 + `inquiry_contacts` の二重管理を解消し、エンドユーザーを独立 `end_users` テーブルに集約する。新スキーマは [../02_基本設計/03_テーブル設計.md](../02_基本設計/03_テーブル設計.md) §3.1 / §3.5 / §3.14 / §3.16 / §3.19 / §3.23 を正本とする。

### 5d.1 マイグレーション順序(forward-only、staging で検証 → prod 適用)

| ステップ | 内容 | 方式 | リスク |
|---|---|---|---|
| M13-1 | 旧 `inquiry_contacts` テーブルを DROP TABLE(MVP 未リリースのため依存データなし)| DDL のみ | 低 |
| M13-2 | `users` テーブルから `role` カラムを撤去 + CHECK `role IN ('admin','end_user')` を撤廃。テーブル再作成(SQLite ALTER 制約) | テーブル再作成 | 低 |
| M13-3 | 新規 DDL を適用(基本設計 03_テーブル設計.md §3.16 を正本):<br>・`end_users`(エンドユーザー、プロジェクト単位スコープ、`(project_id, email_hmac) WHERE valid=1` 部分 UNIQUE、`users` と独立)| CREATE TABLE | 低 |
| M13-4 | `inquiries.end_user_id` を ADD COLUMN(`end_users(id)` FK、NOT NULL、ON DELETE NO ACTION)| テーブル再作成 | 中(参照広範) |
| M13-5 | `chat_messages` を改修:<br>・旧 `actor_id` カラム削除<br>・`actor_user_id` / `actor_end_user_id` の 2 カラムを追加<br>・複合 CHECK で `actor_type` との整合を強制 | テーブル再作成 | 中 |
| M13-6 | `access_tokens` を改修:<br>・`end_user_id` カラムを追加(`end_users(id)` FK、CASCADE)<br>・`user_id` を NULL 許容に変更<br>・複合 CHECK で `(user_id XOR end_user_id)` を強制 | ALTER TABLE | 低 |
| M13-7 | `audit_logs.actor_end_user_id` を ADD COLUMN(`end_users(id)` FK、SET NULL)| ALTER TABLE | 低 |
| M13-8 | インデックス再作成(基本設計各テーブルの §N.5 を反映):<br>・`uq_end_users_project_email_hmac` / `idx_end_users_project` / `idx_end_users_owner` / `idx_end_users_email_hmac` / `idx_end_users_valid`<br>・`idx_inquiries_end_user` / `idx_chat_messages_actor_end_user` / `idx_access_tokens_end_user` / `idx_audit_actor_end_user_created` | CREATE INDEX | 低 |
| M13-9 | seed / E2E fixtures 改修:<br>・エンドユーザーは `end_users` に INSERT(`users` 行を作らない)<br>・再入室トークンは `access_tokens.end_user_id` 経由で発行<br>・チャット投稿は `actor_type='end_user'` 時に `actor_end_user_id` をセット | スクリプト改修 | 低 |
| M13-10 | アプリ層クエリ一括書換:<br>・`users.role='end_user'` 参照を `end_users` テーブル参照に置換<br>・`inquiry_contacts.*` を `end_users.*` に置換<br>・`chat_messages.actor_id` を 2 カラム分離参照に置換<br>・再入室フロー(`purpose='reentry'`)の `user_id` を `end_user_id` に置換 | アプリ改修 | 中 |
| M13-11 | `bash 99_script/check-spec-sync.sh` で SC-001〜SC-011 グリーン確認(SC-002 検査キーワードに `end_users` 追加済) | スクリプト | — |

### 5d.2 想定リスクと緩和策

| リスク | 影響 | 緩和策 |
|---|---|---|
| `chat_messages.actor_id` 単一カラム → 2 カラム分離での参照漏れ | 中(エンドユーザー投稿が記録不能 / 復号失敗) | アプリ層一括 grep + 単体テスト `it-chat-actor-split-001` 必須通過 |
| `inquiries.end_user_id` NOT NULL でも既存案件が NULL のまま残る | 高(整合エラー) | MVP 未リリースなのでデータゼロ前提、staging で fixtures 再投入を必須化 |
| 再入室トークン `purpose='reentry'` の `user_id` 経由運用が残存 | 中(エンドユーザー認証不能) | 認証認可設計書の §6.1 Principal 構築フロー grep + E2E `e2e-reentry-001` 必須通過 |
| `users.role` 撤廃の追従漏れ(認可コード) | 高(認可ロジック退行) | check-spec-sync SC-002 の検査キーワードに `end_users` 追加で機械検出、PR レビュー必須 |

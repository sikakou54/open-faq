...
レート制限は契約単位に残し、質問数上限と課金対象別無料枠をプロジェクト単位へ移行する。FAQ・チャットの旧月次上限は廃止する。
...
| M12-4 | 既存 `question_monthly_limit` は各プロジェクトの質問数上限へ展開。FAQ・チャットは旧上限値と管理者ユーザー由来の無料枠上書きを破棄し、運営者上書きだけを `source='operator'`、`threshold=NULL`、アラート無効で移行 | データ移行 | 中 |
| M12-5 | `owner_quota_overrides.resource_kind` の CHECK から 3 つの `*_monthly_limit` を撤去(レート 5 種別のみに縮小)+ 旧月次上限行を削除 | DDL + DELETE | 中 |
| M12-6 | KV 再構築: 質問数は上限 + 無料枠、FAQ・チャットは無料枠だけを生成。FAQ・チャットの旧上限キーを削除 | スクリプト | 低 |
| M12-7 | 利用者向け API を質問数専用の `PATCH /projects/{id}/quota-limits/questions` に置換。FAQ・チャットの利用者向け設定ルートを撤去し、月次件数による 429・通知・新規作成停止も撤去 | アプリ改修 | 中 |

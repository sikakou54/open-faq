...
§(i) は SCR-008(プロジェクト概要) / SCR-016(契約概要) / SCR-022(利用量と上限)の最終更新タイムスタンプ表示と連動する。画面側で「⚠ 集計遅延」と表示されている期間が長引く場合は監視側でも警告化し、ユーザーの誤解(リアルタイム値だと思い込む / 取得失敗と区別がつかない)を防ぐ。
...
| 契約別利用量 | 質問数、FAQ件数、AI利用コスト |
| 不正検知 | 許可ドメイン外、レート超過(FR-195) |
| レート制限上書き状況(契約単位)| `owner_quota_overrides` の有効件数、しきい値超過契約、有効期限切迫の上書き |
| プロジェクト上限・無料枠状況(プロジェクト単位)| `project_quota_limits` の行数推移、KV `usage-limit:<owner>:<projectId>` ミラー鮮度(≤ 30s)、プロジェクト別の選択閾値/100%/125%到達件数、プロジェクト別アラートメール送信成功率 |
...
- **同期削除**: §14.1.9 `retentionCleanup` で `question_logs` / `notification_logs` / `error_logs` / `access_tokens` を同一ジョブ内で削除(1 transaction)

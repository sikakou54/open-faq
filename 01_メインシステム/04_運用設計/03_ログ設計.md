...
| `action` | string | 監査ログのみ | 例 `prod.direct_change` / `user.activation_completed`(SCR-018 メンバーアカウント有効化完了、`meta={project_id, role}`、`retention_class='general'`)|
| `message` | string | 必須 | 人間可読の本文 |

### 4.1 主要 `audit_logs.action` コード一覧(抜粋)

| action | 発生契機 | actor / target | meta | retention_class |
|---|---|---|---|---|
| `user.activation_completed` | SCR-018 メンバーアカウント有効化ページで「登録を完了する」押下成功時(`POST /auth/invitations/{token}/activate`)| actor=本人 user_id / target=同 user_id | `{ project_id, role }` | `general`(1y) |
| `project.contact_email_verified` | SCR-019 プロジェクト連絡先メール確認完了ページでトークン検証成功時(`POST /auth/contact-verifications/{token}`)| actor=なし(未認証、トークン保有者)/ target=project_id | `{ email_hmac }` | `general`(1y) |
| `prod.direct_change` | 運用直接操作 | (運営者経路を別途参照)| - | `general` |
...
| 同期削除 | 詳細設計の `retentionCleanup` で `question_logs` / `notification_logs` / `error_logs` / `access_tokens` を同一ジョブ内で削除(1 transaction) |

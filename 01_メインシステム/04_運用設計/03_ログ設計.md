...
| `action` | string | 監査ログのみ | 例 `prod.direct_change` / `user.activation_completed`(SCR-031 メンバーアカウント有効化完了、`meta={project_id, role}`、`retention_class='general'`)|
...
|---|---|---|---|---|
| `user.activation_completed` | SCR-031 メンバーアカウント有効化ページで「登録を完了する」押下成功時(`POST /auth/invitations/{token}/activate`)| actor=本人 user_id / target=同 user_id | `{ project_id, role }` | `general`(1y) |
| `project.contact_email_verified` | SCR-032 プロジェクト連絡先メール確認完了ページでトークン検証成功時(`POST /auth/contact-verifications/{token}`)| actor=なし(未認証、トークン保有者)/ target=project_id | `{ email_hmac }` | `general`(1y) |

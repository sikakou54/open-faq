...
| 関連機能ID | FR-102〜091（個別チャット部屋 / メッセージ / 再入室）/ AC-011 / AC-025 |
| 作成日 | 2026-05-17 |
| 版数 | v1.3 |
| ステータス | Future |
...
| 種別 | ID | 名称 |
|----|----|----|
| 機能 | FR-102〜091 | 独立した個別チャット部屋 / メッセージ送受信 |
| 機能 | FR-085a | 個別チャット部屋への担当管理者ユーザー割当（担当者の指定は個別チャットのみの概念。担当者は常に 1 名・未割当不可。未解決質問は担当者を持たない） |
| 機能 | FR-085b | 新規チャット部屋作成時の担当者自動割当（デフォルト担当者→未設定時オーナーへフォールバック） |
| 機能 | FR-085c | プロジェクト単位デフォルト担当者設定（FUT-SCR-04 / `M_PROJECTS.default_assignee_user_id`） |
| 機能 | FR-113 / FR-114 | 個別チャット再入室（FUT-SCR-02 連携） |
| 画面 | FUT-SCR-01 | 個別チャット一覧（利用者側 / エンドユーザー側部屋ビューを含む） |
| 画面 | FUT-SCR-03 | 個別チャット部屋（利用者側、担当者プルダウン） |
| 画面 | FUT-SCR-04 | 自動割り当て（FUT-SCR-01 チャット配下タブ） |
| 画面 | FUT-SCR-02 | 個別チャット再入室（再入室トークン） |
...
| 元章 | 元タイトル | 概要 |
|----|----|----|
| §6 FUT-SCR-01 / FUT-SCR-03 | 画面詳細設計 | 個別チャット一覧 / エンドユーザー部屋 / 管理者向けチャット部屋 |
| §6 FUT-SCR-02 / FUT-SCR-04 | 画面詳細設計 | 個別チャット再入室 / デフォルト担当者設定 |
...
| 画面 | 利用者 | 主な表示・操作 | 主な遷移 |
|----|----|----|----|
| FUT-SCR-01 個別チャット一覧 | 管理者ユーザー | チャットID、最終メッセージ、未読数、更新日時、プロジェクト・未読絞り込み | 行選択で FUT-SCR-03 |
| FUT-SCR-01 エンドユーザー部屋 | エンドユーザー | チャットID、会話履歴、メッセージ入力、機密情報注意 | AI未解決時に同一ウィジェット内で切替 |
| FUT-SCR-03 個別チャット部屋 | 管理者ユーザー | 会話履歴、返信入力、担当者変更、一覧へ戻る | FUT-SCR-01から遷移 |
| FUT-SCR-04 自動割り当て | オーナー / メンバー | デフォルト担当者の選択・保存 | FUT-SCR-01の管理タブから遷移 |
| FUT-SCR-02 個別チャット再入室 | エンドユーザー | チャットID、期限、再開ボタン、期限切れ案内 | ウィジェットメニューまたは再入室URLから遷移 |
...
| `PATCH /projects/{id}` | デフォルト担当者変更 | オーナー / 対象プロジェクトのメンバー |
...
│   └── reentry.ts            # FUT-SCR-02 連携、再入室トークン検証
└── middleware/
    └── widget-session.ts
```
...
// 必ず担当者を確定でき、未割当の部屋は生成されない(FR-085a)。
...
- **担当者変更(FUT-SCR-03, `PATCH /chat-rooms/{id}`)**: `assigneeUserId` は必須・`null` 不可。当該プロジェクトのメンバー(オーナーを含む)に有効割当を持つ利用者のみ受理。それ以外は 400 `E-INPUT-ASSIGNEE`。変更操作自体は当該プロジェクトのメンバー(オーナーを含む)の誰でも可。
- **自動割り当て設定(FUT-SCR-04, `PATCH /projects/{id}` `defaultAssigneeUserId`)**: FUT-SCR-01チャット配下の「自動割り当て」タブから操作する。当該プロジェクトのメンバー(オーナーを含む)が操作可。`null`を許容し、未設定時はオーナーへフォールバックする。
- **担当メンバーの論理削除時の付け替え**: 担当者であったメンバーがアカウント論理削除(`users.valid=0`)される場合、当該メンバーが担当する全部屋の `assignee_user_id` を `resolveInitialAssignee` と同じ優先順位(デフォルト担当者→オーナー)で付け替えてから `valid=0` に更新する。これにより NOT NULL 制約と「未割当を作らない」不変条件を維持する。デフォルト担当者(`M_PROJECTS.default_assignee_user_id`)が当該削除ユーザーを指していた場合も同フローで `NULL` 化(以後オーナーへフォールバック)する。
...
| 異常系 | 期限切れ再入室トークン使用時の 400 `TOKEN_EXPIRED` / 担当者に `null` または当該 PJ 未割当ユーザーを指定 → 400 `E-INPUT-ASSIGNEE` / 当該 PJ 未割当ユーザーがデフォルト担当者設定 → 403 |
| 境界値 | 再入室トークン TTL 30 日ぴったり / 同一ウィジェットセッションの新規チャット 24 時間 5 件 |
| 性能 | `POST /api/v1/chats/.../messages` p95 \< 500ms（NFR-104） |
| ショートカット | FUT-SCR-03 個別チャット部屋で `Ctrl+Enter`（macOS では `Cmd+Enter`）でメッセージ送信 |

# <span id="FUT-03"></span>FUT-03: 国際化・リージョン

## <span id="0-文書情報"></span>0. 文書情報

| 項目         | 内容                                      |
|--------------|-------------------------------------------|
| 文書名       | FUT-03: 国際化・リージョン                 |
| 将来対応ID   | FUT-03                                    |
...
| 背景 | 海外オーナー・海外拠点管理者のユースケースを取り込むには、UI 多言語化、データ越境制限、TZ 上書きの 3 軸が必要となる。 |
...
| マルチリージョン | オーナー作成時の region 決定、データ配置、越境移転ガード | FUT-REQ-I18N-002 |
| 管理者単位 TZ | オーナー TZ と個人 TZ の優先順位、監査ログ時刻表示、レポート集計境界 | FUT-REQ-I18N-003 |
...
| 種別 | 影響内容 |
|----|----|
| 要件 | NFR-059(言語)、NFR-060(TZ)、region 属性導入によるオーナーモデル拡張 |
| 画面 | UI 言語スイッチャー、オーナー作成画面(region 選択)、アカウント設定(個人 TZ) |
...
| FUT-I18N-Q-002 | マルチリージョンオーナーでの region 変更可否(原則固定 / 例外手順 RB-014 連携) | 中 | 検討中 |
| FUT-I18N-Q-003 | 管理者単位 TZ とオーナー TZ の優先順位、監査ログ表示時刻のルール | 中 | 検討中 |

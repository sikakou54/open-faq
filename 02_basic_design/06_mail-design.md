...
> [!NOTE]
> **本書の範囲と対象外** 本書は §4 のテンプレート ID `TPL-*` 全件の件名 / テキスト本文 / HTML 本文・送信契機(`NOTIF-*`)・配信先 / 重要度 / 添付 / リンク有効期限・メール共通要件(送信元・i18n・サニタイズ・配信信頼性)を扱います。アプリ内お知らせ受信箱(`inbox` 限定の `NOTIF-CHAT_HOLD_CHECK` 等)は 本書、画面文言(`MSG-SCR-*`)は [画面設計.md](01_screens/index.md) が正本です。配信信頼性(DMARC 等)は 要件のセキュリティ NFR、通知契機・配信先は 本書 が正本です。
...
**発火条件**: FR-121 月次請求確定バッチ完了時、当該契約のオーナーへ送信。
...
**強制送信**: 規約改定・価格改定・重要セキュリティに該当するお知らせは重要度 `critical` を選択して配信し、受信者はオプトアウトできない(FR-122 / BR-078。利用規約改定そのものは専用の `TPL-TERMS_REVISION` を用いる)。それ以外の `normal` / `high` は §5.2 のルールに従う。

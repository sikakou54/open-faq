...
**メインシステムのデータベース(Cloudflare D1 / SQLite)全 33 テーブルを機能ドメイン別に定義する設計書です。** 全ユーザーは `M_USER`、契約は `M_CONTRACT`(オーナー判定 + プロジェクトの親)で管理します。各テーブルの詳細はテーブル名のリンクから辿れます。

*版数 v3.7 ・ 更新 2026-06-24 ・ テーブル数 33 ・ 独立設計書*

## <span id="store"></span>1.データストア構成

<div class="card-grid cols-3">
<div class="card"><div class="lead-ico">D1</div><h4>Cloudflare D1(SQLite)</h4><p>全 33 テーブル。契約境界は <code>contract_id</code>(<code>M_CONTRACT.id</code>)で表す。</p></div>
...
| 物理名 | 論理名 | 分類 / 保持 | 保持期間 | 概要 |
|----|----|----|----|----|
| <span id="TBL-023"></span>[`T_WITHDRAW_REQ`](TBL-023.md) | 退会記録 | トランザクション | 7年 | 即時退会の実行記録(退会日時・退会者・理由・削除予定日=退会日+7年)。 |
#### システム・ログ・運用 (4)
...
erDiagram
  ...
  T_WITHDRAW_REQ { TEXT id PK
    TEXT contract_id FK "→M_CONTRACT.id"
    TEXT applied_by_id FK "→M_CONTRACT.id(オーナー)" }
  M_CONTRACT ||--o{ T_WITHDRAW_REQ : "退会記録"
```

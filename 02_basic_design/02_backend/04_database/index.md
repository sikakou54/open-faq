...
| <span id="TBL-002"></span>[`M_BILLING_ACCOUNT`](TBL-002.md) | 課金アカウントマスタ | マスタ | 30日 | ユーザー単位の課金アカウント(user_id ごとに 1 件)。サブスク・支払方法・請求・課金状態を束ねる。所有・ロール判定には用いない。 |
...
| 物理名 | 論理名 | 分類 / 保持 | 保持期間 | 概要 |
|----|----|----|----|----|
| <span id="TBL-004"></span>[`M_PROJECTS`](TBL-004.md) | プロジェクト | マスタ | 30日 | FAQ プロジェクトとウィジェット設定。owner_user_id でオーナー(作成者)を表す。 |
...
| 物理名 | 論理名 | 分類 / 保持 | 保持期間 | 概要 |
|----|----|----|----|----|
| <span id="TBL-006"></span>[`M_FAQS`](TBL-006.md) | FAQ | マスタ | 30日 | FAQ 本体(質問・回答・公開状態)。テナント境界は project_id で表す。 |
...
| <span id="TBL-033"></span>[`TP_IMPORT_JOBS`](TBL-033.md) | FAQ取込ジョブ | ワーク | 30日 | FAQ CSV 一括取込ジョブの状態・進捗・結果サマリ(完了後 TTL で消去)。 |
#### 利用量・課金・上限 (6)
...
| <span id="TBL-008"></span>[`M_OWNER_QUOTA_OVR`](TBL-008.md) | オーナー別レート上書き | マスタ | 30日 | オーナー単位のレート制限上書き(owner_user_id 単位)。 |
#### お知らせ・通知 (5)
...
| <span id="TBL-011"></span>[`M_ANNOUNCE_AUD`](TBL-011.md) | お知らせ配信対象(M:N) | マスタ | 30日 | 配信先を限定指定(対象ユーザー)。 |
...
| <span id="TBL-007"></span>[`M_EMAIL_SUPPRESS`](TBL-007.md) | メールサプレスリスト | マスタ | 30日 | バウンス・苦情アドレス(全アカウント横断)。 |

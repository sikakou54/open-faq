...
|----|----|----|----|----|
| <span id="TBL-001"></span>[`M_USER`](TBL-001.md) | ユーザーマスタ | マスタ | 退会後7年 | オーナー・メンバーを含む全ユーザーの認証情報を一元保持。 |
| <span id="TBL-002"></span>[`M_BILLING_ACCOUNT`](TBL-002.md) | 課金アカウントマスタ | マスタ | 退会後7年 | ユーザー単位の課金アカウント(user_id ごとに 1 件)。サブスク・支払方法・請求・課金状態を束ねる。所有・ロール判定には用いない。 |
| <span id="TBL-003"></span>[`M_PRJ_USERS`](TBL-003.md) | プロジェクトメンバー(割当) | マスタ | 削除後90日 | ユーザーをプロジェクトへ割り当て(役割差は持たない)。 |
| <span id="TBL-013"></span>[`T_SESSIONS`](TBL-013.md) | セッション | トランザクション | 失効後30日 | 複数デバイス対応のログインセッション。 |
| <span id="TBL-014"></span>[`T_ACCESS_TOKENS`](TBL-014.md) | アクセストークン | トランザクション | 失効後30日 | 招待・パスワード再設定・メール確認などの短期トークン。 |
| <span id="TBL-012"></span>[`M_TERMS_VER`](TBL-012.md) | 規約版数 | マスタ | 7年 | 利用規約・プライバシーポリシーの版。 |
| <span id="TBL-024"></span>[`T_TERMS_AGREE`](TBL-024.md) | 規約同意 | トランザクション | 退会後7年 | 利用者ごとの規約同意履歴。 |
#### プロジェクト・ウィジェット (3)
...
| 物理名 | 論理名 | 分類 / 保持 | 保持期間 | 概要 |
|----|----|----|----|----|
| <span id="TBL-004"></span>[`M_PROJECTS`](TBL-004.md) | プロジェクト | マスタ | 削除後90日 | FAQ プロジェクトとウィジェット設定。owner_user_id でオーナー(作成者)を表す。 |
| <span id="TBL-005"></span>[`M_ALLOWED_DOMAINS`](TBL-005.md) | 許可ドメイン | マスタ | 削除後90日 | ウィジェット埋め込みを許可するドメイン。 |
| <span id="TBL-015"></span>[`T_PRJ_LEGACY_KEYS`](TBL-015.md) | レガシー API キー | トランザクション | 失効後30日 | 鍵ローテーション時に旧キーを 24 時間だけ有効化。 |
#### FAQ・質問・未解決 (7)
...
| 物理名 | 論理名 | 分類 / 保持 | 保持期間 | 概要 |
|----|----|----|----|----|
| <span id="TBL-006"></span>[`M_FAQS`](TBL-006.md) | FAQ | マスタ | 削除後90日 | FAQ 本体(質問・回答・公開状態)。テナント境界は project_id で表す。 |
...
| <span id="TBL-017"></span>[`T_INQUIRIES`](TBL-017.md) | 未解決質問 | トランザクション | 1年 | FAQ 登録前の未解決質問。 |
| <span id="TBL-029"></span>[`H_INQUIRY_FAQ`](TBL-029.md) | 未解決質問 FAQ 化履歴 | 履歴 | 1年 | 未解決質問から FAQ への移行履歴(データコピー方式)。 |
| <span id="TBL-033"></span>[`TP_IMPORT_JOBS`](TBL-033.md) | FAQ取込ジョブ | ワーク | 短期(完了後TTL) | FAQ CSV 一括取込ジョブの状態・進捗・結果サマリ(完了後 TTL で消去)。 |
#### 利用量・課金・上限 (6)
...
| 物理名 | 論理名 | 分類 / 保持 | 保持期間 | 概要 |
|----|----|----|----|----|
| <span id="TBL-020"></span>[`T_USAGE_METER`](TBL-020.md) | 利用量計測 | トランザクション 課金7年 | 7年 | 質問数・FAQ 件数をプロジェクト単位で計測しオーナー単位で集計。 |
| <span id="TBL-018"></span>[`T_BILL_SUBS`](TBL-018.md) | 課金サブスクリプション | トランザクション 課金7年 | 7年 | Stripe サブスクと連動(課金アカウント単位)。 |
| <span id="TBL-019"></span>[`T_BILL_INVOICES`](TBL-019.md) | 請求書 | トランザクション 課金7年 | 7年 | 月次請求書(電子帳簿保存法 7 年。課金アカウント=オーナー単位、各プロジェクト内訳は明細)。 |
| <span id="TBL-032"></span>[`T_BILLING_WEBHOOK_LOG`](TBL-032.md) | 課金Webhook受信ログ | トランザクション 課金 | 7年 | 課金プロバイダ通知の受信・検証・取込状態を記録(重複検出・失敗再処理)。 |
| <span id="TBL-009"></span>[`M_PRJ_QUOTA_LIMITS`](TBL-009.md) | プロジェクト別利用設定 | マスタ | 削除後90日 | 質問数の月次上限・無料枠・アラート。 |
| <span id="TBL-008"></span>[`M_OWNER_QUOTA_OVR`](TBL-008.md) | オーナー別レート上書き | マスタ | 削除後90日 | オーナー単位のレート制限上書き(owner_user_id 単位)。 |
#### お知らせ・通知 (5)
...
| 物理名 | 論理名 | 分類 / 保持 | 保持期間 | 概要 |
|----|----|----|----|----|
| <span id="TBL-010"></span>[`M_SERVICE_ANNOUNCE`](TBL-010.md) | お知らせ(Control Plane) | マスタ | 1年 | お知らせ本体。 |
| <span id="TBL-011"></span>[`M_ANNOUNCE_AUD`](TBL-011.md) | お知らせ配信対象(M:N) | マスタ | 1年 | 配信先を限定指定(対象ユーザー)。 |
| <span id="TBL-021"></span>[`T_ANNOUNCE_RCPT`](TBL-021.md) | お知らせ受信者 | トランザクション | 1年 | 実配信先・配信集計・監査。 |
| <span id="TBL-022"></span>[`T_INBOX_MSG`](TBL-022.md) | 受信箱(Tenant Plane) | トランザクション | 1年 | 利用者が受け取る通知の既読状態。 |
...
| 物理名 | 論理名 | 分類 / 保持 | 保持期間 | 概要 |
|----|----|----|----|----|
| <span id="TBL-023"></span>[`T_WITHDRAW_REQ`](TBL-023.md) | 退会記録 | トランザクション | 7年 | 即時退会(アカウント単位)の実行記録(退会日時・退会者・理由・削除予定日=退会日+7年)。 |
#### システム・ログ・運用 (4)
...
| <span id="TBL-031"></span>[`TP_AI_THRESH_CACHE`](TBL-031.md) | AI しきい値キャッシュ | ワーク | 短期(キャッシュ) | 3 階層しきい値の永続キャッシュ。 |

> [!NOTE]
> **保持期間**は論理削除(ログ系は記録)から物理削除までの保持基準です。値の正本は[システム仕様書 §4](../../07_system-spec.md#4-データ保持期間削除猶予)。区分: プロジェクト運用データ(FAQ・プロジェクト・割当・許可ドメイン・利用設定等)は**削除後 90 日**([RULE-008](../../../01_requirements/01_business_requirement/08_rule.md#RULE-008) / [NFR-048](../../../01_requirements/03_non_functional_requirement/07_nfr.md#NFR-048))、システムログ(質問ログ・参照FAQ・通知ログ・エラーログ・FAQ化履歴・未解決質問・お知らせ受信箱等)は **1 年**([NFR-049](../../../01_requirements/03_non_functional_requirement/07_nfr.md#NFR-049))、アカウント・課金・請求関連(利用者・課金アカウント・利用量計測・サブスク・請求書・課金Webhook受信ログ・規約同意・退会記録・課金関連監査を含む監査ログ)は法令対応で**退会後 7 年**([RULE-022](../../../01_requirements/01_business_requirement/08_rule.md#RULE-022))とします。セッション・トークンは失効後の短期保持、ワーク/キャッシュは TTL・派生のため随時消去。物理削除は [SYS-027](../01_system/SYS-027.md#SYS-027)(退会即時・論理削除90日)・[SYS-032](../01_system/SYS-032.md#SYS-032)(保持1年)・[SYS-034](../01_system/SYS-034.md#SYS-034)(保持7年)が保持期間を判定して実施します。

> [!NOTE]
> **退会時の扱い(2 区分)** アカウント退会時、各テーブルは保持期間欄により次の 2 区分に分かれます。**(A) 運用データ(削除後 90 日 / 1 年)**=退会成立と同時に論理削除し速やかに物理削除(プロジェクト・FAQ・割当・許可ドメイン・利用設定・質問ログ・未解決質問・通知・受信箱・お知らせ受信状況等)。**(B) アカウント・課金・請求(退会後 7 年)**=`M_BILLING_ACCOUNT.withdrawn_at` を起点に 7 年保持し、経過後に [SYS-034](../01_system/SYS-034.md#SYS-034) が物理削除(利用者・課金アカウント・サブスク・請求書・利用量計測・課金Webhook受信ログ・規約同意・退会記録・課金関連監査)。論理削除データの物理削除起算点は各テーブルの `deleted_at`(保持する場合)、課金・請求は `withdrawn_at` です。

<!-- portal-top -->
[設計ポータル](../../../README.md) ／ [基本設計](../../index.md) ／ [バックエンド設計](../index.md) ／ [DB設計](index.md) ／ **H_ERROR_LOGS(エラーログ)**
<!-- /portal-top -->

# <span id="TBL-028"></span>H_ERROR_LOGS(エラーログ)

履歴

サーバーエラーを記録するエラーログです。

<!-- p5-item -->
### <span id="item"></span>項目

本テーブルの基本情報と、参照元の業務ユースケース / API(逆引き)です。

| 項目 | 内容 |
|---|---|
| テーブルID | `TBL-028` |
| 物理名 | `H_ERROR_LOGS` |
| 論理名 | エラーログ |
| 概要 | サーバーエラーを記録するエラーログです。 |
| 主キー | `id` |
| 論理削除 | なし(物理削除 / 追記専用) |
| 監査項目(日時) | — |
| 監査項目(実施者) | `actor_id` |
| 対応業務UC(逆引き) | — (横断 ・ エラーログ基盤。特定業務UCに紐付かない) ・ [UC-050](../../../01_requirements/04_business_usecases/UC-050.md#UC-050) ・ [UC-076](../../../01_requirements/04_business_usecases/UC-076.md#UC-076) |
| 利用API(逆引き) | — (Control Plane / 内部処理・横断基盤、HTTP API 非経由) |
<!-- /p5-item -->

### <span id="概要"></span>概要

| 項目       | 内容           |
|------------|----------------|
| テーブル名 | `H_ERROR_LOGS` |
| 論理名     | エラーログ     |
| 主キー     | `id`           |
| 外部キー   | —              |

### <span id="カラム定義"></span>カラム定義

| No | 論理名 | 物理名 | データ型 | 桁数 | NULL | PK | FK | UNIQUE | DEFAULT | 制約 |
|---:|----|----|----|---:|----|----|----|----|----|----|
| 1 | ID | `id` | TEXT | \- | NO | ○ |  |  |  |  |
| 2 | 契約 owner ID | `contract_id` | TEXT | \- | YES |  |  |  |  |  |
| 3 | 操作者種別 | `actor_type` | TEXT | \- | YES |  |  |  |  | `actor_type IN ('owner','project_user')` |
| 4 | 操作者 ID | `actor_id` | TEXT | \- | YES |  |  |  |  | `actor_type` に応じ `M_CONTRACT(id)` / `M_PRJ_USERS(id)` を指す(論理参照) |
| 5 | URL | `url` | TEXT | \- | YES |  |  |  |  |  |
| 6 | エラー種別 | `error_type` | TEXT | \- | YES |  |  |  |  |  |
| 7 | スタック | `stack` | TEXT | \- | YES |  |  |  |  |  |
| 8 | 発生日時 | `occurred_at` | TEXT | \- | NO |  |  |  |  |  |

### <span id="インデックス"></span>インデックス

| No | インデックス名 | 対象カラム | UNIQUE | 用途 |
|---:|----|----|----|----|
| 1 | `idx_error_logs_owner_occurred` | `(contract_id, occurred_at DESC)` |  | オーナー別新着 |

### <span id="コード値区分値"></span>コード値・区分値

(なし)

### <span id="usedby"></span>使用元(画面 / API)

このテーブルを読み書きする画面と API です(逆引き)。

**画面** — **API** —

---

<!-- portal-bottom -->
[← DB設計](index.md) ・ [基本設計](../../index.md) ・ [↑ 設計ポータル](../../../README.md)
<!-- /portal-bottom -->

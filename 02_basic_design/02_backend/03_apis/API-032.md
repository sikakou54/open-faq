...
- 行選択による詳細表示は、本 API の一覧レスポンス `items` の各項目(`question` / `answerType` / `confidence` / `inquiryId` / `createdAt`)を行内で展開して表示するものであり、別途の質問ログ詳細取得 API・詳細画面は設けない。AI 応答本文・結果理由などの追加情報を詳細で見せる要件は無いため、詳細表示のための項目追加は行わない。

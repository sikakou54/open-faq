...
> **このページは、メインシステムが提供する全 69 エンドポイント(`API-001`〜`API-069`)を、機能グループ別に一覧する索引です。** 各行は `API ID` / `API名` / `METHOD` / `エンドポイント` を示し、詳細は各 API 個別ページ(項目 / 処理概要 / リクエスト(認証ヘッダ含む) / レスポンス / バリデーション / エラー / 利用テーブル)を参照します。共通の入出力・エラー規約は次の「§0 共通仕様」を正本とします。
...
| 挙動 | 同一キーの再送は初回結果(同一ステータス・ボディ)を再現する。保持期間は [システム仕様書 §4](../../07_system-spec.md#4-データ保持期間削除猶予) 参照。キー一致でボディ不一致の場合は 409 `IDEMPOTENT_REPLAY` → [ERR-032](../../05_errors/ERR-032.md#ERR-032)。 |
...
| <span id="API-069"></span>[`API-069`](API-069.md#API-069) | ログイン失敗ロック解除 | POST | `/projects/{id}/members/{userId}/unlock` |
...
| <span id="API-067"></span>[`API-067`](API-067.md#API-067) | FAQ取込ジョブ状態取得 | GET | `/faqs/import/{jobId}` |
...
| <span id="API-068"></span>[`API-068`](API-068.md#API-068) | ウィジェット回答フィードバック送信 | POST | `/widget/v1/feedback` |
| <span id="API-066"></span>[`API-066`](API-066.md#API-066) | AIしきい値設定取得・更新 | GET / PUT | `/projects/{id}/ai-thresholds` |

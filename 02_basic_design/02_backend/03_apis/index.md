...
> **このページは、メインシステムが提供する全 67 エンドポイント(`API-001`〜`API-067`)を、機能グループ別に一覧する索引です。** 各行は `API ID` / `API名` / `METHOD` / `エンドポイント` を示し、詳細は各 API 個別ページ(項目 / 処理概要 / リクエスト(認証ヘッダ含む) / レスポンス / バリデーション / エラー / 利用テーブル)を参照します。

...
> [!IMPORTANT]
> **構造的に孤立しない** 1 プロジェクト = 1 オーナー(作成者)であり、オーナーはプロジェクト作成者として固定されるため、当該プロジェクトがオーナーを失う状態は構造的に発生しません(降格・譲渡・割当解除を受け付けない)。自身のアカウント削除は常に不可で、他人を経由して自身を削除する経路も設けません。拒否時は `E-AUTHZ-OWNER-PROTECTED` / `E-AUTHZ-SELF-MUTATION`。

<!-- portal-top -->
[設計ポータル](../../README.md) ／ [基本設計](../index.md) ／ [API設計](index.md) ／ **利用量・課金 API**
<!-- /portal-top -->
...
支払方法はクレジットカードのみ(FR-096)。カード番号等の生データはサーバで保持せず、課金プロバイダのトークンを受領して紐付ける(PCI スコープ最小化、正本: 要件のセキュリティ NFR)。サスペンション中もオーナーは本 API を実行できる(FR-098)。

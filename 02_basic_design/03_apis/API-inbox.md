<!-- portal-top -->
[設計ポータル](../../README.md) ／ [基本設計](../index.md) ／ [API設計](index.md) ／ **お知らせ受信箱 API**
<!-- /portal-top -->
...
選択した複数のお知らせを一括で既読にする。1 リクエストの上限は **100 件**。超過時は 400 `VALIDATION_ERROR`。冪等(既読済みは再記録しない)。一括既読操作は操作者・対象件数・実行日時を監査記録する(FR-128 相当、正本: 要件のセキュリティ NFR)。

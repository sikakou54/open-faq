<!-- portal-top -->
[設計ポータル](../../README.md) ／ [基本設計](../index.md) ／ [API設計](index.md) ／ **API-057: AI 推論 IF(`AnswerProvider`)**
<!-- /portal-top -->

# <span id="API-057"></span>API-057: AI 推論 IF(`AnswerProvider`)

> **このページは AI 推論 IF(`AnswerProvider`) API の契約を定義します。** 回答生成(`generate`)とヘルスチェック(`healthcheck`)を持つ内部抽象インターフェースです。エンドポイントは持たず、MVP は `WorkersAIAnswerProvider` として実装します。
...
|---|---|
| API ID | `API-057` |
| API名 | AI 推論 IF(`AnswerProvider`) |
| 対応業務UC | [UC-245](../../01_requirements/04_business_usecases/UC-245.md#UC-245) |
| 対応画面ID | — |
| 対応画面イベントID | — |
| エンドポイント | — |
| HTTPメソッド | — |
...
再構成 P4 で旧 API ID `API-057`(旧グループ別 API 設計)から導出・フラット採番。

...
|----|----|----|----|----|----|----|
| `FR01` | **アカウント管理** | 新規登録 / ログイン / 再認証 / ログアウト | SCR-001〜003 / SCR-013 | `POST /auth/signup` ・ `/auth/login` | `M_CONTRACT` `T_SESSIONS` `T_ACCESS_TOKENS` `T_TERMS_AGREE` | [サービスにアクセスする](../01_requirements/04_business_usecases/index.md#act-account) / [サービス利用を開始する](../01_requirements/04_business_usecases/index.md#act-owner) |
| `FR02` | **ユーザー管理** | プロジェクト単位のメンバー管理 | SCR-013 / SCR-018 | `POST /projects/{id}/members` | `M_PRJ_USERS` | [チームを編成して共同運用する](../01_requirements/04_business_usecases/index.md#act-owner) |
| `FR03` | **プロジェクト管理** | プロジェクト作成 / 編集 / 削除 | SCR-004 | `POST/PATCH/DELETE /projects` | `M_PROJECTS` `M_ALLOWED_DOMAINS` | [FAQ 提供基盤を構築する](../01_requirements/04_business_usecases/index.md#act-owner) |
| `FR04` | **FAQ管理** | FAQ CRUD + 公開 | SCR-008 | `PATCH /faqs/{id}` | `M_FAQS` `TP_FAQ_FTS` | [FAQ を整備して公開する](../01_requirements/04_business_usecases/index.md#act-member) |
...
| `FR06` | **未解決質問登録** | 未解決質問の登録 / 一覧 | SCR-006 | `GET /inquiries` | `T_INQUIRIES` `H_QUESTION_LOGS` | [問い合わせから FAQ を改善する](../01_requirements/04_business_usecases/index.md#act-member) |
| `FR07` | **未解決→FAQ登録** | 未解決 → FAQ 候補化 / 公開 | SCR-006 | `POST /faqs` | `M_FAQS` `T_INQUIRIES` | [問い合わせから FAQ を改善する](../01_requirements/04_business_usecases/index.md#act-member) |
| `FR08` | **処理エラー** | エラー一覧 / 再実行 | (運用) | — | `H_ERROR_LOGS` | — |
| `FR09` | **利用量・課金** | 利用量メータリング / 請求 | SCR-021 / SCR-022 | `GET /usage` ・ `/billing/summary` | `T_USAGE_METER` `T_BILL_SUBS` `T_BILL_INVOICES` `M_PRJ_QUOTA_LIMITS` | [利用量と費用を管理する](../01_requirements/04_business_usecases/index.md#act-owner) |

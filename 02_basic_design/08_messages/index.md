...
> **このページは、メインシステムが送信する全メールテンプレート(`MSG-NNN`)の一覧と、メール共通基準・配信運用ルールの正本です。** 各テンプレートの件名・本文・変数・送信契機は個別 `MSG-NNN.md` が正本です。画面に表示する確認・完了・エラーメッセージの文言は各 [画面設計](../01_screens/index.md)(SCR の §4 / §6)、拒否時のエラーコードは [エラー設計](../07_errors/index.md) が正本です。

*ステータス ドラフト ・ 再構成 P6b*

## <span id="reading"></span>読み順

画面設計(画面文言)/ API設計 / エラー設計 ＞ 本メッセージ設計(メールテンプレート)。配信宛先解決ロジックは [権限設計 PERM-011](../06_permissions/PERM-011.md#PERM-011) が正本です。
...
> [!NOTE]
> **本書の範囲と対象外** 本書は `MSG-*`(= `TPL-*`)各件の件名 / テキスト本文 / HTML 本文・送信契機(`NOTIF-*`)・配信先 / 重要度 / 添付 / リンク有効期限・メール共通要件(送信元・i18n・サニタイズ・配信信頼性)を扱います。`NOTIF-CHAT_HOLD_CHECK` はチャネルが `inbox` 限定(メール送信なし)のため対象外、画面文言は [画面設計](../01_screens/index.md) が正本です。
...
ウィジェット利用者の入力をメールに流用する際の安全策を定めます。次の各項目は、件名・送信元への流用禁止と本文埋め込み時のエスケープ・字数制限を定め、スパム埋め込みやなりすましを防ぎます(FR-118)。

- **ウィジェット利用者の入力文字列(質問本文等)を、メール件名・送信元(Header From / Envelope From)・Reply-To へそのまま使用しない**(FR-118。スパム埋め込み・なりすまし対策)。送信元は §2.1 のとおり `no-reply` 固定とする。
...
通知重要度 4 値ごとにオプトアウト可否と強制送信の扱いを定めます。次の各項目は `critical` / `high` の強制送信と `normal` / `low` のオプトアウト方式を示し、重要度の正本ルールは [`M_CONTRACT`](../04_database/TBL-002.md#TBL-002) に従います。
...
全送信を記録する配信ログの保存先・列構成・保持期間を定めます。次の各項目は `H_NOTIF_LOGS` に記録する内容を示し、テーブル定義は [データベース設計.md](../04_database/index.md) が正本です。
...
- 配信先・重要度変更: 本書 §5.1 / §5.2 + [`M_CONTRACT`](../04_database/TBL-002.md#TBL-002) の整合確認
...
| メッセージ種別 | 正本 |
|----|----|
| 画面の入力検証・エラー表示文言 | 各 [画面設計](../01_screens/index.md) SCR の §4 画面項目定義 |
| 確認ダイアログ・完了トースト文言 | 各 [画面設計](../01_screens/index.md) SCR の §6 画面イベント一覧 |
| API エラーコード(HTTP / 分類) | [エラー設計](../07_errors/index.md) |

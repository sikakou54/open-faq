...
| エラー (ERR) | [ERR-001](../05_errors/ERR-001.md#ERR-001) ・ [ERR-013](../05_errors/ERR-013.md#ERR-013) ・ [ERR-021](../05_errors/ERR-021.md#ERR-021) |
| メッセージ (MSG) | — |

## 概要

回答可否の信頼度・関連度しきい値を 3 階層(グローバル / 契約 / プロジェクト)で取得・更新する。より具体的な階層を優先適用し、未設定の階層は上位を継承する。更新後はシステム処理 SYS-017 が AI しきい値キャッシュへ伝播する。割当のないプロジェクトへのアクセス・入力値不正・存在しないプロジェクトはエラーを返す。

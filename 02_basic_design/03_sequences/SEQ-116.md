# <span id="SEQ-116"></span>SEQ-116: 送信品質監視による通知送信抑制

> **このページは、業務ユースケース UC-069(システムが送信指標を監視して送信を抑制する)のシーケンス図を定義します。**

## 項目

| 項目 | 内容 |
|---|---|
| SEQ ID | `SEQ-116` |
| 対応業務ユースケース | [UC-069](../../01_requirements/04_business_usecases/UC-069.md#UC-069) |
| 業務要件 (BR) | [BR-083](../../01_requirements/01_business_requirement/05_notification-br.md#BR-083) |
| 機能要件 (FR) | [FR-117](../../01_requirements/02_functional_requirement/05_notification-fr.md#FR-117) |
| 画面イベント (EVT) | — |
| 関連画面 | — |
| 関連 API | [API-058](../02_backend/03_apis/API-058.md#API-058) ・ [API-059](../02_backend/03_apis/API-059.md#API-059) |
| 関連テーブル | [TBL-002](../02_backend/04_database/TBL-002.md#TBL-002) ・ [TBL-007](../02_backend/04_database/TBL-007.md#TBL-007) ・ [TBL-026](../02_backend/04_database/TBL-026.md#TBL-026) |
| エラー (ERR) | — |
| メッセージ (MSG) | [MSG-013](../06_messages/MSG-013.md#MSG-013) |

## 概要

システムは、通知送信の発生または定期監視を契機として、配信事業者からの配信結果をもとに契約・プロジェクト単位の送信品質を評価する。直近の送信レート・バウンス率・苦情率を集計して許容水準と照合し、いずれかの指標が許容水準を超えて悪化した契約・プロジェクト宛の通知送信を抑制状態にする。抑制の有無と対象を記録し、必要に応じて管理者へ知らせる。評価に足る実績が無い契約は判定対象から除外し、集計が未完了の場合は当該回の評価を見送って次の監視機会に再評価する。

## シーケンス図

```mermaid
sequenceDiagram
  autonumber
  participant ESP as 配信事業者(外部)
  participant SCH as スケジューラ
  participant Server as サーバー
  participant DB as DB
  ESP->>Server: 配信結果の通知(送信・バウンス・苦情)
  Server->>DB: 配信結果を契約・プロジェクト単位の送信実績へ反映する
  DB-->>Server: 反映完了
  SCH->>Server: 定期の品質監視を起動する
  Note over Server: 通知送信の発生時にも評価を起動する
  loop 契約・プロジェクトごと
    alt 評価に足る送信実績がある
      alt 送信実績の集計が完了している
        Server->>DB: 直近の送信レート・バウンス率・苦情率を集計する
        DB-->>Server: 集計結果
        Server->>Server: 各指標を許容水準と照合する
        alt いずれかの指標が許容水準を超えて悪化している
          Server->>DB: 当該契約・プロジェクト宛の通知送信を抑制状態にする
          DB-->>Server: 更新完了
          Server->>DB: 抑制の有無と対象を記録する
          DB-->>Server: 記録完了
          Server->>ESP: 抑制状態に基づき以後の通知送信を抑制する
          ESP-->>Server: 送信抑制の反映を受け付ける
          opt 管理者への通知が必要な場合
            Server->>ESP: 抑制を行った旨と対象を管理者へ通知する
            ESP-->>Server: 通知メールの受付を返す
          end
        else すべての指標が許容水準内
          Server->>Server: 送信を継続できる状態を維持する
        end
      else 集計が未完了
        Server->>Server: 当該回の評価を見送り次の監視機会に再評価する
      end
    else 評価に足る送信実績が無い
      Server->>Server: 当該契約を判定対象から除外する
    end
  end
```

## 備考

- 本図は基本設計レベルの抽象度(システム起点は外部システム・スケジューラ・バッチを参加者に置く)で記述する。DB 操作は DB アクターへのメッセージで表し、テーブル別 CRUD は本図に書かず 関連テーブル 欄で示す。
- 図の出典は業務ユースケース [UC-069](../../01_requirements/04_business_usecases/UC-069.md#UC-069)。

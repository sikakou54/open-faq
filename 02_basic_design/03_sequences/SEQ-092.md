# <span id="SEQ-092"></span>SEQ-092: 運用イベントのシステム通知自動生成

> **このページは、業務ユースケース UC-060（運用イベントのシステム通知自動生成）のシーケンス図を定義します。**

| ID | 業務ユースケースID | イベント(画面ID EVT-NN) | テーブルID |
|----|----|----|----|
| SEQ-092 | [UC-060](../../01_requirements/04_business_usecases/UC-060.md#UC-060) | — | [TBL-022](../02_backend/04_database/TBL-022.md#TBL-022) ・ [TBL-026](../02_backend/04_database/TBL-026.md#TBL-026) |

## 概要

運用イベント(利用上限接近・AI 利用上限到達・通知失敗急増・サスペンション・復元・規約改定・価格改定 等)の発生を契機に、対象アカウント利用者の受信箱へ「システム通知」のお知らせを自動生成し、メール対象の契機ではシステム通知メールも送信して配信ログを記録する。

## シーケンス図

```mermaid
sequenceDiagram
  autonumber
  participant E as 運用イベント発生元
  participant Server as サーバー
  participant DB as DB
  participant MAIL as メール配信IF
  actor U as アカウント利用者
  E->>Server: 運用イベント発生を通知
  Server->>Server: サブ契機判定・宛先解決
  alt 宛先あり
    Server->>DB: システム通知を受信箱へ生成
    DB-->>Server: 生成完了
    alt メール対象契機
      Server->>MAIL: システム通知メールを送信
      MAIL-->>U: システム通知メール
      Server->>DB: 配信ログを記録
      DB-->>Server: 記録完了
    else メール対象外契機
      Server-->>U: 受信箱のみで通知
    end
  else 宛先なし
    Server-->>E: 正常終了(通知なし)
  end
```

## 例外フロー

- **宛先なし**: 対象アカウント利用者が解決できない場合は通知を生成せず、正常終了する。
- **メール配信失敗**: 受信箱お知らせは生成済みとし、メール送信失敗は配信ログに失敗として記録する(再送は通知再送ユースケースが扱う)。

## 備考

- 本図は基本設計レベルの抽象度(ユーザー / 画面 / サーバー、システム起点は外部システム・スケジューラ・バッチを加える)で記述する。DB 操作は DB アクターへのメッセージで表し、テーブル別 CRUD は本図に書かず 関連テーブル 欄で示す。
- 図の出典は業務ユースケース [UC-060](../../01_requirements/04_business_usecases/UC-060.md#UC-060)。画面イベントとの対応は UC-060 を参照。

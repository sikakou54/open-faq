...
> [!NOTE]
> **応答コードの正本と未整備点** 質問数 100% 停止・支払方法ゲート停止は「受付停止」を示す graceful 応答(エラー応答にはしない)、サスペンションは [ERR-004](05_errors/ERR-004.md#ERR-004)。ウィジェット質問送信 API([API-038](02_backend/03_apis/API-038.md#API-038))は現状レート制限(429)・許可ドメイン外(403)のみエラー定義しており、上記 3 停止の応答(graceful 停止表示 / `BILLING_ACCOUNT_SUSPENDED`)を API-038 のエラー / 応答仕様へ追記して確定する(詳細設計で確定)。
...
> [!IMPORTANT]
> **退会は即時・請求は 7 年保持** 退会は猶予なく即時成立(`withdrawn`)し、運用データを速やかに削除する。アカウント・請求データは退会時刻(`M_BILLING_ACCOUNT.withdrawn_at`)を起点に **7 年保持**(RULE-022)し、7 年経過後に [SYS-034](02_backend/01_system/SYS-034.md#SYS-034)(保持期間経過アカウントの物理削除)が `deleted` へ確定させる。退会撤回・90 日猶予・退会発効日(当月末)・退会取消 URL は持たない。退会時の運用データ削除は [SYS-027](02_backend/01_system/SYS-027.md#SYS-027)、7 年経過後のアカウント・請求データ物理削除は [SYS-034](02_backend/01_system/SYS-034.md#SYS-034) が担う。

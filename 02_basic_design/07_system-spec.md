...
> **このページは、サービス提供に関わる設計値(しきい値・課金単価・無料枠・タイムアウト・保持期間・AI しきい値の階層継承)を一元管理する正本です。** 各設計書は本書を参照する。値の業務的根拠は対応する RULE / FR / NFR を併記する。

## <span id="1-aiしきい値3階層"></span>1. AI しきい値(3 階層)

回答可否判定(関連度・信頼度)のしきい値は、グローバル / オーナー別 / プロジェクト別の 3 階層で持ち、より具体的な階層を優先適用する。各しきい値は階層単位ではなく**項目単位(関連度・信頼度それぞれ)**で継承解決する。グローバル既定値は常に存在し、最終フォールバック値とする。

| 階層 | 適用優先 | 設定主体 | 既定値の有無 |
|----|----|----|----|
| `global`(グローバル) | 最下位(フォールバック) | 運営 | 関連度 0.50 / 信頼度 0.60 を常時保持 |
| `owner`(オーナー別) | 中位 | オーナー | 未設定可(未設定項目は上位を継承) |
| `project`(プロジェクト別) | 最上位 | オーナー / 当該プロジェクトのメンバー | 未設定可(未設定項目は上位を継承) |

継承解決は項目単位で行う。例:プロジェクト階層で関連度のみ設定しオーナー階層で信頼度のみ設定した場合、関連度=プロジェクト値・信頼度=オーナー値・両未設定項目=グローバル既定値となる。適用順序は プロジェクト設定 > オーナー設定 > グローバル既定値。

| 項目 | 値 | 単位/条件 | 根拠 | 格納先 |
|----|----|----|----|----|
| 関連度しきい値(グローバル既定) | 0.50 | 0.0〜1.0 | [RULE-012](../01_requirements/01_business_requirement/08_rule.md#RULE-012) | [`TP_AI_THRESH_CACHE`](02_backend/04_database/TBL-031.md#TBL-031)(`scope='global'`) |
| 信頼度しきい値(グローバル既定) | 0.60 | 0.0〜1.0 | [RULE-012](../01_requirements/01_business_requirement/08_rule.md#RULE-012) | [`TP_AI_THRESH_CACHE`](02_backend/04_database/TBL-031.md#TBL-031)(`scope='global'`) |
| オーナー別しきい値 | 上書き値(未設定可) | 0.0〜1.0 | [RULE-012](../01_requirements/01_business_requirement/08_rule.md#RULE-012) | [`TP_AI_THRESH_CACHE`](02_backend/04_database/TBL-031.md#TBL-031)(`scope='owner'`) |
| プロジェクト別しきい値 | 上書き値(未設定可) | 0.0〜1.0 | [RULE-012](../01_requirements/01_business_requirement/08_rule.md#RULE-012) | [`TP_AI_THRESH_CACHE`](02_backend/04_database/TBL-031.md#TBL-031)(`scope='project'`) |

> [!NOTE]
> **取得・更新と伝播** 3 階層の取得・更新は [API-067](02_backend/03_apis/API-067.md#API-067)、更新値の AI 推論への伝播・取得失敗時のフォールバックは [SYS-015](02_backend/01_system/SYS-015.md#SYS-015) が担う。長期に取得不能のときはグローバル既定値で推論を継続しアラートを上げる。

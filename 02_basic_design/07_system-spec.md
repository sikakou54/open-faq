...
回答可否判定(関連度・信頼度)のしきい値は、プロジェクト単位の設定値のみを持つ。しきい値はプロジェクト作成時に必ず設定され、推論時は常にそのプロジェクト固有の設定値を使用する。グローバル既定値などのフォールバック先は存在しない。

プロジェクト設定値は関連度・信頼度を 1 セットとして扱い、両方を必須とする(片方だけの登録は許容しない)。

| 項目 | 値 | 単位/条件 | 根拠 | 格納先 |
|----|----|----|----|----|
| プロジェクト設定値 | プロジェクト作成時に設定(必須) | 0.0〜1.0、関連度・信頼度を同時に保持 | [RULE-012](../01_requirements/01_business_requirement/08_rule.md#RULE-012) | [`TP_AI_THRESH_CACHE`](02_backend/04_database/TBL-031.md#TBL-031)(`project_id`) |

> [!NOTE]
> **取得・更新と伝播** プロジェクト設定値の取得・更新は [API-067](02_backend/03_apis/API-067.md#API-067)、更新値の AI 推論への伝播は [SYS-015](02_backend/01_system/SYS-015.md#SYS-015) が担う。プロジェクト設定値を取得できない場合はフォールバック先が存在しないため、回答可否判定を行わず推論不可として扱い(未解決質問として記録)、運用へアラートを上げる。

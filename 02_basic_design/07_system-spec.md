...
> [!NOTE]
> **取得・更新・削除と伝播** プロジェクト設定値の取得・更新・削除は [API-066](02_backend/03_apis/API-066.md#API-066)、変更後の設定状態の AI 推論への伝播・取得失敗時のフォールバックは [SYS-015](02_backend/01_system/SYS-015.md#SYS-015) が担う。対象プロジェクトの設定値が未登録または取得不能の場合は、グローバル既定値で推論を継続する。取得不能時はアラートを上げる。
...
| 受付済み FAQ 取込 CSV の保持期間 | 7 | 日(取込ジョブ完了後。R2 に一時保管し経過後に削除) | 設計判断(運用確認対象) | [`TP_IMPORT_JOBS`](02_backend/04_database/TBL-033.md#TBL-033)(R2 オブジェクトキー参照) |
...
| 課金 Webhook 再処理 1回あたり最大抽出件数 | 100 | 件 / 実行(滞留時のタイムアウト回避。超過分は次回起動へ持ち越し) | 設計判断(運用確認対象) | 定数(再処理バッチ) |
| FAQ 取込 失敗明細(`error_summary.errors`)最大保持件数 | 100 | 件(超過分は切り捨て・総失敗件数は `failed_count` に保持) | 設計判断(運用確認対象) | [`TP_IMPORT_JOBS`](02_backend/04_database/TBL-033.md#TBL-033) `error_summary` |
...
| Cron スケジュール SYS-020 | 0 * * * *(UTC、毎時 0 分) | 決済失敗猶予の経過判定(7 日猶予の確定遅延を防ぐ粒度) | 設計判断(運用確認対象) | 定数(Cron 設定) |
| Cron スケジュール SYS-029 | */5 * * * *(UTC、5 分ごと) | ログイン失敗ロック期間(15 分)の経過解除を遅延なく行う粒度 | 設計判断(運用確認対象) | 定数(Cron 設定) |

...
| <span id="IT-04"></span>`IT-04` | 業種選択 | 利用者の業種を選択する(任意)。選択肢は固定の業種マスタ(下記コード値 ↔ 表示名)から選ぶ。未選択時は値を送信しない | ドロップダウン | — | 先頭に「選択してください」(未選択)。以降の選択肢(コード値 = 表示名・設計値): `it` = IT・ソフトウェア / `retail` = 小売・EC / `education` = 教育 / `finance` = 金融・保険 / `manufacturing` = 製造 / `healthcare` = 医療・ヘルスケア / `realestate` = 不動産 / `food` = 飲食 / `public` = 公共・自治体 / `other` = その他。高規制業界は `finance` / `healthcare`(選択時に標準提供範囲外の案内を表示する。EVT-011 参照) |
...
<td>—</td>
<td>初期表示</td>
<td>登録フォームを空の状態で表示する。業種ドロップダウン(IT-04)は固定の業種マスタ(IT-04 の選択肢一覧・設計値)から選択肢を表示する。Turnstile ウィジェット(IT-11)を初期化してトークン取得を開始する</td>
</tr>
<tr>
...
<td>業種を選択</td>
<td>高規制業界(コード値 <code>finance</code>(金融・保険)/ <code>healthcare</code>(医療・ヘルスケア)= 設計値)を選択した場合、標準提供範囲外である旨の注意メッセージとサポート窓口の案内をフォーム上にインライン表示する。登録ボタンは非活性にせず、登録自体は許容する。それ以外の選択(<code>it</code> / <code>retail</code> / <code>education</code> / <code>manufacturing</code> / <code>realestate</code> / <code>food</code> / <code>public</code> / <code>other</code>)および未選択では追加表示なし</td>
</tr>
<tr>

<!-- portal-top -->
[設計ポータル](../README.md) ／ [ユースケース](index.md) ／ **UC-SYSTEM-005: 運営お知らせ配信**
<!-- /portal-top -->
...
| 関連要件 | [FR-091](../01_requirements/FR11.md#FR-091) 任意お知らせのメール通知 ・ [FR-116](../01_requirements/FR15.md#FR-116) お知らせ一覧 |
| 主テーブル | `M_SERVICE_ANNOUNCE(R)` ・ `T_INBOX_MSG(C)` ・ `H_NOTIF_LOGS(C)` |
| 関連 API | [API-ANN-001](../02_basic-design/API-inbox.md#API-ANN-001) お知らせ一覧(受信側参照) ・ [API-MAIL-001](../02_basic-design/API-mail.md#API-MAIL-001) メール配信 IF |
...
4. 重要度を判定し、メール送信対象の宛先へお知らせメールをメール配信 IF([API-MAIL-001](../02_basic-design/API-mail.md#API-MAIL-001))で送信する。`critical` は強制送信(オプトアウト不可)、`normal` はオプトアウト設定に従う。
5. メール送信結果を `H_NOTIF_LOGS(C)` に配信ログとして記録する。
6. 受信者は管理画面のお知らせ一覧([API-ANN-001](../02_basic-design/API-inbox.md#API-ANN-001))で当該お知らせを確認できる([FR-116](../01_requirements/FR15.md#FR-116))。
...
- 対象アカウント利用者の受信箱に「運営お知らせ」が生成される([FR-116](../01_requirements/FR15.md#FR-116))。

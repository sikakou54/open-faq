<!-- portal-top -->
[設計ポータル](../README.md) ／ [ユースケース](index.md) ／ **UC-SYSTEM-006: 運用イベントのシステム通知自動生成**
<!-- /portal-top -->
...
4. メール送信対象の契機かを判定する。
   1. メール対象(質問数上限関連 等): メール配信 IF([API-MAIL-001](../02_basic-design/API-mail.md#API-MAIL-001))でシステム通知メールを送信し、`H_NOTIF_LOGS(C)` に配信ログを記録する。
   2. メール対象外(AI 利用上限到達 等): 受信箱お知らせのみで通知し、メールは送らない。
5. 受信者は管理画面のお知らせ一覧([API-ANN-001](../02_basic-design/API-inbox.md#API-ANN-001))で当該システム通知を確認できる。

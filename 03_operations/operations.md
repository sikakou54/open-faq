...
| 切り分け | 影響範囲(全テナント/特定テナント/特定機能)を判定できること |
...
| テナント分離検証 | クロステナントアクセス試行(自動 50 ケース + 手動 5 ケース) | カスタムスイート | 月次自動 + 年次手動レビュー(AC-017 と整合) |
...
| 6 撤回(FR-010) | 物理削除前であれば撤回可 | 30 日以内 |
...
| 非同意時の挙動 | テナント管理者が「不同意」を明示した場合は退会フロー(SCR-024)へ誘導 |
...
| ログ形式 | JSON Lines(構造化)。必須フィールド = `timestamp`(ISO 8601 / UTC)、`level`、`tenant_id`(該当時)、`request_id`、`actor_account_id`(該当時)、`action`(監査ログのみ)、`message` |
...
| 検索性能 | 1 テナント 7 日分のログ検索を 30 秒以内(管理画面の運営者用ログビューア)。それ以上は非同期エクスポート |
...
| **月次集計バッチ** | 月次(UTC 15:00) | テナント利用量集計、`usage_metering` 更新 |
| **月次確定 cron** | 月次(JST 02:00) | 請求書発行、idempotent key `(tenant_id, billing_year_month)`(FR-148) |
...
| テナント分離テスト | 月次自動 + 年次手動 | クロステナントアクセス試行(AC-017 / §14.3) |
...
| 切り分け | 影響範囲(全テナント / 特定テナント / 特定機能)を判定 |
...
| レート制限上書き状況 | `tenant_quota_overrides` の有効件数、しきい値超過テナント、有効期限切迫の上書き |
| AI 品質メトリクス | 回答可能率・解決率・矛盾理由コード分布・PII 検出件数(NFR-804 (g)) |
| D1 容量 | NFR-804 (h)、8GB 警告 |
| 削除請求 SLA | 残時間、5 営業日経過リマインド、7 営業日超過アラート(NFR-804 (i)) |
...
| 管理画面のお知らせ受信箱(system 種別) | テナント管理者向けの個別通知 |
| 重大障害時のメール通知 | 全テナント管理者・運営者へ |
...
| KPI | しきい値 | 通知先 | 重要度 |
|-----|---------|-------|--------|
| (a) テナント別 5xx 率 | > 1% (5 分窓) | Slack `#ops-warn` | normal |
...
| (i) 削除請求 SLA 残時間 | 残 2 営業日 | 運営者 inbox | normal |
...
| 第 3 階層 (on-call manager) | リソース調整 / 外部連絡可否 | サービス縮退 / 計画停止判断、テナント周知の判断 |
...
- alert: TenantErrorRateHigh
  ...
  annotations: { summary: "テナント {{$labels.tenant}} の 5xx 率 > 1%" }
...
| 段階 | 期間 | 対象 | 判定 KPI |
|-----|------|------|---------|
| MVP リリース判定期間 | 4 週間 | 本番環境の招待テナント | §18.3 参照 |
...
wrangler d1 migrations create main-db "0042_add_tenants_timezone_column"
# → migrations/0042_add_tenants_timezone_column.sql が生成される
...
wrangler d1 migrations create main-db "0043_rollback_tenants_timezone"
...
v2 (breaking change) リリース時の **ウィジェット埋込先テナントへの周知** プロトコル:

| タイミング | チャネル | 内容 |
|---|---|---|
| 60 日前 | 開発者ブログ (`https://docs.example.com/changelog/v2-migration`) | 概要 + 移行ガイド公開 |
| 30 日前 | テナント管理者宛 **in-app 通知** (重要度 high) | 期限 + 移行リンク + 影響範囲 |
| 30 日前 | メール (テナント代表者宛、Resend 送信) | 同上 |
| 7 日前 | テナント管理者宛 **in-app 通知 (critical)** + メール | 最終警告 |
| 廃止日前日 | テナント管理者宛 **in-app 通知 (critical)** + メール + ウィジェット側の sentry 風バナー (本番 v1 アクセス時に「24h 後に廃止」表示) | 翌日廃止 |
...
| 切り分け | 影響範囲(全テナント/特定テナント/特定機能)を判定できること。判定手順は §14.8 Runbook で定義 |
...
| E2E テスト(運営画面) | SCR-090〜097 + 連携 IF #1〜12 | Playwright | nightly |
| 負荷試験 | 100 RPS / テナント 200 / FAQ 1 万 | k6 | 月次 + 主要リリース前 |
| テナント分離検証 | クロステナントアクセス試行(自動 50 + 手動 5) | カスタムスイート | 月次自動 + 年次手動レビュー |
| AI 品質回帰 | FAQ × 想定質問のペア(MVP **50 組以上**) | カスタムバッチ | モデル更新時・プロンプト変更時 |
...
| RB-003 | D1 容量逼迫 | NFR-115 アラート(8GB / 80% 到達) | (1) NFR-115 自動緩和の実行状況確認 → (2) シャーディング設計起動判断 → (3) 古データのアーカイブ進捗確認 → (4) 必要に応じ新規テナント受付一時停止(NFR-110) | NFR-110 / NFR-115 |
| RB-004 | 運営者アカウント侵害疑義 | NFR-311 異常検知(IP 許可リスト外アクセス、運営者操作頻度異常) / R-015 | (1) 該当運営者セッション全無効化 → (2) MFA 強制再設定要求 → (3) 全運営者操作ログを直近 30 日に渡って洗出し → (4) 影響テナントへ inbox/high 通知 → (5) ポストモーテム + 法務報告判断 | NFR-311 / R-015 / FR-211 |
| RB-005 | バウンス率・苦情率の急増 | NFR-503 / NFR-504 アラート | (1) 急増対象テナントの絞込み → (2) 必要に応じテナント別送信一時停止 → (3) サプレスリスト確認 → (4) DKIM/SPF/DMARC 状態確認 → (5) Resend 側状況確認 | NFR-503 / NFR-504 / FR-230 |
| RB-006 | テナント緊急停止(規約違反、要件 §6.2.1 区分3 セキュリティインシデント発動時など) | 通報 / 自動検知 / FR-224 | (1) 4-eyes 承認(§6.2.3) → (2) `accounts.status='suspended'` 遷移 → (3) ウィジェット強制停止 → (4) テナント管理者へ通知(FR-211) → (5) 異議受付窓口の用意 | FR-224 / FR-211 / §6.2.3 |
...
| RB-008 | 削除請求 SLA 違反予兆 | NFR-804 KPI(削除請求 SLA 違反率) | (1) 滞留請求の確認 → (2) 本人確認トークン発行状況確認(FR-228)→ (3) ベンダー側遅延有無確認 → (4) 7 営業日 SLA 遵守判定 | FR-227 / FR-228 / NFR-804 |
...
| ログ形式 | JSON Lines(構造化)。必須フィールド = `timestamp`(ISO 8601 / UTC)、`level`、`tenant_id`(該当時)、`request_id`、`actor_account_id`(該当時)、`action`(監査ログのみ)、`message`(メインシステム §14.8 と同一) |
...
| 検索性能 | 運営者ダッシュボード経由で 1 テナント 7 日分のログ検索を 30 秒以内、全テナント横断の検索は非同期エクスポートで対応(メインシステム §14.8 と整合) |
...
| 切り分け | 影響範囲判定(全テナント/特定テナント/特定機能)、手順は §12.3 Runbook |
...
| RB-004 | 運営者アカウント侵害疑義 | NFR-311 異常検知 / R-015 | セッション全無効化 → MFA 強制再設定 → 30 日操作ログ洗出し → 影響テナント通知 → ポストモーテム+法務報告 | P0 |
| RB-005 | バウンス率・苦情率の急増 | NFR-503 / NFR-504 アラート | テナント絞込 → 送信一時停止 → サプレスリスト確認 → DKIM/SPF/DMARC 確認 → Resend 状況 | P0 |
| RB-006 | テナント緊急停止(規約違反) | 通報 / 自動検知 / FR-224 | 4-eyes 承認(MVP は Log Only)→ `suspended` 遷移 → ウィジェット停止 → テナント管理者通知 → 異議受付 | P0 |
| RB-007 | バックアップ復元 | データ毀損 / RTO 4h | 影響範囲特定 → 復元ポイント選定 → staging 検証 → 本番適用 → 整合性 4 項目検証 → ポストモーテム | P0 |
| RB-008 | 削除請求 SLA 違反予兆 | NFR-804(j)KPI | 滞留請求確認 → 本人確認トークン状況 → 遅延有無確認 → 7 営業日 SLA 判定 | P0 |
| RB-009 | AI 誤回答に関する重大苦情対応 | サポート問い合わせ | 個別事案調査、原因 FAQ 修正提案、運営者 inbox に集約 | P0 |
| RB-010 | サスペンション解除フロー | 課金 Webhook(支払成功)/ 手動解除 | 支払完了確認 → `active` 復帰 → テナント管理者通知 | P0 |
| RB-011 | 削除データ復元と物理削除ジョブの競合 | FR-204 異常系 | ロック取得失敗時のロールバック、運営者 high 通知、再試行判断 | P0 |
| RB-012 | 規約改定 14 日不同意テナントの強制制限移行 | バッチ起動 | SCR-025 割込制御、機能制限の自動適用 | P1 |
| RB-013 | DKIM/SPF/DMARC レコード変更時の段階展開 | DNS 変更計画 | テスト送信 → 部分テナント先行 → 全体展開 | P1 |
...
| RB-019 | 祝日マスタ更新時の SLA 計測切替確認 | 内閣府 CSV 年次更新 | 取込実行 → 単体テスト再実行 → 結果通知 → 前年マスタとの差分確認 | P1 |
...
| RB-022 | 削除請求 GDPR 期限リスク検知 | §12.6.2 検証バッチ `gdpr_risk_flag=true` | (1) 運営者 inbox(legal/high)+ メール通知、(2) 法務担当エスカレーション、(3) 運営者が `sla_due_at` を GDPR 期限内に手動短縮(`deletion.sla.gdpr_override` action、retention_class='5y') | P0 |
| RB-023 | EU 顧客の登録試行検知 | テナント登録時の §1.5.1 EU/EEA/UK 国コード判定ヒット | (1) `accounts.status='pending_legal_review'` 保留、(2) 運営者 inbox(legal/high)+ メール通知、(3) 法務担当エスカレーション、(4) 受入判定: 拒否 or 個別契約締結後の承認、(5) 同意取得書面 ID を `audit_logs(tenant.legal_review.approve, retention_class='5y')` に記録 | P0 |
...
#### 12.6 SLA 計測ロジック(D-16、正本)

本節は削除請求(FR-227)SLA 計測ロジックの **正本**。§4.2 状態遷移図 / §6.2.5 フロー / §5.3.6 SCR-095 画面から本節を参照する。

| 項目 | 仕様 |
|---|---|
| 起点 | `deletion_requests.state='pending'` 入力時刻 |
| 終点 | `state='completed'` 遷移時刻 |
| SLA | 7 営業日以内 |
| 営業日定義 | 月〜金 - `holiday_master` × kind ∈ {national_holiday, substitute, national_rest, special} - {12/29, 12/30, 12/31, 1/1, 1/2, 1/3} |
| リマインド | 5 営業日経過時点 |
| エスカレーション | 7 営業日超過時点 |
| 自動 expired | `in_review` 滞在 14 日(暦日基準、営業日換算しない) |

##### 12.6.1 営業日計算アルゴリズム(疑似コード)

```text
function calculate_sla_due_at(received_at: UTC datetime) -> UTC datetime:
    # 1. JST に変換
    jst = received_at.to_timezone("Asia/Tokyo")
    # 2. 受領時刻が JST 営業日外なら翌営業日 00:00 JST にリベース(起点正規化)
    if not is_business_day(jst.date):
        jst = next_business_day_start(jst.date)
    # 3. 7 営業日を加算(初日不算入: 起点日を 0 日目として 7 営業日後の終業 23:59:59 JST)
    counted = 0
    cursor = jst.date
    while counted < 7:
        cursor = cursor + 1 day
        if is_business_day(cursor):
            counted += 1
    sla_due_at_jst = datetime(cursor, 23:59:59, tz="Asia/Tokyo")
    return sla_due_at_jst.to_utc()

function is_business_day(date) -> bool:
    if date.weekday in {SAT, SUN}: return False
    if date in HOLIDAY_MASTER where kind in {national_holiday, substitute, national_rest, special}: return False
    if date.month_day in {(12,29), (12,30), (12,31), (1,1), (1,2), (1,3)}: return False
    return True
```

##### 12.6.2 連続休暇境界判定(GDPR 整合性検証)

海外顧客の削除請求は MVP では日本祝日基準で運用するが、**連続休暇(GW・年末年始等)を跨ぐ場合は日本基準の `sla_due_at` が GDPR 1 ヶ月期限(GDPR 12 条 3 項、受領日から 30 暦日以内)を超過しないか追加検証** する。

| 観点 | 仕様 |
|---|---|
| 検証起動 | 全削除請求の `pending` 状態 INSERT 時(同期)、および日次 03:00 JST バッチ(滞留分の再検証) |
| 検証式 | `gdpr_due_at = received_at + 30 暦日`(GDPR 受領から 1 ヶ月、暦日基準)。`sla_due_at_jst > gdpr_due_at` なら GDPR リスクと判定 |
| 連続休暇定義 | (a) GW 連続休暇(`holiday_master` で 4/29〜5/5 期間内に 5 連続以上の非営業日)、(b) 年末年始(12/29〜1/3 固定除外)、(c) その他 4 連続以上の非営業日を含む期間 |
| 連続休暇跨ぎ判定 | `received_at` から `sla_due_at_jst` までの期間が、上記 (a)〜(c) のいずれかの連続休暇期間と重なる場合に「跨ぎ」と判定 |
| GDPR リスク検知時の挙動 | (1) `deletion_requests.gdpr_risk_flag=true` 立て、(2) 運営者 inbox(legal/high)通知 + メール、(3) SCR-095 で対象請求を「GDPR 期限警告」バッジ表示、(4) 運営者が `sla_due_at` を GDPR 期限内に手動短縮可(`deletion.sla.gdpr_override` action コード、retention_class='5y')、(5) 短縮不可な場合は法務エスカレーション(RB-022 新設) |
| 計算例 1(GW 跨ぎ) | 受領 2026-04-28(火)→ 翌営業日起算 → 7 営業日後の終業 = GW を含むため 2026-05-12(火) 23:59:59 JST = 受領後 14 暦日 < 30 暦日 → **GDPR 期限内、リスクなし** |
| 計算例 2(年末年始跨ぎ) | 受領 2026-12-26(土)→ 翌営業日 2026-12-28(月)起算 → 7 営業日後の終業 = 年末年始を含むため 2027-01-14(木) 23:59:59 JST = 受領後 19 暦日 < 30 暦日 → **GDPR 期限内、リスクなし** |
| 計算例 3(極端ケース) | 受領 2026-12-31(木 / 固定除外)→ 翌営業日 2027-01-05(火)起算 → 7 営業日後 = 2027-01-14(木) 23:59:59 JST = 受領後 14 暦日 → **GDPR 期限内、リスクなし**。ただし極端な祝日追加で日本基準が 30 暦日を超える可能性に備えて検証は常時実施 |
| 計算例 4(リスク発火、仮想) | 内閣府特別措置法で連続 14 営業日除外が宣言された場合、日本基準 7 営業日が 30 暦日を超過しうる。本検証はこれを検知し運営者にエスカレーション |

##### 12.6.3 MVP の削除請求 SLA 基準

MVP は日本祝日基準で削除請求 SLA を計算する。GDPR 期限リスク検証バッチで 30 暦日超過リスクを検知し、運営者に通知する。
...
| SCR-095 | 削除請求状態別カウンタ、SLA 期限、祝日マスタ |
| SCR-099 | ペイロード差分検出件数 |
...
| 削除請求 transition | < 500ms | < 750ms | < 1500ms | p95 | 5 分連続超過 |
...
実装上の留意点:
- `audit_logs` は actor / target / action / tenant / retention_class の 5 軸インデックス(§8.4)で網羅
...
- **NFR-205 計画停止との関係**: 計画停止 (D-09 / NFR-205) は **事前告知 (運営者 inbox + テナント管理者宛 in-app 通知 7 日前)** のうえ実施し、**SLO 計測対象外** とする。
...
| 削除請求 SLA 違反率 (KPI(j)) | D1 (deletion_requests) | cron 集計、日次 02:30 JST | — | 運営 |
...
KPI 一覧(メイン NFR-804 (a)〜(i) を正本参照 + 本書独自 (j)(k)(l)):

| KPI | 主管 | しきい値 / 重大度 | 通知先 | データソース |
|---|---|---|---|---|
| (a)〜(i) | メイン | メイン NFR-804 | - | メイン |
| (j) 削除請求 SLA 違反率 | 本書 | > 5% で `high` | 全運営者 | `deletion_requests` |
| (k) 運営者操作頻度異常 | 本書 | 同一運営者 1h で 50 件超 | 全運営者 | `audit_logs(actor_id, occurred_at)` |
| (l) 全テナント横断 MAU・解決率 | 本書 | 急減で `high`(直近 7 日平均から 20pt 低下) | 運営者 inbox | メイン側集計を参照 |
...
| **AI 内部原価(FR-304)** | 本書 | 月次集計、テナント別、Workers AI 実績 token 数 × 単価。集計遅延 24h 超で normal、72h 超で high | 運営者 inbox(SCR-096) | `question_logs.ai_token_count_input` / `question_logs.ai_token_count_output` + KV `ai-cost:unit-prices` |

計測基盤: Cloudflare Workers Analytics Engine、5 分窓集計、日次評価。

AI 内部原価集計仕様(FR-304):
- 集計粒度: テナント別月次(粗利分析用)、グローバル日次(運用監視用)
- 表示先: SCR-096 KPI セクション、SCR-093 テナント別利用状況参考値
- 用途制限: テナント請求には含めない(FR-304(d))
...
|---|---|---|---|
| 個人情報保護法 (APPI) | 適用 | §5.3.6 削除請求 / §13.4 保持期間 / §12.7 PII 検出 | 第 28 条 越境移転規制は apac リージョン固定 |
| GDPR | 適用 (海外テナント) | §13.9 削除請求 SLA / §5.3.6 | <!-- TBD: GDPR 適用テナント判定。担当: PO + 法務 --> |
...
| AI 内部原価のテナント別分布 | KV `ai-cost:unit-prices` (顧管 §13.3) の単価管理と質問ログ集計の組合せで月次レポート (CSV) として出力 |
| 値上げ / 解約判断材料 | 上記 AI 内部原価分布 + テナント別 D1 行数 + Workers AI Neurons 消費の 3 指標を組合せ、四半期で運営判断 (担当: PO + SRE) |
...
| 行件数検証 動的閾値 | メイン §13.6.5 と同様 (主要テーブル `audit_logs` / `operator_approvals` / `webhook_events` / `deletion_requests` / `service_operators` で過去 30 日 ±2σ または ±5%)。`SLAComputeWorker` (顧管 §14.2 / 新設) が σ を日次集計 |
...
- **キー命名**: 運営者画面は `screen.scr_09X.*` / `errors.<feature>.<code>` 命名規則に従う（メイン §13.5.X 参照）。共通部品は `cmp.<CMP_ID>.*` 名前空間を使い、テナント側 (`screen.scr_0XX.*`) と衝突しない
- **カタログ管理場所**: `app/shared/src/i18n/admin/ja.ts` を運営者画面専用カタログ、`app/shared/src/i18n/ja.ts` (テナント側) と分離。同じ namespace 構造で管理
- **運営者向けメールテンプレート 15 種**（§11.1）は MVP は ja 固定。
- **テナント管理者・エンドユーザー向けメールテンプレート**（顧管 → メイン経由 IF #7 / 通知 14 種）はメイン §11.5 と同基盤で多言語化、特定電子メール法対応フィールド (COMPANY_LEGAL_NAME, UNSUBSCRIBE_URL) は言語別に保持 <!-- TBD: 翻訳責任者。担当: PO -->
- **タイムゾーン**: 運営者画面の日時表示は **メイン側と同じ `formatJst` を再利用** し JST 固定。SLA 計算 (§13.9) で営業日加算する祝日マスタも JST 基準（`holiday_master.locale='jp'`）。
...
| 削除確認トークン | TTL 24 時間 | HKDF `deletion-confirm` |
| 削除請求 SLA | 7 営業日 | `deletion_requests.sla_due_at` |
| 削除請求 in_review TTL | 14 日(暦日) | `deletion_requests.expires_at` |
| 運営者セッション | 8 時間 | D-18 |
...
| 重大度 | アサイン | 着手 | 告知 |
|---|---|---|---|
| P1(全テナント停止 / データ毀損) | 15 分以内 | 30 分以内 | ステータスページ + 運営者 inbox(high)+ 対象テナント管理者 |
| P2(機能影響 / SLO 違反疑い) | 1 時間以内 | 1 時間以内 | 運営者 inbox + 関係者 |
| P3(軽微・非機能影響なし) | 翌営業日内 | 翌営業日内 | 運営者 inbox |

オンコール体制は MVP では営業時間 + 緊急時ベストエフォートとする。

#### 13.9 SLA 計測ロジック(D-16、正本)

削除請求(FR-227)SLA 計測の **正本**。§4.2 / §6.2.5 / §5.3.6 から本節を参照する。

| 項目 | 仕様 |
|---|---|
| 起点 | `deletion_requests.pending_at`(`state='pending'` 入力時刻) |
| 終点 | `deletion_requests.completed_at`(`state='completed'` 遷移時刻) |
| SLA | 7 営業日以内 |
| 営業日定義 | 月〜金 − `holiday_master` × kind ∈ {national_holiday, substitute, national_rest, special} − {12/29, 12/30, 12/31, 1/1, 1/2, 1/3} |
| 海外顧客(GDPR) | MVP では日本祝日基準 |
| リマインド | 5 営業日経過時点 |
| エスカレーション | 7 営業日超過時点 |
| 自動 expired | `in_review` 滞在 14 日(暦日基準) |

##### 13.9.1 営業日加算アルゴリズム(擬似コード)

```text
function addBusinessDays(start: ISO8601, bd: integer): ISO8601:
    cur = parseDate(start)
    remaining = bd
    while remaining > 0:
        cur = cur + 1 day
...
function isBusinessDay(date: Date): boolean:
    if dayOfWeek(date) in {Sat, Sun}: return false
    if date in {Dec 29, Dec 30, Dec 31, Jan 1, Jan 2, Jan 3}: return false
    if exists(SELECT 1 FROM holiday_master WHERE date=?): return false
    return true
```

##### 13.9.2 営業日加算 SQL(D1 で計算する場合の参考)

```sql
-- sla_due_at を計算する SQL(疑似)
-- 注: D1 は CTE をサポートするが、複雑な営業日計算はアプリ層 + KV holiday-cache 推奨
WITH RECURSIVE bd_count(date, bd) AS (
  SELECT date(?), 0           -- ? = pending_at
  UNION ALL
  SELECT date(date, '+1 day'),
         bd + CASE
           WHEN strftime('%w', date(date, '+1 day')) IN ('0', '6') THEN 0
           WHEN date(date, '+1 day') IN ('YYYY-12-29','YYYY-12-30','YYYY-12-31','YYYY-01-01','YYYY-01-02','YYYY-01-03') THEN 0
           WHEN EXISTS (SELECT 1 FROM holiday_master h WHERE h.date = date(date, '+1 day')) THEN 0
           ELSE 1
         END
    FROM bd_count
   WHERE bd < 7
)
SELECT date FROM bd_count WHERE bd = 7 LIMIT 1;
```

実装上は `HolidayMasterFetchWorker` 取込結果を KV `holiday-cache:<year>` にプリロードし、アプリ層で計算。
...
| RB-004 | 運営者アカウント侵害疑義 | NFR-311 / R-015 | セッション全無効化 → MFA 強制再設定 → 30 日操作ログ洗出し → 影響テナント通知 → ポストモーテム+法務 | P0 |
| RB-005 | バウンス・苦情急増 | NFR-503/504 | テナント絞込 → 送信一時停止 → サプレスリスト → DKIM/SPF/DMARC → Resend 状況 | P0 |
| RB-006 | テナント緊急停止 | 通報 / 自動 / FR-224 | 4-eyes(MVP Log Only)→ `suspended` → ウィジェット停止 → テナント通知 → 異議受付 | P0 |
| RB-007 | バックアップ復元 | データ毀損 / RTO 4h | 影響範囲 → 復元ポイント → staging 検証 → 本番適用 → 4 項目検証 → ポストモーテム | P0 |
| RB-008 | 削除請求 SLA 違反予兆 | NFR-804(j) | 滞留請求 → トークン状況 → 遅延有無 → 7 営業日判定 | P0 |
| RB-009 | AI 誤回答重大苦情 | サポート | 個別事案調査 → 原因 FAQ 修正提案 → 運営者 inbox 集約 | P0 |
| RB-010 | サスペンション解除 | 課金 Webhook(成功)/ 手動 | 支払完了確認 → `active` 復帰 → テナント通知 | P0 |
| RB-011 | 復元と物理削除ジョブ競合 | FR-204 異常系 | ロック取得失敗時のロールバック、運営者 high、再試行判断 | P0 |
| RB-012 | 規約改定 14 日不同意の強制制限移行 | バッチ | SCR-025 割込制御、機能制限の自動適用 | P1 |
| RB-013 | DKIM/SPF/DMARC 段階展開 | DNS 変更計画 | テスト送信 → 部分テナント先行 → 全体展開 | P1 |
...
| RB-019 | 祝日マスタ更新時 SLA 切替確認 | CSV 年次 | 取込 → 単体テスト → 通知 → 前年差分 | P1 |
...
| 段階 | チェックリスト |
|---|---|
| MVP | P0 機能(SCR-090〜097 + 主要 cron + 監査ハッシュチェーン)、4-eyes ハードゲート 3 操作 + 承認ログ 7 操作、PII 第 1 / 第 3 層、AC-038 監査通知、AC-039 SLA、AC-041 冪等、AC-042 請求冪等 |
...
| FUT-REQ-OPS-003 | 利用規約・プライバシーポリシー正式版 | 限定公開対象テナントとの個別契約で補完 | GA | 法務最終承認版が完成していること |
...
| NFR-804 | 監視 | 主要 KPI を監視し、しきい値超過時にアラート通知できること。具体 KPI / しきい値 / 通知先 / 重大度はメインシステム NFR-804 を正本とし、本書側でも同一基準で監視する。運営者ダッシュボード(FR-230)で参照する KPI はメインシステム NFR-804 の (a)〜(i) に加え、(j) 削除請求 SLA 違反率、(k) 運営者操作頻度異常検知(FR-229(c))、(l) 全テナント横断 MAU・解決率を含めること |
...
| 原因切り分け | 30 分 | テナント特定、復旧対象データ特定、復旧手段(Time Travel / R2 リストア)選択 |
...
| (c) チェックサム比較 | テナント単位のテーブルチェックサム(SHA-256)を計算し、復元前後で比較 |
...
MVP はリリース判定期間を設け、招待制 10 テナントで稼働確認を行う。判定 KPI は §13.3 に従う。

#### 13.2 機能優先度のリリース対応(参照: 要件 §18.2)

| 優先度 | 対象機能 |
|---|---|
| P0 | FAQ 管理(下書き含む)、FAQ 根拠限定回答、未解決質問登録、問い合わせ ID、個別チャット、メール登録、メール通知、未解決から FAQ 登録、処理エラー表示、課金基本機能、ユーザー管理、テナント分離、HTTPS・レート制限・操作ログ |
...
|---|---|---|---|---|
| (3-a) D1 単一インスタンス障害 | 全テナント書込不可 | 検知 5 分 + Cloudflare 自動フェイルオーバー 30 分 + 整合性検証 1 時間 = **約 1.6 時間** | D1 Time Travel = **15 分以内** | RTO 4h 以内 / RPO < 1h ⇒ 達成 |
...
| 顧客管理側 Worker 障害 | メイン側は §8.7.6 単独動作可能範囲で継続 | 課金画面・新規テナント登録は遅延、他は継続 | §8.7.6 |
| メイン側 Worker 障害(顧客管理側から見て) | 顧客管理側は §11.5.1(顧客管理基本設計)縮退運転で継続 | 監査・削除請求 SLA は継続、削除実行・サスペンション伝達は遅延 | 顧客管理 §11.5.1 / RB-021 |
...
|---|---|---|---|
| (a)〜(i) | メイン | メイン NFR-804 | - |
| (j) 削除請求 SLA 違反率 | 本書 | > 5% で `high` | 全運営者 inbox + メール |
| (k) 運営者操作頻度異常 | 本書 | 同一運営者 1h で 50 件超 | 全運営者 inbox + メール |
| (l) 全テナント横断 MAU・解決率 | 本書 | 急減で `high`(直近 7 日平均から 20 ポイント低下)| 運営者 inbox |
...
| **AI 内部原価(FR-304)** | 本書 | 月次集計・テナント別表示・Workers AI 実績 token 数 × 社内原価単価(詳細設計 §13.3 と整合)。集計遅延 24h 超で `normal`、72h 超で `high` | 運営者 inbox(原価データは SCR-096 経由で運営者のみ閲覧、テナント請求項目には含めない FR-304(d)) |
...
- 表示先: SCR-096 KPI セクション、SCR-093 テナント別利用状況参考値
- 用途制限: テナント請求には含めない(FR-304(d))、運営者の単価設定の根拠データおよび粗利分析のみ
...
メインシステム(テナント向け Worker / D1 / KV)が部分または全断した場合の、運営者側で「継続可能な操作」「不可能な操作」を以下に定義する。メイン §8.7.6(顧客管理障害時のメイン単独動作)と対称配置する正本表である。

| 機能 | メイン障害時の挙動 | 実装方針 |
|---|---|---|
| 監査ログ閲覧・エクスポート(SCR-096 / FR-232) | **継続可** | 監査ログは本書側 D1 に独立保管。`audit_logs` への参照系操作はメインへの依存なし |
| 削除請求 SLA 監視(SCR-095 / FR-227) | **継続可(タイマー計測のみ)** | `deletion_requests` は本書側 D1。SLA 違反検知・運営者 inbox 通知は継続。**ただし削除実行(`pending → in_review → completed`)はメイン側 PII 削除 API(連携 IF #11)に依存するため、メイン断中は `in_review` 滞留**(`sla_due_at` を超過する場合は high alert 発火、復旧後に運営者判断で SLA 除外申請を `audit_logs(deletion.sla.exclusion)` で記録) |
...
| 月次請求確定 cron(FR-303) | **継続可(本書側集計のみ)** | `billing_invoices` 確定は本書側 D1 で完結。テナント管理者通知は連携 IF #12 経由のため **遅延処理**(キュー退避) |
| ハッシュチェーン日次検証(D-04) | **継続可** | 本書側で完結、メイン依存なし |
| 祝日マスタ取込(RB-019) | **継続可** | 外部 CSV → 本書側 KV / D1 で完結 |
...
| テナント別レート/予算上書き(SCR-093 / FR-121, FR-128) | **継続可(本書側 KV 書込まで)/ メイン反映は遅延** | 上記 AI パラメータと同様 |
...
| テナント物理削除(`deleted_pending → deleted`、ハードゲート) | **不可** | メイン側 D1 への削除カスケード(連携 IF #1)が必須。**メイン断中は申請受付のみ可、実行は復旧後** |
...
| 運営者操作 → テナント管理者通知(FR-211 / 連携 IF #12) | **遅延処理** | キュー退避、復旧後に集約窓 10 分(D-19)で再集約配信 |

**復旧時の整合性確保手順**(メイン §8.7.6 対称、本書側からの送信側補完):

1. 連携 IF #10 課金 Webhook の DLQ → メインへのリプレイ完了確認(SCR-097)
2. 連携 IF #1 テナント停止イベントの未送信キュー消化(運営者操作通知の集約配信完了)
...
**メイン断中の運営者画面表示**: SCR-090〜099 の上部に「メインシステム障害中(継続不可機能: テナント物理削除 / 削除データ復元 / 緊急ウィジェット停止)」のバナーを表示し、不可機能のボタンは disabled 化する(誤操作防止)。
...
| 区分 | 内容 |
|---|---|
| MVP | P0 機能、4-eyes ハードゲート 3 操作、PII 第 1 / 第 3 層、AI モデル初期値、削除請求 SLA、課金 Webhook、監査ログ、運営者 MFA / IP 許可 / 再認証 |
| MVP リリース判定期間 | 招待制テナントで NFR-101〜106 p95 を 4 週間連続計測し、AC-046 を確認 |

#### 13.2 4-eyes MVP 適用範囲

| 区分 | 適用範囲 |
|---|---|
| MVP ハードゲート(3) | テナント物理削除 / AI 推論パラメータ変更 / マスター鍵ローテーション |
| MVP 承認ログのみ(7) | テナント無効化、テナント復元、単価表・無料枠改定、テナント別レート制限・予算上限上書き、緊急ウィジェット強制停止、削除データ復元、ハードゲート KV フラグ切替 |

#### 13.3 機能優先度(要件 §18.2)

| 優先度 | 顧客管理側の対象 |
|---|---|
| P0 | SCR-090〜097、4-eyes ハードゲート 3、削除請求 SLA、Stripe Webhook、月次請求確定、運営者 MFA・IP 許可・再認証、監査ハッシュチェーン、AI パラメータ 3 階層、レート/予算上書き、お知らせ配信、運営者操作通知、PII 第 1・第 3 層 |
| P1 | SCR-098、SCR-099、AI モデル切替、SCA 週次、ペネトレ年 1、テナント別単価上書き、訂正請求発行 UI |
| P2 | MVP では定義しない |
...
| 項目 | 仕様 |
|---|---|
| データ | KV `feature:<name>:<scope>`(scope = `global` / `tenant:<id>` / `operator:<id>`) |
...
| 項目 | 移行方針 |
|---|---|
| テナント MFA | カラム追加後、既存テナントは `off` で開始。限定公開対象だけ opt-in を許可し、ベータで enforcement 値を広げる |
...
| 優先度 | 対象 |
|---|---|
| P0 | FAQ管理(下書き含む)、FAQ根拠限定回答、未解決質問登録、問い合わせID、個別チャット、メール登録、メール通知、未解決からFAQ登録、処理エラー表示、課金基本機能、ユーザー管理、テナント分離、HTTPS・レート制限・操作ログ |
...
| 観点 | 判定基準 |
|---|---|
| 機能 | 全 P0 機能が実装され、受入条件 **AC-001〜AC-044(分解された AC-039a〜c / AC-040a〜c / AC-044a〜b を含む)・AC-046・AC-047** を満たす |
| 性能 | AC-046 のとおり、MVP リリース判定期間で NFR-101〜106 の p95 目標を 4 週間連続達成 |
| セキュリティ | テナント分離、ドメイン検証、レート制限、HTTPSが動作する |
| プライバシー | 利用目的表示・削除請求受付フローが用意されている |

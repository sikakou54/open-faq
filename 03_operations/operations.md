...
| GDPR | 適用 (海外契約) | §13.9 削除請求 SLA / §5.3.6 | <!-- TBD: GDPR 適用契約判定。担当: PO + 法務 --> |
...
| NFR-804 | 監視 | 主要 KPI を監視し、しきい値超過時にアラート通知できること。具体 KPI / しきい値 / 通知先 / 重大度はメインシステム NFR-804 を正本とし、本書側でも同一基準で監視する。運営者ダッシュボード(FR-230)で参照する KPI はメインシステム NFR-804 の (a)〜(i) に加え、(j) 削除請求 SLA 違反率、(k) 運営者操作頻度異常検知(FR-229(c))、(l) 全契約横断 MAU・解決率を含めること |

...
| **UC新設候補(判断待ち)** | 57 | 下記の操作系。離散 UC 化の要否はオーナー判断 |

FR→UC 被覆は **対応 62 → 107**(+45)、受容 30、未割当 57 に整理された。

### 8.1 UC 新設候補(57件・機能グループ別・要オーナー判断)

下表の操作系 FR は現行の画面イベント由来 UC 集合に対応操作が無い。離散 UC を新設するか、既存 UC の一部とみなすか、バックログ化するかはオーナー判断。
...
> [!NOTE]
> 第2段の割当 45 件は機械導出のため「要レビュー」。受容 30 件は横断設計(権限 `PERM-*` / エラー `ERR-*` / 非機能 NFR / AI 推論)で担保され、業務 UC を持たないのが正しい。UC 新設候補 57 件の扱い(新設 / 既存 UC へ吸収 / バックログ)は次段でオーナー確認のうえ確定する。

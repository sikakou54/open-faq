...
|---|---|---|---|
| API→UC 無し | 2 | API-031 API-032 | 検索系。UC-304 へ連結(解消) |
...
| **UC新設(実施済み)** | 57 | 下記の操作系。オーナー承認のもと UC-248〜UC-304 を新設(§8.2) |

FR→UC 被覆は **対応 62 → 164**(+45 既存連結 +57 新設)、受容 30、**未割当 0**(ギャップ完全解消)に整理された。

### 8.1 UC 新設候補(57件・機能グループ別)

下表の操作系 FR は現行の画面イベント由来 UC 集合に対応操作が無かった。オーナー判断「業務UCを新設」に従い、§8.2 で UC-248〜UC-304 を新設した。
...
### 8.2 UC 新設 実施結果(UC-248〜UC-304・57件)

オーナー判断「業務UCを新設」に従い、§8.1 の操作系 FR 57 件に対し業務ユースケース **UC-248〜UC-304** を新設した(15項目テンプレ・FR 番号昇順)。各 UC は対応 FR と関連 BR を `対応要件ID` にリンクし、対応する各 FR の `対応業務UC` も双方向更新した。画面操作を伴わないもの(ウィジェット稼働・通知・バッチ・非同期)はシステム起点として基本フローにトリガーを明記。新設 UC は `02_business_usecases/index.md` の新セクション「5. 要件起点ユースケース(第2段新設)」に機能グループ別で索引化した。

| 指標 | 第2段前 | 第2段後 | 備考 |
|---|---|---|---|
| 業務 UC 総数 | 247 | **304** | +57(UC-248〜UC-304) |
...
| BR-145 | 未解決質問の状況管理 | [UC-268](../01_requirements/02_business_usecases/UC-268.md#UC-268) へ連結(双方向) |
...
| API | 名称 | 対応 |
|---|---|---|
| API-031 | FAQ 全文検索 | `対応業務UC` に [UC-304](../01_requirements/02_business_usecases/UC-304.md#UC-304)(FAQ・質問ログを検索する)を連結。UC-304 の `関連API` も双方向更新 |
| API-032 | 質問ログ検索 | 同上(UC-304 へ連結・双方向) |
...
> [!NOTE]
> 構造ギャップは「未使用テーブルの削除」ではなく、**連結補完(検索 API↔UC-304、TBL-012↔API)と性質分類(Control Plane / 横断基盤の受容)** で解消した。削除対象の不要要素は検出されなかった。これにより 要件↔UC↔SCR↔EVT↔API↔TBL の主要トレースは全系列で結線または明示分類された。

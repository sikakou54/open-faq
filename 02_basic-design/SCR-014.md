...
> [!NOTE]
> **補足** 退会はオーナー専有機能です。メンバーには本画面を提供せず、URL 直アクセスは権限不足表示となります。退会申請には確認ダイアログと再認証が必須です。退会フローのタイムライン表示・データエクスポート導線は設けません(データエクスポート機能は MVP 対象外、05_future 参照)。
...
<td>「退会を申請する」を押下</td>
<td><ul>
<li>確認ダイアログ + 再認証(<a href="API-auth.md#API-AUTH-005">再認証</a> API)</li>
<li>再認証後: <a href="API-terms.md#API-TRM-005">退会申請</a> API を実行し status=deleted_pending を設定</li>
</ul></td>
</tr>
<tr>

<!-- portal-top -->
[設計ポータル](../../README.md) ／ [基本設計](../index.md) ／ [ユースケース設計](index.md) ／ **UC-SYSTEM-011: 上限到達ウィジェット受付停止**
<!-- /portal-top -->
...
| 関連要件 | [FR-066](../../01_requirements/FR09.md#FR-066) 上限到達時の受付停止 ・ [FR-064](../../01_requirements/FR09.md#FR-064) 利用量集計 |
| 主テーブル | `M_PRJ_QUOTA_LIMITS(R)` ・ `T_USAGE_METER(R)` |
| 関連 API | [API-BIL-006](../02_api-design/API-billing.md#API-BIL-006) プロジェクト上限参照 |
...
4. 到達済みの場合、支払方法の有無に関わらず当該プロジェクトのウィジェット新規質問受付を停止し、プロジェクト連絡先メールへの問い合わせを促す安全な定型文をチャット内システム返信として返し、入力・送信を無効化する。契約は `active` を維持する([FR-066](../../01_requirements/FR09.md#FR-066))。
...
- 100% 到達時は当該プロジェクトのウィジェット新規質問受付が停止し、安全な定型文が返り、入力・送信が無効化される([FR-066](../../01_requirements/FR09.md#FR-066))。

<!-- portal-top -->
[設計ポータル](../../README.md) ／ [基本設計](../index.md) ／ [ユースケース設計](index.md) ／ **UC-BIZ-010: ウィジェットの応答を最適化する(設定・しきい値・許可ドメイン)**
<!-- /portal-top -->
...
| 関連要件 | [FR-092](../../01_requirements/FR12.md#FR-092) 埋め込みコードの取得・設置 ・ [FR-093](../../01_requirements/FR12.md#FR-093) 許可ドメイン上での動作 ・ [FR-094](../../01_requirements/FR12.md#FR-094) 外観の設定 ・ [FR-098c](../../01_requirements/FR12.md#FR-098c) 受付制限中の案内 ・ [FR-154](../../01_requirements/FR20.md#FR-154) しきい値の即時反映とフォールバック |
...
- 対象プロジェクトが選択されており、ウィジェット設定([SCR-007](../01_screen-design/SCR-007.md#SCR-007))へ到達できる。
- AI しきい値の既定値(信頼度 0.60 / 関連度 0.50)が定義されている([FR-154](../../01_requirements/FR20.md#FR-154))。
...
1. メンバーがウィジェット設定([SCR-007](../01_screen-design/SCR-007.md#SCR-007))を開き、現在の外観・公開キー・埋め込みコードを確認する([UC-SCR-007](UC-SCR-007.md))。
2. メンバーが外観(主色・表示位置・見出し・初期メッセージ)を編集し、プレビューで確認して設定を保存する([FR-094](../../01_requirements/FR12.md#FR-094))。
3. 必要に応じて、メンバーが公開キーを再発行し、新しい埋め込みコードを自社サイトへ設置する([FR-092](../../01_requirements/FR12.md#FR-092))。
4. メンバーが動作を許可するドメインを限定し、許可ドメイン上のみでウィジェットが動作するようにする([FR-093](../../01_requirements/FR12.md#FR-093))。許可ドメインの設定はプロジェクト編集([SCR-004](../01_screen-design/SCR-004.md#SCR-004))で行い、詳細は [UC-SCR-004](UC-SCR-004.md) に委譲する。
...
- **受付制限の到達**: 質問数の月次上限到達時はウィジェットが受付制限中の案内を行い、新規質問の入力・送信を無効化する([FR-098c](../../01_requirements/FR12.md#FR-098c))。
- **権限なし**: 当該プロジェクト未割当のメンバーが直アクセスした場合、操作不可を提示する。

## 7. 事後条件

- ウィジェットの外観・公開キー・埋め込みコードが、保存した内容で運用に反映される。
- 許可ドメイン上のみでウィジェットが動作し、他契約・他プロジェクトのデータ参照を防ぐ([FR-093](../../01_requirements/FR12.md#FR-093))。
- AI しきい値の変更が短時間で以降の推論動作へ反映される([FR-154](../../01_requirements/FR20.md#FR-154))。

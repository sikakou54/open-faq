<!-- portal-top -->
[設計ポータル](../../README.md) ／ [基本設計](../index.md) ／ [ユースケース設計](index.md) ／ **UC-SYSTEM-005: 運営お知らせ配信**
<!-- /portal-top -->
...
4. 重要度を判定し、メール送信対象の宛先へお知らせメールをメール配信 IF([API-MAIL-001](../02_api-design/API-mail.md#API-MAIL-001))で送信する。`critical` は強制送信(オプトアウト不可)、`normal` はオプトアウト設定に従う。
5. メール送信結果を `H_NOTIF_LOGS(C)` に配信ログとして記録する。
6. 受信者は管理画面のお知らせ一覧([API-ANN-001](../02_api-design/API-inbox.md#API-ANN-001))で当該お知らせを確認できる([FR-116](../../01_requirements/FR15.md#FR-116))。

<!-- portal-top -->
[設計ポータル](../../README.md) ／ [基本設計](../index.md) ／ [ユースケース設計](index.md) ／ **UC-SYSTEM-004: 月次請求確定バッチ**
<!-- /portal-top -->
...
| 関連要件 | [FR-064](../../01_requirements/FR09.md#FR-064) 利用量集計 ・ [FR-090](../../01_requirements/FR11.md#FR-090) 月次請求確定通知 |
| 主テーブル | `T_USAGE_METER(R)` ・ `M_PRJ_QUOTA_LIMITS(R)` ・ `M_CONTRACT(R)` ・ `T_BILL_INVOICES(C)` ・ `T_INBOX_MSG(C)` ・ `H_NOTIF_LOGS(C)` |
| 関連 API | [API-BIL-003](../02_api-design/API-billing.md#API-BIL-003) 請求サマリ(確定結果の参照) ・ [API-MAIL-001](../02_api-design/API-mail.md#API-MAIL-001) メール配信 IF |
...
6. バッチが当該契約のオーナーへ月次請求確定通知をメール配信 IF([API-MAIL-001](../02_api-design/API-mail.md#API-MAIL-001))で送信し、`H_NOTIF_LOGS(C)` に配信ログを記録する。
7. 同時に当該オーナーのお知らせ受信箱 `T_INBOX_MSG(C)` へ「請求確定」のお知らせを生成する([FR-090](../../01_requirements/FR11.md#FR-090))。
...
- オーナーへ確定通知メールが送信され、お知らせ受信箱に「請求確定」のお知らせが生成される([FR-090](../../01_requirements/FR11.md#FR-090))。

<!-- portal-top -->
[設計ポータル](../../README.md) ／ [基本設計](../index.md) ／ [ユースケース設計](index.md) ／ **UC-BIZ-011: 疑問をその場で自己解決する**
<!-- /portal-top -->
...
| 関連要件 | [FR-034](../../01_requirements/FR05.md#FR-034) FAQ を根拠とした AI 回答 ・ [FR-045](../../01_requirements/FR06.md#FR-045) 未解決質問の登録 ・ [FR-153](../../01_requirements/FR20.md#FR-153) AI 推論動作 |
...
3. システムが公開中の登録済み FAQ のみを根拠に AI 回答を生成する([FR-034](../../01_requirements/FR05.md#FR-034))。
...
| 区分 | 契機 | 業務上の扱い | 参照 |
|---|---|---|---|
| 回答不能(未解決) | FAQ に根拠がない / 矛盾で断定できない | 未解決の旨を表示し、未解決質問として記録のうえ連絡先メールへ誘導する。引き続き別の質問は可能。 | [UC-SCR-WIDGET](UC-SCR-WIDGET.md) EV-06 ・ [FR-045](../../01_requirements/FR06.md#FR-045) |
...
| 処理エラー | 通信障害・上流障害・推論タイムアウト等 | エラーと再試行案内を表示する。処理エラーは未解決質問として自動登録しない。 | [UC-SCR-WIDGET](UC-SCR-WIDGET.md) EV-08 ・ [FR-153](../../01_requirements/FR20.md#FR-153) |

## 7. 事後条件

- 回答できた質問は会話内で解決し、有人問い合わせを発生させずに完結する。
- 回答できなかった質問は未解決質問として記録され、連絡先メールへの誘導が行われる([FR-045](../../01_requirements/FR06.md#FR-045))。
...
flowchart TD
  ...
  D --> E["FAQ を根拠に AI 回答を生成<br/>(FR-034)"]
  E --> F{"回答結果は?"}
  F -- "回答できた" --> G["AI 回答を表示し<br/>その場で自己解決"]
  F -- "未解決" --> H["未解決質問を記録<br/>連絡先メールへ誘導<br/>(UC-SCR-WIDGET EV-06 / FR-045)"]
  F -- "処理エラー" --> I["エラー・再試行案内を表示<br/>未解決登録はしない<br/>(UC-SCR-WIDGET EV-08 / FR-153)"]
  G --> J{"追加の疑問あり?"}
  H --> J
  I --> J
  J -- "あり" --> D

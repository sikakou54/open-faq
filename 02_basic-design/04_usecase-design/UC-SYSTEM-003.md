<!-- portal-top -->
[設計ポータル](../../README.md) ／ [基本設計](../index.md) ／ [ユースケース設計](index.md) ／ **UC-SYSTEM-003: 90 日物理削除バッチ**
<!-- /portal-top -->
...
退会申請([SCR-014](../01_screen-design/SCR-014.md#SCR-014) / 設定 [SCR-023](../01_screen-design/SCR-023.md#SCR-023))や各種削除操作により論理削除されたデータを対象に、日次バッチが論理削除日からの経過日数を判定する。90 日(猶予期間)を経過したデータを、依存関係の順序(契約 → 利用者 → FAQ / 質問 等)で物理削除する。削除は監査ログ `H_AUDIT_LOGS(C)` に記録する。
...
| 関連画面 | [SCR-014](../01_screen-design/SCR-014.md#SCR-014) 退会申請 ・ [SCR-023](../01_screen-design/SCR-023.md#SCR-023) 設定 |

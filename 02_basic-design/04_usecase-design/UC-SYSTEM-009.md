<!-- portal-top -->
[設計ポータル](../../README.md) ／ [基本設計](../index.md) ／ [ユースケース設計](index.md) ／ **UC-SYSTEM-009: 通知再送**
<!-- /portal-top -->
...
| 関連要件 | [FR-083](../../01_requirements/FR11.md#FR-083) 通知再送 ・ [FR-089](../../01_requirements/FR11.md#FR-089) 再送回数上限 |
| 主テーブル | `H_NOTIF_LOGS(RU)` |
| 関連 API | [API-MAIL-001](../02_api-design/API-mail.md#API-MAIL-001) メール配信 IF ・ [API-DASH-001](../02_api-design/API-dashboard.md#API-DASH-001) ダッシュボードサマリ(配信失敗の可視化) |
...
3. 各対象について再送可否を判定する。
   ...
   2. 再送回数が上限([FR-089](../../01_requirements/FR11.md#FR-089))未満であること。
4. 条件を満たす対象をメール配信 IF([API-MAIL-001](../02_api-design/API-mail.md#API-MAIL-001))で再送する。
...
- 再送可能な対象は上限の範囲内で再送され、配信状態が更新される([FR-083](../../01_requirements/FR11.md#FR-083))。
- 再送上限到達分・抑制リスト該当分は確定失敗として記録され、再送されない。
- 配信失敗状況はダッシュボードサマリ([API-DASH-001](../02_api-design/API-dashboard.md#API-DASH-001))で可視化できる。

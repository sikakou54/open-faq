...
| 関連 | 内容 |
|----|----|
| FR / BR | FR-013, FR-013a, FR-013d, FR-013e, FR-006(パスワード強度), FR-099(規約同意), FR-101, FR-111(Turnstile) / BR-011, BR-012 |
...
> [!NOTE]
> **補足** 本画面は**未認証画面**です。唯一の認可条件は `T_ACCESS_TOKENS.purpose='activation'` の有効トークン保持で、オーナー / メンバーのログインロールでは到達しません(サイドメニュー除外)。氏名(表示名)は招待された本人のみが入力でき、招待者(オーナー / メンバー)は事前入力できません(FR-013d 個人情報原則)。本画面は TPL-ADMIN_USER_REGISTER / メンバー招待テンプレートの着地点です(11_メール設計)。
...
本画面が読み書きするテーブルと、呼び出す API の一覧です。テーブルの正本は [データベース設計](03_database-design.md)、API の正本は [API設計](02_api-design.md) / [§5.1.8](02_api-design.md) です。
...
<td>◯</td>
<td>—</td>
<td><code>M_USER</code>(<a href="TBL-M-001.md">テーブル設計</a>)</td>
</tr>
<tr>
<td>プロジェクト割当</td>
...
<td>「登録を完了する」を押下</td>
<td><ul>
<li><a href="API-auth.md#API-AUTH-008">メンバーアカウント有効化</a> API を実行(同一トランザクションで予約 <code>M_USER</code>(氏名・パスワード設定・<code>status='active'</code>)/ <code>M_PRJ_USERS</code>(<code>valid=1</code>)/ <code>T_TERMS_AGREE</code> 更新 + トークン消費)</li>
<li>成功時: 完了画面へ</li>
</ul></td>
</tr>
<tr>

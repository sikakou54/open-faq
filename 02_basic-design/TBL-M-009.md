<!-- portal-top -->
[設計ポータル](../README.md) ／ [基本設計](index.md) ／ [データベース設計](03_database-design.md) ／ **TBL-M-009 M_PRJ_QUOTA_LIMITS**
<!-- /portal-top -->
...
| 3 | 契約 owner ID | `contract_id` | TEXT | \- | NO |  | `M_CONTRACT(id)` |  |  |  |
| 4 | リソース種別 | `resource_kind` | TEXT | \- | NO |  |  | ① |  | `resource_kind IN ('q_monthly_limit','faq_monthly_limit')` |
| 5 | 月次上限件数 | `threshold` | INTEGER | \- | YES |  |  |  |  |  |
| 6 | 月次無料枠 | `free_quota` | INTEGER | \- | YES |  |  |  |  |  |
| 7 | アラート閾値 | `alert_thresholds` | TEXT | \- | NO |  |  |  | `'[]'` | `json_valid(alert_thresholds) AND json_type(alert_thresholds) = 'array'`。要素の許可値・重複排除・昇順化はAPI層で保証 |
| 8 | 設定元 | `source` | TEXT | \- | NO |  |  | ① | `'owner'` | `source IN ('owner')` |
| 9 | 理由 | `reason` | TEXT | \- | YES |  |  |  |  |  |
| 10 | 有効期限 | `valid_until` | TEXT | \- | YES |  |  |  |  |  |
| 11 | 有効フラグ | `valid` | INTEGER | \- | NO |  |  |  | `1` | `valid IN (0,1)` |
| 12 | 設定者種別 | `created_by_type` | TEXT | \- | YES |  |  |  |  | `created_by_type IN ('owner','project_user')` |
| 13 | 設定者 ID | `created_by_id` | TEXT | \- | YES |  |  |  |  | `created_by_type` に応じ `M_CONTRACT(id)` / `M_PRJ_USERS(id)` を指す(論理参照) |
...
**画面** [SCR-016](SCR-016.md) [SCR-021-001](SCR-021-001.md) [SCR-021](SCR-021.md) **API** [API-BIL-006](02_api-design.md#API-BIL-006) [API-BIL-007](02_api-design.md#API-BIL-007)

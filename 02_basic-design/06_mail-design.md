...
ウィジェット利用者の入力をメールに流用する際の安全策を定めます。次の各項目は、件名・送信元への流用禁止と本文埋め込み時のエスケープ・字数制限を定め、スパム埋め込みやなりすましを防ぎます(FR-145)。

- **ウィジェット利用者の入力文字列(質問本文等)を、メール件名・送信元(Header From / Envelope From)・Reply-To へそのまま使用しない**(FR-145。スパム埋め込み・なりすまし対策)。送信元は §2.1 のとおり `no-reply` 固定とする。
...
送信失敗・バウンス・苦情時の再送方式の正本所在と、本書で定めるテンプレート単位の例外を示します。配信信頼性の方式そのものは 09_セキュリティ設計.html §11 を正本とし、本書は `critical` 認証メールの再送続行など差分のみを定めます。

再送回数上限(24 時間内に最大 3 回)・バウンス / 苦情検知・全契約横断の送信停止リストといった**配信信頼性の方式は 09_セキュリティ設計.html §11 を正本**とする(FR-141 / FR-142 / FR-147 / FR-149b)。本書はテンプレート単位の挙動差のみを以下に定める。
...
**発火条件**: 支払方法未登録の契約の **いずれかのプロジェクト** が質問数無料枠を超過し、支払方法ゲート(FR-136)により当該プロジェクトのウィジェットの新規質問受付を停止した時点で送信。FAQ 件数の無料枠超過は本テンプレートの停止契機にしない。
...
**強制送信**: 規約改定・価格改定・重要セキュリティに該当するお知らせは重要度 `critical` を選択して配信し、受信者はオプトアウトできない(FR-149 / BR-105。利用規約改定そのものは専用の `TPL-TERMS_REVISION` を用いる)。それ以外の `normal` / `high` は §5.2 のルールに従う。
...
**発火条件**: 質問数の選択アラート閾値到達、契約上書き反映等の運用系イベント(FR-127 / FR-149a)。サブ契機ごとに別 i18n キーを持つ。質問数上限関連(25% / 50% / 80% / 90% / 100% 閾値到達)はメール送信対象。

**メール対象外**: AI 利用上限到達(FR-149a / FR-189 のシステム通知契機)は MVP ではメールを送信せず、お知らせ受信箱(`inbox` チャネル)のみで通知する。受信箱側の生成は 06_メッセージ一覧.html §6 を正本とする。
...
**備考**: 新規イベント追加時は本書 + `06_メッセージ一覧.html` §6.1 + i18n キー定義の 3 か所を同期更新する。
...
全送信を記録する配信ログの保存先・列構成・保持期間を定めます。次の各項目は `H_NOTIF_LOGS` に記録する内容を示し、テーブル定義は [03_テーブル設計/index.md](03_database-design.md) が正本です。
...
テンプレートを新規追加 / 変更する際に同期すべき作業手順を定めます。本書・06 メッセージ一覧・i18n キーなど複数箇所の整合を保つため、追加と変更それぞれの手順を以降に示します。

### <span id="61-新規テンプレート追加手順"></span>6.1 新規テンプレート追加手順

メールテンプレートを新規に追加する際の作業を順に定めます。次の手順は通知契機の登録から検証スクリプト実行までの一連を示します。

1.  通知契機を 06_メッセージ一覧.html §6.1 に追加(NOTIF-\* 行)
...
- 配信先・重要度変更: 本書 §5.1 / §5.2 + 06 メッセージ一覧 §6.1 + 共有概念.html の整合確認

...
| 処理 ID | 処理内容                                          |
|---------|---------------------------------------------------|
| P-01    | 認証・認可(admin 限定、CSRF)を検証する            |
...
選択した複数のお知らせを一括で既読にする。1 リクエストの上限は **100 件**。超過時は 400 `VALIDATION_ERROR`。冪等(既読済みは再記録しない)。一括既読操作は操作者・対象件数・実行日時を監査記録する(FR-152 相当、正本: 09_セキュリティ設計.html)。

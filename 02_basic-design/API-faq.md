...
選択した複数 FAQ の状態を一括で `draft` / `published` / `hidden` に変更する。1 リクエストの上限は **50 件**(SCR-006 の BulkActionBar 上限、[画面設計.md](01_screen-design.md))。超過時は 400 `VALIDATION_ERROR`。処理は行単位で評価し、対象外(他契約 / 論理削除済み)の ID は当該行のみ失敗として集計する(オーナー境界違反は §2.2 に従い結果から除外し成功扱いにしない)。`published` へ変更した行は `publishedAt` をサーバ側で記録する。
...
全文検索方式・対象列は [データベース設計.md](03_database-design.md)(FTS インデックス)を正本とする。
...
対象列・全文検索方式は [データベース設計.md](03_database-design.md) を正本とする。

<!-- portal-top -->
[設計ポータル](../README.md) ／ [基本設計](index.md) ／ [API設計](02_api-design.md) ／ **FAQ 管理 API**
<!-- /portal-top -->
...
| HTTP メソッド | GET |
| 認証 | Cookie |
| 権限 | 編集系・参照: オーナー / 該当 PJ の `member`+(`admin` または `member`) |
...
403 `PROJECT_ACCESS_DENIED` ほか(05_エラー設計.html 正本)。
...
選択した複数 FAQ の状態を一括で `draft` / `published` / `hidden` に変更する。1 リクエストの上限は **50 件**(SCR-006 の BulkActionBar 上限、[01_画面設計/index.md](01_screen-design.md))。超過時は 400 `VALIDATION_ERROR`。処理は行単位で評価し、対象外(他契約 / 論理削除済み)の ID は当該行のみ失敗として集計する(オーナー境界違反は §2.2 に従い結果から除外し成功扱いにしない)。`published` へ変更した行は `publishedAt` をサーバ側で記録する。
...
受理形式は **CSV のみ**(UTF-8、BOM 許容、ヘッダ行必須、1 ファイル最大 1000 件)。**JSON 形式は受理しない**(不正形式は 415 / 400、E-INPUT-CSV-INVALID)。CSV 列構成は `FAQ ID, 質問, 回答, カテゴリ`。各行の `FAQ ID` で **空欄 = 新規(`status=draft`)/ 既存 ID 一致 = 上書き(状態維持・`H_FAQ_REV` に改訂追記)/ 当該契約に存在しない ID = 当該行を失敗**(行単位エラー、E-INPUT-CSV-FAQID-NOTFOUND)と判定する。`status` 列は受理しない。
...
| HTTP メソッド  | GET                                             |
| 認証           | Cookie                                          |
| 権限           | オーナー / 該当 PJ の `member`+(API-022 と同一) |
...
全文検索方式・対象列は [03_テーブル設計/index.md](03_database-design.md)(FTS インデックス)を正本とする。
...
対象列・全文検索方式は [03_テーブル設計/index.md](03_database-design.md) を正本とする。

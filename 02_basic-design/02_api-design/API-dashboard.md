...
`notification.failedCount` / `bouncedCount` は `H_NOTIF_LOGS` の配信失敗・バウンス集計(列定義は [データベース設計.md](../03_database-design/index.md))。メンバーが `projectId` 指定で呼んだ場合、本ブロックは `null` を返す。

...
| セッション TTL (無操作) | <!-- TBD: メイン §4.4.1 は 30 分。運営者業務 (DLQ リプレイ等) の長時間操作中断を考慮し、MVP 初期値を 60 分とするか確定要。担当: PO --> 60 分を暫定値とし、最終アクセスで更新 | D-18 / FR-005 |
...
| 再認証必須 action(集合) | `owner.restore_data` / `ai_parameter.update` / `rate_limit.override` / `budget_limit.override` / `announcement.schedule` / `announcement.send` / `announcement.test_send` / `webhook.replay` / `pii_rule.update` / `webhook.payload_diff.reprocess` / `master_key.rotate` / `operator.invite` / `operator.disable` / `pricing.update` / `feature.hard_gate.toggle` | SCR-091〜094 / 097 / 098 / 099 |
...
| reported | under_review | `POST /pii-fp-reports/{id}/transition` `to=under_review` | `review_started_at = now`、`review_due_at = + 3 営業日`(営業日計算は §13.9) |
...
| SCR-091 | 削除データ復元 | §7.5 | 承認ログ | 必須 | FR-201〜FR-209, FR-211, FR-222 |
...
| CMP-B | サイドメニュー | 全画面左 | 12 項目(§5.5)+ 各バッジ(承認待ち / DLQ 滞留 / ペイロード差分) |
...
#### 5.2.5 UI/UX 共通要件で追加する部品(参照: 要件 §8.21〜§8.24)

メインシステム詳細設計 §6.2.7（Breadcrumb / PageHeader / QuickFilterChips / AppliedFilterChips / BulkActionBar / SummaryCard / Timeline / AutosaveIndicator / UnsavedChangesGuard / PermissionTooltip / LoadingSkeleton / ProgressText / ReauthBadge）と同一実装を運営者コンソールにも採用する。共通ライブラリとして同一パッケージから提供する。

運営者コンソール特有の追加部品:

##### 5.2.5.1 ReauthCountdown（再認証残時間表示・FR-457）

```tsx
// apps/admin-console/src/components/ReauthCountdown.tsx
export function ReauthCountdown() {
  const reauthState = useReauthState();
  if (!reauthState.active) return null;
  const remaining = Math.max(0, reauthState.expiresAt.getTime() - Date.now());
  const mm = Math.floor(remaining / 60_000);
  const ss = Math.floor((remaining % 60_000) / 1000).toString().padStart(2, '0');
  const urgent = remaining < 60_000;
  return (
    <span role="status" aria-live="polite" className={`reauth-countdown ${urgent ? 'urgent' : ''}`}>
      🛡 再認証残 {mm}:{ss}
    </span>
  );
}
...
ヘッダ右側に常時表示。残 1 分未満は赤強調 + `aria-live="assertive"` で読み上げ通知。

##### 5.2.5.2 HardgateBadge / ApprovalStateBadge

```tsx
export function HardgateBadge() {
  return (
    <span role="img" aria-label="4-eyes 承認が必要" title="別運営者の承認が必要な操作です" className="hardgate-badge">
      ⚠ 4-eyes
    </span>
  );
}

type ApprovalState = 'requested' | 'approved' | 'rejected' | 'withdrawn';
const APPROVAL_STATE_META: Record<ApprovalState, { color: string; icon: string; label: string }> = {
  requested: { color: '#36c', icon: '⏳', label: '承認待ち' },
  approved:  { color: '#393', icon: '✅', label: '承認済' },
  rejected:  { color: '#c33', icon: '❌', label: '却下' },
  withdrawn: { color: '#888', icon: '↩', label: '撤回' },
};

export function ApprovalStateBadge({ state }: { state: ApprovalState }) {
  const meta = APPROVAL_STATE_META[state];
  return (
    <span className="approval-state-badge" style={{ color: meta.color }} aria-label={meta.label}>
...
色 + アイコン + テキストの三重識別（FR-490）。

##### 5.2.5.3 SelfApprovalGuard

```tsx
export function SelfApprovalGuard({ approval, currentOperator, children }: {
  approval: Approval;
  currentOperator: Operator;
  children: React.ReactElement;
}) {
  const isSelfRequest = approval.requestedBy === currentOperator.id;
  if (!isSelfRequest) return children;
  return (
    <div role="region" aria-label="自己承認禁止">
      <p className="self-approval-message">{AdminMessages.selfApprovalBlocked}</p>
...
##### 5.2.5.4 PayloadDiffViewer（CMP-L を要約モード優先に拡張・FR-455）

```tsx
type ViewMode = 'summary' | 'raw';

export function PayloadDiffViewer({ before, after, exclusions = [] }: {
  before: Record<string, unknown>;
  after: Record<string, unknown>;
  exclusions?: string[];
}) {
  const [mode, setMode] = useState<ViewMode>('summary');  // 既定は要約モード(FR-455)
  const diff = computeDiff(before, after, exclusions);
  return (
    <div className="payload-diff-viewer" role="region" aria-label="申請内容の対比">
      <div className="mode-switch">
        <button onClick={() => setMode('summary')} aria-pressed={mode === 'summary'}>要約</button>
        <button onClick={() => setMode('raw')} aria-pressed={mode === 'raw'}>生 JSON</button>
      </div>
      {mode === 'summary' ? <DiffSummaryTable diff={diff} /> : <RawJsonDiff before={before} after={after} exclusions={exclusions} />}
    </div>
  );
}
```

`DiffSummaryTable` は変更前→変更後をキー別に対比表示（例: `rate_limit: 1000/min → 5000/min`）。生 JSON は折り畳み。

##### 5.2.5.5 文言テンプレ集

メインシステム詳細設計 §6.2.8 と同等の文言テンプレ集を運営者コンソール用に分離して持つ。運営者特有の文言:

```tsx
// apps/admin-console/src/i18n/adminMessages.ts
export const AdminMessages = {
  // 4-eyes 関連
  selfApprovalBlocked: '自分の申請は承認できません(他の運営者の承認が必要です)',
  payloadHashMismatch: '申請内容が変更されている可能性があります(payload_hash 不一致)。最新の内容を確認してから承認してください',
  approvalExpired: '申請は期限切れです(72 時間)。再申請が必要です',
  approvalRequested: '申請を送信しました。承認待ち一覧から状況を確認できます',
  approvalApproved: '承認しました。申請者と監査ログに記録されます',
  approvalRejected: '却下しました。申請者に通知されます',

  // 復元・副作用
  precheckAllOk: 'プリチェック全項目 OK。復元できます',
  precheckHasNg: 'プリチェックで NG 項目があります。詳細を確認してください',
  restoreCompleted: '復元しました。副作用 (a)〜(g) の連動回復は監査ログを参照してください',

  // チケット ID
  ticketIdRequired: '対応チケット ID を入力してください(例: JIRA-12345)',
  ticketIdFormat: '対応チケット ID は英数字・ハイフン・アンダースコアのみ(最大 64 文字)',

  // DLQ・Webhook
  webhookReplayCompleted: 'Webhook イベント {eventId} をリプレイしました',
  webhookOutOfWindow: '受信から 30 日を超えたためリプレイできません(R2 退避済み)',

  // PII
  piiRuleUpdated: 'PII ルールを更新しました。次回ハイドレーション(60 秒以内)から有効になります',
} as const;

export const AdminConfirmDialogs = {
  restore: {
    title: 'リソースを復元しますか?',
    body: '{ownerName}の {resourceLabel} を復元します。副作用 (a)〜(g) が連動して回復します。取り消しはできません。',
    primary: '復元する',
    secondary: 'キャンセル',
    requiresReauth: true,
  },
  webhookReplay: {
    title: 'Webhook イベントをリプレイしますか?',
    body: 'イベント {eventId} を再処理キューに投入します。冪等性キーで二重課金は防止されます。',
    primary: 'リプレイする',
    secondary: 'キャンセル',
    requiresReauth: true,
  },
  webhookDiscard: {
    title: 'DLQ イベントを破棄しますか?',
    body: 'イベント {eventId} を破棄します。破棄後は復元できません(4-eyes 承認ログ対象)。',
    primary: '破棄する',
    secondary: 'キャンセル',
    requiresReauth: true,
  },
  immediateAnnouncement: {
    title: 'お知らせを即時配信しますか?',
    body: '配信対象: {audienceSummary}({recipientCount} 件)。即時配信は取り消しできません(4-eyes 承認ログ対象)。',
    primary: '即時配信する',
    secondary: 'キャンセル',
    requiresReauth: true,
  },
  rejectPiiReport: {
    title: '報告を却下しますか?',
    body: '報告者へ却下理由を通知します。ルールは変更されません。',
    primary: '却下する',
    secondary: 'キャンセル',
    requiresReauth: true,
  },
...
呼出 API: `POST /admin/api/v1/restorations`(§7.5)/ 関連 FR: FR-201〜FR-209, FR-211, FR-222
...
| `ERR_SELF_APPROVAL` | `errors.approval.self` | 「自分の申請は承認できません(他の運営者の承認が必要です)」(`AdminMessages.selfApprovalBlocked`) | 403 |
| `ERR_DELETION_IN_PROGRESS` | `errors.restore.in_progress` | 「他の運営者が処理中のため復元できません。少し時間を置いて再度お試しください」 | 423 |
| `ERR_PRECHECK_FAILED` | `errors.restore.precheck_failed` | 「プリチェックで NG 項目があります。詳細を確認してください」(`AdminMessages.precheckHasNg`) | 409 |
| `ERR_DELETED_PHYSICAL` | `errors.restore.physical_deleted` | 「物理削除済のため復元できません」 | 404 |

メッセージキーは 02_admin の i18n に集約(本書 §5.2.5.5 `AdminMessages`)。エラー文は **原因 + 解決策** のセットで表現し、技術的詳細(HTTP コード・エラーコード)は折り畳みで運営者にのみ提示(FR-453)。

##### 5.3.2.5 プリチェック結果 UI 実装

基本設計 §5.3.2 の「プリチェック結果ステータス帯」を `<PrecheckResultPanel />`(§5.2.5)で実装:

```tsx
type PrecheckRow = { id: 'a'|'b'|'c'|'d'|'e'|'f'|'g'; label: string; state: 'ok'|'ng'|'na'|'info'; details?: string };

function PrecheckSummary({ rows }: { rows: PrecheckRow[] }) {
  const ngCount = rows.filter(r => r.state === 'ng').length;
  const variant = ngCount === 0 ? 'success' : ngCount === rows.length ? 'danger' : 'warning';
  const headline = ngCount === 0
    ? AdminMessages.precheckAllOk
    : AdminMessages.precheckHasNg;
  return (
    <Alert variant={variant} role="status">
      {headline}
      <details><summary>詳細を表示</summary>
        <ul>{rows.map(r => <PrecheckRowItem key={r.id} row={r} />)}</ul>
      </details>
    </Alert>
  );
}
...
基本設計 §5.3.2 の「4-eyes 申請 / 即時実行(承認ログ)」2 タブ UI:

```tsx
type ExecMode = 'four-eyes' | 'log-only';

function ExecModeTabs({ resource, precheckResult, onSubmit }: {
  resource: TargetResource;
  precheckResult: PrecheckResult;
  onSubmit: (mode: ExecMode) => void;
}) {
  const allowFourEyes = isHardgateTarget(resource);  // MVP ハードゲート対象のみ
  const canExecute = precheckResult.allOk;
  return (
    <Tabs defaultValue="four-eyes">
      {allowFourEyes && (
        <TabPanel value="four-eyes">
          <button disabled={!canExecute} className="btn-primary" onClick={() => onSubmit('four-eyes')}>
            4-eyes 申請する <HardgateBadge />
          </button>
        </TabPanel>
      )}
      <TabPanel value="log-only">
        <button disabled={!canExecute} className="btn-primary" onClick={() => onSubmit('log-only')}>
          復元を実行する <ReauthBadge />
        </button>
      </TabPanel>
    </Tabs>
...
タブ切替は誤クリック防止のため確認モーダルを経由。同一タブ内での連続実行も再認証 5 分タイマーで防御。
...
- 完了トースト: `AdminMessages.restoreCompleted`「復元しました。副作用 (a)〜(g) の連動回復は監査ログを参照してください」
...
| コード | メッセージキー | 表示文言(運営者向け) | HTTP |
|---|---|---|---|
| `ERR_APPROVAL_REQUIRED` | `errors.approval.required` | 「4-eyes 承認が必要な操作です。先に申請してください」 | 403 |
| `ERR_SELF_APPROVAL` | `errors.approval.self` | `AdminMessages.selfApprovalBlocked` | 403 |
| `ERR_MODEL_UNAVAILABLE` | `errors.ai_param.model_unavailable` | 「指定のモデルは現在利用できません。他のモデルを選択してください」 | 400 |
| `ERR_REAUTH_REQUIRED` | `errors.auth.reauth_required` | 「再認証が必要です(過去 5 分以内の再認証が無効です)」 | 401 |

##### 5.3.3.5 しきい値 UI 実装(スライダ + 数値併記)

基本設計 §5.3.3 の「スライダ + 数値入力併記」を以下で実装(微調整の操作性確保):

```tsx
function ThresholdInput({ label, value, onChange, ariaLabel }: {
  label: string; value: number; onChange: (v: number) => void; ariaLabel: string;
}) {
  return (
    <div className="threshold-input">
      ...
      <div className="threshold-controls">
        <input
          ...
          value={value}
          aria-label={`${ariaLabel}(スライダ)`}
          onChange={(e) => onChange(parseFloat(e.target.value))}
        />
        <input
          type="number" min={0} max={1} step={0.01}
          value={value.toFixed(2)}
          aria-label={`${ariaLabel}(数値入力)`}
          onChange={(e) => {
            const v = parseFloat(e.target.value);
            if (isFinite(v) && v >= 0 && v <= 1) onChange(Math.round(v * 100) / 100);
          }}
        />
      </div>
    </div>
...
両入力は双方向同期。数値入力で微調整、スライダで大きく動かす運用を両立。

##### 5.3.3.6 ロールアウト進行可視化

```tsx
const ROLLOUT_STAGES: { percent: 0|10|50|100; label: string }[] = [
  { percent: 0,   label: '無効化' },
  { percent: 10,  label: '部分公開(10%)' },
  { percent: 50,  label: '半数公開(50%)' },
  { percent: 100, label: '全面公開(100%)' },
];

function RolloutStepper({ current, next }: { current: 0|10|50|100; next?: 0|10|50|100 }) {
  const steps = ROLLOUT_STAGES.map(s => ({
    id: `r-${s.percent}`,
    label: s.label,
    state: s.percent < current ? 'done' : s.percent === current ? 'current' : 'future',
  } as Step));
  return (
    <div role="region" aria-label="ロールアウト進行状況">
      <Timeline steps={steps} />
      {next != null && next !== current && (
        <p>現在 {current}% → 次回 {next}%</p>
      )}
    </div>
  );
}
...
次回ステージへ進める操作は再度 4-eyes 申請の対象(各段階で監査ログ記録、FR-065)。
...
| **即時配信(4-eyes 承認ログ)** | POST | `/admin/api/v1/announcements/{id}:send-now` | `X-Op-Ticket-Id`、`X-Reauth-Token` |
...
| コード | メッセージキー | 表示文言(運営者向け) | HTTP |
|---|---|---|---|
| `ERR_XSS_DETECTED` | `errors.announcement.xss_detected` | 「危険な HTML 要素が含まれています。スクリプトタグなどを取り除いてください」 | 400 |
| `ERR_SCHEDULE_OUT_OF_RANGE` | `errors.announcement.schedule_out_of_range` | 「送信予定日時は現在時刻より未来かつ 30 日以内で指定してください」 | 400 |
| `ERR_CANCEL_TOO_LATE` | `errors.announcement.cancel_too_late` | 「配信開始 5 分前を過ぎたため取消できません」 | 409 |
| `ERR_AUDIENCE_INVALID` | `errors.announcement.audience_invalid` | 「宛先範囲の組み合わせが不正です。種別と対象範囲を確認してください」 | 400 |

##### 5.3.5.6 4 段階ステッパー UI

基本設計 §5.3.5 のステッパー UI を `<Timeline />` ベースで実装:

```tsx
type AnnouncementStep = 1 | 2 | 3 | 4;
type AnnouncementMode = 'draft' | 'tested' | 'scheduled' | 'sent';

function modeToStep(mode: AnnouncementMode): AnnouncementStep {
  return { draft: 1, tested: 2, scheduled: 3, sent: 4 }[mode] as AnnouncementStep;
}
...
即時配信(ステップ 4)は赤枠で視覚的に分離し、`<HardgateBadge />` + `<Danger>` で 4-eyes 承認ログ対象であることを明示。

##### 5.3.5.7 即時配信(`send-now`)実装

```ts
// POST /admin/api/v1/announcements/{id}:send-now
// 1. 再認証検証(X-Reauth-Token)
// 2. AdminConfirmDialogs.immediateAnnouncement の確認後に呼ばれる
// 3. 4-eyes 承認ログ(approval_log only、ハードゲートではない)
// 4. recipientCount + audienceSummary を監査ログに記録
// 5. 連携 IF #7 でメイン側へ即時配信指示
```

確認モーダル本文に `{audienceSummary}` `{recipientCount}` を差し込み、運営者が宛先誤指定に気づける構造にする(FR-454)。
...
| 滞留サマリ取得 | GET | `/admin/api/v1/webhooks/summary` | - |
...
| 手動破棄(4-eyes 承認ログ) | POST | `/admin/api/v1/webhooks/discard` | `X-Op-Ticket-Id`、`X-Reauth-Token` |
...
| コード | メッセージキー | 表示文言(運営者向け) | HTTP |
|---|---|---|---|
| `ERR_AUTO_BO_IN_PROGRESS` | `errors.webhook.auto_bo_in_progress` | 「自動 BO 中はリプレイできません({N}分後に再試行可能になります)」 | 409 |
| `ERR_DLQ_ARCHIVED` | `errors.webhook.dlq_archived` | 「受信から 30 日を超えたためリプレイできません(R2 退避済み)」(`AdminMessages.webhookOutOfWindow`) | 410 |
| `ERR_REAUTH_REQUIRED` | `errors.auth.reauth_required` | 「再認証が必要です(過去 5 分以内の再認証が無効です)」 | 401 |
| `ERR_TICKET_REQUIRED` | `errors.audit.ticket_required` | 「対応チケット ID を入力してください(例: JIRA-12345)」 | 400 |

##### 5.3.8.5 滞留サマリ実装

```ts
// GET /admin/api/v1/webhooks/summary レスポンス例
{
  staleOver24h: 3,
  approachingWindow: 7,  // 30日近接(残3日以内)
  totalDlq: 42,
}
```

クライアント側で `<Alert />` の variant を分岐:

```tsx
function StaleSummaryBanner({ summary }: { summary: WebhookSummary }) {
  if (summary.staleOver24h === 0 && summary.approachingWindow === 0) return null;
  const messages = [];
  if (summary.staleOver24h > 0) {
    messages.push(`⚠ 24 時間以上滞留中のイベントが ${summary.staleOver24h} 件あります。早急に確認してください`);
  }
  if (summary.approachingWindow > 0) {
    messages.push(`⏳ リプレイ枠 30 日に近づくイベントが ${summary.approachingWindow} 件あります(残 3 日以内)`);
  }
  const variant = summary.staleOver24h > 0 ? 'danger' : 'warning';
  return <Alert variant={variant} role="alert">{messages.map((m, i) => <div key={i}>{m}</div>)}</Alert>;
}
```
...
3 営業日タイマーは §13.9 営業日定義に従う(残 1 日で黄バッジ、超過で赤バッジ、AC-036)。
...
```mermaid
flowchart TD
    Login[運営者ログイン<br/>+ MFA + IP 許可] --> Home[ホームダッシュボード<br/>未承認・DLQ滞留]
...
| 7 | Webhook リプレイ(SCR-097) | 7 | DLQ 滞留 1h / 24h(赤) |
| 8 | PII 誤検出報告(SCR-098) | 8 | 3 営業日経過件数 |
| 9 | ペイロード差分(SCR-099) | 9 | detected 件数(赤) |
| 10 | 運営者活動(SCR-096) | 10 | ハッシュチェーン検証 NG(赤) |
| 11 | 承認待ち一覧 | 11 | 承認待ち件数 |
| 12 | 運営者 inbox | 12 | 未読件数 |
| 13 | ログアウト | 13 | - |
...
**営業日定義**: 月〜金 - 国民の祝日・振替休日・国民の休日・特別祝日 - {12/29, 12/30, 12/31, 1/1, 1/2, 1/3}。詳細は §13.9。
...
### 6.AUTH SCR-AUTH 運営者ログイン

画面項目(表示・入力・操作・主要制約)は本書 §5.3.14 SCR-AUTH を正本とする。本節では、当該画面の実装に関する 6 桁分割入力 UI・ロックアウト UI・IP 拒否表示の実装方針を記載する。

#### 6.AUTH.1 6 桁分割入力 UI(TOTP コード)

```tsx
// apps/admin-console/src/components/TotpDigitInput.tsx
export function TotpDigitInput({ value, onChange, autoFocus }: {
  value: string;             // 0-6 文字
  ...
  function setDigit(idx: number, d: string) {
    if (!/^\d?$/.test(d)) return;
    const next = (value.padEnd(6, ' ').substring(0, idx) + d + value.padEnd(6, ' ').substring(idx + 1)).trimEnd();
    onChange(next);
    if (d && idx < 5) refs[idx + 1].current?.focus();
  }

  return (
    <div role="group" aria-label="認証コード(6 桁)" className="totp-digit-input">
      {refs.map((ref, i) => (
        <input
          key={i}
          ref={ref}
          ...
          onPaste={(e) => {
            ...
            if (pasted.length === 6) { e.preventDefault(); onChange(pasted); refs[5].current?.focus(); }
          }}
        />
      ))}
    </div>
...
- ペースト対応: 6 桁数字を 1 回ペーストで全マス埋め(認証アプリのワンタイムコード対応)。
- `autocomplete="one-time-code"` でブラウザ/OS の SMS / アプリ補完を許容(運営者本人デバイス想定)。

#### 6.AUTH.2 ロックアウト残時間表示

```tsx
function LockoutCountdown({ unlockAt }: { unlockAt: Date }) {
  const [remaining, setRemaining] = useState(unlockAt.getTime() - Date.now());
  useEffect(() => {
    const t = setInterval(() => setRemaining(Math.max(0, unlockAt.getTime() - Date.now())), 1000);
    return () => clearInterval(t);
  }, [unlockAt]);
  const mm = Math.floor(remaining / 60_000);
  const ss = Math.floor((remaining % 60_000) / 1000).toString().padStart(2, '0');
  return (
    <div role="alert" className="lockout-alert">
      ログイン試行回数の上限に達しました。{mm}分{ss}秒後にもう一度お試しください
    </div>
  );
}
```

15 分ロックアウト中はパスワード/TOTP 入力欄を disabled、Primary ボタンも非活性化。

#### 6.AUTH.3 IP 拒否時の表示

API 応答 `403 IP_NOT_ALLOWED` を受けた場合、画面上部に固定アラート:

```tsx
<Alert variant="danger" role="alert">
  許可された IP アドレスからアクセスしてください(現在の IP: {currentIp})。管理者にお問い合わせください。
</Alert>
```

`currentIp` はサーバから返却される `X-Forwarded-For` 由来値(マスク済み)。

#### 6.AUTH.4 関連参照

> FR-220 / FR-221 / FR-450 / FR-453 / NFR-311 / 本書 §10.3

### 6.AUTH-M1 SCR-AUTH-M1 MFA 初回セットアップ

画面項目(表示・入力・操作・主要制約)は本書 §5.3.15 SCR-AUTH-M1 を正本とする。本節では、当該画面の実装に関する Zod スキーマ・呼出 API・トークン処理・ウィザード制御・回復コード保管 UI を記載する。
...
export const mfaSetupSchema = z.object({
  ...
  recoveryCodesStored: z.literal(true),         // 回復コード保管確認チェック(FR-454)
});
```
...
function canProceed(state: SetupState): boolean {
  switch (state.step) {
    case 1: return true;                                 // QR を読み取り次へ
    case 2: return /^\d{6}$/.test(state.totpCode);       // TOTP 入力完了
    case 3: return state.storedConfirmed;                 // 保管確認チェック完了
  }
}
```

ステップ 3 で `recoveryCodesStored` が `true` でない場合、Primary「セットアップを完了する」を非活性化。

#### 6.AUTH-M1.4 回復コード保管 UI(3 択)

```tsx
function RecoveryCodesActions({ codes }: { codes: string[] }) {
  const text = codes.join('\n');
  return (
    <div role="group" aria-label="回復コードの保管">
      <button onClick={() => window.print()}>印刷する</button>
      <button onClick={async () => {
        await navigator.clipboard.writeText(text);
        toast.success('回復コードをコピーしました');
      }}>コピーする</button>
      <button onClick={() => downloadRecoveryCodesPdf(codes)}>PDF でダウンロード</button>
    </div>
  );
}
```

回復コードは 1 度しか表示されない旨を `<Alert role="alert">` で常時固定表示(FR-454)。

#### 6.AUTH-M1.5 関連参照

> FR-220 / FR-450 / FR-453 / FR-454 / RB-014 / NFR-311 / 本書 §10.3 / §10.5
...
#### 6.APPROVALS-M1.3 PayloadDiffViewer の要約モード(FR-455)

申請モーダルの主表示は **生 JSON ではなく要約**:

```tsx
function PayloadSummary({ payload, kind }: { payload: unknown; kind: ApprovalActionCode }) {
  switch (kind) {
    case 'ai_parameter.update':
      return <KvDiffTable rows={[
        { key: '信頼度しきい値', before: payload.before.confidence_threshold, after: payload.after.confidence_threshold },
        { key: '関連度しきい値', before: payload.before.relevance_threshold, after: payload.after.relevance_threshold },
        { key: 'モデル',        before: payload.before.model_id,             after: payload.after.model_id },
      ]} />;
    case 'rate_limit.override':
      return <p>契約 {payload.owner_account_id} のレート制限を {payload.before.rate}/min → {payload.after.rate}/min に変更</p>;
    // ...
  }
}
```

`<KvDiffTable />` は変更前→変更後を 2 カラムで対比し、変更箇所のみハイライト。生 JSON が必要な運営者は `<details>` で展開できる(`PayloadDiffViewer.mode='raw'`)。

#### 6.APPROVALS-M1.4 通知先表示(FR-454)

申請ボタン直上に「申請後、運営者 {N} 名(あなたを除く)に通知されます」を表示。N は申請時点の `service_operator` 数 - 1。

#### 6.APPROVALS-M1.5 関連参照

> FR-226 / FR-231 / FR-450 / FR-453 / FR-454 / FR-455 / FR-458 / 要件 §6.2.3 / 本書 §6.4 / §6.5
...
| 却下 | `POST /admin/v1/approvals/{id}/reject` | 別運営者、再認証、コメント必須 |
...
#### 6.APPROVALS-M2.3 承認 / 却下 CTA の視覚分離(FR-454)

承認と却下を **明確に異なる位置・色**に配置して誤クリックを防ぐ:

```tsx
function ApprovalActionFooter({ approval, currentOperator, onApprove, onReject }: {...}) {
  const isSelfRequest = approval.requestedBy === currentOperator.id;
  return (
    <footer className="approval-action-footer">
      ...
      <div className="right">
        ...
        {isSelfRequest && <SelfApprovalGuard approval={approval} currentOperator={currentOperator}>{/* CTA disabled */}</SelfApprovalGuard>}
      </div>
    </footer>
  );
}
...
承認は **右下に緑系 Primary**、却下は **左下に赤系 Danger** で対称配置。視覚的に対極となるよう CSS で配置(キーボード操作時の Tab 順は左→右で却下→承認)。

#### 6.APPROVALS-M2.4 payload_hash 不一致時の UI

```tsx
function HashMismatchAlert({ onRefresh, onResubmit }: { onRefresh: () => void; onResubmit: () => void }) {
  return (
    <Alert variant="danger" role="alert">
      {AdminMessages.payloadHashMismatch}
      <div className="actions">
        <button className="btn-link" onClick={onRefresh}>最新を表示</button>
        <button className="btn-link" onClick={onResubmit}>最新で再申請</button>
      </div>
    </Alert>
  );
}
...
技術用語(`payload_hash`)はカッコ書きでのみ補足し、主文は業務語で説明(FR-455)。

#### 6.APPROVALS-M2.5 却下コメント必須化

却下時の `comment` を必須化するためのクライアント側バリデーション + サーバ側 422:

```ts
const RejectComment = z.string().trim().min(1).max(1000);

// クライアント: 却下ボタン押下時に空文字なら Primary 非活性化 + 入力欄ハイライト
// サーバ: POST /approvals/{id}/reject で comment 空文字なら 422 + ERR_REJECT_COMMENT_REQUIRED
```

却下理由は申請者へ通知される(`AdminMessages.approvalRejected` 連動)ので、空のままの却下を防ぐ。

#### 6.APPROVALS-M2.6 関連参照

> FR-226 / FR-450 / FR-454 / FR-455 / FR-456 / 要件 §6.2.3 / 本書 §6.4 / §6.5
...
| エラー | 404 `NOT_FOUND`(物理削除済 = FR-209) |
...
通信仕様の正本はメイン §11.5.2 / §11.5.3。本書側主管 8 件 + 補助 2 件の URL と JSON Schema 抜粋を §10.1 で確定する。
...
- **対象テーブル優先順位**: `audit_logs` (毎日) > `webhook_events` (毎日) > `operator_approvals` (週次月曜) > その他 (月次)。
...
|---|---|---|---|
| 業務監査(NFR-602(a)) | `1y` | 1 年 | `faq.create`, `faq.update`, `chat.reply`, `account.login`, `inbox.read` |
...
| 3 | PII 誤検出 3 営業日判定リマインド | system / normal | `[リマインド] PII 誤検出報告 {report_id} 判定期限` | `pii-fp-reminder:<report_id>` | `op-pii-fp-reminder` | なし |
| 4 | Webhook ペイロード差分検出 | system / high | `[警告] Stripe Webhook payload 差分検出` | `webhook-diff:<event_id>` | `op-webhook-diff` | 即時 |
| 5 | DLQ 滞留 1h 以上 | system / high | `[警告] Webhook DLQ 滞留 {count} 件` | `dlq-stuck:<batch_id>` | `op-dlq-stuck` | 30 分集約 |
| 6 | ハッシュチェーン不一致 | system / high | `[緊急] 監査ハッシュチェーン検証 NG` | `audit-chain-fail:<verify_date>` | `op-audit-chain-fail` | なし |
| 7 | 運営者操作頻度異常 | system / high | `[警告] 運営者 {operator_id} の操作頻度異常` | `operator-anomaly:<operator_id>:<hour>` | `op-operator-anomaly` | 1h |
| 8 | SCR-094 テスト送信 | テスト | `[テスト] {announcement_subject}` | なし | `op-announcement-test` | なし |
| 9 | 課金 Webhook 署名検証失敗 | system / high | `[緊急] Stripe Webhook 署名検証失敗` | `webhook-sig-fail:<hour>` | `op-webhook-sig-fail` | 1h |
| 10 | 課金 Webhook 受信失敗率 5% 超過 | system / high | `[緊急] Webhook 受信失敗率 {rate}%` | `webhook-fail-rate:<hour>` | `op-webhook-fail-rate` | 1h |
| 11 | 月次請求確定 cron 失敗(3 回連続) | system / high | `[緊急] 月次請求確定 cron 失敗` | `monthly-billing-fail:<month>` | `op-monthly-billing-fail` | なし |
| 12 | 復元時 423 Locked(deletion-queue 競合) | system / high | `[警告] 復元失敗: deletion-queue 競合` | `restore-lock:<restoration_id>` | `op-restore-lock` | なし |

**Resend 連携の実装方針**:
- 起動時ヘルスチェック: アプリ起動時に `GET https://api.resend.com/templates` で全 12 キーの存在を検証、欠落時は `op-resend-template-missing` 内部ログ + 起動失敗
...
| `internal-api` | 連携 IF JWT 署名 | 32B | グローバル | グローバルマスター | §10.1, D-03 | 年次ローテーション |

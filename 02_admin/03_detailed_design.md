...
| `MonthlyBillingCronWorker` | 月初 02:00 JST の請求書発行(D-11)、`(tenant_id, billing_year_month)` 冪等、Stripe Invoice API 発行、テナント通知 + 監査記録 | `D1` / Stripe / Queues | 月次 |
...
| 用途 | メソッド | パス |
|---|---|---|
| 一覧検索 | GET | `/admin/api/v1/deleted-resources?type=&deletion_type=&tenant=&from=&to=&cursor=` |
...
| (b) | KV `dlq:tenant:<id>` カウンタ取得 | 件数表示(警告) |
| (c) | `announcements_deliveries WHERE tenant_id=? AND state='paused'` 件数 | 件数表示(警告) |
| (d) | KV `widget:config:<tenant_id>` 整合確認 | NG → 復元ボタン非活性 |
| (e) | `accounts.stripe_customer_id` と Stripe `customer.id` の一致確認 | NG → 復元ボタン非活性 |
| (f) | `invitations WHERE tenant_id=? AND state IN ('pending','accepted_pending')` 件数 | 件数表示(警告) |
...
呼出 API: `PUT /admin/api/v1/overrides/rate-limit/{tenant_id}`、`PUT /admin/api/v1/overrides/budget/{tenant_id}`(§7.6)/ 関連 FR: FR-121, FR-128, FR-224(b)
...
**MVP ハードゲート対象**: `tenant.physical_delete` / `ai_parameter.update` / `master_key.rotate`(3 操作)
**MVP 承認ログのみ(7 操作区分)**: `tenant.suspend` / `tenant.restore` / `pricing.update` / `rate_limit.override` または `budget_limit.override` / `widget.force_stop` / `tenant.restore_data` / `feature.hard_gate.toggle`
...
#### 6.2.8 テナント別レート/予算上書き適用(FR-121, FR-128, D-14)
...
sequenceDiagram
    ...
    Note over Main: 以降のテナント API リクエストは<br/>KV キャッシュ TTL 30s から上書き値取得
```
...
3 階層 (project > tenant > global) の優先順位に **段階ロールアウト (`rollout_percentage` 0/10/50/100)** が組み合わさった場合の評価ロジック。例: tenant に `rollout_percentage=50` で新パラメータ A、project に `rollout_percentage=0` で新パラメータ B が設定されている場合。
...
| 監査 | `tenant.restore` または `<resource>.restore`(5y) |
...
#### 7.6.2 `PUT /admin/api/v1/overrides/rate-limit/{tenant_id}`
...
#### 7.6.3 `PUT /admin/api/v1/overrides/budget/{tenant_id}`

| Body | `{ "monthlyBudgetYen": 100000, "reason": "..." }` |
| 監査 | `budget_limit.override`(5y) |

#### 7.6.4 `POST /admin/api/v1/overrides/suppress-list/{tenant_id}/restore`(サプレスリスト復帰承認)
...
| 認証 | service_operator + 再認証 + `X-Op-Ticket-Id` |
| 処理 | HKDF info=`deletion-confirm`(テナント派生鍵)で 24h トークン発行 + メール送信 |
...
| クエリ | `action`, `actorId`, `actorType`, `targetId`, `targetType`, `tenantId`, `from`, `to`, `ipMasked`, `retentionClass`, `ticketId`, `cursor`, `limit` |
...
| クエリ | `state[]`, `receivedFrom`, `receivedTo`, `tenantId`, `cursor`, `limit` |
...
ULID の衝突確率 (2^80) は実質ゼロのため tenant_id 重複リスクはない。にもかかわらず本テーブルを **永久保持** で残す目的は以下:
...
|---|---|
| アルゴリズム | SHA-256 |
| 計算式 | `record_hash = sha256(prev_hash || canonical_json({id, actor_id, action, target_id, target_type, tenant_id, before_value, after_value, ip_masked, ticket_id, occurred_at, retention_class, segment_key}))` |
...
|---|---|---|---|---|---|---|
| #1 | 本書→メイン | `POST /internal/admin-integration/v1/tenant/suspend` / `.../tenant/resume` | `{operationId, tenantId, targetStatus, reason, occurredAt, ticketId}` | `(operation_id, tenant_id, target_status)` | 100/30 分 | 5s |
...
| #4 | 本書→メイン | `POST /internal/admin-integration/v1/restore/execute` | `{restoreOperationId, tenantId, restoredAt, ticketId, rollback}` | `(restore_operation_id)` | 50/24h | 10s |
...
| #8 | メイン→本書(補助) | `GET /internal/admin-integration/v1/metrics`(本書から参照、主管はメイン) | (query: `from`, `to`, `tenantId?`) → `{kpis: [...]}` | `(metric_window_id)` | n/a | 10s |
...
| #11 | メイン→本書 | `POST /internal/main-integration/v1/deletion/completed` | `{deletionRequestId, tenantId, completedAt, result, deletedCounts}` | `(deletion_request_id, completed_at)` | 50/24h | 5s |
...
#### 10.1.1 連携 IF #1 テナント停止イベント JSON Schema
...
集約キー: `notify-batch:<tenant_id>:<operation_kind>`(KV TTL 10 分)
...
| Backup Key | Backup Worker のみ参照可、テナント派生鍵とは独立(NFR-324) |
...
|---|---|---|---|---|---|---|---|
| `MonthlyBillingCronWorker` | 月初 1 日 02:00 JST | `0 17 * * *`(毎日 17:00 UTC) | 翌日 02:00 JST | `if (jstNow().day !== 1) return;` | `(tenant_id, billing_year_month)` UNIQUE | 3 回 | D-11 |
...
function monthlyBilling():
    ...
    for owner in owners:
        try:
            ...
            existing = D1.query("SELECT id FROM billing_invoices WHERE tenant_id=? AND billing_year_month=?", tenant.id, target_month)
            if existing:
                continue  // 冪等
            invoiceId = ULID()
            D1.exec("INSERT INTO billing_invoices (id, tenant_id, billing_year_month, amount_yen, status, issued_at) VALUES (?, ?, ?, ?, 'issued', ?)", invoiceId, tenant.id, target_month, amount, now)
            stripeInvoice = stripe.invoiceItems.create({customer: tenant.stripe_customer_id, amount: amount, currency: 'jpy', metadata: {tenant_id: tenant.id, billing_year_month: target_month}})
            stripeInv = stripe.invoices.create({customer: tenant.stripe_customer_id, auto_advance: false, metadata: {tenant_id, billing_year_month}, idempotencyKey: `monthly-billing-${tenant.id}-${target_month}`})
...
| **deletion_request.*** | | | |
| `deletion_request.create` | 1y | - | `{requestId, tenantId, requesterEmailMasked}` |
...
| SCR-093 | FR-121, FR-128, FR-224(b) | AC-040, AC-044 | `PUT /admin/api/v1/overrides/rate-limit/{tenant_id}`, `/overrides/budget/{tenant_id}` | `apps/admin-console/e2e/overrides/rate-budget.spec.ts` | `e2e-scr093-001` |
...
| `e2e-4eyes-expire-001` | 72h 期限切れ自動 expire | `tenant.physical_delete` | `apps/admin-console/e2e/4eyes/expire-flow.spec.ts` |

...
| 関連要件 | [FR-100](../FR13.md#FR-100) 猶予期間後のデータ削除 ・ [FR-109](../FR14.md#FR-109) 操作ログの監査用保持 ・ [NFR-015](../index.md#NFR-015) 監査ログの改ざん検知保持・日次全件検証 |
...
- 監査ログが改ざん検知可能な形で保持されている([NFR-015](../index.md#NFR-015))。
...
2. 退会・削除の猶予が経過すると、日次バッチが対象データを依存順で物理削除し、保持義務データは残す([UC-SYSTEM-003](UC-SYSTEM-003.md#UC-SYSTEM-003) / [FR-100](../FR13.md#FR-100))。
...
6. 日次バッチが監査ログを全件検証し、整合性を確認する([UC-SYSTEM-018](UC-SYSTEM-018.md#UC-SYSTEM-018) / [NFR-015](../index.md#NFR-015))。
...
| 保持義務データ | 監査ログ等の法令・運用上の保持義務 | 物理削除の対象外とし、保持期間に従って別途管理する。 | [FR-100](../FR13.md#FR-100) ・ [FR-109](../FR14.md#FR-109) |
...
| 整合性違反の検出 | 監査ログに改ざん・欠落の疑い | 対象を特定してアラート通知し、走査は中断せず全件継続する(監査ログ自体は変更しない)。 | [UC-SYSTEM-018](UC-SYSTEM-018.md#UC-SYSTEM-018) ・ [NFR-015](../index.md#NFR-015) |

## 7. 事後条件

- 猶予を経たデータは確定削除(不可逆)され、保持義務データは保持される。削除内容は監査ログに記録される([FR-100](../FR13.md#FR-100))。
- 失効・ロックアウト・契約停止に該当するセッションは無効化され、再認証なしには操作できない。停止後の操作には停止時アクセス制限が適用される。
- 監査ログが日次で全件検証され、整合性違反は運用担当へアラート通知される([NFR-015](../index.md#NFR-015) / [FR-109](../FR14.md#FR-109))。

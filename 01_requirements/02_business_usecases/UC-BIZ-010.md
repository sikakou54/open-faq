...
| 関連要件 | [FR-126](../01_specifications/FR-126.md#FR-126) 埋め込みコードの取得・設置 ・ [FR-127](../01_specifications/FR-127.md#FR-127) 許可ドメイン上での動作 ・ [FR-128](../01_specifications/FR-128.md#FR-128) 外観の設定 ・ [FR-135](../01_specifications/FR-135.md#FR-135) 受付制限中の案内 ・ [FR-193](../01_specifications/FR-193.md#FR-193) しきい値の即時反映とフォールバック |
...
- AI しきい値の既定値(信頼度 0.60 / 関連度 0.50)が定義されている([FR-193](../01_specifications/FR-193.md#FR-193))。
...
2. メンバーが外観(主色・表示位置・見出し・初期メッセージ)を編集し、プレビューで確認して設定を保存する([FR-128](../01_specifications/FR-128.md#FR-128))。
3. 必要に応じて、メンバーが公開キーを再発行し、新しい埋め込みコードを自社サイトへ設置する([FR-126](../01_specifications/FR-126.md#FR-126))。
4. メンバーが動作を許可するドメインを限定し、許可ドメイン上のみでウィジェットが動作するようにする([FR-127](../01_specifications/FR-127.md#FR-127))。許可ドメインの設定はプロジェクト編集([SCR-004](../../02_basic_design/01_screens/SCR-004.md#SCR-004))で行い、詳細は [UC-SCR-004](UC-SCR-004.md) に委譲する。
...
- **受付制限の到達**: 質問数の月次上限到達時はウィジェットが受付制限中の案内を行い、新規質問の入力・送信を無効化する([FR-135](../01_specifications/FR-135.md#FR-135))。
- **権限なし**: 当該プロジェクト未割当のメンバーが直アクセスした場合、操作不可を提示する。

## 7. 事後条件

- ウィジェットの外観・公開キー・埋め込みコードが、保存した内容で運用に反映される。
- 許可ドメイン上のみでウィジェットが動作し、他契約・他プロジェクトのデータ参照を防ぐ([FR-127](../01_specifications/FR-127.md#FR-127))。
- AI しきい値の変更が短時間で以降の推論動作へ反映される([FR-193](../01_specifications/FR-193.md#FR-193))。
...
flowchart TD
  ...
  M -->|外観| A[主色・表示位置・見出し・初期メッセージを編集<br/>FR-128]
  A --> AV{入力は妥当か}
  AV -->|不正| AE[エラー提示・修正]
  AE --> A
  AV -->|OK| AS[設定を保存]
  ...
  K --> KE[新しい埋め込みコードを設置<br/>FR-126]

  M -->|許可ドメイン| DM[動作許可ドメインを限定<br/>FR-127 / UC-SCR-004]

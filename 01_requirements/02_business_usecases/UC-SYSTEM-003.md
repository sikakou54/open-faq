<!-- portal-top -->
[設計ポータル](../../README.md) ／ [要件定義](../index.md) ／ [業務ユースケース](index.md) ／ **UC-SYSTEM-003: 90 日物理削除バッチ**
<!-- /portal-top -->
...
退会申請([SCR-014](../../02_basic_design/01_screens/SCR-014.md#SCR-014) / 設定 [SCR-023](../../02_basic_design/01_screens/SCR-023.md#SCR-023))や各種削除操作により論理削除されたデータを対象に、日次バッチが論理削除日からの経過日数を判定する。90 日(猶予期間)を経過したデータを、依存関係の順序(契約 → 利用者 → FAQ / 質問 等)で物理削除する。削除は監査ログ `H_AUDIT_LOGS(C)` に記録する。

| 項目 | 内容 |
|---|---|
| 目的 | 退会・削除後の猶予期間(90 日)経過データを確定削除し、データ最小化を担保する |
| 関連要件 | [FR-138](../01_specifications/FR-138.md#FR-138) 退会時の猶予期間後のデータ削除 |
| 主テーブル | `M_CONTRACT` ・ `M_USER` ・ `M_FAQS` ・ `T_INQUIRIES` ・ `T_WITHDRAW_REQ`(物理削除)・ `H_AUDIT_LOGS(C)` |
| 関連画面 | [SCR-014](../../02_basic_design/01_screens/SCR-014.md#SCR-014) 退会申請 ・ [SCR-023](../../02_basic_design/01_screens/SCR-023.md#SCR-023) 設定 |

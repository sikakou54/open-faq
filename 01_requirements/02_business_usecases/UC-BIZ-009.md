...
| 関連要件 | [FR-068](../01_specifications/FR-068.md#FR-068) 未解決質問の登録 ・ [FR-072](../01_specifications/FR-072.md#FR-072) 対応状況の 2 区分 ・ [FR-075](../01_specifications/FR-075.md#FR-075) 状況の手動切替 ・ [FR-076](../01_specifications/FR-076.md#FR-076) 未解決質問から FAQ 登録開始 ・ [FR-077](../01_specifications/FR-077.md#FR-077) 質問文の初期反映 ・ [FR-079](../01_specifications/FR-079.md#FR-079) 登録前の確認・編集 ・ [FR-080](../01_specifications/FR-080.md#FR-080) FAQ 操作と状況の非連動 ・ [FR-081](../01_specifications/FR-081.md#FR-081) 登録先 FAQ のトレース |
...
- AI が解決できなかった、またはウィジェット利用者が「解決しなかった」とした未解決質問が登録されている([FR-068](../01_specifications/FR-068.md#FR-068))。
...
3. 必要に応じて、メンバーが対応状況を「対応中 / 対応済み」で手動更新する([FR-075](../01_specifications/FR-075.md#FR-075))。
4. メンバーが「FAQ 登録へ」で当該質問を起点に FAQ 編集([SCR-006-001](../../02_basic_design/01_screens/SCR-006-001.md#SCR-006-001))を開く。質問文が質問欄へ初期反映される([FR-077](../01_specifications/FR-077.md#FR-077))。
5. メンバーが回答を自分で入力・編集し、質問・回答・公開状態を確認して FAQ を保存・公開する([UC-SCR-006-001](UC-SCR-006-001.md))。
6. 以降、未解決質問から登録先 FAQ をたどって改善結果を追跡する([FR-081](../01_specifications/FR-081.md#FR-081))。

## 6. 例外・代替フロー(業務レベル)

- **FAQ 化で解決しない**: 入力した FAQ では質問を網羅できない場合、メンバーは質問を分割または候補 FAQ を見直して再整備する。
- **対応状況の独立**: FAQ の下書き保存・公開を行っても、未解決質問の対応状況は自動で変わらない。状況は詳細画面の手動操作のみで切り替える([FR-080](../01_specifications/FR-080.md#FR-080))。
- **回答の自動取得なし**: 質問文以外(過去の AI 回答など)は回答欄へ自動反映されないため、メンバーが回答を作成する([FR-078](../01_specifications/FR-078.md#FR-078))。
...
- 未解決質問の対応状況が、メンバーの手動操作に基づいて反映される([FR-072](../01_specifications/FR-072.md#FR-072))。
- 未解決質問から登録先 FAQ をたどれる状態になり、改善ループが追跡可能になる([FR-081](../01_specifications/FR-081.md#FR-081))。
...
flowchart TD
  ...
  D --> ST{対応状況を更新するか}
  ST -->|更新する| UP[対応中 / 対応済みを手動切替<br/>FR-075]
  ST -->|更新しない| F0
  UP --> F0[FAQ 登録へ進む]
  F0 --> EDIT[質問文を初期反映し FAQ 編集を開く<br/>FR-077]
  EDIT --> ANS[回答を入力・編集し内容を確認<br/>FR-079]
  ANS --> OK{保存・公開できるか}
  OK -->|不足| RE[質問の分割・候補見直しで再整備]
  RE --> ANS
  OK -->|OK| PUB[FAQ を保存・公開<br/>UC-SCR-006-001]
  PUB --> TR[未解決質問から登録先 FAQ をトレース<br/>FR-081]

<!-- portal-top -->
[設計ポータル](../../README.md) ／ [要件定義](../index.md) ／ [業務ユースケース](index.md) ／ **UC-SYSTEM-006: 運用イベントのシステム通知自動生成**
<!-- /portal-top -->
...
| 関連要件 | [FR-123](../01_specifications/FR-123.md#FR-123) システム通知の自動生成 ・ [FR-113](../01_specifications/FR-113.md#FR-113) 契機による通知 |
...
4. メール送信対象の契機かを判定する。
   1. メール対象(質問数上限関連 等): メール配信 IF([API-MAIL-001](../../02_basic_design/03_apis/API-mail.md#API-MAIL-001))でシステム通知メールを送信し、`H_NOTIF_LOGS(C)` に配信ログを記録する。
   2. メール対象外(AI 利用上限到達 等): 受信箱お知らせのみで通知し、メールは送らない。
5. 受信者は管理画面のお知らせ一覧([API-ANN-001](../../02_basic_design/03_apis/API-inbox.md#API-ANN-001))で当該システム通知を確認できる。

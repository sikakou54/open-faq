...
## <span id="FR-046"></span>FR-046: FAQ の登録・編集・削除
...
## <span id="FR-047"></span>FR-047: 管理画面からの削除の不可逆性
...
FAQ・プロジェクト・メンバーユーザー・お知らせ・アカウントの削除は管理画面から復元できないこと(誤削除時はサポート窓口経由でのみ救済される)
...
## <span id="FR-048"></span>FR-048: FAQ に質問と回答を登録
...
## <span id="FR-049"></span>FR-049: FAQ を下書き、公開中、非公開の状態で管理
...
## <span id="FR-050"></span>FR-050: FAQ をカテゴリで整理
...
## <span id="FR-051"></span>FR-051: FAQ の検索・並び替え・絞り込み
...
## <span id="FR-052"></span>FR-052: 公開前の内容確認
...
## <span id="FR-053"></span>FR-053: FAQ 件数・文字数の上限管理
...
FAQ 件数および 1 件あたりの文字数の上限は全プロジェクト共通の基準とし、極端に大きい場合は警告または登録拒否できること。
1. 1 プロジェクトあたり FAQ 件数 = 警告 **8,000 件**、登録拒否 **12,000 件**
...
## <span id="FR-054"></span>FR-054: 同時編集の競合検出
...
## <span id="FR-055"></span>FR-055: FAQ 登録元の保持
...
## <span id="FR-056"></span>FR-056: 公開中 FAQ のみを根拠とする回答
...
## <span id="FR-057"></span>FR-057: 根拠なき独自生成の抑止
...
## <span id="FR-058"></span>FR-058: FAQ 表現の整理にとどめる加工
...
## <span id="FR-059"></span>FR-059: 参照 FAQ の記録と提示
...
## <span id="FR-060"></span>FR-060: FAQ 矛盾時の未解決扱い
...
## <span id="FR-061"></span>FR-061: しきい値の 3 階層調整
...
回答可否の判定に信頼度・関連度のしきい値をグローバル / オーナー別 / プロジェクト別の **3 階層**で調整できること。
...
## <span id="FR-062"></span>FR-062: 問い合わせ ID 付与と回答不可案内
...
## <span id="FR-063"></span>FR-063: 処理エラー時のエラー表示
...
## <span id="FR-064"></span>FR-064: プロンプト注入耐性
...
## <span id="FR-065"></span>FR-065: モデル切替時の品質回帰とロールバック
...
## <span id="FR-066"></span>FR-066: 回答出力前の検査
...
## <span id="FR-067"></span>FR-067: 回答不可質問の未解決登録
...
## <span id="FR-068"></span>FR-068: 未解決申告による未解決登録
...
## <span id="FR-069"></span>FR-069: 未解決質問の記録項目
...
## <span id="FR-070"></span>FR-070: 未解決質問への問い合わせ ID 付与
...
## <span id="FR-071"></span>FR-071: 状況の 2 区分管理
...
## <span id="FR-072"></span>FR-072: 登録時の初期状況「対応中」
...
## <span id="FR-073"></span>FR-073: 状況の一覧・詳細表示
...
## <span id="FR-074"></span>FR-074: 状況変更の手動操作限定
...
## <span id="FR-075"></span>FR-075: 未解決質問からの FAQ 登録開始
...
## <span id="FR-076"></span>FR-076: 質問文の質問欄への初期反映
...
## <span id="FR-077"></span>FR-077: 回答の手動入力と自動反映の禁止
...
## <span id="FR-078"></span>FR-078: 登録前の内容確認・編集
...
## <span id="FR-079"></span>FR-079: FAQ 操作と状況の非連動
...
## <span id="FR-080"></span>FR-080: 登録先 FAQ の参照
...
## <span id="FR-081"></span>FR-081: 処理エラーの検知とエラー応答
...
処理エラー(通信障害・上流障害・入力不備・認可エラー等)を検知し、ウィジェット利用者向けにエラー表示と、状況に応じた応答ができること。「状況に応じた応答」とは、次の各ケースに対しウィジェット利用者を行き止まりにせず次の行動へ誘導することをいう。
1. 回答可能: AI が回答できた場合は回答内容を返す
2. 未解決: AI が回答できなかった場合(該当 FAQ なし・信頼度不足・FAQ 矛盾)は、回答できなかった旨を会話内に表示し、未解決の記録として残す(処理エラーとは区別する。FR-082)
3. 受付上限到達: 月次上限件数への到達・支払方法の未整備により新規質問を受け付けられない場合は、受付制限中である旨を表示し、新規質問の入力・送信を無効化する
4. 処理エラー: 通信障害・上流障害・入力不備・認可エラー等の処理上の失敗時は、エラーが起きた旨を表示し、再試行が妥当なときは再試行案内を示す(FR-083)。処理エラーは未解決の記録として残さない(FR-082)
5. レート制限: 短時間の過大リクエストにより抑制された場合は、時間をおいて再試行する旨を案内する

いずれのケースでも、連絡先メールが設定済みであればその連絡先を代替の問い合わせ先として案内し、未設定の場合は状況説明のみを表示する。AI 推論のタイムアウト・AI プロバイダ障害時は、未解決として扱う定型の案内文へフォールバックする。
...
## <span id="FR-082"></span>FR-082: 処理エラーと未解決登録の区別
...
## <span id="FR-083"></span>FR-083: 再試行案内の表示
...
## <span id="FR-084"></span>FR-084: 内部エラーの運用記録と識別子提示
...
## <span id="FR-085"></span>FR-085: エラー記録の機密を保護
...
## <span id="FR-188"></span>FR-188: AI 推論のタイムアウト上限
...
## <span id="FR-189"></span>FR-189: しきい値変更の推論反映とフォールバック

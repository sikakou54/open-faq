...
処理エラー(通信障害・上流障害・入力不備・認可エラー等)を検知し、ウィジェット利用者向けにエラー表示と、状況に応じた応答ができること。「状況に応じた応答」とは、次の各ケースに対しウィジェット利用者を行き止まりにせず次の行動へ誘導することをいう(各案内の流れはウィジェット利用者への状況案内 UC に準拠する)。

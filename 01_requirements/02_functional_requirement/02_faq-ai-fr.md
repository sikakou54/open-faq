...
2. 未解決: AI が回答できなかった場合(該当 FAQ なし・信頼度不足・FAQ 矛盾)は、回答できなかった旨を会話内に表示し、未解決の記録として残す(処理エラーとは区別する。FR-083)
3. 受付上限到達: 月次上限件数への到達・支払方法の未整備により新規質問を受け付けられない場合は、受付制限中である旨を表示し、新規質問の入力・送信を無効化する
4. 処理エラー: 通信障害・上流障害・入力不備・認可エラー等の処理上の失敗時は、エラーが起きた旨を表示し、再試行が妥当なときは再試行案内を示す(FR-084)。処理エラーは未解決の記録として残さない(FR-083)
...
## <span id="FR-189"></span>FR-189: AI 推論のタイムアウト上限
...
## <span id="FR-190"></span>FR-190: しきい値変更の推論反映とフォールバック

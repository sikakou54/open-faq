...
| ダッシュボード | SCR-095 KPI セクション(NFR-804 (a)〜(l)、グラフ + 直近 30 日履歴)、SCR-096(DLQ 滞留、Webhook 受信失敗率、自動 BO 進捗)、SCR-097(PII 誤検出報告状態別)、SCR-098(ペイロード差分検出件数) |
...
| **AI 内部原価(FR-304)** | 本書 | 月次集計、契約別、Workers AI 実績 token 数 × 単価。集計遅延 24h 超で normal、72h 超で high | 運営者 inbox(SCR-095) | `question_logs.ai_token_count_input` / `question_logs.ai_token_count_output` + KV `ai-cost:unit-prices` |
| **SCR-HOME / SCR-095 ダッシュボード集計遅延**(運営者画面 FR-135d / FR-135g 相当) | 本書 | 集計バッチ最終実行時刻が 5 分以上前で normal、15 分以上で high | 運営者 inbox(SCR-HOME 上の集計遅延バナーと連動) | `GET /admin/v1/home` の `meta.last_aggregated_at` |

**計測基盤**: Cloudflare Workers Analytics Engine、5 分窓集計、日次評価。AC-046(MVP リリース判定期間 NFR-101〜106 p95 4 週間連続達成)はメイン主管だが、本書側は SCR-095 KPI ダッシュボードで観測。
...
- 表示先: SCR-095 KPI セクション、SCR-093 契約別利用状況参考値
- 用途制限: 契約請求には含めない(FR-304(d))、運営者の単価設定の根拠データおよび粗利分析のみ

## 4. AC-046 達成判定 SLA エンジン仕様

AC-046(MVP リリース判定期間 NFR-101〜106 p95 4 週間連続達成、メイン主管)を本書側 SCR-095 で観測する際の **除外条件と判定主体**:
...
- 集計バッチ(`AnalyticsBatchWorker`、日次 04:00 JST): SCR-095 ダッシュボード用に AC-046 達成可否を `analytics_kpi_daily` テーブルへ書き出し
...
|---|---|
| ホーム | サイドメニューのバッジ集約、KPI ハイライト、未承認件数、SLA 違反、DLQ 滞留 |
| SCR-095 KPI セクション | NFR-804 (a)〜(l)、グラフ + 直近 30 日履歴 |
| SCR-096 | DLQ 滞留状況、Webhook 受信失敗率、自動 BO 進捗 |
| SCR-098 | ペイロード差分検出件数 |
| SCR-097 | PII 誤検出報告状態別カウンタ、3 営業日タイマー |

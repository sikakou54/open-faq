...
- 対象外: 利用者向けテーブル(`accounts` / `projects` / `faqs` 等)── [01_メインシステム/個別設計書群/04_テーブル定義書.md](../../01_メインシステム/個別設計書群/04_テーブル定義書.md) を参照(**再掲なし、共有概念正本**)
...
| 13 | `accounts_retired` | 物理削除契約証跡 | **永久保持**(slug 再利用防止)| `owner_account_id` | NFR-707 | **永久** | `7y`(物理削除 action のみ)| - |
...
- **slug 再利用防止が主目的**: `accounts.id` および `accounts.slug` の UNIQUE 制約に WHERE 句を含めない(NFR-707)
...
| 主管 FR | NFR-402 |
| 想定スキーマ | `id` / `owner_account_id` / `region`(`eu` / `eea` / `uk` / `jp` 等)/ `state`(`pending` / `under_review` / `approved` / `rejected`)/ `consent_document_url` / `reviewer_id`(4-eyes)/ `reviewed_at` / `created_at` |
...
| 想定スキーマ | `id` / `owner_account_id` / `endpoint_kind` / `rps` / `rpm` / `valid_until` / `reason` / `created_by` / `created_at` |
...
| 主管 FR | FR-055, FR-061 |
| 想定スキーマ | `id` / `scope`(`global` / `owner` / `project`)/ `owner_account_id` / `project_id` / `confidence_threshold` / `relevance_threshold` / `version` / `created_by` / `created_at` |
...
| カラム名 | snake_case 単数形(`owner_account_id`、`occurred_at`)|
...
### 8.6 契約状態(`accounts.contract_status`(オーナー行)、付録 B.7)

メイン側 [テーブル定義書 §8.5](../../01_メインシステム/個別設計書群/04_テーブル定義書.md#85-契約状態遷移accountscontract_status共有概念正本) を参照。運営者側責務は IF #1 経由の遷移トリガーとサスペンション解除(4-eyes 承認)。
...
| テーブル | インデックス | 用途 |
|---|---|---|
| `webhook_events` | `(state, received_at)`, `(owner_account_id, received_at)`, `next_retry_at`(WHERE NOT NULL)| DLQ 監視 / リプレイ検索 |

...
| 17 | `usage_limit_overrides` | 運営者上書き月次上限件数(論理名)| プロジェクト別上限件数。物理実装はメイン `project_quota_limits`(`source='operator'`)| `id` | FR-121(c) | 5 年 | `5y` | - |
...
| BR-018 | SaaS 全体利用状況把握(運営者)| FR-303, FR-304, FR-232 | SCR-095 |
| BR-021 | 月次請求確定通知 | FR-303 | SCR-095 |
| BR-022 | 運営からのお知らせ配信 | FR-149, FR-188, FR-189 | SCR-094 |
| BR-028 | 内部統制機能 | FR-204, FR-211, FR-220〜232, FR-302〜304 | SCR-090〜094, SCR-095〜098 |
...
> **メイン側との関係**: メイン側では **レート制限を `owner_quota_overrides`(契約単位、`resource_kind` = `*_per_*`)**、**月次上限件数・無料枠を `project_quota_limits`(プロジェクト単位、§3.24、`source='operator'`)** に分離実装する。運営者側基本設計では `rate_limit_overrides` / `usage_limit_overrides` を論理名として言及するが、DDL 未収載のため、**実装はメイン側の `owner_quota_overrides`(レート)/ `project_quota_limits`(月次上限件数)に準拠** を方針とする。
...
### 3.17 `usage_limit_overrides`(プロジェクト別月次上限件数 ── 実装はメイン側 `project_quota_limits`)

§3.15 と同様、詳細設計内 DDL 未収載。**運営者側論理名としては残すが、物理実装はメイン側 `project_quota_limits`(プロジェクト単位、`source='operator'`)を正本とする**(v2.x で月次上限件数をプロジェクト単位化したため、契約単位の独立テーブルは作成しない)。

| 項目 | 内容 |
|---|---|
| 用途 | 質問数の月次上限件数・無料枠と、FAQ 件数 / 個別チャット部屋数の無料枠の運営者上書き。質問数の `threshold=NULL` は上限OFFを表す。FAQ・チャットの月次上限は扱わない |
| 主管 FR | FR-121(c) |
...
| 項目 | 内容 |
|---|---|
| 用途 | SCR-096 からのリプレイ要求の受付(`dlq_replay_log` と対になる申請レコード)|
...
erDiagram
    ...
    usage_limit_overrides ||--|| service_operators : authored_by
...
| `checking_idempotency` | `duplicate_diff_detected_high_alert` | 既処理 + ハッシュ不一致 | 200 OK + SCR-098 + high |
...
| `dlq_retrying` | `dlq_manual_replay` | 1 時間経過 | SCR-096 待ち |
...
| `detected` | `reviewed` | 運営者 SCR-098 確認 |
| `reviewed` | `reprocessed_manually` | SCR-096 手動再処理 |
...
`rate_limit.override` / `usage_limit.override` /
`announcement.send` /
...
| `audit_logs` | `(actor_id, occurred_at)`, `(target_id, occurred_at)`, `(action, occurred_at)`, `(contract_owner_user_id, occurred_at)`, `(retention_class, occurred_at)`, `ticket_id` | SCR-095 検索 / 保持区分別削除 |
...
| 2 | `rate_limit_overrides` / `usage_limit_overrides` / `ai_parameter_overrides` の DDL 未収載 | 02_運営者システム/03_詳細設計書.md | メイン側 `owner_quota_overrides`(レート・契約単位)/ `project_quota_limits`(月次上限件数・プロジェクト単位)に準拠の方針 |

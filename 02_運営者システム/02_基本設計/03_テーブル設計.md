...
> **メイン側との関係**: メイン側では **レート制限を `owner_quota_overrides`(契約単位、`resource_kind` = `*_per_*`)**、**月次上限件数・無料枠を `project_quota_limits`(プロジェクト単位、§3.36、`source='operator'`)** に分離実装する。運営者側基本設計では `rate_limit_overrides` / `usage_limit_overrides` を論理名として言及するが、DDL 未収載のため、**実装はメイン側の `owner_quota_overrides`(レート)/ `project_quota_limits`(月次上限件数)に準拠** を方針とする。
...
| 項目 | 内容 |
|---|---|
| 用途 | 課金対象ごとの月次上限件数・無料枠(質問数 / FAQ 件数 / 個別チャット部屋数)の運営者上書き(SCR-093、4-eyes 承認ログ、**プロジェクト単位**)。物理実装はメイン `project_quota_limits` |
| 主管 FR | FR-121(c) |

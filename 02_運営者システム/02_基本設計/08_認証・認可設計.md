...
| **再認証(FR-005)** | **5 分以内、操作 1 回限り**(SCR-091 / SCR-092 / SCR-093 / SCR-094 / SCR-096 / SCR-097 / SCR-098 の高権限操作の前)|
...
| Webhook リプレイ | SCR-096 | `POST /admin/v1/webhooks/replay` |
| PII ルール更新 | SCR-097 | `POST /admin/v1/pii-rules/revisions` |
| ペイロード差分手動再処理 | SCR-098 | `POST /admin/v1/webhook-diffs/{id}/reprocess` |
...
| 入力場所 | SCR-091〜094 / SCR-096(リプレイ操作)/ SCR-097(ルール更新)/ SCR-098(差分再処理)の各操作前モーダル |
| 監査記録 | `audit_logs.ticket_id` 列に保存、SCR-095 から逆引き検索可 |
...
flowchart TD
    ...
    E -->|OK| F{5. 操作が再認証対象?<br/>SCR-091/092/093/094/096/097/098}

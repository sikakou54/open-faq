...
| テーブル | `contract_owners.contract_status`(参照、メイン主管) | 4 値 |
...
### 3.6 `contract_owners.contract_status`(参考、メイン主管)

`contract_owners` 行(= オーナー)のみで意味を持つ 4 値:
...
stateDiagram-v2
    state pii_false_positive_reports {
        [*] --> reported
        reported --> under_review: SCR-097 開始

...
任意フィールド: `trace_id`、`operator_id`、`owner_account_id`、`action`、`endpoint`、`method`、`status_code`、`duration_ms`、`error_code`、`error_message`、`stack_trace`。
...
| アラート | `level=critical` で運営者 critical alert(オーナー + 全プロジェクト管理者(`account_project_grants.role='admin'` を 1 件でも保持するメンバー)、強制送信) |

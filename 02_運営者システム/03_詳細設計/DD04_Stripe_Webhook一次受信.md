...
| 画面 | SCR-097 | Webhook リプレイ・DLQ 操作 |
| 画面 | SCR-099 | Webhook ペイロード差分検出レビュー |
...
sequenceDiagram
    ...
    alt 署名 NG
        ...
        alt 既処理 + payload_hash 一致
            ...
            Worker->>Main: 通知 (SCR-099 へ運営者誘導)
            Worker-->>Stripe: 200(差分は SCR-099 レビュー待ち)
        else 未処理
...
| 同 `event_id` 異 API バージョン | 既処理 `event_id` + `api_version` 不一致 → 通常フローでは `webhook_payload_diffs` に記録するが、`stripe_api_version_mismatch` フラグを立てて SCR-099 で見分け可能にする |
...
### 3.9 ペイロード差分検出後のレビュー(SCR-099)

差分検出後は `webhook_payload_diffs.state` で SCR-099 上のレビューフローを管理:
...
| `e2e-scr099-001` | 差分検出 → SCR-099 表示 → reprocess / dismiss |

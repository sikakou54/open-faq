...
運営者操作の事後監査(操作者・操作元 IP・対応チケット ID・操作種別・対象・操作前後の値の記録)、運営者活動ダッシュボード(直近 90 日の時系列表示)、操作頻度異常の検知、監査ログ閲覧・検索・エクスポート機能(SCR-095)を扱う。本書側で正本管理する監査ログ閲覧・エクスポート機能(FR-232)もここに含める。
...
| FR-232 | 運営者は監査ログを Web UI(**SCR-095 運営者活動ダッシュボード(監査)**)で閲覧・検索・エクスポートできること。検索・エクスポート操作自体も監査ログに記録し、誰がどの条件で監査ログへアクセスしたかを追跡できること。検索項目、ページング、エクスポート形式、改ざん検知方式、上限超過時の扱いは基本設計 / 詳細設計を正本とする。**派生元**: BR-028 | P0 |
...
- **NFR-311**: 運営者画面(SCR-090〜098)へのアクセスログは全件保持(サンプリング省略不可)、NFR-602d に従い 5 年。
...
- [../02_基本設計/01_画面設計.md](../02_基本設計/01_画面設計.md) §SCR-095(運営者活動ダッシュボード(監査))

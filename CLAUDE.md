...
ファイル名はすべて英名(ASCII)。ページの表示タイトル・見出しは日本語。トップは 3 グループ(`01_requirements` / `02_basic-design` / `03_future`)。基本設計はセクションごとにフォルダ(`index.md` + 配下ファイル)で構成する。
...
├── index.md                    #   基本設計トップ
├── 01_screen-design/           #   画面設計 : index.md(画面一覧)+ SCR-*.md
...
├── 04_usecase-design/          #   ユースケース : index.md(一覧)+ 横断 UC-01〜10 / 画面 UC-SCR-* / システム UC-SYSTEM-*
...
├── usecase-review-log.md       #   ユースケース再編成のレビュー記録(provenance)
└── …                           #   移行記録(html2md.py / imagify_mocks.py 等)
```

- ファイル名は ID ベースの英名(例 `FR01.md` / `SCR-001.md` / `TBL-M-001.md` / `API-auth.md` / `UC-SCR-001.md` / `FUT06-detail.md`)。
- 基本設計はセクション別フォルダの**入れ子構成**。各セクション直下は同一階層で兄弟参照(例 画面設計内 `SCR-002.md`、DB設計内 `TBL-M-001.md`)。
- ページ間リンクはすべて `.md`。同一フォルダ内は兄弟参照、フォルダ/グループをまたぐ参照は相対パス(例 画面設計から API設計へ `../02_api-design/API-faq.md`、基本設計から要件定義へ `../../01_requirements/index.md`)。
...
- **ポータルトップ** = ルート [`README.md`](README.md)。3 グループ(要件定義 / 基本設計 / 将来対応)の全文書を入れ子で索引する(基本設計は 画面設計 / API設計 / データベース設計 / ユースケース設計 / 横断設計 に細分。ユースケース設計は 画面起点 `UC-SCR-*` / システム起点 `UC-SYSTEM-*`)。
- **各セクションの一覧** = 各フォルダの `index.md`(基本設計の `01_screen-design/index.md` / `02_api-design/index.md` / `03_database-design/index.md` / `04_usecase-design/index.md` が画面/API/テーブル/ユースケースの一覧を兼ねる)。
- **各ページの上下ナビ** = 全ページの先頭にパンくず、末尾に戻り導線を、次のマーカで囲って埋め込む:
  - 先頭: `<!-- portal-top -->` 〜 `<!-- /portal-top -->`(例 `[設計ポータル](../../README.md) ／ [基本設計](../index.md) ／ [画面設計](index.md) ／ **SCR-001 ログイン**`)
...
GitHub は埋め込み HTML の `style=`/`class=` を除去するため、画面モック(`02_basic-design/01_screen-design/SCR-*.md` の §3 画面レイアウト)は **PNG 画像で表示**する。HTML ソースは MD に埋め込まず、**`02_basic-design/01_screen-design/mocks/<画面>-<n>.html`** に外出しして保持する(MD は画像 1 行のみ)。画像も同じ `02_basic-design/01_screen-design/mocks/<画面>-<n>.png` に置き、各 SCR からは `mocks/<画面>-<n>.png`(子階層相対・`../` なし)で参照する。**`../` 親参照にすると Cursor / VS Code のプレビューが画像を読み込めない**ため、必ず各ページと同階層配下に置く。

`mocks/<画面>-<n>.html` は単体で開けて図と同じ見た目になる完結 HTML(クロップ用に `#wrap` を持つ)。**モックを変更するときはこの .html を直接編集**し、PNG を再生成する(ルートで):

```sh
node ~/.claude/skills/html-to-png/scripts/html_to_png.js 02_basic-design/01_screen-design/mocks --selector "#wrap"
```
...
- 同一ページ: `[FR-005](#FR-005)`
- 同一フォルダの他ページ: `[API-AUTH-002](API-auth.md#API-AUTH-002)`(API設計フォルダ内)
- 基本設計の別フォルダ: `[SCR-001](../01_screen-design/SCR-001.md#SCR-001)` / `[TBL-M-006](../03_database-design/TBL-M-006.md#TBL-M-006)`
- 他グループ: `[NFR-304](../../01_requirements/index.md#NFR-304)`(基本設計の各フォルダから。NFR の正本は要件定義 index)
...
### 基本設計の文書(`02_basic-design/02_api-design/API-*`、`05〜07_*-design.md` ほか)

- 各節は **タイトル → 説明(リード文)→ 表 → 補足(callout)** の順。
- **内容は原則すべて表**。散文・箇条書きは最小限。
- フロー解説はシーケンス図(mermaid)。DDL・API スキーマ等はコードフェンス。

### 画面設計(`02_basic-design/01_screen-design/SCR-*.md`、1 画面 = 1 ファイル)
...
### テーブル設計(`02_basic-design/03_database-design/TBL-*.md`、1 テーブル = 1 ファイル)

- ファイル名 `TBL-<分類>-<連番>.md`。分類接頭辞は **`M_`(マスタ)/ `T_`(トランザクション)/ `H_`(履歴)/ `TP_`(ワーク・一時)**。
- 各テーブルは 概要 / カラム定義 / 主キー / 外部キー / インデックス / 制約 / コード値 の構成で表記する。
- **データモデルの正本は `02_basic-design/03_database-design/index.md` および各 `TBL-*`**。利用者モデルは `M_USER`(利用者マスタ)+ `M_CONTRACT`(契約マスタ)を中核とする。
...
- **正本カタログ** = `index.md`(画面起点 UC 索引 / システム起点 UC 索引 / 横断ユースケースフロー一覧 / 要件トレーサビリティ)。
- **横断フロー** = `UC-01.md`〜`UC-10.md`(複数画面・非同期処理をまたぐ業務フローを 1 UC 1 ファイル)。システム起点 UC と同じ 8 セクション(概要 / 利用者 / 事前条件 / トリガー / 基本フロー / 異常系フロー / 事後条件 / シーケンス図)。アンカーは `UC-01`〜`UC-10`。
...
- ID 体系: `FR-###` / `BR-###` / `NFR-###`(要件)、`SCR-<番号>(-NNN)`(画面)、`IT-##` / `EV-##`(画面内連番)、`API-*`(API)、`TBL-<分類>-<連番>`(テーブル)、`UC-<SCRID>-EV<nn>` / `UC-SYSTEM-<nnn>`(ユースケース)、`FUT##`(将来対応)。
- 定義箇所は `<span id="ID"></span>`(アンカー)。参照は定義へリンク(同一ページ `#ID`、他ページ `相対パス.md#ID`、NFR は基本設計の各フォルダから `../../01_requirements/index.md#NFR-###`)。
...
> [!NOTE]
> **壊れリンク・壊れアンカーは 0 件に解消済み**(検証スクリプトで `broken file links: 0 / broken anchor links: 0`)。一覧/索引ページ(`02_api-design/index.md` の API、`03_database-design/index.md` §2 のテーブル、`01_screen-design/index.md` の画面カード、`04_usecase-design/index.md` の UC、`01_requirements/index.md` の NFR)は各項目に `<span id>` を付与し、`#API-*` / `#TBL-*` / `#SCR-*` / `#UC-*` / `#NFR-*` の参照が解決する。**この 0 / 0 を維持すること**(編集後は必ず検証スクリプトで確認)。
...
- **登録**: その場で解決できない課題・仕様とドキュメントの矛盾・環境制約(例: Chromium 不在でモック PNG を再生成できない)が生じたら、`gh issue create` で Issue を起票する。本文には **背景 / 対応(具体手順)/ 完了条件** を書く。仕様判断が絡む場合はオーナー確定事項も明記する。
- **クローズ**: 課題を解決したら、解決コミットの本文に `Closes #<番号>` を入れるか `gh issue close <番号>` で当該 Issue をクローズする。チェックリスト付き Issue は消化状況を反映する。
...
- 定義行・見出しを追加したら `<span id="…"></span>` を必ず付ける。削除・改名した識別子(FR ID・列挙値・列名・画面/項目ラベル等)は、`01_requirements` / `02_basic-design`(配下フォルダ含む)/ `03_future` を検索して取りこぼしを修正する。

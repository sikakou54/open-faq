...
- **NFR-ID はカテゴリ別採番(`1xx` 性能 / `2xx` 可用性 / `3xx` セキュリティ / `4xx` プライバシー / `5xx` 通知 / `6xx` 監査 / `7xx` データ保持 / `9xx` 拡張性)を維持し、連番化の対象外**(範囲行 `NFR-320〜324` や接尾辞付き `NFR-602a` を含む独自体系のため)。NFR の正本は `01_requirements/index.md`。

...
> [!NOTE]
> **壊れリンク・壊れアンカーは 0 件に解消済み**(検証スクリプトで `broken file links: 0 / broken anchor links: 0`)。HTML 時代から残っていた未移行ドキュメント(詳細設計 / 運用設計 / エラー設計 / メッセージ一覧 / 共有概念 等)への参照は、現構成への張替または非リンク化で整理した。一覧/索引ページ(`02_api-design.md` の API、`03_database-design.md` §2 のテーブル、`01_screen-design.md` の画面カード、`01_requirements/index.md` の NFR)は各項目に `<span id>` を付与し、`#API-*` / `#TBL-*` / `#SCR-*` / `#NFR-*` の参照が解決する。**この 0 / 0 を維持すること**(編集後は必ず検証スクリプトで確認)。

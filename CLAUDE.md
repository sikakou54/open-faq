...
- **正本カタログ** = `index.md`(画面起点 UC 索引 / システム起点 UC 索引 / 業務ユースケース(アクター別)索引 / 要件トレーサビリティ)。
- **業務起点** = `UC-BIZ-<nnn>.md`(アクター(役割)別の業務目的を 1 UC 1 ファイル)。8 セクション(概要 / アクター / 事前条件 / トリガー / 主成功シナリオ / 例外・代替フロー / 事後条件 / **業務アクティビティ図**)。**詳細シーケンスは持たず**、画面起点 UC・システム起点 UC へトレースする(粒度の重複を避ける)。`§8` は業務レベルの `flowchart`(API・テーブル・SQL は描かない)。

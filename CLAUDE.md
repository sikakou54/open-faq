...
├── 01_specifications/             #   要件仕様 : index.md + カテゴリ別ファイル(01_account / 02_faq-ai / 03_usage /
│                                  #            04_widget / 05_notification / 06_security / 07_nfr / 08_rule)。各 BR/FR/NFR/RULE は節(#<ID>)で保持
...
- ファイル名は原則 ID 1 件 = 1 ファイル(例 `UC-001.md` / `SCR-001.md` / `EVT-001.md` / `API-001.md` / `TBL-001.md` / `SEQ-001.md` / `PERM-001.md` / `ERR-001.md` / `MSG-001.md`)。**ただし要件仕様(BR/FR/NFR/RULE)はファイル数削減のためカテゴリ別ファイルへ統合**し、各 ID は当該ファイル内の節 `### <span id="ID"></span>ID: 名称` として保持する(`01_specifications/` 配下。ID・アンカー・採番は不変)。
...
- 同一ページ: `[FR-001](#FR-001)`
- 要件仕様(カテゴリ別ファイル内の節へ): `[FR-007](02_faq-ai.md#FR-007)` / `[BR-001](01_account.md#BR-001)`(同一フォルダ)。**ファイル名は ID ではなくカテゴリ名**、アンカーは `#<ID>`。対応表は `01_specifications/index.md`。
...
- グループをまたぐ: 業務UC → 要件 `[FR-001](../01_specifications/01_account.md#FR-001)` / 画面 → 業務UC `[UC-001](../../01_requirements/02_business_usecases/UC-001.md#UC-001)` / API → 業務UC `[UC-016](../../01_requirements/02_business_usecases/UC-016.md#UC-016)`
...
BR / FR / NFR / RULE を**カテゴリ別ファイルへ統合**し、各要件は節として保持する。ファイル構成は次の 8 つ + `index.md`:

| ファイル | 内容 |
|----|----|
| `01_account.md` | アカウント・ユーザー・アクセス制御(BR / FR) |
| `02_faq-ai.md` | FAQ・AI 回答・未解決質問・処理エラー(BR / FR) |
| `03_usage.md` | 利用量・課金・ダッシュボード・運用(BR / FR) |
| `04_widget.md` | ウィジェット・検索・入出力(BR / FR) |
| `05_notification.md` | 通知・お知らせ(BR / FR) |
| `06_security.md` | セキュリティ・プライバシー(BR / FR) |
| `07_nfr.md` | 非機能要件(`## <分類>` 節別) |
| `08_rule.md` | 業務ルール |

- ファイル先頭はカテゴリの `# 見出し` + 要約 + メタ。カテゴリ↔要件の対応は `index.md`。BR/FR ファイルは `## 業務要件` / `## 機能要件` の種別節を持つ。
- 各要件節の骨格: `### <span id="ID"></span>ID: 名称` → 要約 → メタ → `#### 要件`(RULE は `#### ルール`)→ `#### 関連`。**リード文は置かない。** ID アンカーは要件見出しに付け、節見出し(要件 / 関連 / 由来 等)には付けない(ファイル内で重複するため)。
- **業務要件 `BR-*`**: `#### 関連` 表(列 = `観点 / ステークホルダ / 関連 NFR / 対応業務UC`)。要件本文は役割別の箇条書き可。
- **機能要件 `FR-*`**: `#### 関連` 表(列 = `関連業務要件 / 優先度 / 対応業務UC`)。優先度 `P0`/`P1`/`P2`。メタに `機能グループ`(カテゴリ振り分けの根拠)。
...
- 要件を追加するときは当該カテゴリファイルの種別節へ `### <span id="ID"></span>…` を追加し、`機能グループ` / `分類` で振り分ける。

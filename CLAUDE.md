...
- 各要件節の骨格: `## <span id="ID"></span>ID: 名称` → 要約 → メタ → `### 要件`(RULE は `### ルール`)→ **`### シーケンス`(FR のみ)** → `### 関連`。**リード文は置かない。** ID アンカーは要件見出しに付け、節見出し(要件 / シーケンス / 関連 / 適用UC 等)には付けない(ファイル内で重複するため)。
- **後工程参照を置かない**: 要件本文(`### 要件` / `### ルール`)には、後工程(機能要件・業務ルール・基本設計)へのリンクや「…を正本とする」等の委譲文を書かない。前後関係は `### 関連` で表す。
- **`### 関連` は後続ドキュメントのみ**: 上流・兄弟(BR↔FR、→NFR、RULE→FR 由来)へのリンクは持たず、下流の業務UC のみを記す。
...
- **業務要件 `BR-*`**: `### 関連` 表(列 = `観点 / ステークホルダ / 対応業務UC`)。要件本文は役割別の箇条書き可(ただし上記粒度に従い実装詳細は持たない)。
- **機能要件 `FR-*`**: `### 関連` 表(列 = `優先度 / 対応業務UC`)。優先度 `P0`/`P1`/`P2`。メタに `機能グループ`(カテゴリ振り分けの根拠)。

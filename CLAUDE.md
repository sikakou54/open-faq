...
| 画面イベント | `EVT-` | `EVT-001`〜 | 画面設計 SCR §6 のイベント一覧に `<span id>` アンカー(非リンク)として保持(独立ファイル・フォルダは廃止)。シーケンスがプレーン ID で参照 |
...
- **イベント ID の特例**: 画面イベント `EVT-xxx` / システムイベント `SEV-xxx` は独立ページを持たず、画面設計 SCR §6 / システム設計 SYS §6 のイベント一覧に **非リンクのアンカー** `<span id="EVT-001"></span>EVT-001` として定義する。これを参照する側(シーケンス SEQ・将来のシステム起点シーケンス等)は、リンクではなく **プレーンテキストの広域 ID**(例 `EVT-001`)で記す。

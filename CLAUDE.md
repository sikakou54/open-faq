...
- **BR と FR の記載粒度(重複を作らない)**: BR=業務意図(誰が・何のために・何を可能にするか + 業務上の制約/リスク/方針の質的言明)を **1〜2 文**で。FR=システム機能の細粒度。**数値しきい値・手順番号・対象操作の列挙・状態名・画面/UI 文言・FR とほぼ同文の機能記述は BR に書かず**、対応 FR(数値は `08_rule.md` の RULE)を正本として委ねる(本文に「(詳細は対応機能要件・業務ルールを正本とする)」等と記し、新規リンクは貼らない)。FR に無い業務理由・価値・ステークホルダ別役割・固有方針は BR に残す。FR から BR の業務観点を再掲しない。
- **業務要件 `BR-*`**: `### 関連` 表(列 = `観点 / ステークホルダ / 関連 NFR / 対応業務UC`)。要件本文は役割別の箇条書き可(ただし上記粒度に従い実装詳細は持たない)。

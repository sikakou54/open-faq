...
本リポジトリは「FAQ AI ウィジェット SaaS / メインシステム」の **要件定義書**・**基本設計書** を **Markdown** で管理する。1 つの識別子(要件 / UC / 画面 / イベント / API / テーブル …)= 1 ファイルの**個別ページ構成**を正本とし、ルートの [`README.md`](README.md) が入口(各セクション `index.md` への手書きトップページ)である。各識別子はフラット連番でゼロ詰め 3 桁・欠番なし。
...
| 基本設計(SCR/EVT/SYS/SEV/API/TBL/SEQ ほか) | 要件をシステムとしてどう実現するかを記載する |
...
ページ内ナビ(パンくず・戻り導線)の自動付与は廃止した。ナビゲーションは次の手動保守で行う。

- **トップページ** = ルート [`README.md`](README.md)(手書き・最小)。各セクションの `index.md` への入口。
- **各セクションの一覧** = 各フォルダの `index.md`(要件仕様 / 業務UC / 画面 / 画面イベント / システム / システムイベント / API / テーブル / シーケンス / 権限 / エラー / メッセージ の一覧を兼ねる)。一覧・索引は手動で保守する。
...
| 基本設計 同グルーピング内(兄弟フォルダ) | 画面イベント → 画面 `[SCR-001](../01_screens/SCR-001.md#SCR-001)` / システムイベント → システム `[SEV-001 → SYS-001](../01_system/SYS-001.md#SYS-001)` / API → テーブル `[TBL-001](../04_database/TBL-001.md#TBL-001)` / システム → API `[API-060](../03_apis/API-060.md#API-060)` |
...
各ドキュメント層の骨格・章立て・記載例・体裁指示は [`templates/`](templates/README.md) 配下に分離した。**新規ページを作成するときは、該当テンプレートを雛形にして本文を起こす。** テンプレートは記載スタイル標準(共通)+ 各層テンプレート(要件仕様 / 業務UC / 画面 / 画面イベント / システム / システムイベント / API / DB / シーケンス / 権限・エラー・メッセージ / 将来対応)で構成する。
...
| [templates/07_screen-event.md](templates/07_screen-event.md) | 画面イベント設計 `EVT` |
| [templates/08_system.md](templates/08_system.md) | システム設計 `SYS` |
| [templates/09_system-event.md](templates/09_system-event.md) | システムイベント設計 `SEV` |

...
- **順引き**: 各ページの `関連` / `項目` 表で下流(UC は SCR/EVT/API/TBL、SCR §1 は UC、API は UC/SCR/EVT/TBL、TBL は UC/API)へリンク。**ただし要件仕様(BR/FR/NFR/RULE)は下流へのリンクを持たない**(`### 関連` を置かない)。要件 ↔ UC の結線は UC 側の `対応要件ID`(および SCR/PERM の由来要件)を正本とする一方向参照とする。
- **逆引き**: 要件→下流(対応業務UC 等)や TBL の `対応業務UC / 利用API` は、下流側の参照(UC の `対応要件ID` 等)を機械集計して辿る。要件ページ自体には逆引き欄を持たせない。

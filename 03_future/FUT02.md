...
| 種別 | 影響内容 |
|----|----|
| 要件 | FR-168(検索方式)、FR-173 / FR-192(応答方式)、FR-061(矛盾検知)、FR-066(AI 品質テストセット)、FR-067(PII 検出層) |
| 画面 | ウィジェット応答表示(ストリーミング)、FAQ 画面(矛盾検知結果表示)、利用者の PII 誤検出報告画面、契約別プロンプト選択 UI(SCR-029 設定で具体化) |

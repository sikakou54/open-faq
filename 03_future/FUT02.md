<!-- portal-top -->
[設計ポータル](../README.md) ／ [将来対応](index.md) ／ **FUT02: 検索・AI 強化**
<!-- /portal-top -->
...
| 種別 | 影響内容 |
|----|----|
| 要件 | FR-129(検索方式)、FR-134 / FR-153(応答方式)、FR-038(矛盾検知)、FR-043(AI 品質テストセット)、FR-044(PII 検出層) |
| 画面 | ウィジェット応答表示(ストリーミング)、FAQ 画面(矛盾検知結果表示)、利用者の PII 誤検出報告画面、契約別プロンプト選択 UI(SCR-023 設定で具体化) |
...
| FUT-AI-Q-004 | プロンプトカスタマイズの注入対策(システム管理プロンプト優先・サニタイズ) | 中 | 検討中 |

...
| SCR-034 自動割り当て | オーナー / メンバー | デフォルト担当者の選択・保存 | SCR-013の管理タブから遷移 |
...
- **自動割り当て設定(SCR-034, `PATCH /projects/{id}` `defaultAssigneeUserId`)**: SCR-013チャット配下の「自動割り当て」タブから操作する。当該プロジェクトのメンバー(オーナーを含む)が操作可。`null`を許容し、未設定時はオーナーへフォールバックする。

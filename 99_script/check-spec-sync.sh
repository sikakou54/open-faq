...
require_pattern "users" "01_メインシステム/02_基本設計/03_テーブル設計/index.html"
require_pattern "contract_owners" "01_メインシステム/02_基本設計/03_テーブル設計/index.html"
require_pattern "project_users" "01_メインシステム/02_基本設計/03_テーブル設計/index.html"
require_pattern "contract_owner_user_id" "01_メインシステム/02_基本設計/03_テーブル設計/index.html"
...
for v in "active" "suspended" "deleted_pending" "deleted"; do
  ...
  require_pattern ">${v}<|'${v}'" "01_メインシステム/02_基本設計/03_テーブル設計/index.html"
done
...
# (b) テーブル詳細は個別ファイル化。index 2.1 一覧に 31 件の個別リンクが存在
t31 = read(M + "03_テーブル設計/index.html")
seg31 = re.search(r'<h3[^>]*>2\.1.*?(?=<h3[^>]*>2\.2|<h2)', t31, re.S).group(0)
links31 = len(re.findall(r'href="[a-z_]+\.html"', re.search(r'<table>.*?</table>', seg31, re.S).group(0)))

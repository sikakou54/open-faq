...
require_pattern "M_project_users" "01_メインシステム/02_基本設計/03_テーブル設計/index.html"
require_pattern "M_project_user_assignments" "01_メインシステム/02_基本設計/03_テーブル設計/index.html"
...
for v in "active" "suspended" "deleted_pending" "deleted"; do
  ...
  require_pattern ">${v}<|'${v}'" "01_メインシステム/02_基本設計/03_テーブル設計/M_owners.html"
done
...
links31 = len(re.findall(r'href="[A-Za-z_]+\.html"', re.search(r'<table>.*?</table>', seg31, re.S).group(0)))

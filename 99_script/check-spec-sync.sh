...
# 画面遷移図: メインシステムはアカウント種別別 4 ファイル、運営者システムは統合 1 ファイル
...
MAIN_SCREEN_SECTIONS=$(grep -E '^### 5\.[0-9]+ ' "01_メインシステム/02_基本設計/01_画面設計.md" \
  | sed -E 's/^### 5\.([0-9]+).*/\1/')
require_sequence "メイン画面詳細節" "$MAIN_SCREEN_SECTIONS" "$(seq 0 28)"

MAIN_MESSAGE_SECTIONS=$(grep -E '^### 4\.[0-9]+ (SCR-|エンドユーザー)' "01_メインシステム/02_基本設計/06_メッセージ一覧.md" \
  | sed -E 's/^### 4\.([0-9]+).*/\1/')
require_sequence "メイン画面メッセージ節" "$MAIN_MESSAGE_SECTIONS" "$(seq 1 27)"

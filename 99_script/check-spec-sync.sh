...
# 画面遷移図(原 HTML)。メインはアカウント種別別 4、運営者は統合 1。
require_file "01_メインシステム/画面遷移図_ウィジェット利用者.html"
require_file "01_メインシステム/画面遷移図_プロジェクトメンバー.html"
require_file "01_メインシステム/画面遷移図_プロジェクト管理者.html"
require_file "01_メインシステム/画面遷移図_オーナー.html"
...
for system in "01_メインシステム" "02_運営者システム"; do
  for f in "${BASIC_DESIGN_FILES[@]}"; do require_file "${system}/02_基本設計/${f}"; done
done
...
for system in "01_メインシステム" "02_運営者システム"; do
  for f in "${BASIC_DESIGN_FILES[@]}"; do
    require_pattern "<h1[ >]" "${system}/02_基本設計/${f}"
    require_pattern "<h2[ >]" "${system}/02_基本設計/${f}"
  done
done
...
# ============================================================================
MAIN_SCRS=("SCR-001" "SCR-002" "SCR-003" "SCR-004" "SCR-004-M1" "SCR-005" "SCR-006" "SCR-006-M1" "SCR-007" "SCR-008" "SCR-009" "SCR-009-M1" "SCR-010" "SCR-011" "SCR-012" "SCR-013" "SCR-014" "SCR-015" "SCR-016" "SCR-017" "SCR-018" "SCR-019" "SCR-020" "SCR-021" "SCR-021-M1" "SCR-022" "SCR-023")
for scr in "${MAIN_SCRS[@]}"; do
  require_pattern "${scr}" "01_メインシステム/02_基本設計/01_画面設計.html"
...
for if_num in 1 2 4 5 6 7 8 9 10 12; do
  require_pattern "IF #${if_num}" "01_メインシステム/02_基本設計/02_API設計.html"
done
...
# (c) 画面詳細節 5.N = 0..28
check_seq("メイン画面詳細節", heading_nums(M+"01_画面設計.html", 3, "5."), list(range(0, 29)))
...
if grep -R -E --include='*.html' '\(旧\)サポート時間について|JSON 形式は廃止|旧 \.left::after は廃止' \
  "01_メインシステム/画面遷移図_オーナー.html" "01_メインシステム/画面遷移図_プロジェクト管理者.html" \
  "01_メインシステム/画面遷移図_プロジェクトメンバー.html" "01_メインシステム/画面遷移図_ウィジェット利用者.html" >/dev/null; then

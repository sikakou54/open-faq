...
- 横断コンポーネントの単一実装検証: `AuditLogService`(M-01)/`AuditLogRepository`(M-02)を本機能領域内の M-08・M-11・M-14 が同一インスタンス相当として共用し、重複実装しないこと。[MOD-003](MOD-003.md#MOD-003) M-12/M-19・[MOD-005](MOD-005.md#MOD-005) M-17 の同名コンポーネントは本 platform の単一共有実装(M-01/M-02)を指す参照であり、ドメインごとに再実装しない(監査ログの集約先は platform に確定)。

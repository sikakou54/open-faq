...
> **本構造図は「プロジェクトの一覧・作成・更新・削除・許可ドメイン管理・ウィジェット公開鍵ローテーション・データ概要/削除影響プレビュー取得」機能領域のモジュール分割と内向き依存の方向を定義します。**
...
| M-02 | `app/(dashboard)/projects/[id]`(プロジェクト概要画面) | frontend | プロジェクト範囲データ概要・削除影響プレビューの表示を担う([SCR-012](../../02_basic_design/01_frontend/01_screens/SCR-012.md#SCR-012)) | 利用者操作(画面遷移) | プロジェクト API 呼び出し |
...
| M-06 | `app/api/projects/[id]/overview/route.ts` ・ `app/api/projects/[id]/deletion-impact/route.ts` | api | プロジェクト範囲データ概要・削除影響プレビューの受付(参照系)([API-065](../../02_basic_design/02_backend/03_apis/API-065.md#API-065) ・ [API-066](../../02_basic_design/02_backend/03_apis/API-066.md#API-066)) | HTTP リクエスト(プロジェクト ID) | Service 呼び出し・HTTP レスポンス |
| M-07 | `lib/service/project`(`ProjectService`) | service | 一覧絞り込み(`scope`)・作成(オーナー設定・許可ドメイン保存・課金アカウント遅延作成・作成者メンバー自動登録・しきい値任意保存)・更新(コア項目 / `settings` の権限分岐)・論理削除(メンバー割当解除・アカウント論理削除判定・関連データ論理削除伝播・監査記録)・データ概要/削除影響プレビュー集計を統括する([CLS-003](../10_class/CLS-003.md#CLS-003)) | 検証済みプロジェクト操作要求 | Repository 呼び出し・応答 DTO |
...
| M-09 | `lib/guard/project-ownership`(`ProjectOwnershipGuard`) | 共通 | 対象プロジェクトの `owner_user_id` が要求元と一致するかを判定する(コア項目更新・削除・鍵ローテーション・削除影響プレビューの権限境界) | 要求元・対象プロジェクト | 許可 / 拒否(境界秘匿 or オーナー限定) |
...
| M-16 | `lib/repository/project-deletion-impact`(`ProjectDeletionImpactRepository`) | repository | プロジェクト削除で論理削除される FAQ・質問ログ・未解決質問・メンバー割当の件数集計を D1 へ行う | Service からの集計要求 | 集計結果 |
| M-17 | `lib/repository/project-member`(`ProjectMemberRepository`) | repository | プロジェクトメンバー割当の自動登録(作成者)・プロジェクト単位一括論理削除・ユーザー単位有効割当数照会を D1 へ行う | Service からの参照・更新要求 | 取得結果 / 更新結果 |
| M-18 | `lib/repository/billing-account`(`BillingAccountRepository`) | repository | 作成者の課金アカウント照会・未作成時の遅延作成を D1 へ行う | Service からの参照・更新要求 | 取得結果 / 更新結果 |
...
flowchart TB
    ...
    M06["概要/削除影響プレビュー API(api)"]
    M09["ProjectOwnershipGuard(共通)"]
    M10["ProjectMembershipGuard(共通)"]
    M11["ReauthGuard(共通)"]
    M07["ProjectService(service)"]
    M08["WidgetKeyService(service)"]
    REPO["リポジトリ群(project / allowed-domain / legacy-widget-key / project-usage-overview / project-deletion-impact / project-member / billing-account)"]
    M19["D1 クライアント(共通)"]
    DB["D1(TBL 参照)"]
    M01 --> M03
    M01 --> M04
...
| M-02 プロジェクト概要画面 | M-06 概要/削除影響プレビュー API | データ概要・削除影響プレビューの取得 | 同期 | — |
...
| M-06 概要/削除影響プレビュー API | M-10 ProjectMembershipGuard | データ概要取得の割当判定 | 同期 | 割当なしは 403([ERR-019](../../02_basic_design/05_errors/ERR-019.md#ERR-019)) |
| M-06 概要/削除影響プレビュー API | M-09 ProjectOwnershipGuard | 削除影響プレビュー取得のオーナー限定判定 | 同期 | オーナー以外は 403([ERR-015](../../02_basic_design/05_errors/ERR-015.md#ERR-015)) |
| M-07 ProjectService | M-09 ProjectOwnershipGuard ・ M-10 ProjectMembershipGuard | 更新・削除・データ概要/削除影響プレビューの権限境界判定 | 同期 | — |
...
| M-07 ProjectService | M-17 プロジェクトメンバーリポジトリ | 作成者のメンバー自動登録・全メンバー割当論理削除・有効割当数照会 | 同期 | [TBL-003](../../02_basic_design/02_backend/04_database/TBL-003.md#TBL-003)。物理項目対応は [DBP-005](../07_db_physical/DBP-005.md#DBP-005) |
| M-07 ProjectService | M-18 課金アカウントリポジトリ | 作成者の課金アカウント照会・未作成時の遅延作成 | 同期 | [TBL-002](../../02_basic_design/02_backend/04_database/TBL-002.md#TBL-002)。物理項目対応は [DBP-004](../07_db_physical/DBP-004.md#DBP-004) |
...
| M-07 ProjectService | M-16 プロジェクト削除影響リポジトリ | 削除影響プレビューのための関連データ件数集計 | 同期 | [API-066](../../02_basic_design/02_backend/03_apis/API-066.md#API-066) |
...
| M-12〜M-18 各リポジトリ | M-19 D1 クライアント | クエリ実行・トランザクション境界 | 同期 | Repository のみが D1 を利用(内向き依存) |
...
| M-06 概要/削除影響プレビュー API | 権限境界判定を経て当該プロジェクトに範囲を限定した集計結果を返す(参照のみ) | 割当なしは 403([ERR-019](../../02_basic_design/05_errors/ERR-019.md#ERR-019))、オーナー限定違反は 403([ERR-015](../../02_basic_design/05_errors/ERR-015.md#ERR-015))、対象なしは 404([ERR-011](../../02_basic_design/05_errors/ERR-011.md#ERR-011)) | 他プロジェクトのデータを混在させない(プロジェクト単位のデータ分離) |
| M-07 ProjectService | 一覧絞り込み・作成(許可ドメイン保存・課金アカウント遅延作成・作成者メンバー自動登録・しきい値任意保存)・更新・論理削除(伝播順序を含む)・データ概要/削除影響プレビュー集計を統括する | 名称重複は作成を中断し 409 を返す | 詳細ロジックは [CLS-003](../10_class/CLS-003.md#CLS-003) §5 メソッド一覧 |
...
| M-12〜M-18 リポジトリ群 | プロジェクト・許可ドメイン・旧公開鍵・データ概要/削除影響集計・メンバー・課金アカウントの D1 アクセスを担い、Service からの参照・更新要求を実行する | 一時障害は呼び出し元へ伝播し Tx をロールバック | 物理設計は [DBP-006](../07_db_physical/DBP-006.md#DBP-006) ・ [DBP-005](../07_db_physical/DBP-005.md#DBP-005) ・ [DBP-004](../07_db_physical/DBP-004.md#DBP-004) |

## 6. 後続工程への引き継ぎ事項

実装・テスト設計へ引き継ぐ観点(依存方向の逸脱検出・非同期境界・外部連携の切り離しテスト)を箇条書きで示す。

- 内向き依存の逸脱検証: D1 クライアント(M-19)を利用するのは Repository 群のみで、Service/Guard/API から直接 D1 を触らないこと。逆依存(Repository → Service)・循環依存が生じていないこと。
...
- ガード使い分けの境界検証: コア項目更新・削除・鍵ローテーション・削除影響プレビューはオーナー限定(M-09)、`settings` のみの更新とデータ概要取得は有効メンバー許可(M-10)である分岐をテスト設計でケース化すること。

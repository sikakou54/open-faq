...
| 関連 API / SYS | [API-037](../../02_basic_design/02_backend/03_apis/API-037.md#API-037) ・ [API-038](../../02_basic_design/02_backend/03_apis/API-038.md#API-038) ・ [API-039](../../02_basic_design/02_backend/03_apis/API-039.md#API-039) ・ [API-057](../../02_basic_design/02_backend/03_apis/API-057.md#API-057) ・ [API-068](../../02_basic_design/02_backend/03_apis/API-068.md#API-068) ・ [SYS-001](../../02_basic_design/02_backend/01_system/SYS-001.md#SYS-001) ・ [SYS-003](../../02_basic_design/02_backend/01_system/SYS-003.md#SYS-003) ・ [SYS-005](../../02_basic_design/02_backend/01_system/SYS-005.md#SYS-005) ・ [SYS-008](../../02_basic_design/02_backend/01_system/SYS-008.md#SYS-008) ・ [SYS-016](../../02_basic_design/02_backend/01_system/SYS-016.md#SYS-016) ・ [SYS-018](../../02_basic_design/02_backend/01_system/SYS-018.md#SYS-018) |
...
本機能領域は、設置サイトのウィジェットから届いた質問を受け取り、AI 推論で回答可否を判定して回答または未解決案内を返し、質問数は同期(同一トランザクション)で計上したうえで、非同期に後処理(参照 FAQ 記録・未解決記録・利用量集計の閾値判定/サマリ反映)を進める一連の実装単位を定義する。モジュール分割は Next.js on Cloudflare の物理配置(`app/`・`lib/service`・`lib/repository`・`lib/gateway`・`workers/queues`・横断ガード)へ写像し、依存は内向き(frontend → api → service → repository、AI 連携は service → external-gateway)に統一して逆依存・循環依存を作らない。
...
| M-02 | `app/api/widget/ask/route.ts` | api | 質問送信の受付。認証・入力検証を経て Service へ委譲し回答/未解決/処理エラーを応答する。質問数計上は Service が同一トランザクション内で確定する(本 API 経路が計上の正本)([API-038](../../02_basic_design/02_backend/03_apis/API-038.md#API-038)) | HTTP リクエスト(質問文・対象プロジェクト・セッション) | Service 呼び出し・HTTP レスポンス |
| M-03 | `app/api/widget/bootstrap/route.ts` ・ `app/api/widget/inquiries/route.ts` ・ `app/api/widget/feedback/route.ts` | api | ウィジェット起動・未解決申告登録・回答フィードバックの受付と Service 委譲([API-037](../../02_basic_design/02_backend/03_apis/API-037.md#API-037) ・ [API-039](../../02_basic_design/02_backend/03_apis/API-039.md#API-039) ・ [API-068](../../02_basic_design/02_backend/03_apis/API-068.md#API-068)) | HTTP リクエスト | Service 呼び出し・HTTP レスポンス |
| M-04 | `lib/service/answer`(`AnswerService`) | service | 回答可否判定の中核。しきい値解決・候補 FAQ 取得(全文検索・関連度スコア付き)・AI 推論呼出・結果判定(信頼度 かつ 関連度=候補最上位 FTS スコア がしきい値以上で `answered`)・回答文 PII マスキング適用・質問ログ記録・質問数の同期計上(同一 Tx で [TBL-020](../../02_basic_design/02_backend/04_database/TBL-020.md#TBL-020) 加算)・未回答時の未解決同一 Tx 生成・非同期後処理の投入を統括する([IPO-001](../04_ipo/IPO-001.md#IPO-001)) | 検証済み質問要求・適用しきい値 | Repository/Gateway 呼び出し・Queues 投入・応答 DTO |
| M-04a | `lib/service/answer/pii-masker`(`PiiMasker`) | service | AI 回答生成後の回答文からメール・電話・カード番号を検出し種別ラベル付き伏字へ置換し、検出有無・種別を返す(AnswerProvider の外・応答整形前。氏名・住所は MVP 対象外・[FUT-07](../../04_future/FUT-07.md#FUT-07)) | 生成された回答文 | マスク済回答文・検出種別([RULE-024](../../01_requirements/01_business_requirement/08_rule.md#RULE-024)) |
...
| M-06 | `lib/repository/question-log`(`QuestionLogRepository`) | repository | 質問ログの記録(結果種別・結果理由・信頼度/関連度スコア・課金対象区分・PII マスク済/検出種別)を D1 へ書き込む | Service からの記録要求 | 記録結果([TBL-025](../../02_basic_design/02_backend/04_database/TBL-025.md#TBL-025)) |
...
| M-08 | `lib/repository/faq`(`FaqRepository`) | repository | 候補 FAQ(公開かつ当該プロジェクト分)の全文検索(一致スコア降順で上位 N 件を関連度スコア付きで返す)と参照 FAQ 紐づけの記録を D1 へ行う | Service からの参照/記録要求 | FAQ 取得結果(関連度スコア付き)・紐づけ記録([TBL-006](../../02_basic_design/02_backend/04_database/TBL-006.md#TBL-006) ・ [TBL-016](../../02_basic_design/02_backend/04_database/TBL-016.md#TBL-016)・全文検索は [TBL-030](../../02_basic_design/02_backend/04_database/TBL-030.md#TBL-030)) |
| M-09 | `lib/repository/usage`(`UsageMeterRepository`) | repository | 当月利用量計測の参照・更新を D1 へ行う。質問数(`question_count`)・課金対象外件数(`unbillable_question_count`)の同期加算(AnswerService の Tx 内)と、コンシューマ/モニタからの参照を担う | Service/Guard/コンシューマからの参照・更新要求 | 利用量取得/更新結果([TBL-020](../../02_basic_design/02_backend/04_database/TBL-020.md#TBL-020)) |
...
| M-17 | `workers/queues/metering-consumer` | batch | 利用量集計の非同期後処理を消費する。質問数は加算せず(同期加算済み値=正本を参照)、閾値到達判定・アラート/受付停止/最終ガード契機の引き渡し・利用量サマリ反映のみを行う([SYS-016](../../02_basic_design/02_backend/01_system/SYS-016.md#SYS-016) ・ [BAT-001](../05_batch/BAT-001.md#BAT-001)) | Queues メッセージ(質問到達イベント) | 判定・後続契機引き渡し・サマリ反映 |
...
| M-04 AnswerService | M-04a PiiMasker | `answered` 時の回答文 PII マスキング(応答整形前) | 同期 | 基準は [RULE-024](../../01_requirements/01_business_requirement/08_rule.md#RULE-024)。氏名・住所は対象外([FUT-07](../../04_future/FUT-07.md#FUT-07)) |
| M-04 AnswerService | M-06 質問ログリポジトリ | 質問ログの記録(回答/未回答/処理エラー) | 同期 | 物理カラムは [DBP-001](../07_db_physical/DBP-001.md#DBP-001) |
| M-04 AnswerService | M-07 未解決リポジトリ | 未回答時の未解決質問を同一 Tx で生成 | 同期 | 未回答 3 理由(`no_faq_match` / `low_confidence` / `contradiction`)のみ。処理エラー時は生成しない([FR-082](../../01_requirements/02_functional_requirement/02_faq-ai-fr.md#FR-082)) |
| M-04 AnswerService | M-08 FAQ リポジトリ | 候補 FAQ(公開分)の全文検索(関連度スコア付き上位 N 件) | 同期 | 参照 FAQ 紐づけ記録は非同期後処理で実施([SYS-001](../../02_basic_design/02_backend/01_system/SYS-001.md#SYS-001))。上位 N の正本は [システム仕様書](../../02_basic_design/07_system-spec.md) |
| M-04 AnswerService | M-09 利用量リポジトリ | 質問数(`question_count`)・課金対象外件数(`unbillable_question_count`)の同期加算(質問ログ ID で冪等・同一 Tx。計上の正本) | 同期 | [TBL-020](../../02_basic_design/02_backend/04_database/TBL-020.md#TBL-020)。分母=`question_count - unbillable_question_count`([RULE-013](../../01_requirements/01_business_requirement/08_rule.md#RULE-013)) |
| M-04 AnswerService | Cloudflare Queues | 参照 FAQ 記録・未解決記録・利用量集計(閾値判定・サマリ)の後処理投入 | 非同期(Queues 経由) | 投入先コンシューマは M-15/M-16/M-17。M-17 は質問数を加算しない |
...
| M-17 利用量コンシューマ | M-09 利用量リポジトリ | 同期加算済み利用量の参照(閾値到達判定・サマリ反映用。質問数は加算しない) | 非同期(Queues 経由) | 判定・契機引き渡し・サマリ反映は [BAT-001](../05_batch/BAT-001.md#BAT-001)。上限超過分は DLQ へ退避 |
...
| M-04 AnswerService | 適用しきい値を解決し候補 FAQ を全文検索(関連度スコア付き)して AI 推論を呼び出し、信頼度 かつ 関連度(候補最上位 FTS スコア)がしきい値以上なら `answered` と判定する。`answered` は回答文を PII マスキング(M-04a)してから提示・`unanswered` は質問ログと未解決を同一 Tx で生成・`error` は処理エラーとして確定し、いずれも質問数を同一 Tx で同期計上したうえで後処理を Queues へ投入する | AI タイムアウト/プロバイダエラーは処理エラーとして未解決を生成しない([FR-082](../../01_requirements/02_functional_requirement/02_faq-ai-fr.md#FR-082))。推論失敗は課金対象外件数へ計上 | 判定・マスキング順序の詳細は [IPO-001](../04_ipo/IPO-001.md#IPO-001) |
| M-04a PiiMasker | AI 回答生成後の回答文からメール・電話・カード番号を検出し種別ラベル付き伏字へ置換し、検出有無・種別を AnswerService へ返す | 検出パターン不一致時はマスクせず素通し(誤検出・過検出の扱いはテスト設計) | 氏名・住所は対象外([FUT-07](../../04_future/FUT-07.md#FUT-07))。基準は [RULE-024](../../01_requirements/01_business_requirement/08_rule.md#RULE-024) |
...
| M-15〜M-17 非同期コンシューマ群 | 参照 FAQ 記録・未解決記録を Queues メッセージから消費して Repository を起動する。利用量コンシューマ(M-17)は質問数を加算せず、同期加算済み値を参照して閾値到達判定・契機引き渡し・サマリ反映のみを行う | 消費失敗は自動リトライ、上限超過分は DLQ へ退避 | 冪等キーで二重処理を抑止([BAT-001](../05_batch/BAT-001.md#BAT-001)) |
...
- 非同期境界の検証: AnswerService からの Queues 投入(参照 FAQ 記録・未解決記録・利用量集計の閾値判定/サマリ)と各コンシューマ消費の投入・消費・冪等・DLQ 滞留([BAT-001](../05_batch/BAT-001.md#BAT-001))。
- 計上の正本境界: 質問数計上は AnswerService(M-04)が同一 Tx で同期加算(質問ログ ID で冪等)する経路を正本とし、利用量コンシューマ(M-17)は加算せず判定・サマリのみを行うこと。無料枠・受付停止の分母は `question_count - unbillable_question_count`([RULE-013](../../01_requirements/01_business_requirement/08_rule.md#RULE-013))で、推論失敗は課金対象外件数へ計上されること。
- external-gateway スタブ化: AnswerProvider(M-05)をスタブ化した AnswerService 単体テストで `answered` / `unanswerable` / `error` の各分岐を分離検証すること。回答文 PII マスキング(M-04a)は `answered` 経路で検出/非検出の双方をケース化すること。
...
- 未解決同一 Tx の境界: 未回答 3 理由(`no_faq_match` / `low_confidence` / `contradiction`)時に質問ログ(M-06)と未解決(M-07)を同一トランザクションで生成し、処理エラー時は未解決を生成しないモジュール分担([FR-082](../../01_requirements/02_functional_requirement/02_faq-ai-fr.md#FR-082))。

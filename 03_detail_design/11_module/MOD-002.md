...
- 権限者による手動ロック解除の受付経路は [API-069](../../02_basic_design/02_backend/03_apis/API-069.md#API-069)(ログイン失敗ロック解除・[PERM-007](../../02_basic_design/04_permissions/PERM-007.md#PERM-007)・判定は [IPO-014](../04_ipo/IPO-014.md#IPO-014) が正本)で確定済みである。対応する Route Handler(`app/api/projects/[id]/members/[userId]/unlock/route.ts`)は AuthService(M-11)経由で操作権限判定([PERM-007](../../02_basic_design/04_permissions/PERM-007.md#PERM-007))を通過後に LockoutGuard(M-13)の解除を委譲し、UserRepository(M-15)の `login_failed_count` / `locked_until` を初期化・解除する([CLS-002](../10_class/CLS-002.md#CLS-002) と整合)。実装時はこの受付経路を含めて Route Handler → AuthService → LockoutGuard → UserRepository の同期経路をテスト設計へ引き継ぐこと。

...
flowchart TB
    ...
    FAQMOD -.->|"H_INQUIRY_FAQ 書き込み(経路未確定)"| M10
```
...
- FAQ 化履歴の書き込み経路の未確定: `InquiryFaqHistoryRepository`(M-10)は参照メソッドのみを本モジュールで定義し、未解決質問から FAQ への移行時に `H_INQUIRY_FAQ` へ書き込む処理主体・トランザクション境界(FAQ 作成モジュール側か本モジュール側か)が基本設計未確定である。関係者確認のうえ担当モジュールを確定する(課題候補)。

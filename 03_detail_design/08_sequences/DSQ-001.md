...
質問送信から受付ガード通過・候補 FAQ 全文検索・AI 推論・回答可否判定・回答文マスキング・質問ログ記録・非同期後処理投入までの内部コンポーネント連携を、回答時と未回答時のトランザクション境界とともに示す。未回答時は質問ログと未解決質問を同一トランザクションで生成する([API-038](../../02_basic_design/02_backend/03_apis/API-038.md#API-038) P-06)。質問数の計上は同期トランザクション内で加算する([API-038](../../02_basic_design/02_backend/03_apis/API-038.md#API-038) が正本)。非同期後処理(参照 FAQ 記録・未解決記録・利用量サマリ反映・閾値到達判定)はコミット後に Queue へ投入する。
...
sequenceDiagram
    ...
    Svc->>Repo: 候補 FAQ を全文検索(公開分・上位 N 件)と適用しきい値を取得
    Repo->>D1: 公開 FAQ を全文検索(FTS 一致スコア降順・上位 N)としきい値を照会
    D1-->>Repo: 候補 FAQ 上位 N 件・関連度スコア(FTS)・適用しきい値
    Repo-->>Svc: 候補 FAQ・関連度スコア・適用しきい値
    Svc->>AI: 回答生成をタイムアウト付きで呼出(候補 FAQ・信頼度を返す)
    AI-->>Svc: 回答結果(回答 / 回答不能 / エラー)
    Svc->>Svc: 回答可否を判定(信頼度 かつ 関連度・IPO-001)
    alt 回答可(answered)
        Svc->>Svc: 回答文の個人情報をマスキング(RULE-024・AnswerService)
        Note over Svc,D1: トランザクション開始
        Svc->>Repo: 質問ログを記録(結果種別 answered・信頼度・関連度・質問数を同期加算)
        Repo->>D1: 質問ログを登録・利用量を同期加算
        D1-->>Repo: 登録完了
        Note over Svc,D1: コミット
        Svc-->>RH: 回答(マスキング済)・参照 FAQ・関連質問サジェスト・質問ログ識別子
        RH->>Q: 参照 FAQ 記録・利用量サマリ反映を投入
        Q-->>RH: 受理
        RH-->>Client: 回答・参照 FAQ・サジェストを提示
    else 回答不可(unanswered / 未回答理由 3 値)
        Note over Svc,D1: トランザクション開始
        Svc->>Repo: 質問ログを記録(結果種別 unanswered・未回答理由・質問数を同期加算)
        Repo->>D1: 質問ログを登録・利用量を同期加算
        D1-->>Repo: 登録完了
...
| 3 | AnswerService | 候補 FAQ を全文検索し(公開分・一致スコア降順で上位 N 件)適用しきい値を取得する | プロジェクト・質問文 | 候補 FAQ 上位 N 件・関連度スコア(FTS)・適用しきい値 | 候補 FAQ は公開かつ当該プロジェクトのみ対象([TBL-006](../../02_basic_design/02_backend/04_database/TBL-006.md#TBL-006))を全文検索([TBL-030](../../02_basic_design/02_backend/04_database/TBL-030.md#TBL-030))。N・しきい値は正本([システム仕様書 §1](../../02_basic_design/07_system-spec.md#1-aiしきい値)・[FR-198](../../01_requirements/02_functional_requirement/02_faq-ai-fr.md#FR-198)) | 候補 0 件は未回答(`no_faq_match`)へ。しきい値取得不能時はグローバル既定で継続([IPO-001](../04_ipo/IPO-001.md#IPO-001) No.1) |
| 4 | AnswerService → AnswerProvider | AI 推論をタイムアウト付きで呼出し回答を生成する([API-057](../../02_basic_design/02_backend/03_apis/API-057.md#API-057) `generate`) | 質問文・候補 FAQ・適用しきい値 | 回答結果(回答 / 回答不能 / エラー・信頼度) | タイムアウト超過([システム仕様書 §3](../../02_basic_design/07_system-spec.md#3-タイムアウトセッション認証))・プロバイダエラーは §5 No.4 へ分岐。矛盾検出時は回答不能(`contradiction`・[EIF-001](../06_external_if/EIF-001.md#EIF-001)) | 呼出例外は処理エラーとして確定([IPO-001](../04_ipo/IPO-001.md#IPO-001) No.5) |
| 5 | AnswerService | 回答可否を判定する([IPO-001](../04_ipo/IPO-001.md#IPO-001)) | 回答結果(信頼度)・候補最上位の関連度(FTS)・適用しきい値 | 回答可否と未回答理由 | 信頼度・関連度がともにしきい値以上なら回答可、いずれか未満または回答不能なら未回答、エラーなら処理エラー | スコア欠損時は未回答(信頼度不足)として未解決へ回す |
| 6 | AnswerService | 回答可時、回答文の個人情報をマスキング([RULE-024](../../01_requirements/01_business_requirement/08_rule.md#RULE-024))し、質問ログを記録する(結果種別 `answered`・質問数を同期加算) | 回答本文・信頼度・関連度・参照 FAQ | 質問ログ識別子(マスキング済回答) | マスキングは応答整形前に実施(`AnswerProvider` の外・[EIF-001](../06_external_if/EIF-001.md#EIF-001))。トランザクション境界内で登録し即コミット | 書込失敗はロールバックし §5 No.5 の処理エラーへ |
| 7 | AnswerService | 回答不可時、質問ログと未解決質問を同一トランザクションで作成する(質問数を同期加算・[API-038](../../02_basic_design/02_backend/03_apis/API-038.md#API-038) P-06) | 質問文・未回答理由・プロジェクト | 質問ログ識別子・未解決識別子 | 対象理由は未回答理由 3 値 `no_faq_match` / `low_confidence` / `contradiction`。未解決は状態 `open` で作成([STS-001](../01_state_transitions/STS-001.md#STS-001)) | いずれかの書込失敗で同一トランザクションをロールバックし §5 No.5 へ(未解決を残さない) |
| 8 | Route Handler → Queue | コミット後に非同期後処理を投入する(質問数の計上は同期側 No.6・No.7 で確定済のため加算しない) | 質問ログ識別子・回答可否 | 投入受理 | 回答可時は参照 FAQ 記録([SYS-001](../../02_basic_design/02_backend/01_system/SYS-001.md#SYS-001))・利用量サマリ反映([SYS-016](../../02_basic_design/02_backend/01_system/SYS-016.md#SYS-016))、回答不可時は未解決記録([SYS-003](../../02_basic_design/02_backend/01_system/SYS-003.md#SYS-003))・サマリ反映。閾値到達判定・消費は [BAT-001](../05_batch/BAT-001.md#BAT-001) | 投入失敗時の再試行方針は [BAT-001](../05_batch/BAT-001.md#BAT-001) が担う(同期応答は返却済) |
| 9 | Route Handler | 応答を返却する([API-038](../../02_basic_design/02_backend/03_apis/API-038.md#API-038) P-07) | 回答可否・回答/未回答理由・連絡先メール | ウィジェット応答([IO-001](../03_io_specs/IO-001.md#IO-001)) | 回答可時は回答・参照 FAQ・サジェスト、回答不可時は未回答理由・連絡先メール(未設定時は連絡先を `null`) | — |
...
- 候補 FAQ 全文検索(公開かつ当該プロジェクト・一致スコア降順で上位 N 件・[TBL-030](../../02_basic_design/02_backend/04_database/TBL-030.md#TBL-030))と、関連度(候補最上位の FTS スコア)を用いた回答可否判定(信頼度 かつ 関連度)の分岐をテスト設計でケース化する([IPO-001](../04_ipo/IPO-001.md#IPO-001) No.0・No.3)。
- 回答可時の回答文個人情報マスキング([RULE-024](../../01_requirements/01_business_requirement/08_rule.md#RULE-024))が応答整形前・`AnswerProvider` の外(AnswerService)で実施されること、マスキング済回答が返却・記録されることを検証する。
- 質問数の計上を同期トランザクション内で加算し([API-038](../../02_basic_design/02_backend/03_apis/API-038.md#API-038) が正本)、非同期後処理はサマリ反映・閾値到達判定のみで二重加算しないことを検証する。
- コミット後の非同期後処理投入(参照 FAQ 記録・未解決記録・利用量サマリ反映・閾値到達判定)の投入失敗・再試行と消費順序は [BAT-001](../05_batch/BAT-001.md#BAT-001) と結線して確定する。

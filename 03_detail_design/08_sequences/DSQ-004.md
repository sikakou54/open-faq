...
| 対象 API | [API-028](../../02_basic_design/02_backend/03_apis/API-028.md#API-028)(`POST /faqs/import`) ・ [API-067](../../02_basic_design/02_backend/03_apis/API-067.md#API-067)(`GET /faqs/import/{jobId}`) |
...
sequenceDiagram
    ...
    participant PollRH as "Route Handler(API-067)"
...
| 5 | Route Handler(API-067) | 進捗ポーリング要求のたびにジョブの状態・進捗・失敗明細を照会する | ジョブ ID | ジョブ状態・進捗・失敗明細 | 指定ジョブが当該プロジェクトに属し存在することを検証する([API-067](../../02_basic_design/02_backend/03_apis/API-067.md#API-067) P-02/P-03) | 不一致は §5 No.3、不在は §5 No.4 へ分岐 |

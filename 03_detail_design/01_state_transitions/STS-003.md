...
| 詳細設計 | 手動停止の実行系 | `active → suspended` の「手動停止」契機を起こす運用管理者向け API・画面は Post-MVP([FUT-08](../../04_future/FUT-08.md#FUT-08))。MVP のサスペンション移行はシステム駆動([SYS-020](../../02_basic_design/02_backend/01_system/SYS-020.md#SYS-020)・決済失敗猶予経過)のみで、本 MVP 詳細設計の対象外 |

...
| `failed` | 再取込・再反映(定期再処理) | Cron Triggers が定期スケジュールで起動し、`status = 'failed'` を [`idx_billing_wh_status`](../../02_basic_design/02_backend/04_database/TBL-032.md#インデックス) で抽出した対象について再取込・再反映が成功する | `processed` | `status` を `processed` へ更新し `processed_at` を記録する([SYS-033](../../02_basic_design/02_backend/01_system/SYS-033.md#SYS-033) PR-02・PR-03・Repository 更新あり) | システム(Cron Triggers 起点バッチ。人手操作なし) | — | 再処理対象の抽出単位・実行間隔は詳細設計で確定する(§7 引き継ぎ) |
| `failed` | 再取込・再反映が失敗し上限未到達 | 再反映が失敗し、再処理回数が上限回数(実装確定値。§7 引き継ぎ)未到達 | `failed` | `status` を `failed` のまま維持し `error_detail` を最新の失敗内容へ更新する(Repository 更新あり) | システム(Cron Triggers 起点バッチ) | 次回スケジュールへ持ち越す(即時エスカレーションしない) | 再処理回数のカウント方法(受信ログへの回数保持要否)は詳細設計で確定する |
| `failed` | 再取込・再反映が失敗し上限到達 | 再反映が失敗し、再処理回数が上限回数(実装確定値。§7 引き継ぎ)に到達する | `failed` | `status` を `failed` のまま維持する(状態遷移なし。Repository 更新なし) | システム(Cron Triggers 起点バッチ) | 運用者へエスカレーション通知する([SYS-033](../../02_basic_design/02_backend/01_system/SYS-033.md#SYS-033) PR-04) | `failed` は状態としては終端に遷移せず、通知のみで運用対応へ引き継ぐ |
...
| 詳細設計 | 再処理上限値・カウント方式 | `failed` の再処理上限回数(基本設計・システム仕様書に具体値なし)と、再処理回数の保持要否・カウント方式(受信ログへの回数カラム追加要否等)を実装で確定する |
| 詳細設計 | バッチ実行間隔・抽出単位 | [SYS-033](../../02_basic_design/02_backend/01_system/SYS-033.md#SYS-033) の定期スケジュール周期と `failed` 対象の抽出単位(1 回あたりの処理件数等)を実装で確定する |

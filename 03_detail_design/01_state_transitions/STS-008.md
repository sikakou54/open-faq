...
|----|----|----|----|----|----|
| S1 | `draft` | [状態モデル §7.2](../../02_basic_design/08_state-model.md#72-請求書状態) | — | — | 状態モデル §7.2 の状態一覧に定義されるが、遷移図・SYS-019・API-060 のいずれにも遷移契機の記載がない(課題候補) |
...
| S5 | `refunded` | [状態モデル §7.2](../../02_basic_design/08_state-model.md#72-請求書状態) | — | — | 状態モデル §7.2 の状態一覧に定義されるが、遷移図・SYS-019・API-060 のいずれにも遷移契機の記載がない(課題候補) |
...
> [!NOTE]
> **月次請求確定は `issued` を既定状態として作成し、`draft` で作成する経路は存在しない。** `draft` / `refunded` への遷移契機は SYS-019・billing-design・API-060 のいずれにも記載がなく、本書では遷移一覧に含めない(課題候補として §7 に引き継ぐ)。反映処理の実体は [SYS-004](../../02_basic_design/02_backend/01_system/SYS-004.md#SYS-004)(課金プロバイダ通知の受信・検証・取込)が担い、[SYS-019](../../02_basic_design/02_backend/01_system/SYS-019.md#SYS-019) は `issued` の発行契機のみを担う。
...
| 詳細設計 | 課題候補 | `draft` / `refunded` への遷移契機が状態モデル・SYS-019・API-060 のいずれにも定義されていない点の解消方針を委ねる(本書 §3・§5 参照) |

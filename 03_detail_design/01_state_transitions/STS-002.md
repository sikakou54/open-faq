...
| S3 | `suspended` | [状態モデル §1](../../02_basic_design/08_state-model.md#1-アカウント状態) | — | — | [SYS-030](../../02_basic_design/02_backend/01_system/SYS-030.md#SYS-030)が消費する状態として保持。遷移契機となる管理停止操作は Post-MVP([FUT-08](../../04_future/FUT-08.md#FUT-08)) |
...
> [!NOTE]
> **`suspended` は状態モデル・物理定義(CHECK 制約)上は存在し、[SYS-030](../../02_basic_design/02_backend/01_system/SYS-030.md#SYS-030)(停止時セッション一斉無効化)が消費する状態として保持する。** ただし `suspended` への遷移を発生させる操作(管理停止・停止解除の実行)は Post-MVP スコープであり、運用管理者ロール・バックオフィス機能とあわせて [FUT-08](../../04_future/FUT-08.md#FUT-08) で具体化する。課金アカウント(`M_BILLING_ACCOUNT.status='suspended'`。[状態モデル §2](../../02_basic_design/08_state-model.md#2-課金アカウント状態))のサスペンションとは別軸であり、本書では両者を混同せず `M_USER.status` のみを対象とする。
...
> [!NOTE]
> **`active → suspended`(管理停止)・`suspended → active`(停止解除)・`suspended → deleted` の 3 遷移は、状態モデル・CHECK 制約上は定義されているが、遷移を発生させる実行操作(運用管理者操作)は Post-MVP スコープであり本リポジトリの基本設計に未実装である。** 対応は [FUT-08](../../04_future/FUT-08.md#FUT-08) に委ねる(§7 参照)。
...
| `suspended` | — | — | — | 遷移契機となる管理停止操作が Post-MVP([FUT-08](../../04_future/FUT-08.md#FUT-08))のため、状態別の許可操作は本書のスコープ外(§7 参照) |
...
| 詳細設計 | `suspended` 遷移契機の実装(Post-MVP) | `active ⇄ suspended` および `suspended → deleted` を発動する運用管理者操作・API・実行可能ロール・ガード条件・エラー時挙動は Post-MVP スコープとして確定済み([FUT-08](../../04_future/FUT-08.md#FUT-08))。MVP では [SYS-030](../../02_basic_design/02_backend/01_system/SYS-030.md#SYS-030)が `suspended` を消費する状態として保持するのみとし、本書は遷移契機を実装しない |

...
| 関連テーブル | [TBL-031](../../02_basic_design/02_backend/04_database/TBL-031.md#TBL-031) |
...
| `candidate_faqs` | array\<object\> | ◯ | 回答の根拠として渡す候補 FAQ 配列。要素は `id` / `question` / `answer` | 候補 FAQ の抽出方式は本連携外(§7) |
...
> [!NOTE]
> **回答可否のしきい値(信頼度・関連度)は本連携の入力として渡す。** 対象プロジェクトに設定値があれば [`TP_AI_THRESH_CACHE`](../../02_basic_design/02_backend/04_database/TBL-031.md#TBL-031) の値を、未登録・取得不能時はグローバル既定値を渡す。値の正本は [システム仕様書 §1](../../02_basic_design/07_system-spec.md#1-aiしきい値)、取得・フォールバックの担い手は [SYS-015](../../02_basic_design/02_backend/01_system/SYS-015.md#SYS-015)。
...
- **候補 FAQ 抽出方式(要確認)**: 送信 `candidate_faqs` の選定・並び順は本連携の入力前段であり別工程。抽出・スコアリング方式は基本設計に未定義のため確定できない。
- **回答可否スコア算出方式(要確認)**: `confidence` の算出根拠、候補 FAQ 間の矛盾(`contradiction`)検出方式は基本設計([RULE-023](../../01_requirements/01_business_requirement/08_rule.md#RULE-023))に業務仕様が定義されていないため確定できない。
- **個人情報検出・マスキング方式(要確認)**: 質問文の個人情報検出および回答文のマスキング([RULE-024](../../01_requirements/01_business_requirement/08_rule.md#RULE-024))の具体ルールは基本設計に未定義のため確定できない。

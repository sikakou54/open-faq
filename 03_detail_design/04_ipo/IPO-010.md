...
- **課題候補**: [SYS-027](../../02_basic_design/02_backend/01_system/SYS-027.md#SYS-027)・[SYS-032](../../02_basic_design/02_backend/01_system/SYS-032.md#SYS-032)・[SYS-034](../../02_basic_design/02_backend/01_system/SYS-034.md#SYS-034) に対応するバッチ処理設計書(BAT)が未作成であり、実行契機・スケジュール・冪等性・部分失敗時の再評価・削除順序(FK 依存順)の実装機構を委ねる先が本書執筆時点で存在しない。BAT 起票要否を確認したい。
- **課題候補**: [システム仕様書 §4](../../02_basic_design/07_system-spec.md#4-データ保持期間削除猶予)の表に、質問ログ・未解決質問・通知ログ・お知らせ受信箱の保持期間(1 年。[NFR-045](../../01_requirements/03_non_functional_requirement/07_nfr.md#NFR-045)・[NFR-049](../../01_requirements/03_non_functional_requirement/07_nfr.md#NFR-049)が根拠)が未掲載であり、正本への値集約が完了していない。設計値の重複定義を避けるため本書では両 NFR を根拠として引用したが、システム仕様書 §4 側への追記要否を確認したい。

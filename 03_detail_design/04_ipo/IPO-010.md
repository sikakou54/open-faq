...
- [TBL-002](../../02_basic_design/02_backend/04_database/TBL-002.md#TBL-002)(課金アカウント)自身は `deleted_at` 列を持たず、[SYS-034](../../02_basic_design/02_backend/01_system/SYS-034.md#SYS-034) 側での削除対象確定は `withdrawn_at` 起点の保持期間経過のみで判定し、`valid`/`deleted_at` 判定(No.1〜No.3a)を経由しない(§4 No.5 で確定)。

...
- 対象プロジェクトの有効性: 処理開始時および**反映フェーズ開始前**に対象プロジェクトが有効(`active`・未削除)であることを確認する。取込処理中に対象プロジェクトが論理削除([SYS-010](../../02_basic_design/02_backend/01_system/SYS-010.md#SYS-010))された場合は 1 行も反映せずジョブを中断し `failed` とする(論理削除済みプロジェクトへ FAQ を流入させない)。
...
| 4 | 既存上書き反映 | `UPDATE`([TBL-006](../../02_basic_design/02_backend/04_database/TBL-006.md#TBL-006))。質問・回答・カテゴリを取込時点の CSV 内容で上書き(後勝ち / Last-Write-Wins)し、`status` は既存値を維持する(取込 CSV の値では変更しない)。取込処理中に同一 FAQ が画面([API-026](../../02_basic_design/02_backend/03_apis/API-026.md#API-026))から編集されても取込を優先する(MVP 方針・楽観ロックは適用しない) | 既存上書き判定行(質問 / 回答 / カテゴリ)、対象 FAQ の現行 `status` | 更新済み FAQ | 既存上書き判定時 | 反映失敗(制約違反等)は当該行のみ行失敗へ振り替え、他行の処理は継続する |

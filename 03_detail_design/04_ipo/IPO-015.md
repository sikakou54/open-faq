...
| 6 | 集計確定 | `succeeded_count = 新規登録数 + 既存上書き数`、`failed_count = errors.length`、`processed_count = succeeded_count + failed_count` | 全行の判定・反映・記録結果 | `succeeded_count` / `failed_count` / `processed_count` / `error_summary` | 全行の処理完了後 | 集計不能(異常終了)時の扱いは [SYS-014](../../02_basic_design/02_backend/01_system/SYS-014.md#SYS-014) に委ねる(実行機構は [BAT-003](../05_batch/BAT-003.md#BAT-003) を参照) |

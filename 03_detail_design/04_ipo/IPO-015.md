...
| 6 | 全行の新規 / 上書き判定結果、対象プロジェクトの現有効 FAQ 件数 | 現有効件数 + 新規登録判定件数が[RULE-010](../../01_requirements/01_business_requirement/08_rule.md#RULE-010)の強制拒否しきい値を超えるかをジョブ単位で判定する | 判定結果(範囲内 / 超過) | 超過時は取込ジョブを失敗とし反映した全行をロールバックする(部分取込しない) | 失敗理由コード `CSV_FAQ_LIMIT_EXCEEDED`([ERR-037](../../02_basic_design/05_errors/ERR-037.md#ERR-037)) |
...
| 6 | 件数上限ガード(ジョブ単位) | `if 現有効FAQ件数 + 新規登録判定件数 > RULE-010強制拒否しきい値 → ジョブ失敗(CSV_FAQ_LIMIT_EXCEEDED)かつ当該ジョブで反映した全行をロールバック else → 通過`。取込は単一トランザクションで行い、反映後・コミット前に本ガードを評価する。しきい値の正本は[RULE-010](../../01_requirements/01_business_requirement/08_rule.md#RULE-010)(値は再定義しない) | 対象プロジェクトの現有効 FAQ 件数、新規登録判定件数 | 判定結果(範囲内 / 超過) | 反映後・集計確定前 | 超過時はトランザクションをロールバックしジョブを `failed` とする(理由コード `CSV_FAQ_LIMIT_EXCEEDED`・[ERR-037](../../02_basic_design/05_errors/ERR-037.md#ERR-037)・ジョブ単位)。部分取込しない |
...
ジョブ単位の失敗理由 `CSV_FAQ_LIMIT_EXCEEDED`([ERR-037](../../02_basic_design/05_errors/ERR-037.md#ERR-037))は No.6 件数上限ガードで発生し、行明細(`errors[]`)ではなく取込ジョブ自体の失敗理由として記録する(ジョブ状態は `failed`・反映済み全行はロールバック)。
...
flowchart TB
  ...
  F2 --> R
  R -- 残っている --> S
  R -- 全行処理済み --> G{"件数上限ガード 現有効件数+新規登録数 が RULE-010強制拒否しきい値 以内か"}
  G -- 超過 --> RB[全行ロールバック ジョブ失敗 CSV_FAQ_LIMIT_EXCEEDED]
  G -- 範囲内 --> C[集計確定 succeeded/failed/processed]
  RB --> E([取込ジョブへ結果確定])
  C --> E
```

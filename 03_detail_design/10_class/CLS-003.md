...
> **本クラス図は「プロジェクトの一覧・作成・更新・削除・ウィジェット公開鍵ローテーション・データ概要取得を実装する Route Handler・Service・Repository・DTO/Entity の構成と責務」を定義します。**
...
本クラス図は、プロジェクト一覧([API-016](../../02_basic_design/02_backend/03_apis/API-016.md#API-016))・新規作成([API-017](../../02_basic_design/02_backend/03_apis/API-017.md#API-017))・更新/削除([API-018](../../02_basic_design/02_backend/03_apis/API-018.md#API-018))・ウィジェット鍵ローテーション([API-019](../../02_basic_design/02_backend/03_apis/API-019.md#API-019))・データ概要取得([API-065](../../02_basic_design/02_backend/03_apis/API-065.md#API-065))を Next.js(App Router)+ Repository 層のレイヤーへ配置し、実装者がクラス構成・責務・シグネチャ・データ構造の境界を迷わず組み立てられる粒度を確定する。依存方向は内向き(Route Handler → Service → Repository → D1)に固定し、逆流させない。
...
| ProjectService | Service | 一覧絞り込み(`scope`)・作成(オーナー設定・許可ドメイン保存・課金アカウント遅延作成・作成者メンバー自動登録)・更新(コア項目/`settings` の権限分岐)・論理削除(関連データへの論理削除伝播・メンバー割当解除)・概要の集計統括を担う | `list` / `create` / `update` / `delete` / `getOverview` | 削除時のメンバー整理・関連データ論理削除の順序は [API-018](../../02_basic_design/02_backend/03_apis/API-018.md#API-018) 処理概要 P-01〜P-05 に従う。しきい値保存分岐(任意指定)は [システム仕様書 §1](../../02_basic_design/07_system-spec.md#1-aiしきい値) |
...
| ProjectOwnershipGuard | ガード | 対象プロジェクトの `owner_user_id` が要求元と一致するかを判定する(コア項目更新・削除・鍵ローテーションの権限境界) | `check` | 不一致時は境界秘匿([ERR-017](../../02_basic_design/05_errors/ERR-017.md#ERR-017))またはオーナー限定拒否([ERR-015](../../02_basic_design/05_errors/ERR-015.md#ERR-015))。判定の使い分けは API ごとの許容範囲に従う |
...
- `ProjectOwnershipGuard` と `ProjectMembershipGuard` の使い分け(コア項目更新/削除/鍵ローテーションはオーナー限定、`settings` のみの更新とデータ概要取得は有効メンバー許可)をテスト設計でケース化する。

...
| PaymentGateway | `attachPaymentMethod` | 支払方法トークンを課金プロバイダの顧客へ紐付ける | AttachPaymentMethodRequest(顧客参照・支払方法トークン) | PaymentMethodResult | カード拒否 | 生カード情報は自システムで保持しない。顧客参照の実体は [TBL-002](../../02_basic_design/02_backend/04_database/TBL-002.md#TBL-002) `stripe_customer_id` |
| PaymentGateway | `getPaymentMethod` | 課金プロバイダ側の登録済み支払方法を照会する | 顧客参照 | PaymentMethodResult | — | GET 経路で利用。顧客参照の実体は [TBL-002](../../02_basic_design/02_backend/04_database/TBL-002.md#TBL-002) `stripe_customer_id` |
...
| AttachPaymentMethodRequest | DTO(Gateway 境界入力) | 課金プロバイダ顧客参照([TBL-002](../../02_basic_design/02_backend/04_database/TBL-002.md#TBL-002) `stripe_customer_id`)・支払方法トークン | `PaymentGateway.attachPaymentMethod` への入力 |
...
| BillingAccountEntity | Entity | 課金アカウント ID・ユーザー ID・課金状態・決済プロバイダ顧客 ID(`stripe_customer_id`)・支払方法登録状態情報 | 永続ドメインモデル([TBL-002](../../02_basic_design/02_backend/04_database/TBL-002.md#TBL-002) 由来) |

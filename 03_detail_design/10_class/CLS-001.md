...
| AnswerService | Service | しきい値解決・AI 呼び出し・回答可否判定・質問ログ/未解決質問の永続化を統括する。未回答時は質問ログと未解決質問を同一トランザクションで生成する | `ask` / `registerInquiry` / `recordFeedback` | 判定条件・スコア比較の詳細は [IPO-001](../04_ipo/IPO-001.md#IPO-001) |
...
| InquiryRepository | Repository | 未解決質問の生成・冪等キー照会(D1) | `create` / `findByIdempotencyKey` | [TBL-017](../../02_basic_design/02_backend/04_database/TBL-017.md#TBL-017)。`inquiry_code` 採番は [DBP-001](../07_db_physical/DBP-001.md#DBP-001) |
| FaqRepository | Repository | 候補 FAQ の全文検索・照会(D1 / FTS)| `searchCandidates` | 公開状態の FAQ を対象([TBL-006](../../02_basic_design/02_backend/04_database/TBL-006.md#TBL-006)) |
...
| AnswerService | `ask` | しきい値解決・AI 呼出・可否判定・永続化を統括する | AskInput(論理項目) | AnswerResult | AI 不通([ERR-036](../../02_basic_design/05_errors/ERR-036.md#ERR-036))・書込失敗([ERR-014](../../02_basic_design/05_errors/ERR-014.md#ERR-014) 系の一意制約含む) | 未回答時は質問ログ+未解決を同一 Tx で生成。判定順序は [IPO-001](../04_ipo/IPO-001.md#IPO-001) |
...
| FaqRepository | `searchCandidates` | 候補 FAQ を全文検索で抽出する | プロジェクト ID・質問文 | FaqEntity 配列 | — | 公開状態の FAQ を対象([TBL-006](../../02_basic_design/02_backend/04_database/TBL-006.md#TBL-006)) |
...
| QuestionLogEntity | Entity | 質問ログ ID・プロジェクト ID・質問本文・AI 応答・解決フラグ・課金確定フラグ・信頼度/関連度スコア・結果種別・結果理由コード・PII マスク済フラグ・有効フラグ | 永続ドメインモデル([TBL-025](../../02_basic_design/02_backend/04_database/TBL-025.md#TBL-025) 由来) |
...
| FaqEntity | Entity | FAQ ID・プロジェクト ID・質問・回答・状態 | 候補 FAQ の永続ドメインモデル([TBL-006](../../02_basic_design/02_backend/04_database/TBL-006.md#TBL-006) 由来) |
...
- 回答可否判定(しきい値比較・未解決理由の写像・PII/矛盾判定の順序)は [IPO-001](../04_ipo/IPO-001.md#IPO-001) で確定する。

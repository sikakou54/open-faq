...
| 関連テーブル | [TBL-006](../../02_basic_design/02_backend/04_database/TBL-006.md#TBL-006) ・ [TBL-030](../../02_basic_design/02_backend/04_database/TBL-030.md#TBL-030) ・ [TBL-029](../../02_basic_design/02_backend/04_database/TBL-029.md#TBL-029) |
| 関連 SYS | — |
...
| 対象外 | AI 回答経路での候補 FAQ 全文検索・参照 FAQ 記録([CLS-001](CLS-001.md#CLS-001) `FaqRepository` が担う。本図の `FaqRepository` は管理画面 CRUD・検索を担当する別クラスとして分離する)・CSV インポート/エクスポート(別 CLS)・状態遷移の判定条件詳細([STS-005](../01_state_transitions/STS-005.md#STS-005))・検索スコア算出やページング構築の内部アルゴリズム([IPO](../04_ipo/index.md) が担う)・未解決質問一覧/詳細・FAQ 化履歴の照会([CLS-007](CLS-007.md#CLS-007) `InquiryFaqHistoryRepository` が参照専用で担う。本図は起点未解決質問指定時の `H_INQUIRY_FAQ` への書き込みのみを担う) |
...
classDiagram
    ...
    class InquiryFaqHistoryRepository {
        +create(InquiryFaqHistoryEntity) InquiryFaqHistoryEntity
    }
...
| FaqService | Service | FAQ の一覧・個別取得・作成・更新・削除・一括状態変更の業務処理を統括する。楽観ロック(`version`)判定・状態遷移の可否判定・`draft`/`published` での必須項目切替を担う。新規作成時に起点未解決質問(`sourceInquiryId`)が指定された場合は FAQ 作成に合わせて `H_INQUIRY_FAQ` へ移行履歴を記録する | `list` / `getById` / `create` / `update` / `delete` / `bulkChangeStatus` | 状態遷移の許可条件は [STS-005](../01_state_transitions/STS-005.md#STS-005)。判定詳細は [IPO](../04_ipo/index.md) へ委譲。移行履歴記録は [API-026](../../02_basic_design/02_backend/03_apis/API-026.md#API-026) P-06 |
...
| InquiryFaqHistoryRepository | Repository | 未解決質問から FAQ への移行履歴を作成する(D1)。書き込み専用であり、未解決質問側からの参照は [CLS-007](CLS-007.md#CLS-007) `InquiryFaqHistoryRepository`(参照専用)が別途担う | `create` | [TBL-029](../../02_basic_design/02_backend/04_database/TBL-029.md#TBL-029) |
...
| FaqService | `create` | FAQ を新規作成する。`published` 遷移時は質問・回答必須を判定する。起点未解決質問(`sourceInquiryId`)指定時は `InquiryFaqHistoryRepository.create` で移行履歴を記録する | FaqCreateInput(論理項目・起点未解決質問 ID を任意で含む) | FaqDetailResult | 必須項目未充足([ERR-001](../../02_basic_design/05_errors/ERR-001.md#ERR-001)) | `published` 化時は `publishedAt` を記録。移行履歴記録は [API-026](../../02_basic_design/02_backend/03_apis/API-026.md#API-026) P-06 |
...
| InquiryFaqHistoryRepository | `create` | 未解決質問から FAQ への移行履歴を永続化する | InquiryFaqHistoryEntity | InquiryFaqHistoryEntity | — | [TBL-029](../../02_basic_design/02_backend/04_database/TBL-029.md#TBL-029)。ID 採番は [DBP-007](../07_db_physical/DBP-007.md#DBP-007) と同方式 |
...
| FaqUpsertRequestDto | DTO | 質問・回答・カテゴリ・状態・バージョン・起点未解決質問 ID(任意・作成時のみ有効) | FAQ 作成・更新 API 境界の入力(FaqRouteHandler で受領) |
...
| FaqListInput / FaqCreateInput / FaqUpdateInput / FaqSearchInput / FaqBulkStatusInput | DTO(Service 内部入力) | Route Handler から Service への論理項目(DTO を業務入力へ変換した形)。FaqCreateInput は起点未解決質問 ID(任意)を含む | Service メソッドの入力(Route Handler で DTO から変換) |
...
| FaqEntity | Entity | FAQ ID・プロジェクト ID・質問(タイトル)・回答(本文)・カテゴリ・状態・バージョン・タグ・有効フラグ・起点未解決質問 ID・作成者/更新者・削除日時・作成日時・更新日時 | 永続ドメインモデル([TBL-006](../../02_basic_design/02_backend/04_database/TBL-006.md#TBL-006) 由来) |
...
| InquiryFaqHistoryEntity | Entity | ID・プロジェクト ID・元未解決質問 ID・作成 FAQ ID・質問スナップショット・実行者種別・実行者 ID・移行日時 | 永続ドメインモデル([TBL-029](../../02_basic_design/02_backend/04_database/TBL-029.md#TBL-029) 由来。[CLS-007](CLS-007.md#CLS-007) の InquiryFaqHistoryEntity と同一クラスを共有) |
...
- `FaqRepository.create` と `InquiryFaqHistoryRepository.create`(起点未解決質問指定時のみ実行)を単一トランザクションで扱うか否かの実装方針は詳細ロジック設計(IPO)・[MOD-007](../11_module/MOD-007.md#MOD-007) で確定する。

...
- 規約・退会・監査ドメインのモジュール分割は [MOD-011](../11_module/MOD-011.md#MOD-011)(platform=監査ログ等の横断コンポーネント)ほかを参照する。`AuditLogService` / `AuditLogRepository` は platform の単一共有実装([MOD-011](../11_module/MOD-011.md#MOD-011) M-01/M-02)であり、本図・[CLS-004](CLS-004.md#CLS-004) の記載は同一共有コンポーネントへの参照である(ドメインごとに再実装しない)。

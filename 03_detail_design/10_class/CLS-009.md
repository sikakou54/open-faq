...
- 利用量・上限ドメイン(ダッシュボード/利用量サマリ/上限設定)のモジュール配置(`app/api/dashboard/**`・`app/api/usage/**`・`app/api/owner/projects/usage/**`・`app/api/projects/[id]/quota-limits/**`)は [MOD-009](../11_module/MOD-009.md#MOD-009)(usage-billing モジュール構造)が扱う(既存 [MOD-001](../11_module/MOD-001.md#MOD-001) はウィジェット/AI 回答ドメインのため対象外)。

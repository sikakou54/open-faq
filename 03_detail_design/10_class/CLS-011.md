...
- 通知送信抑制(プロジェクト単位の送信品質監視・送信時サプレス照合)の判定枠組み・許容水準は [IPO-016](../04_ipo/IPO-016.md#IPO-016)([システム仕様書 §7](../../02_basic_design/07_system-spec.md#7-バッチ運用設計値)を正本とするバウンス率・苦情率・評価対象下限件数・評価窓)で確定済み。`NotificationDispatchService.getDeliveryStatusSummary` の抑制対象加味ロジックはこれに従う。

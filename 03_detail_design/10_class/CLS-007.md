...
- `InquiryRepository`・`QuestionLogRepository` は [CLS-001](CLS-001.md#CLS-001) と本書とで同一クラスを共有するため、両 CLS のメソッド一覧を合算した完全なインターフェースをモジュール構造(MOD)で確定する。本ドメインの管理機能側モジュール構造図(MOD)は本書執筆時点で未作成であり、起票要否を確認する(課題候補)。

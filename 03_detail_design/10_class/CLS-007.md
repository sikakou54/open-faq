...
| 対象外 | ウィジェット側の未解決質問登録(`InquiryRouteHandler` の `post`・`InquiryRepository` の `create` / `findByIdempotencyKey` は [CLS-001](CLS-001.md#CLS-001) が定義。本書はその `InquiryRepository` に一覧・詳細・状況更新のメソッドを追加する形で共有する)・元質問ログの記録/フィードバック([CLS-001](CLS-001.md#CLS-001) が担う)・未解決質問から FAQ への移行操作そのもの(`M_FAQS` への作成と `H_INQUIRY_FAQ` への移行履歴書き込みは [API-026](../../02_basic_design/02_backend/03_apis/API-026.md#API-026) が担い [CLS-005](CLS-005.md#CLS-005) の対象。本書は `H_INQUIRY_FAQ` 履歴テーブルの参照専用 Repository 配置のみを示す)・状態遷移の契機/ガード条件の詳細([STS-001](../01_state_transitions/STS-001.md#STS-001))・論理項目 ↔ 物理カラム対応([DBP-008](../07_db_physical/DBP-008.md#DBP-008)) |
...
| InquiryFaqHistoryRepository | Repository | 未解決質問から FAQ への移行履歴を未解決質問 ID で照会する(D1)。移行履歴の書き込みは [CLS-005](CLS-005.md#CLS-005) `FaqRepository`(FAQ 作成経路)が担い、本クラスは参照専用 | `findByInquiry` | [TBL-029](../../02_basic_design/02_backend/04_database/TBL-029.md#TBL-029) |
...
| InquiryFaqHistoryRepository | `findByInquiry` | 未解決質問 ID で FAQ 化履歴を照会する | 未解決質問 ID | InquiryFaqHistoryEntity 配列 | — | 書き込みは [API-026](../../02_basic_design/02_backend/03_apis/API-026.md#API-026)(FAQ 作成)起点。本クラスは参照専用 |
...
- `InquiryFaqHistoryRepository`(`H_INQUIRY_FAQ`)は本書では参照専用(`findByInquiry`)の Repository 配置のみを示す。書き込みは [API-026](../../02_basic_design/02_backend/03_apis/API-026.md#API-026)(FAQ 作成)起点で [CLS-005](CLS-005.md#CLS-005) `FaqRepository` が担う。両クラスの書き込み/参照分担とトランザクション境界の実装詳細は [MOD-007](../11_module/MOD-007.md#MOD-007)・詳細ロジック設計(IPO)で確定する。

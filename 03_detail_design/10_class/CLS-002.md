...
| 関連 API | [API-001](../../02_basic_design/02_backend/03_apis/API-001.md#API-001) ・ [API-002](../../02_basic_design/02_backend/03_apis/API-002.md#API-002) ・ [API-003](../../02_basic_design/02_backend/03_apis/API-003.md#API-003) ・ [API-005](../../02_basic_design/02_backend/03_apis/API-005.md#API-005) ・ [API-006](../../02_basic_design/02_backend/03_apis/API-006.md#API-006) ・ [API-010](../../02_basic_design/02_backend/03_apis/API-010.md#API-010) ・ [API-013](../../02_basic_design/02_backend/03_apis/API-013.md#API-013) ・ [API-015](../../02_basic_design/02_backend/03_apis/API-015.md#API-015) ・ [API-069](../../02_basic_design/02_backend/03_apis/API-069.md#API-069) |
...
| 区分 | 対象 |
|----|----|
| 対象機能 | 新規登録・確認メール再送([API-001](../../02_basic_design/02_backend/03_apis/API-001.md#API-001))・ログイン([API-002](../../02_basic_design/02_backend/03_apis/API-002.md#API-002))・ログアウト([API-003](../../02_basic_design/02_backend/03_apis/API-003.md#API-003))・再認証([API-005](../../02_basic_design/02_backend/03_apis/API-005.md#API-005))・メール確認([API-006](../../02_basic_design/02_backend/03_apis/API-006.md#API-006))・パスワード再設定確定([API-010](../../02_basic_design/02_backend/03_apis/API-010.md#API-010))・自己パスワード変更([API-013](../../02_basic_design/02_backend/03_apis/API-013.md#API-013))・セキュリティ設定更新(メール / パスワード。[API-015](../../02_basic_design/02_backend/03_apis/API-015.md#API-015))・権限者によるログイン失敗ロック解除([API-069](../../02_basic_design/02_backend/03_apis/API-069.md#API-069))・セッション失効判定・ログイン失敗ロックアウト判定 |
...
classDiagram
    ...
    class MemberUnlockRouteHandler {
        +post(MemberUnlockRequestDto) MemberUnlockResponseDto
    }
    class AuthService {
        ...
        +unlockLoginLockout(MemberUnlockInput) MemberUnlockResult
    }
    class ProjectMembershipGuard {
        +check(SessionContext, projectId, targetUserId) GuardVerdict
    }
    ...
    MemberUnlockRouteHandler --> AuthService
    AuthService --> SessionService
    AuthService --> LockoutGuard
    AuthService --> ProjectMembershipGuard
...
| MemberUnlockRouteHandler | Route Handler(Controller 相当) | 権限者によるログイン失敗ロック解除要求を受理し、解除を Service へ委譲する | `post` | `app/api/projects/[id]/members/[userId]/unlock/route.ts` 相当(`POST /projects/{id}/members/{userId}/unlock`。[API-069](../../02_basic_design/02_backend/03_apis/API-069.md#API-069)) |
| AuthService | Service | 新規登録・ログイン・ログアウト・再認証・メール確認・パスワード再設定・自己パスワード変更・セキュリティ設定更新・権限者によるログイン失敗ロック解除の業務判定を統括する。判定の中で `SessionService`・`LockoutGuard`・`ProjectMembershipGuard`・`TokenService` を呼び出す | `signup` / `resendVerification` / `login` / `logout` / `reAuthenticate` / `verifyEmail` / `resetPassword` / `changeOwnPassword` / `updateSecuritySettings` / `unlockLoginLockout` | 各判定の詳細は [IPO-013](../04_ipo/IPO-013.md#IPO-013) ・ [IPO-014](../04_ipo/IPO-014.md#IPO-014) へ委譲 |
...
| ProjectMembershipGuard | ガード | ログイン失敗ロック解除の操作権限(オーナー / 当該プロジェクトの有効メンバー)と、対象利用者が当該プロジェクトの割当を持つことを判定する | `check` | 判定条件は [PERM-007](../../02_basic_design/04_permissions/PERM-007.md#PERM-007)。割当照会リポジトリは本 CLS 対象外(メンバー管理系 CLS が担う) |
...
| MemberUnlockRouteHandler | `post` | 権限者によるログイン失敗ロック解除要求を受理し解除結果を返す | MemberUnlockRequestDto(対象プロジェクト・対象利用者) | MemberUnlockResponseDto | 権限なし([ERR-004](../../02_basic_design/05_errors/ERR-004.md#ERR-004)) | HTTP 境界。ロック中でない対象も同一完了応答を返す(冪等。[API-069](../../02_basic_design/02_backend/03_apis/API-069.md#API-069)) |
...
| AuthService | `unlockLoginLockout` | 権限者の操作で対象利用者のログイン失敗ロックを解除する | MemberUnlockInput(論理項目。対象プロジェクト・対象利用者) | MemberUnlockResult(解除の要否・実施有無) | 権限なし | `ProjectMembershipGuard.check` を先行判定し `LockoutGuard.reset` に委譲。ロック中でなくても冪等に完了。手動解除の判定条件は [IPO-014](../04_ipo/IPO-014.md#IPO-014) No.6〜No.8 |
...
| ProjectMembershipGuard | `check` | ログイン失敗ロック解除の操作権限と対象利用者の割当有無を判定する | SessionContext・対象プロジェクト・対象利用者 | GuardVerdict(許可 / 拒否) | — | 判定条件は [PERM-007](../../02_basic_design/04_permissions/PERM-007.md#PERM-007) |
...
| MemberUnlockRequestDto | DTO | 対象プロジェクト・対象利用者 | ログイン失敗ロック解除 API 境界の入力([API-069](../../02_basic_design/02_backend/03_apis/API-069.md#API-069)) |
| MemberUnlockResponseDto | DTO | 解除の要否・実施有無 | ログイン失敗ロック解除 API 境界の出力 |
...
- ログイン失敗ロックアウトの判定順序(ロック中判定 → 資格情報照合 → 失敗計上 → しきい値到達判定)・解除契機(自動 / 手動)の詳細条件は [IPO-014](../04_ipo/IPO-014.md#IPO-014) で確定済み。`LockoutGuard` の実装はこれに従う。権限者による手動解除の受付経路は [API-069](../../02_basic_design/02_backend/03_apis/API-069.md#API-069)(`POST /projects/{id}/members/{userId}/unlock`)で確定済みであり、`MemberUnlockRouteHandler` → `AuthService.unlockLoginLockout`(`ProjectMembershipGuard` 認可・[PERM-007](../../02_basic_design/04_permissions/PERM-007.md#PERM-007))→ `LockoutGuard.reset` として本図に含める。手動解除の判定は [IPO-014](../04_ipo/IPO-014.md#IPO-014)、操作権限は [PERM-007](../../02_basic_design/04_permissions/PERM-007.md#PERM-007) を正本とする。

...
| マルチプロジェクト | 1テナント内で複数サイト/製品を並行管理 |
...
| 用語 | 定義 |
|---|---|
| オーナーアカウント(オーナー) | 本サービスを契約するアカウント。利用契約・課金・データ分離・GDPR削除請求の単位を兼ねる。1 アカウント = 1 オーナー固定で、譲渡・降格・停止・削除はできない。配下に複数のプロジェクトを所有し、複数のメンバーアカウントを招待できる |
| メンバーアカウント(メンバー) | オーナーまたはメンバー権限「ユーザー管理」保持者から招待された利用者で、いずれか 1 つのオーナーに紐づく。メンバー権限フラグ 5 種と、プロジェクト割当の組み合わせで操作範囲が決まる |
| テナント側ユーザー | オーナーとメンバーの総称。本書で「管理者ユーザー」の語が残る箇所はすべて本概念を指す |
...
| ユーザー種別 | アカウントが本サービス上で持つ立場の区分。`admin`(テナント側ユーザー=オーナーまたはメンバー)、`end_user`(エンドユーザー)、`service_operator`(サービス運営者)の3種類。本サービスの認可(=操作の可否判定)は、オーナー境界(オーナー配下に閉じる範囲)・プロジェクト境界(メンバー割当)・メンバー権限フラグ・ユーザー種別の組み合わせで適用する |
...
| テナント分離 | 他テナント(=他の契約組織)のデータが論理的にも物理的にも分離されるようにする |
...
| 対象 | 内容 |
|---|---|
| 管理画面 | アカウント、テナント、管理者ユーザー、プロジェクト、FAQ、ウィジェット、質問ログ、未解決質問、個別チャット、課金、設定を管理する |
...
本サービスは、立場の異なる利用者を想定する。「認可範囲(=操作してよい対象の範囲)」は、オーナー境界(=オーナー配下に閉じる範囲)・メンバー権限フラグ・メンバーのプロジェクト割当・ユーザー種別の組み合わせで決定する。テナント側ユーザーは「オーナー(1 アカウント = 1 契約単位、全機能恒久保持)」と「メンバー(0 名以上、オーナーに紐づき、権限フラグとプロジェクト割当で操作範囲を制御)」の 2 区分から成り、有人のチャット対応は「個別チャット対応」権限を付与されたテナント側ユーザーが実施する。
...
| オーナー配下データ物理削除 | × | × | × | × | × | × | × | ○(削除請求 / 退会後の自動バッチ) |
| プロンプトテンプレート編集(FR-342) | × | × | × | × | × | × | × | ○(運営者のみ) |

#### 6.2.0 メンバー権限フラグの定義と適用範囲(WHAT)

テナントメンバーに付与可能なメンバー権限フラグは MVP では 5 種固定で、ロール(プリセット)概念は持たない。各フラグの責務は次のとおりとし、組み合わせ自由とする。
...
| ユーザー管理 | ユーザー管理 | テナントメンバーの招待・権限フラグ変更・停止・削除、招待再送・取消(FR-015〜021 系列) |
| プロジェクト設定 | プロジェクト設定 | プロジェクトの作成・編集・削除・アーカイブ、ウィジェット設定、許可ドメイン設定(FR-030〜036、FR-150 等) |
| ログ参照 | ログ参照 | 質問ログ閲覧、自オーナー配下の監査ログ閲覧、未解決質問・個別チャット内容の閲覧(返信権はない)(FR-070、NFR-602 系列) |

オーナー専有機能(課金情報の操作、退会申請、規約再同意の承諾)はメンバー権限フラグでは付与できない。オーナーは常に全権限フラグ相当の操作とオーナー専有機能を実行可能で、権限フラグの編集対象外とする。テナント側ユーザーは権限フラグの保有有無に関わらずダッシュボード(自分の通知・お知らせ受信箱)を利用できる。§6.2.1 の「△ 緊急時のみ可」操作はメンバー権限フラグでは付与できず、運営者専有とする。
...
| 全員ロックアウト | 該当テナントの全管理者ユーザーがログイン不能、または承認権限のある運営者 2 名以上が同時に通常承認フローを実行不能 | ロックアウト集約、サポート受付、承認者可用性 |
| セキュリティインシデント | 不正アクセス、監査ログ完全性検証失敗、鍵漏洩疑義など、本サービスの機密性・完全性に重大な疑義がある場合 | セキュリティ監視、監査ログ完全性検証、運営者通報 |
| 法令対応即応 | 個人情報削除請求(NFR-404)の GDPR 期限が 24 時間以内、または規制当局からの照会で当日対応要 | 削除請求 SLA、法務エスカレーション |
...
| BR-014 | エンドユーザーからの個人情報削除請求を、管理者経由または運営者経由で受け付けて対応できること | P0 |
| BR-015 | 管理者はテナント内のデータをエクスポートでき、退会時に削除されること | P0 |
...
| BR-022 | サービス運営者は、メンテナンス予告・機能追加・規約改定・価格改定などのお知らせを、全テナントまたは特定テナントの管理者ユーザーにメールで通知できること | P0 |
...
| BR-014 | 個人情報削除請求 | FR-161, §9.9 | NFR-404 | AC-039a〜c | 両方(受付=メイン、SLA監視=顧客管理) |
| BR-015 | データエクスポート / 退会削除 | FR-009, FR-010, FR-162, FR-163, FR-167 | NFR-703, NFR-704, NFR-803 | AC-022 | メイン |
...
| プライバシー・データ管理 | 削除請求、エクスポート、退会、規約再同意 |
...
| FR-010 | 退会申請後、猶予期間中は撤回できること | P1 |
| FR-011 | 利用規約・プライバシーポリシー改定時に再同意を取得できること。フロー詳細(発効 30 日前通知、同意期限 = 発効日 + 14 日、期限超過時は SCR-025 強制割込み + 機能制限、非同意時は退会フロー誘導、テナント単位段階的実施)は §14.7.3 を参照 | P0 |
| FR-022 | 運営者操作によりテナントが停止された場合、配下の管理者ユーザーの全セッションを **5 秒以内**に無効化すること(連携 IF #1 タイムアウト 5 秒と整合)。進行中の長時間リクエスト(エクスポート・大量インポート等)は**完了まで継続**し、中断はしないが、完了後の次回操作で再認証を要求する。再ログイン時はサスペンション状態の場合 §9.10 のルールに従う。失効方式の実装手段は[基本設計書 §10.1](02_basic_design.md) を正本とする | P0 |
...
| FR-016 | 招待されたメンバーは招待リンクからアカウントを有効化(本人によるパスワード設定)できること。招待リンクの有効期限は **7 日**(FR-020 と整合)。期限切れの場合はテナントオーナーまたは「ユーザー管理」保持者が再送信する | P0 |
| FR-016a | 招待時に指定されたメンバー権限フラグは、招待対象者がアクティベーション(パスワード設定)を完了した時点で有効化されること | P0 |
| FR-016b | 招待リンクの再送信および取消は、テナントオーナーまたは「ユーザー管理」保持者のみが実行できること。再送信時は旧リンクを失効させ、新リンクを発行する | P0 |
| FR-016c | アクティベーション完了前のメンバーは、招待時に指定された権限フラグを予約状態として保持するが、ログインおよび認可チェックは通らないこと(アクティベーション完了時点で初めて有効化される) | P0 |
| FR-017 | テナント側ユーザーは同一テナント内のオーナーおよびメンバーの一覧および詳細を参照できること。詳細にはロール種別(オーナー / メンバー)、利用状態、保持するメンバー権限フラグ、最終ログイン日時、招待有効期限を含めること | P0 |
...
| FR-019 | テナントオーナーおよび「ユーザー管理」保持者は、同一テナント内のメンバーを停止・削除できること。テナントオーナーは停止・削除の対象外とし、自分自身の停止・削除も不可とする(FR-333 / FR-334 と整合) | P0 |
| FR-020 | 招待リンクには有効期限を設けること。**有効期限: 7 日**(顧客管理システム FR-020 と整合)。期限切れ後は招待の再送信で新リンクが発行される | P0 |
| FR-021 | テナントメンバー数に固定の上限を設けず、利用量しきい値に基づいて上限接近や急増を検知し、テナント側ユーザーへアラート通知できること | P0 |
| FR-021a | メンバー数しきい値接近・超過時のアラート通知先は、テナントオーナーおよび「ユーザー管理」保持メンバーとすること | P1 |
| FR-021b | テナントにメンバーが 1 人も登録されていない状態でも、テナントオーナーが単独でテナント運営を継続できること(オーナーは全機能保持) | P0 |
...
| FR-040b | FAQ・プロジェクト・管理者ユーザー・お知らせ・テナントの削除は管理画面から復元できないこと(誤削除時はサポート窓口経由でのみ救済される) | P0 |
...
| FR-046 | FAQ件数および1件あたりの文字数の上限はテナント共通の基準とし、極端に大きい場合は警告または登録拒否できること。**MVP 初期値**: (a) 1 テナントあたり FAQ 件数 = 警告 **8,000 件**、登録拒否 **12,000 件**(NFR-111 と整合)、(b) FAQ 質問文 文字数上限 = **500 文字**、(c) FAQ 回答文 文字数上限 = **5,000 文字**。文字数超過は登録時にエラー、件数上限は段階通知(80%/100%)で警告し 120% で新規登録拒否 | P0 |
...
|---|---|---|---|
| 関連度(relevance) | 質問に該当する FAQ が存在するかを判定する指標 | 0.50 | グローバル < テナント < プロジェクト |
| 信頼度(confidence) | AI 回答を確定回答として提示してよいかを判定する指標 | 0.60 | グローバル < テナント < プロジェクト |
...
| FR-055 | 回答可否の判定に信頼度・関連度のしきい値を運営者がグローバル / テナント別 / プロジェクト別の **3 階層**で調整できること。優先順位は **プロジェクト > テナント > グローバル**(より具体的な設定が優先)。設定値は保存と同時に有効化されること。MVP 初期値は **信頼度 0.60 / 関連度 0.50**(顧客管理システム FR-061 と整合)。動作ルールは基本設計 §6.4.0 を正本とする | P1 |
...
| FR-090 | チャット投稿に上限文字数および投稿頻度制限を設定できること。**MVP 初期値**: 1 メッセージ文字数上限 = **2,000 文字**、投稿頻度上限 = エンドユーザー **10 件/分**・管理者ユーザー **無制限**。テナントごとに運営者が調整可 | P0 |
| FR-091 | 機密情報(パスワード、決済情報等)入力を控えるよう注意喚起すること | P0 |
| FR-082a | 同一テナント内で同一エンドユーザーメールアドレスが保有可能な「未終了案件(対応状態が『未対応』)」は **最大 5 件** までとすること。超過時は新規問い合わせを受付不可とし、既存案件のクローズを促すメッセージをエンドユーザーに表示すること(BR-006 関連) | P1 |
...
| FR-121 | テナントごとに利用上限(質問数、FAQ 件数、個別チャット部屋数の月次無料枠および月次予算上限(円))を任意に設定できること。設定したしきい値に基づき利用量の計測および制限を行えること(顧客管理システム FR-121 と整合) | P0 |
...
| FR-128 | 各APIやチャット機能に対してテナント・API種別別のレート制限を DDoS / Bot / 暴走対策(セキュリティ目的)として適用し、運営者がテナント単位でしきい値を調整できること(具体しきい値は基本設計書 §11.7 リミット設計表を参照) | P0 |
| FR-129 | 無料トライアル(**MVP 初期値 14 日間**、§8.11.1 と整合)の開始・残り 3 日・終了日にテナントへ通知すること(請求確定の受信箱お知らせ + メール)。トライアル終了時に支払方法未登録の場合は終了日翌日に **自動サスペンションへ遷移すること(猶予期間なしで即時、§9.10 のサスペンション支払い失敗フローとは別経路、§9.10 末尾の「トライアル終了経路」を参照)**。サスペンション後の挙動は §9.10 段階 5 と同一(課金画面・データエクスポート画面・退会画面のみアクセス可、ウィジェット応答は機能停止の旨を示す応答) | P0 |
| FR-136 | 支払い方式はクレジットカード払いを必須とすること | P0 |
| FR-137 | 支払い失敗を検知した場合、猶予期間(7 日。§9.10 サスペンションフロー、§20.2 参照)を経過後にテナントをサスペンション状態に移行すること | P0 |
...
| FR-144 | テナント/プロジェクト単位で送信レート制限・バウンス率・苦情率を監視し、しきい値超過時に送信抑制・運営者通知ができること | P0 |
...
| FR-148 | 月次の請求が確定したタイミング(月次の確定処理タイミングは §8.11.1 と整合)で、管理者ユーザーにメール通知できること。通知には請求対象期間、請求金額、内訳(質問数・各種上限の超過分など)、請求書/明細の確認導線を含めること。同タイミングで対象管理者のお知らせ受信箱に「請求確定」のお知らせを生成すること(FR-187 と整合)。同一テナント・同一請求月での重複請求を行わず、月次確定処理は冪等に動作すること(冪等キーの構成等の実装仕様は基本設計書を正本とする。顧客管理システム FR-303 と整合) | P0 |
| FR-149 | サービス運営者が、特定テナントまたは全テナントに対して任意のお知らせ(メンテナンス予告、機能追加、規約改定、価格改定など)をメール通知できること。通知の作成・送信は運営者のみが行い、宛先範囲(全テナント/特定テナント/特定ユーザー種別)、件名、本文、送信予定日時を指定できること。受信者は管理者ユーザーを対象とし、種別ごとにオプトアウトの可否を設定できること(ただし規約改定・重要セキュリティ通知などは強制送信としオプトアウト不可)。同タイミングで対象テナントの管理者ユーザーのお知らせ受信箱に「運営お知らせ」のお知らせを生成すること(FR-188 と整合) | P0 |
...
| FR-149b | バウンス・苦情検知時のメールサプレスリストはテナント横断で共有し、宛先別に再送停止すること(R-003 共通ドメイン使用に伴う対策) | P0 |
| FR-145a | テナントメンバーのメンバー権限フラグが追加・剥奪された場合、当該メンバーへお知らせ受信箱(「運営お知らせ」種別・「通常」重要度)とメールで通知できること(FR-018b と整合) | P0 |
...
| FR-161 | エンドユーザーからの個人情報削除請求を、管理者または運営者が受け付けて対応できること | P0 |
| FR-162 | 管理者はテナント内データをエクスポートできること | P1 |
| FR-163 | 退会時にテナントデータを定められた猶予期間後に削除できること | P0 |
...
エンドユーザーからの削除請求を本サービス側で直接受け付けるための画面要件は **SCR-026 エンドユーザー削除請求受付フォーム**(FR-161 関連)で扱う。詳細は基本設計書 §5 の画面一覧を参照。
...
| FR-176 | アクセス制御は最小権限の原則に従うこと(テナント分離、ユーザー種別別認可) | P0 |
| FR-177 | Bot対策(CAPTCHA等)を必要に応じて実施できること | P1 |
| FR-178 | 不審なリクエスト(大量アクセス、未許可ドメイン等)を検知・記録できること | P0 |
| FR-178a | テナントメンバー招待、招待再送、招待取消、メンバー権限フラグ変更、メンバー停止、メンバー削除のいずれの操作も監査ログに記録できること。記録項目は操作者・対象メンバー・変更前後の権限セット差分(該当時)・対応理由(任意)・実行時刻を含むこと | P0 |
| FR-179 | 管理画面に対するテナント単位の IP 許可リスト機能を提供すること(オプトイン、初期設定空 = 制限なし) | P1 |
...
| FR-195 | 不正利用検知(大量送信・Bot 疑い・規約違反)について、(a) 自動検知はメインシステム側で行うこと、(b) 判定ルール・しきい値は顧客管理システムからテナント別に上書き可能とすること、(c) 自動ブロックはメインシステム側で実行すること、(d) 自動ブロック発火時は管理者ユーザーへ即時通知し、申し立て窓口を案内すること、(e) 運営者の手動判断による緊急停止は顧客管理システム FR-224 から実行されること。連携方式と通知種別は基本設計を正本とする | P1 |

### 8.17 お知らせ

| ID | 要件 | 優先度 |
|---|---|:---:|
| FR-180 | 管理者は自テナント宛のお知らせ一覧を取得・表示できること | P0 |
...
| SCR-016 | 設定(退会・課金画面入口) | 管理者 | FR-009, FR-010, FR-125, FR-162 | P0 | メイン |
...
| SCR-024 | 退会申請・撤回 | 管理者 | FR-009, FR-010 | P0 | メイン |
| SCR-025 | 規約再同意割込み | 管理者ユーザー | FR-011, FR-164 | P0 | メイン |
| SCR-026 | エンドユーザー削除請求受付フォーム | エンドユーザー | FR-161, NFR-404 | P0 | メイン |
...
| SCR-092 | AI 推論パラメータ設定(テナント別上書き) | サービス運営者 | 顧客管理 FR-061〜066, FR-222 / メイン FR-055 | P0 | 顧客管理 |
| SCR-093 | テナント別レート制限・予算上限管理(サプレスリスト復帰承認を含む) | サービス運営者 | 顧客管理 FR-121, FR-224(b), §11.3 サプレスリスト復帰承認 / メイン FR-121, FR-122 | P0 | 顧客管理 |
| SCR-094 | お知らせ作成・配信(運営者) | サービス運営者 | 顧客管理 FR-149, FR-188〜189 | P0 | 顧客管理 |
| SCR-095 | 削除請求処理ダッシュボード(運営者) | サービス運営者 | 顧客管理 FR-227, FR-228 / メイン NFR-404 | P0 | 顧客管理 |
...
| ID | 要件 | 優先度 |
|---|---|:---:|
| FR-310 | FAQ インポートは CSV および JSON 形式を受け入れること。同一質問文が既存テナント内に存在する場合の **競合処理は管理者が選択**できること(スキップ / 上書き / 別件として追加)。部分失敗時は成功分を活かし、失敗分の理由を管理者が確認できること。上書き時は改訂履歴に残すこと。大容量インポートのジョブ化、同一判定、ロールバック単位、エラーログ形式は基本設計 / 詳細設計を正本とする | P1 |
| FR-311 | データエクスポート(FR-162)は CSV / JSON 形式で提供し、スキーマ定義書を別ファイルで提供すること。テナント全データのエクスポートは非同期ジョブ化し、完了時にメール通知 | P1 |
...
| FR-331 | 海外 IP 遮断は **運営者画面のみデフォルト ON** とする。テナント側 API およびウィジェットは海外エンドユーザーから到達するため対象外 | P1 |
...
| FR-334 | テナント側ユーザーは **自分自身の利用状態変更・削除・権限フラグ剥奪・プロジェクト割当変更不可** とすること。特に「ユーザー管理」権限フラグの自己剥奪は不可で、別の保持者(またはオーナー)経由で実行する必要がある | P0 |
...
| 3 | テナントを猶予期間状態に変更する |
| 4 | 猶予期間中、管理者は撤回できる |
| 5 | 猶予期間経過後、テナントデータを削除する |
| 6 | 削除実行を運営者が確認できる(監査ログ) |

なお、解約申請(サブスクリプション終了)と退会(テナント削除)は別フローとする。解約後も猶予期間中はデータ参照可能であり、猶予期間終了時に削除キューへ投入される。
...
| 復元対象外 | - | 物理削除済みデータ、個人情報削除請求(GDPR対応)で削除されたデータ |
...
エンドユーザーや管理者からの個人情報削除請求(GDPR等)は、メインシステムの管理画面 SCR-026 で受け付ける場合と、運営者へのメール・サポート窓口経由で受け付ける場合の双方が存在する。運営者直接受付の場合は、運営者が顧客管理システムに削除指示を入力し、メインシステム側で削除キューに投入する。受付から **7 営業日以内** に削除実行を完了することを SLA とする(NFR-404 および 顧客管理システム FR-227 / AC-039 と整合)。

**削除請求の状態機械および SLA 監視主体**: 削除請求の状態(受付待ち → レビュー中 → 処理中 → 完了、期限切れ / キャンセルを含む)および SLA 監視は **顧客管理システムが単独で管理する**(正本: 顧客管理 §9.9, FR-227)。メインシステム側は (1) SCR-026 経由での受付 → 顧客管理への通知、(2) 顧客管理からの「削除実行」指示の受信 → 物理削除実行、(3) 削除完了通知の顧客管理への返送(連携 IF #11)、の 3 工程のみを担当する。営業日定義は日本のカレンダー(土日 + 国民の祝日 + 年末年始 12/29〜1/3)とし、5 営業日経過時点で顧客管理側から運営者へリマインド、SLA 超過(7 営業日超)で高重要度アラートを発火する(連携 IF #11)。状態値の実装名は基本設計を正本とする。

#### 9.9.1 削除データ取扱の責任分界マトリクス

|  | メインシステム | 顧客管理システム |
|---|---|---|
| 利用者からの削除リクエスト受付 | SCR-026 でエンドユーザー受付、管理者は管理画面で受付 | サポート窓口経由受付、運営者が代行入力 |
| 削除確認ダイアログ表示 | 表示する(取消不能の明記必須) | 運営者代行時は確認ダイアログ表示 |
| 削除実行(論理削除→キュー投入) | 利用者操作によりメインシステムで実行 | 代行時のみ顧客管理システム経由でメインシステム API を呼出 |
| 物理削除実行(猶予期間後) | バッチ実行 | 直接の物理削除権限は持たない |
| 復元(誤削除救済) | **提供しない** | 運営者のみ提供(物理削除前のもののみ) |
| 監査ログ記録 | 削除操作の記録 | 復元操作の記録。具体コード体系は基本設計 / 詳細設計 |

### 9.10 サスペンションフロー

支払い失敗・規約違反等によりテナントを機能制限する場合のフロー。FR-137〜FR-139 と連動する。**本節はメインシステム / 顧客管理システム共通の正本**(顧客管理システム §9.10 から参照される)。
...
| サスペンション状態遷移(利用中 → サスペンション中) | メイン | テナント状態をサスペンション中へ更新し、関連キャッシュを無効化 |
| ウィジェット停止応答 | メイン | エッジでテナント状態を判定し、機能停止の旨を示す応答に切り替える |
| 管理画面アクセス制限 | メイン | 認可ゲートで「課金画面 / エクスポート画面 / 退会画面」以外を利用不可とする |
| 解除条件判定(支払い成功) | 顧客管理 | 課金プロバイダからの支払い成功イベントで判定 |
| 解除実行(サスペンション中 → 利用中) | メイン | 顧客管理からの「解除」イベントでテナント状態を利用中へ更新 |
...
| テナント状態遷移 | 利用中 → サスペンション中(猶予経過後) | 利用中 → サスペンション中(終了日翌日) |
...
**重要原則**: トライアル終了経路は課金プロバイダのイベントを経由しないため、§9.10.1 の支払い失敗イベントフローは発火しない。メイン側で完結する状態遷移であり、顧客管理側へは連携 IF #1 ではなく **連携 IF #8 監視メトリクス** 経由でテナント数増減として通知される。
...
**重大障害の定義(§6.2.1 緊急区分 1 と整合)**: 本サービスの主要 KPI のいずれか((a) サーバー側エラー率が 5% 超、(b) 公開ウィジェット可用性が NFR-201 違反、(c) AI 推論応答 p95 が 5.0s 超 のいずれか)が **連続 5 分以上** 継続した状態、または全テナントへの影響が想定される事象を「重大障害」と定義する。重大障害発生時は §6.2.1 の緊急区分 1 として、§6.2.2 の発動条件下で △ 操作を実行可能とする。
...
| NFR-302 | セキュリティ | 他テナントのFAQ、質問ログ、個別チャットを参照できないこと |
...
| NFR-305 | セキュリティ | トークンはハッシュ化し、平文保存しないこと。アクセストークン(登録完了・再入室・削除確認)はテナント派生鍵で、API キー(ウィジェット公開キー)はグローバル派生鍵で鍵付きハッシュを計算する。鍵階層は NFR-320 を参照(採用アルゴリズム名は詳細設計) |
| NFR-306 | セキュリティ | 監査ログを改ざん検知可能な形で保持すること。レコード列が改ざんされた場合に検知できる仕組みを設け、**日次バッチで全件検証**し、不一致検出時に運営者へ高重要度通知を送出すること。鍵管理は単一鍵で年次ローテーションとする。**IP マスク**: IP アドレスは匿名化して記録する(顧客管理システム NFR-306 と整合、検知方式・採用アルゴリズム・列設計の詳細は基本設計および詳細設計)。**保持期間経過時の削除整合性**: NFR-602a / NFR-602b の保持期間経過後の削除、および GDPR 削除請求によるテナントスコープ削除を行う際は、§12.2.1 の方針に従い改ざん検知の連続性を維持すること。削除済みレコードは日次検証でも整合判定対象とする(物理的な削除方式は詳細設計) |
...
| NFR-312 | セキュリティ | ウィジェット配信オリジンを限定すること。(a) 管理画面 API は管理画面オリジンのみ許容(ワイルドカード不可)、(b) ウィジェット配信はテナント設定の許可ドメイン(FR-151, AC-020)に従い、サブドメインのワイルドカード許容のみ受け付ける、(c) 上記を実現するためのレスポンスヘッダおよびプリフライト設定は基本設計を正本とする |
...
| NFR-316 | セキュリティ | レート制限はテナント・機能種別の組み合わせで DDoS / Bot / 暴走対策(セキュリティ目的)として設定可能とすること(具体しきい値は基本設計で規定) |
...
| NFR-319 | セキュリティ | 個別チャット本文・エンドユーザー入力は、問い合わせ対応のため本文として保存し、テナント分離 + プロジェクト分離により参照制御を行うこと。**本文保存の補完策**: (a) 本番データへ直接アクセス可能な運営者を 2 名以下に限定、(b) 月次棚卸しで運営者リストを PO 承認、(c) 直接アクセスを監査ログに記録、(d) 直接アクセス時は §14.5「本番直接変更」のチケット ID 必須ルールを適用。削除請求 PII(SCR-026 受付分)は削除完了後に復元不能な方式で破棄すること(NFR-404 と整合、採用方式は基本設計および詳細設計)。エンドユーザーメールアドレスは安全に保管し、復号せずに突合可能な検索手段を提供すること(列構造・採用方式は基本設計および詳細設計)。カード情報は本サービスでは保存せず課金プロバイダへ委譲し、PCI-DSS 範囲外化を維持すること |
...
| NFR-402 | プライバシー | 日本国内利用者を対象とする SaaS として提供し、EU/EEA 居住者を主たる利用者とする SaaS としては提供しない旨を利用規約およびプライバシーポリシーに明記すること。個人情報保護法(APPI)については正本要件として対応し、利用目的・第三者提供・保持期間・削除請求への対応導線(NFR-401 / NFR-404)を提供すること |
| NFR-403 | プライバシー | データ最小化原則に従い、業務に不要な個人情報を取得しないこと |
| NFR-404 | プライバシー | 削除請求に対し受付から **7 営業日以内** に対応を完了できること(§9.9 削除データ取扱、顧客管理システム FR-227 / AC-039 と整合) |
...
| NFR-602a | 監査 | **一般監査ログ(操作・チャット対応・FAQ 編集・ログイン)の保持期間 = 1 年**(§12.2 と整合)。期間経過後は §12.2.1 の方針に従い本文を物理削除しつつ改ざん検知用の連続性を維持できる方式で削除すること(具体方式は基本設計および詳細設計)。**1 年保持の根拠(3 点)**: (a) 業務改善の振り返り期間として一般的に 1 年(年次振り返り・年度報告・四半期 KPI 比較)が妥当、(b) 一般的なセキュリティインシデント調査の遡及範囲として最低 1 年(BCP/IR ベストプラクティス)、(c) GDPR 等の削除請求対応(7 営業日 SLA、NFR-404)時の照会において削除前データの参照が必要となるケースへの備え |
| NFR-602b | 監査 | **課金関連監査ログ(請求発行・支払い結果・課金設定変更・サスペンション遷移)の保持期間 = 7 年**(国税通則法・法人税法施行規則。§12.1 / §16.1 と整合)。電子帳簿保存法対応のため真正性・可視性・検索性を確保すること |
| NFR-602d | 監査 | **運営者高権限操作監査ログの保持期間 = 5 年**。対象操作は (a) 削除データ復元、(b) テナント停止/復元/物理削除、(c) AI 推論パラメータ変更、(d) テナント別レート制限/予算上書き、(e) お知らせ配信、(f) 運営者アカウント発行/無効化、(g) 課金 Webhook リプレイ、(h) 本番直接変更、(i) 監査ログエクスポート操作。内部統制(SOX 類似)・係争耐性のため。保持区分の管理、定期削除、保持期間経過後の削除方式は基本設計および詳細設計を正本とする(顧客管理システム NFR-602(b) と整合) |
...
| NFR-605 | 監査 | 監査ログエクスポート機能を運営者向けに提供すること(SCR-096 経由)。形式は表形式・行区切りの構造化テキストなどから選択可、期間・テナント・操作種別でフィルタ可、テナント単位での「自テナント分のみのエクスポート」は管理者にも提供する。エクスポート操作自体も監査ログに記録 |
...
| NFR-703 | データ削除 | 退会・削除請求に応じてデータを削除できること |
...
| NFR-707 | データ削除 | 物理削除完了後のテナント識別子は **再利用しない** こと。同一組織が再契約する場合も新規 ID を発行する。**理由**: 監査ログ・エラーログ・課金履歴(NFR-602b の電帳法 5 年・国税法 7 年保持)の参照整合性を保ち、削除後に同一識別子で別データが紐付くことによる監査汚染を防止するため。実装方針(削除済 ID の永続保持方式・一意性制約の設計)は基本設計書で定義(顧客管理システム NFR-707 と整合) |
...
| ID | 区分 | 要件 |
|---|---|---|
| NFR-320 | 鍵管理 | マスター鍵はクラウドプロバイダの機密保管領域に保管し、構成ファイル・ソースコードに直書きしないこと。マスター鍵からテナント別派生鍵およびグローバル派生鍵を生成し、本番アプリケーションコードから直接マスター鍵を参照しないこと(採用する鍵派生方式・派生パラメータ・投入手順は基本設計および詳細設計) |
| NFR-321 | 鍵管理 | 鍵ローテーション周期は以下とする: (a) マスター鍵 = **年次**、(b) テナント派生鍵 = マスター鍵更新時に再派生、(c) API キー(ウィジェット公開キー、FR-194)= 管理者操作で任意。マスター鍵ローテーション中は新旧両鍵での復号を一定期間並行運用し、運用中の暗号化データを再暗号化なしに切替可能とすること。並行期間の長さ・運用手順は基本設計を正本とする |
| NFR-322 | 鍵管理 | API キー(ウィジェット公開キー)のローテーション猶予期間は **30 日**(FR-193 と整合)。猶予期間中は旧キーを警告付きで受け付け、管理画面 SCR-014 に「旧キー使用中」表示を行うこと |
| NFR-323 | 鍵管理 | 外部プロバイダの API キー(メール配信プロバイダ・課金プロバイダ等)はクラウドプロバイダの機密保管領域に保管し、**90 日ローテーション** を目標とすること。漏洩を検知した場合は (a) 機密保管領域の即時ローテーション、(b) 該当プロバイダの管理画面から旧キーを失効、(c) 影響テナントへ高重要度通知、(d) 全運営者・管理者セッションの無効化、を実施できること |
...
| 同時アクティブテナント数 | NFR-110 | (受け側機能要件なし、本節が正本) |

| ID | 区分 | 要件(MVP 初期値) |
|---|---|---|
| NFR-110 | キャパシティ | 同時アクティブテナント数の上限を **200** とする(150 到達時にデータ層スケール設計を着手)。超過時は新規テナント受付を一時停止し運営者通知 |
| NFR-111 | キャパシティ | 1 テナントあたり FAQ 件数の **運用想定上限を 10,000 件** とする。具体しきい値は **警告 8,000 件 / 強制拒否 12,000 件**(FR-046 を正本とする)。10,000 件は「想定運用範囲の上限」を示す目安であり、警告(80%)と強制拒否(120%)の中間値として超過監視の起点に用いる |
| NFR-112 | キャパシティ | 1 テナントあたり質問ログ件数の想定上限を **100 万件/月** とする。超過時はプラン変更案内を運営者から発出する |
| NFR-113 | キャパシティ | 1 テナントあたりプロジェクト数の上限を **50**、管理者ユーザー数の上限を **100** とする |
...
| NFR-117 | キャパシティ | AI 推論並行数の上限を テナントあたり **10 並列**、全体 **200 並列** とし、超過時は利用者にレート制限超過を示す応答を返却する。本要件は NFR-103 / NFR-805 と整合 |
...
| 課金プロバイダ | 決済結果・サブスクリプション変更を顧客管理システム経由で受け取り、テナント状態および課金履歴へ反映できること | [基本設計書 §8.7](02_basic_design.md) / [顧客管理 基本設計書 §8](../02_admin/02_basic_design.md) |
| 管理者サイト(エンドユーザー) | 管理者サイトに埋め込むウィジェットを配信し、許可されたサイトからのみ質問送信を受け付けること | [基本設計書 §8.3](02_basic_design.md) |
| 顧客管理システム(サービス内連携) | テナント停止・削除・復元・しきい値変更・お知らせ配信・監視メトリクス等を双方向連携できること | [基本設計書 §8.7](02_basic_design.md) |
...
| 開発時例外 | 管理者ユーザーが開発モードを有効化した場合のみ、ローカルホスト系ドメインを一時許可できること |
...
| 送信元 | サービス共通ドメインを使用し、From 表示名にテナント / プロジェクト名を含められること |
| Reply-To | プロジェクトごとに設定でき、確認完了前は利用できないこと |
| 配信状態 | メール配信プロバイダから配信結果を受信・記録できること |
| サプレス | バウンス・苦情を検知して送信停止でき、必要な復帰申請を運営者が判断できること |
| 送信制御 | テナント単位および同一宛先単位で送信量を制御できること |
...
|---|---|---|---|---|
| 1 | テナント停止イベント(支払い失敗確定 / 解除 / 規約違反緊急停止) | サスペンション・規約違反対応を顧客管理から反映できること | FR-022, FR-124, FR-137, §9.10 | FR-124, FR-224(a) |
| 2 | 強制ログアウト | テナント停止連動・本人不正検知時にセッションを失効できること | FR-022 | FR-224 |
| 3 | 削除請求受付通知 / 削除実行指示 | メイン受付の削除請求を顧客管理で SLA 監視し、確認後に削除実行を指示できること | FR-161, §9.9.1 | FR-227, FR-228 |
| 4 | 復元実行 | 運営者承認後に論理削除済みデータを差し戻せること | §9.9 | FR-200〜FR-211 |
| 5 | テナント別レート制限上書き | 緊急時にテナント別 API レート制限を上書きできること | NFR-316 | FR-121, FR-224(b) |
| 6 | AI 推論パラメータ上書き | 信頼度・関連度しきい値をテナント別 / プロジェクト別に調整できること | FR-055, FR-058〜FR-059 | FR-055, FR-061 |
...
| 11 | 削除完了通知・SLA 超過アラート | 削除完了を顧客管理へ通知し、SLA 超過の監視・エスカレーションに利用できること | FR-161, NFR-404 | FR-227, AC-039 |
...
| データ | 保持目的 | 補足 |
|---|---|---|
| テナント | 課金単位、データ分離単位 | 課金設定、状態、規約同意状況、利用上限しきい値 |
...
**主要なカーディナリティ規約**:
- 1 アカウント = 1 テナントに固定する(§8.22 FR-335 と整合)。
...
データ分類ごとに「物理削除」「匿名化」「ハッシュチェーン保持 + テナントスコープ削除」のいずれを適用するかは下表に従う。

| データ種別 | 削除方式 | 理由 |
|---|---|---|
| 業務データ(FAQ、質問ログ、チャットメッセージ) | 匿名化を原則とする(統計利用と利用者特定回避の両立)。テナント要請がある場合は物理削除に切替可能 | 統計分析・サービス改善のため、識別子を除去した匿名形式での保持を許容する |
| アカウント・認証情報・トークン | 物理削除 | 認証情報の残存はセキュリティリスクとなるため |
| 監査ログ | 改ざん検知用の連続性を維持しつつ、テナントスコープのみ削除 | NFR-306 改ざん検知要件と GDPR 削除請求の両立のため。**削除実装方針**: テナントスコープ削除時は当該レコードの本文(操作者・対象・IP 等の業務情報)を物理削除しつつ、改ざん検知の連続性を担保する最低限の情報のみを残すこと。NFR-306 / NFR-602c / NFR-604 の日次検証で「改ざん検知の連続性」が維持されること。連続性確保用レコード自体は監査ログの保持期間を超えても削除しない(連続性破断防止)。**具体スキーマ・列構成は基本設計および詳細設計参照** |
| 通知ログ | 集計後削除(個人識別情報を除去のうえ集計値のみ残す) | 配信品質分析と個人情報削除の両立のため |
...
| 項目 | 要件 |
|---|---|
| テナント分離 | テナントAのデータをテナントBから論理的・物理的に分離する |
| プロジェクト分離 | 同一テナント内のFAQ・質問ログ・未解決質問・個別チャットをプロジェクト単位で分離して管理できる |
...
| アクセス制御 | テナント、プロジェクト、ユーザー種別、所有者によるチェック |
...
| 運営お知らせ | 通常 | メインシステム(自動) | 未解決質問の手動「終了」(FR-079)、復元実行通知(顧客管理システム §9.9)、テナントメンバー権限フラグの追加・剥奪通知(FR-145a / FR-018b) | 可(権限変更通知は不可) | 受信箱 + メール(権限変更通知時) |
...
| システム通知 | 重要 | 顧客管理システム(運営者操作) | テナント停止・強制ログアウト実行通知、不正利用検知時の自動ブロック通知 | 不可 | 受信箱 + メール |
| システム通知 | 通常 | メインシステム(自動) | ログインロックアウト発動(FR-007)= 管理者ユーザーへの情報提供。該当本人へは別途メール直送 | 不可 | 受信箱(管理者宛) + メール(本人宛) |

**重複生成対策**: 同一テナント・同一イベント種別での 60 分以内の連続発火は 1 件に集約する(R-012 と整合)。重複判定キーの設計は基本設計で確定する。
...
| 認可境界をまたぐ操作 | テナント切替不可だが SCR-XXX 経由で他リソース参照、テナントメンバーの招待・権限フラグ変更・停止・削除、招待再送・取消、テナントオーナーへの変更試行の拒否 | §6.2 権限マトリクスの全 △ 操作およびユーザー管理操作 |
| 状態変更を伴う P0 機能 | FAQ 公開 / 取り下げ、未解決質問の終了 / FAQ 登録、テナントメンバーの招待 / 権限フラグ変更 / 停止 / 削除、テナント設定変更 | P0 全 FR の作成・更新・削除・公開・終了等の状態変更操作 |
...
| データ削除・復元 | 論理削除、物理削除、運営者復元、連続性維持用レコード化(§12.2.1)、削除請求受付・完了 | §9.9 / §12.2.1 全件 |
...
本サービスは新規 SaaS であり、既存の本サービス利用テナントは存在しない。MVP では「テナント移行」「アカウント移行」「FAQ 移行」は発生しない。メイン / 顧客管理間でデータ移管が発生する場合は §11.5 連携 IF #3 / #4(削除・復元)で対応する。

#### 13.1.1 既存テナントの権限モデル移行(オーナー + メンバー化)

本サービスはリリース後に、テナント側ロールを「単一の管理者ユーザー」から「テナントオーナー(1 名固定) + テナントメンバー(権限フラグ式)」へ刷新する。既存テナント(本変更前にすでに存在するテナント)に対しては、次の WHAT を満たす一括移行を実施する。実装手順(SQL、バッチ実行順、ロールバック)は詳細設計を正本とする。

| 移行対象 | 移行後の状態 |
|---|---|
| 各テナントにおいて利用状態が有効な管理者ユーザーのうち、最も古く登録された 1 名 | テナントオーナーに昇格(全機能保持・降格不可) |
| 上記以外の有効な管理者ユーザー | テナントメンバーとして残し、メンバー権限フラグ 5 種をすべて ON で付与(運用に支障が出ないよう移行直後は機能差を作らない) |
| 招待中(アクティベーション未完了)の管理者ユーザー | テナントメンバーとして残し、メンバー権限フラグ 5 種をすべて ON で予約付与(アクティベーション完了時点で有効化) |
| 利用停止中の管理者ユーザー | 現状維持(メンバー権限フラグは付与しない) |
| 有効な管理者ユーザーが 0 名のテナント | オーナー昇格は実施せず、運営者による個別対応(サポート窓口経由のオーナー再アサイン)とする。移行スクリプトは該当テナントを監査ログに記録し、運営者へ重要度「重要」のシステム通知を発火する |

移行は不可逆とする(オーナー降格は MVP 機能として提供しないため、誤った昇格は別の有効メンバーへのサポート対応によってのみ訂正可能)。移行完了後、各テナントについて「テナントオーナーが 1 名存在する」または「有効な管理者ユーザーが 0 名でオーナー不在」のいずれかであることを検査する。複数オーナーが発生する状態は受入条件 AC-018g に反するため、移行スクリプトは失敗として扱う。

#### 13.1.2 テナント階層廃止に伴うデータ移行(オーナー直接所有モデルへ)

§13.1.1 完了を前提に、テナント階層を廃止して「オーナーアカウントが直接プロジェクトを所有する」モデルへ移行する。実装手順(SQL、バッチ実行順、ロールバック)は詳細設計を正本とする。本節は WHAT を記述する。

| 移行対象 | 移行後の状態 |
|---|---|
| 既存の各テナント | 当該テナントの `is_owner=1` アカウントを「オーナーアカウント」として再定義し、所属メンバーおよび配下プロジェクトをすべてオーナーアカウントに直接ぶら下げる(中間層としての `tenants` レコードを廃止) |
| 旧テナント状態(`active` / `suspended` / `deleted_pending` / `deleted`) | オーナーアカウントの「契約状態」属性へ移行(実装値の正本は基本設計)。意味は不変 |
| アカウント・プロジェクト・FAQ・問い合わせ・課金・監査ログなど全エンティティの「テナント所属」属性 | 対応するオーナーアカウントを直接参照する形に変更(`tenant_id` → `owner_account_id` 相当の移行) |
| メンバーのプロジェクト割当(本改訂で新設) | 既存メンバーは初期状態で「割当なし」(=ダッシュボードと全社的機能のみ利用可)。運用開始後にオーナーまたは「ユーザー管理」保持者が個別割当を行う |
| オーナーが存在しないテナント(§13.1.1 で運営者対応キューに送られた残骸) | 移行対象外。運営者が個別にオーナー再アサインを完了するまで凍結 |

移行は不可逆。移行完了後、各オーナーアカウントの契約状態が旧テナント状態と一致していること、プロジェクト・メンバー・課金・監査ログ等の総数が移行前後で一致していることを検査する。不一致が検出された場合は移行を失敗扱いとし、ロールバック手順(詳細設計)を実行する。
...
| 項目 | 件数(MVP) | 備考 |
|---|---|---|
| 検証用テナント | 5 件 | 用途別: (a) 通常運用、(b) サスペンション中、(c) 削除予定、(d) トライアル中、(e) 上限近傍 |
| 検証用プロジェクト | テナントあたり 3〜5 件 | アーカイブ済み 1 件含む |
| 検証用 FAQ | プロジェクトあたり 50 件 | 下書き / 公開 / 非公開 / カテゴリ付与のミックス |
| 想定質問セット | 50 件以上 | AI 品質回帰テスト用(FR-059) |
| エンドユーザー疑似データ | 1 テナントあたり 100 件 | メール・問い合わせ ID・チャットメッセージ |
| 検証用運営者アカウント | 3 名 | 役割: (a) 全権、(b) 削除請求対応のみ、(c) 監査ログ閲覧のみ |
...
| 言語 | 初期版UIは日本語のみ |
| ブランド | 送信ドメインはサービス共通(テナント独自送信ドメインなし) |
| 業界 | **申込み時の業種チェックは §14.7.1 ステップ 3「テナント情報入力」で実施** し、高規制業界を選択した申込みは利用規約同意ページで標準提供範囲外であることを明示し、サポート窓口経由での個別相談へ誘導する |
...
|---|---|---|---|
| 個人情報保護法(APPI) | 必須 | 利用目的通知、第三者提供制限、削除請求 7 営業日 | FR-160, FR-161, NFR-404 |
| 改正電気通信事業法(2023) | 必須 | 外部送信規律(Cookie / トラッキング情報の通知)、利用目的の表示位置(ウィジェット初期表示) | FR-160, FR-168 |
| GDPR | EU 在住エンドユーザー対応時 | DPO 任命、削除請求、データ可搬性 | FR-161, FR-162, NFR-404 |
...
| AC-017 | 他テナントのFAQ・質問ログ・個別チャットが参照できないこと。検証は、テナント間API相互アクセス禁止を確認する自動テストに加え、MVP リリース前にペネトレーションテスト1回以上で実施すること | NFR-302 |
...
| AC-018g | 既存テナントの権限モデル移行 + テナント階層廃止移行(§13.1.2)後、各オーナーアカウント配下について「オーナー 1 名存在し、その他の元管理者ユーザーが全権限フラグ ON のメンバーとして残る(プロジェクト割当はゼロ)」、または「有効な管理者ユーザーが 0 名のため運営者対応待ちとなる」のいずれかであること | §13.1.1 / §13.1.2 |
...
| AC-022 | 退会後、定められた猶予期間を経てテナントデータが削除されること | FR-163, NFR-703 |
...
| AC-026 | サービス運営者が運営側のお知らせを作成・送信でき、宛先範囲(全テナント/特定テナント/特定ユーザー種別)、件名、本文、送信予定日時を指定して該当テナントの管理者ユーザーに届けられること。種別によりオプトアウト可否を制御できること | BR-022, FR-149 |
...
| AC-034 | AI 回答の信頼度しきい値・関連度しきい値がグローバル / テナント別 / プロジェクト別の 3 階層で適用され、優先順位はプロジェクト > テナント > グローバルとすること。設定変更は再起動なしで即時有効化されること(顧客管理システム AC-034 と整合) | FR-055 |
...
| AC-038 | 運営者の高権限操作(削除データ復元、テナント停止 / 解除、AI しきい値変更など)が監査ログに記録され、対象テナントの管理者へメールおよび受信箱(system 種別)で通知されること(顧客管理システム AC-038 と整合) | 顧客管理 FR-211, FR-224, FR-229 / NFR-306 |
| AC-039a | 個人情報削除請求が受付待ち → レビュー中 → 処理中 → 完了の状態で管理され、期限切れ・キャンセル分岐も扱えること。状態遷移は監査ログに記録されること(顧客管理システム AC-039a と整合) | FR-161, §9.9 |
| AC-039b | 削除請求 SLA が **7 営業日以内** に達成され、5 営業日経過時点で運営者へリマインド通知が発火すること。営業日は日本のカレンダー(土日 + 国民の祝日 + 年末年始 12/29〜1/3)で計算されること(顧客管理システム AC-039b と整合) | FR-161, NFR-404 |
| AC-039c | SLA 超過(7 営業日超)時に運営者へ高重要度のエスカレーション通知が発火し、対象管理者ユーザーにも進捗通知が送信されること(顧客管理システム AC-039c と整合) | FR-161, NFR-404, §9.9 |
...
| AC-043 | 支払い失敗から猶予 7 日経過でテナントがサスペンション中へ遷移し、サスペンション中も管理者ユーザーは管理画面の課金画面・データエクスポート画面・退会画面のみアクセス可能であること。ウィジェット応答方式は基本設計を正本とする(顧客管理システム AC-043 と整合) | FR-137, §9.10 |
...
| FR-009, FR-010(退会・撤回) | AC-022(退会後削除) | §14.6 E2E + バッチ検証 |
...
| FR-160〜168(プライバシー) | AC-039a〜c | §14.6 E2E + 法務レビュー |
...
| AC-039a | 削除請求の状態遷移と監査ログ記録が動作すること |
| AC-039b | 5 営業日リマインドが発火すること(日本営業日基準) |
| AC-039c | 7 営業日超過時のエスカレーション通知が発火すること |
| AC-040a | 80% 到達で警告通知 |
...
| AC-039a〜c | 削除請求状態遷移・5 営業日リマインド・SLA 超過エスカレーション | FR-161, NFR-404, §9.9 | FR-227, FR-228 | 顧客管理(状態管理・SLA監視) / メイン(削除実行) |
...
| AC-044a〜b | テナント月次予算上限設定・通知連動 | FR-005, FR-121, FR-122, FR-125 | FR-121, FR-122, FR-125, FR-307 | メイン(UI・適用) / 顧客管理(全体把握) |
| AC-047 | アクセシビリティ受入条件(WCAG 2.1 AA / 自動 + 手動評価) | NFR-1001〜1003 | NFR-1001〜1003 | メイン(自動チェック・テナント側 SCR)/ 顧客管理(運営者画面 SCR-090〜099) |
...
| R-003 | 共通送信ドメインのレピュテーション低下 | 全テナントへ影響 | テナント別レート/バウンス/苦情監視、抑制 | L | H | M |
| R-004 | 他テナントへのデータ漏えい | 重大インシデント | テナント分離、最小権限、テスト・監査 | L | H | L |
...
| R-014 | 運営者(顧客管理システム側)による高権限操作のテナントへの影響波及(誤った復元・配信・サスペンション) | テナント業務停止、エンドユーザー混乱 | 運営者操作の管理者ユーザーへの即時通知(FR-211)、運営者監査ログのテナント側参照可能化(NFR-306)、操作前再認証(FR-222)、顧客管理 §6.2.3 のハードゲート / 承認ログによる統制 | M | H | M |
| R-015 | 運営者アカウントの侵害(MFA バイパス、IP 制限突破、認証情報漏えい) | 全テナントへの影響、データ漏えい、不正な復元・配信 | 運営者 MFA 必須、IP 許可リスト、運営者操作の即時通知、自己リセット禁止、4-eyes 原則対象操作の統制(FR-220, FR-221, FR-226, NFR-311)、**侵害疑義時は RB-004 Runbook に従い対応** | L | H | M |
...
| R-017 | AI モデル切替時の品質回帰(回答可能率低下、誤回答増加、矛盾率増加) | エンドユーザー体験劣化、テナント解約 | 回答可能率・解決率・矛盾率分布・PII 検出件数の時系列監視、回帰テストセット、即時ロールバック(FR-063, FR-065, FR-066, NFR-807 / メインシステム FR-059 と整合) | M | M | L |
...
| テナント分離 | テナント境界を認可・テストで担保 | 基本設計 §2.6 / §10 |
...
<a id="glossary-tenant"></a>
- **テナント** — 本サービスを契約している会社や団体ごとの「仕切り」です。他の会社のデータは見えません。
  - (技術的な定義)本サービスを契約する組織単位。課金・データ分離の単位。

<a id="glossary-admin"></a>
- **管理者ユーザー** — 契約した会社のなかで、本サービスの管理画面を使い、個別チャット対応も行う利用者です。
  - (技術的な定義)テナントに所属し、テナント内の全機能を操作できる利用者。個別チャット対応も管理者ユーザーが行う。

<a id="glossary-project"></a>
- **プロジェクト** — 1つの会社のなかで、製品ごと・サイトごとにFAQやログを分けて管理する「箱」です。
  - (技術的な定義)1テナント内でFAQ・ウィジェット・ログを分けて管理する単位(例:製品別、サイト別)。
...
- **ユーザー種別** — そのアカウントが本サービスでどの立場かを表すラベルで、`admin`(管理者ユーザー)・`end_user`(エンドユーザー)・`service_operator`(サービス運営者)の3種類があります。
  - (技術的な定義)アカウントが本サービス上で持つ立場の区分。`admin`(管理者ユーザー)、`end_user`(エンドユーザー)、`service_operator`(サービス運営者)の3種類。本サービスの認可は、テナント境界とユーザー種別を条件として適用する。
...
<a id="glossary-tenant-isolation"></a>
- **テナント分離** — 別の会社(=別のテナント)のデータが絶対に混ざらないように、データベースやログを仕切ることです。
  - (技術的な定義)複数テナントのデータが互いに参照・混在しないよう、論理的または物理的に隔離する設計上の方針。
...
#### 識別子
- **やさしい説明**: 対象を一意に区別するための値。問い合わせID、SCR-XXX、tenant_id など。
- **本書での使われ方**: 各種採番の総称。
...
#### テナント
- **やさしい説明**: 本サービスを契約する組織単位。データと課金はテナントごとに分離する。
- **本書での使われ方**: 認可・データ保持・課金の最小単位として全章で多用する。

#### テナント分離
- **やさしい説明**: あるテナントのデータを、別のテナントから一切参照できないように区切る設計。
...
- **本書での使われ方**: テナント・プロジェクト境界 + ユーザー種別を条件として適用する。
...
#### プロジェクト
- **やさしい説明**: 1テナント内で FAQ・ウィジェット・ログを分けて管理する単位。製品別・サイト別など。
...
- **本書での使われ方**: 削除請求対応(本書 FR-161 / 顧客管理システム側 FR-210、NFR-402)。

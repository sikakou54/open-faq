...
| D1 | 単一 D1 + owner_account_id 分離方式(要件 §20.2 確定)。FAQ、質問ログ、未解決質問、チャット、通知ログ、アカウント、課金、お知らせ、監査ログ、AI しきい値永続キャッシュ(`ai_threshold_persistent_cache`)、削除請求(`deletion_requests`)、サプレスリスト(`email_suppression_list`)、メール配信メタなど |
...
| SCR-017 | ユーザー管理(招待 / 権限変更 / 停止 / 削除) | オーナー / `users:manage` 保持メンバー | FR-015〜021, FR-015a〜c, FR-016a〜c, FR-018a〜b, FR-021a〜c, FR-333, FR-334, FR-335, FR-336〜338 | メイン | P0 |
...
| 区分 | 項目 | 種類 | 概要 |
|---|---|---|---|
| 入力 | サービス名 / 組織名 | テキストボックス | 必須。組織 / サービス名 |
...
| 区分 | 項目 | 種類 | 概要 |
|---|---|---|---|
| 表示 | 契約情報 | テキスト表示(編集不可) | 表示名、連絡先メール |
| 入力 | サービス名 / 組織名 | テキストボックス | 編集可能 |
...
注: データ削除モード(物理削除 / 匿名化)・利用目的文言・削除請求受付方法・規約同意状況・データエクスポート機能は機能自体は要件 §8.15 / NFR-704 / §7.8 等で MVP 機能として残るが、SCR-016 画面上では別画面(SCR-018 規約閲覧、SCR-026 個人情報削除受付)または別経路(退会フロー内のエクスポート)に分離する。本画面はアカウント設定(オーナー設定)の編集ハブとしての配置のみを示す。

#### 5.4.11 SCR-017 ユーザー管理(オーナー + メンバー)

本画面はオーナーおよび「ユーザー管理」(`users:manage`)権限フラグ保持メンバーがアクセスできる。それ以外のメンバーはサイドメニューに項目が表示されず、URL 直アクセス時は 403 でダッシュボードへ戻される。

| 区分 | 項目 | 種類 | 概要 |
|---|---|---|---|
| 表示 | 利用者(オーナー / メンバー)表示名 | テキスト表示 | - |
| 表示 | メールアドレス | テキスト表示 | - |
...
**鍵管理方針(§7.10 と整合)**: 削除完了時に **オーナー派生鍵を使った列単位暗号化** を採用。削除完了時に当該レコードを物理削除する。鍵自体は契約単位で年次ローテーション(NFR-321)、ローテ前の旧鍵で暗号化されたレコードは 60 日 dual-decrypt 期間内に削除完了する。
...
flowchart TD
  ...
  P8 --> P9[アカウント設定(オーナー設定)削除モードに従い 物理削除 / 匿名化]
...
| 暗号化(SCR-026 受付分) | AES-256-GCM 列単位暗号化、オーナー派生鍵(NFR-319 / §7.10) |
...
| 削除モード | アカウント設定(オーナー設定)の `data_deletion_mode`(`physical` / `anonymize`)に従う(§7.8) |
...
| 管理者ユーザーから(自契約全データ) | 退会フロー(SCR-024)で実行 | admin | admin + 再認証 |
...
| `GET /api/v1/data/deletion-requests` | admin | 一覧取得(自契約) |
...
| 削除請求 | エンドユーザーからの削除請求時は当該契約のモードに従う(本人意思での選択は提供しない) |
...
**tombstone 方式**: 保持期間経過後の削除、および GDPR 削除請求による契約スコープ削除時は **本文(actor / target / IP / payload)を物理削除しつつ、`log_id` / `prev_hash` / `current_hash` / `deleted_at` を保持** する。tombstone は同一の `current_hash` を保持するため、NFR-306 / NFR-602c / NFR-604 の日次再計算で「ハッシュチェーンの連続性」は維持される(削除済み区間も含めて整合検証可能)。tombstone 自体は監査ログの保持期間を超えても削除しない(チェーン破断防止)。
...
| 検索用ハッシュ | `requester_email_hmac`(HMAC-SHA256、オーナー派生鍵)で暗号化解除なしの突合を可能化 |
| 削除完了時 | 当該レコードを物理削除。GDPR 復元対象外(`gdpr_deleted=true`) |
| 鍵ローテーション | オーナー派生鍵は Master Key 年次ローテーション時に再 KDF。60 日 dual-decrypt 期間内に旧鍵で暗号化されたレコードは削除完了するか再暗号化(NFR-321) |
...
| localhost 例外 | 利用者管理画面で「開発モード」ON 時のみ `localhost` / `127.0.0.1` / `*.local` を一時許可(24 時間で自動 OFF) |
...
| ADMIN_USER_REGISTER | 「{サービス名} の管理者ユーザー登録を完了してください」 | 登録元オーナー、登録者名、有効期限 | 登録完了 URL(7 日有効) |
...
| 削除請求 PII | AES-256-GCM 列単位暗号化、オーナー派生鍵、削除完了時に鍵 + データ同時破棄(§7.10) |
...
NFR-602a / NFR-602b / NFR-602d の保持期間経過後の削除、および GDPR 削除請求による契約スコープ削除を行う際は、tombstone 方式に従う。
...
| **エンドユーザーのメールアドレス変更を新規問い合わせ扱いに固定(FR-082b)** | エンドユーザーがメールアドレスを変更したい場合は、新メールで新規問い合わせを行うものとする。過去案件は元のメールアドレスでのみ再入室可能(FR-083 / FR-084 と整合) | 削除請求 PII の暗号化境界・トークン HMAC のオーナー派生鍵境界を簡素化、name 変更による誤転送リスクを排除 |

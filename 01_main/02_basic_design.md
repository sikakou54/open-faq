...
| StatusBadge | FAQ 状態、案件状態、通知状態、契約状態 | DB 値 → 表示文言マッピング。**色 + アイコン + テキスト** の三重識別を必須とし、色のみに依存しない(FR-390) |
| ConfirmDialog | 削除、公開、退会など | 重要操作は再認証(FR-005)。削除系ダイアログには「この操作は取り消せません。誤って削除した場合はサポートにお問い合わせください」を必須表示(FR-354)。Primary ボタン文言は対象操作の具体動詞、Secondary は「キャンセル」固定(FR-357) |
| ErrorAlert | 入力エラー、通信エラー、処理エラー | エラーコード別文言。**原因 + 解決策** をセットで表現(FR-353) |
| Pagination | 一覧画面 | カーソル方式 + 件数表示「1-50 / 全 254 件」併記(FR-358) |
| EmptyState | データなし | アイコン + 説明文 + 次操作への CTA を必須(FR-351)。「データがありません」単独表示は禁止 |
| Toast | 成功・失敗通知 | 操作結果のフィードバック。右上固定、成功 4 秒・警告 6 秒・エラーは手動閉じ(FR-382) |
| FormField | 入力部品 | バリデーションメッセージ表示。必須印、補助文(label 下)、エラー文(欄下)を構成要素として持つ(FR-359) |
...
| UsageBar | 利用量バー(80% / 100% / 125%) | 80% で黄色、100% で赤、125% でアニメーション強調。色 + ラベル(「⚠ 80% 警告」等)を併記(FR-390) |

#### 5.3.2 UI/UX 共通要件で追加する部品(参照: §8.24〜§8.27)

| 部品 | 用途 | 主な仕様 |
|---|---|---|
| Breadcrumb | パンくず(画面の現在位置) | 認証フローを除く全画面に表示。最大 3 階層「ホーム / 親 / 現在」(FR-370) |
| PageHeader | 画面タイトル + 1 行説明文 | 全画面の最上部(ヘッダー直下)に配置。タイトルと説明文は SCR 別画面表を正本とする(FR-350) |
| QuickFilterChips | クイックフィルタ(よく使う条件) | 一覧画面の上部に配置。チップ選択で詳細フィルタを自動適用、URL クエリで状態保持。詳細フィルタは折り畳みパネル(FR-371) |
| AppliedFilterChips | 適用済フィルタ表示 + クリア | 検索条件適用時にチップ表示。「すべてクリア」リンクを常設(FR-371) |
| BulkActionBar | 一括操作バー(複数選択時のみ表示) | 1 件以上選択時に画面下部固定で表示。「○件選択中: 操作1 / 操作2 / クリア」(FR-372)。一括操作上限は FR-323 を準用 |
| SummaryCard | 数値サマリーカード(ダッシュボード) | 数字 + 単位 + 補助バー(UsageBar 等)+ ラベル の 4 要素構成。SCR-015 等で使用 |
| Timeline | 段階・フローの可視化 | 縦タイムライン、現在位置を強調(`aria-current="step"`)。SCR-024 退会フロー、SCR-013 自動クローズ段階等で使用 |
| AutosaveIndicator | 自動保存状態の表示 | 緑点「30 秒前に保存しました」/ 黄点「保存中…」/ 赤点「保存できませんでした」(FR-321 連動) |
| UnsavedChangesGuard | 未保存変更時の離脱警告 | フォーム編集中に未保存変更があるとき、ブラウザの戻る・タブ閉じ・リロード時に警告ダイアログ表示(FR-373) |
| PermissionTooltip | 権限不足時のグレーアウト + 説明 | 操作ボタンに権限がない場合、ボタンをグレーアウトし tooltip で「○○権限が必要です」と表示(FR-356)。UI 非表示はサイドメニュー除外時のみ |
| LoadingSkeleton | スケルトン UI(初期データ取得時) | テーブルは行スケルトン、カードはカードスケルトン形状。スピナー単独表示を避ける(FR-380) |
| ProgressText | 進捗テキスト(3 秒超の操作) | 「保存中…」「処理中…(2/5 段階)」など、ユーザーが処理停止と誤認しない粒度で表示(FR-381) |
| ReauthBadge | 再認証必須操作の可視化 | Primary ボタン横に 🛡 アイコン + tooltip「再認証が必要」を表示(FR-360)。再認証必須操作一覧は詳細設計を正本 |

#### 5.3.3 文言の共通基準(参照: §8.24)

詳細設計の文言一覧を正本とするが、設計レベルの基準として以下を固定する:

| 種別 | テンプレ | 例 |
|---|---|---|
| 画面タイトル | 名詞(機能名) | 「未解決質問」「FAQ 管理」「利用状況」 |
| 画面説明文 | 動詞句(1 行) | 「ウィジェットで回答できなかった質問を確認・対応します」 |
| ボタン(主操作) | 具体動詞 + する | 「削除する」「招待メールを送信」「変更を保存」 |
| ボタン(取消) | 「キャンセル」固定 | — |
| 確認モーダル タイトル | 「{動詞}しますか?」 | 「FAQ を公開しますか?」「メンバーを削除しますか?」 |
| 確認モーダル 本文 | 操作結果 + 取消可否 | 「公開するとウィジェット利用者に表示されます。」/ 「削除すると元に戻せません(運営者への復元依頼が必要です)。」 |
| エラー文(入力) | 原因 + 修正例 | 「メールアドレスの形式が正しくありません(例: name@example.com)」 |
| エラー文(通信) | 原因 + 再試行案内 | 「ネットワークの一時的な問題のため保存できませんでした。少し時間を置いて再度お試しください」 |
| 成功トースト | 完了操作 + (任意で)次の導線 | 「FAQ を公開しました。ウィジェットで利用可能です。[FAQ 一覧へ]」 |
| 空状態 | 状況説明 + 次の行動 | 「FAQ がまだありません。最初の FAQ を作成しましょう。[+ 新規作成]」 |
| 権限不足 | 必要権限 + 解決方法 | 「この操作には FAQ 管理権限が必要です。オーナーまたはユーザー管理権限保持者に依頼してください」 |

**禁則語(単独使用を禁止)**: 「OK」「実行」「処理する」「確定」「送信」「こちら」「エラーが発生しました」「不正な値です」「失敗しました」(FR-352 / FR-353)。代替表現は詳細設計の文言一覧を参照。
...
タッチターゲットは PC 36×36px / SP 44×44px 以上、隣接要素間 8px 以上の余白を確保(FR-392)。
...
**ページヘッダ(参照: FR-350)**:
- 段階 1: タイトル「パスワードの再設定」/ 説明「ご登録のメールアドレスに再設定リンクをお送りします」
- 段階 1 送信後: タイトル「メールを送信しました」/ 説明「メールに記載のリンクから再設定を続けてください(リンクは 1 時間有効)」
- 段階 2: タイトル「新しいパスワードを設定」/ 説明「12 文字以上、英大文字・小文字・数字・記号のうち 3 種類以上を含めてください」

**画面構成**: 段階 1 → 「メール送信済み」中間表示 → 段階 2 の 3 ステップを Timeline で可視化。

| 区分 | 項目 | 種類 | 概要 |
|---|---|---|---|
| 表示 | ステップタイムライン | Timeline | 「① メールアドレス入力 → ② 受信メールのリンクをクリック → ③ 新しいパスワードを設定」を縦タイムラインで表示。現在ステップを `aria-current="step"` で強調(FR-350) |
| 段階 1 入力 | メールアドレス | メールアドレスボックス | 必須、形式チェック |
| 段階 1 操作 | 「再設定リンクを送信」 | ボタン(Primary) | リクエスト発行(存在有無は同一応答、列挙攻撃対策) |
| 段階 1 送信後 表示 | 完了画面 | 中間表示 | 「{メールアドレス} にメールを送信しました。1 時間以内にメールのリンクをクリックして再設定を完了してください」+ 「メールが届かない場合は迷惑メールフォルダもご確認ください。再送信は {N} 分後から可能です」 |
| 段階 1 送信後 操作 | 「メールを再送信する」 | ボタン(Secondary、レート制限カウントダウン併記) | 5 分以内は disabled、残時間表示 |
| 段階 2 入力 | 新パスワード | パスワードボックス + **強度メーター** | 必須。FR-006 強度要件(12 文字以上 + 3 種類以上)。入力中にリアルタイムで強度 5 段階(極弱 / 弱 / 中 / 強 / 極強)+ 不足要件を表示 |
| 段階 2 入力 | 新パスワード(確認) | パスワードボックス | 必須。一致確認。一致時に緑チェック、不一致時にエラー表示 |
| 段階 2 操作 | 「新しいパスワードを設定する」 | ボタン(Primary) | パスワード更新 → 全セッション失効 → 完了画面 → 自動的に SCR-001 へ |
| 段階 2 表示 | トークン無効 / 期限切れエラー | アラート帯(画面上部固定) + 再送 CTA | 段階 2 でリンク不正の場合、「再設定リンクが期限切れ、または無効です(有効期限 1 時間)。新しいリンクを再送してください」+ 「再送する」ボタンを直結配置(FR-453) |
...
|---|---|
| ステップ 1 | 「① メールアドレスを入力」 |
| ステップ 2 | 「② 受信メールのリンクをクリック」 |
| ステップ 3 | 「③ 新しいパスワードを設定」 |
| Primary ボタン(段階 1) | 「再設定リンクを送信」 |
| Primary ボタン(段階 2) | 「新しいパスワードを設定する」 |
| 強度メーター下の不足要件 | 「あと {N} 文字 / あと {種類数} 種類の文字種が必要です」 |
| トークン無効・期限切れ | 「再設定リンクが期限切れ、または無効です(有効期限 1 時間)。新しいリンクを再送してください」 |
| 完了メッセージ(`SuccessMessages.passwordReset`) | 「新しいパスワードを設定しました。ログインしてください」 |
| メール再送残時間 | 「メールを再送信する({N}分後から可能)」 |

#### 5.4.4 SCR-010 プロジェクト一覧

**ページヘッダ(参照: FR-350)**:
- タイトル「プロジェクト」
- 説明文「ウィジェットを設置するサイトごとのプロジェクトを管理します」

| 区分 | 項目 | 種類 | 概要 |
|---|---|---|---|
| 表示 | パンくず | Breadcrumb | 「ホーム / プロジェクト」(FR-370) |
| 表示 | プロジェクト ID | テキスト表示(テーブル列) | 内部 ID(短縮表示 + tooltip でフル表示) |
| 表示 | プロジェクト名 | テキスト表示(主) | カード型レイアウト時は見出しとして強調 |
| 表示 | 許可ドメイン | タグバッジ群 | 設定値を最大 3 件まで表示、超過分は `+N 件` で折り畳み(クリックで残り表示)。サブドメインワイルドカード `*.example.com` も表示 |
| 表示 | 連絡先メール | テキスト + 状態バッジ(色 + アイコン + テキスト) | 「✅ 確認済み」(緑)/「⏳ 確認待ち」(黄、常時表示で目立たせる)/「未設定」(灰)。色のみ依存禁止(FR-390) |
| 表示 | 更新日時 | テキスト表示(テーブル列) | 相対表記「3 時間前」+ tooltip で絶対日時 |
| 表示 | 件数表示 | テキスト | 「1-50 / 全 N 件」(FR-358) |
| 操作 | + 新規プロジェクトを作成 | ボタン(Primary) | SCR-010-M1 プロジェクト設定モーダルを「新規作成」モードで開く |
| 操作 | 編集 | ボタン(行アクション) | SCR-010-M1 プロジェクト設定モーダルを「編集」モードで開く |
| 操作 | 削除する | ボタン(Danger / 行アクション / 確認ダイアログ + 再認証 🛡) | 確認のうえ実施。管理画面からの復元手段は提供しない(FR-040b、サポート窓口経由)。再認証必須(FR-005 重要操作: プロジェクト削除はメンバーアクセス権・FAQ・チャットに影響) |
| 表示 | 空状態 | EmptyState | 「プロジェクトがまだありません。最初のプロジェクトを作成しましょう」+ 「+ 新規プロジェクトを作成」CTA(FR-351) |
| 表示 | ローディング状態 | LoadingSkeleton | テーブル/カード行スケルトン 3 行(FR-380) |
...
| 用途 | 文言 |
|---|---|
| 連絡先メール状態(確認待ち) | 「⏳ 確認待ち — このプロジェクトは確認完了まで連絡先メールを表示しません」 |
| 削除確認モーダル(`ConfirmDialogs.deleteProject`) タイトル | 「プロジェクトを削除しますか?」 |
| 同上 本文 | 「削除するとプロジェクト配下の FAQ・未解決質問・チャットも削除されます。元に戻せません(運営者への復元依頼が必要です)」 |
| 同上 Primary | 「削除する」(再認証必須 🛡) |
| 完了トースト(削除) | 「プロジェクトを削除しました」 |

#### 5.4.4a SCR-010-M1 プロジェクト設定モーダル(新規作成 / 編集)

**モーダルヘッダ(参照: FR-350)**:
- 新規作成: タイトル「新規プロジェクトを作成」/ 説明「ウィジェットを埋め込むサイトのドメインと連絡先メールを設定します」
- 編集: タイトル「プロジェクト設定 — {プロジェクト名}」/ 説明「プロジェクトの設定を変更します」

SCR-010 の「+ 新規プロジェクトを作成」または行アクション「編集」から開く全画面割込みモーダル。新規作成 / 編集の双方を同一画面で扱う(モード切替で見出し・説明文・初期値・CTA を分岐)。本モーダルは独立した画面 ID を持ち、サイドメニューには表示しない。

| 区分 | 項目 | 種類 | 概要 |
|---|---|---|---|
| 表示 | モード見出し | 見出し | 新規作成「新規プロジェクトを作成」/ 編集「プロジェクト設定 — {プロジェクト名}」 |
| 入力 | プロジェクト名 *(必須) | テキストボックス | 1〜100 文字、placeholder「例: サポートサイト」、文字数カウンタ |
| 入力 | 許可ドメイン *(必須) | タグ入力(複数値) | placeholder「例: https://example.com、*.example.com」。Enter または カンマで 1 件追加、即時検証。完全一致 + `*.example.com` 形式可。IP アドレス・プロトコル指定は不可(要件 §11.2) |
| 表示 | 許可ドメイン補足ヘルプ | helper | 「ウィジェット埋め込みを許可するドメイン。サブドメインを許可するには `*.example.com` のように記載します」 |
| 入力 | プロジェクト連絡先メール | メールアドレスボックス | 任意(FR-033a)。placeholder「例: support@example.com」。設定時に確認メール送信 → リンククリックで `contact_email_verified_at` が設定されるまでウィジェット表示に利用しない。確認完了後は SCR-013 ウィジェットチャット上の「お問い合わせ先」表示(FR-033c)にのみ利用する。メール送信時の Reply-To には利用しない(本サービスからのメールは常に no-reply 送信) |
| 表示 | 連絡先メール 状態バッジ | バッジ | 編集モード時のみ。「✅ 確認済み」/「⏳ 確認待ち(残 N 時間)」/「未設定」 |
| 操作 | 確認メールを再送 | ボタン(Secondary、レート制限カウントダウン併記) | 編集モードかつ確認未完了のときのみ活性。再送時に旧確認トークンは失効する。レート制限(再送間隔・1 時間あたりの上限)は詳細設計 §6.7a を正本とする |
| 操作 | キャンセル | ボタン(Secondary) | 変更を破棄してモーダルを閉じる(連絡先メール変更による確認トークン送信を撤回するわけではない)。未保存変更があれば UnsavedChangesGuard で警告(FR-373) |
| 操作 | 新規作成: 「プロジェクトを作成」 / 編集: 「変更を保存」 | ボタン(Primary) | バリデーション通過時に値を保存。新規作成時はウィジェット公開キーの初回発行も実施。完了後モーダルを閉じて SCR-010 に戻る |

**呼出元**: SCR-010 行アクション「編集」/「+ 新規プロジェクトを作成」ボタン。URL は SCR-010 のままで、UI 上はモーダルオーバーレイとして提示する。

**主要文言(SCR-010-M1)**:

| 用途 | 文言 |
|---|---|
| 新規作成 Primary | 「プロジェクトを作成」 |
| 編集 Primary | 「変更を保存」 |
| 完了トースト(新規) | 「プロジェクトを作成しました。ウィジェット設定を確認してください」+ 「ウィジェット設定へ」リンク |
| 完了トースト(編集) | 「変更を保存しました」 |
| 連絡先メール確認案内 | 「連絡先メールに確認メールを送信しました。メールに記載のリンクから確認を完了するまで、ウィジェット上に連絡先は表示されません」 |
| 確認メール再送中 | 「確認メールを再送する({N}分後から可能)」 |
| 許可ドメイン バリデーション エラー | 「許可ドメインの形式が正しくありません(例: https://example.com、*.example.com)」 |

#### 5.4.5 SCR-011 未解決質問一覧 / 詳細

**ページヘッダ(参照: FR-350)**:
- タイトル「未解決質問」
- 説明文「ウィジェットで回答できなかった質問を確認・対応します」

**一覧画面**:

| 区分 | 項目 | 種類 | 概要 |
|---|---|---|---|
| 入力 | クイックフィルタチップ | チップグループ | 「自分の未対応」「期日切れ」「今日来た」「すべて」の 4 プリセット。選択で詳細フィルタを自動適用、URL クエリで状態保持(FR-371)。各チップに件数バッジ |
| 入力 | 詳細フィルタ(折り畳み) | 折り畳みパネル | 以下 3 項目を含む |
| 入力 | ┗ 状況フィルタ | チェックボックスグループ | 未解決 / 対応中 / 解決済み / 終了(派生値、§4.6) |
| 入力 | ┗ 管理者ユーザーフィルタ | プルダウン | 自分 / 未割当 / 特定管理者ユーザー |
| 入力 | ┗ 期間 | 日付ピッカー(from/to) | 発生日時 |
| 表示 | 適用済フィルタチップ | チップ + クリア | 検索条件適用時に表示、「すべてクリア」を常設(FR-371) |
| 表示 | 件数表示 | テキスト | 「1-50 / 全 248 件」(FR-358) |
| 操作 | CSV エクスポート | ボタン | フィルタ適用結果をエクスポート |
| 表示 | 問い合わせ ID | テキスト表示(テーブル列) | `inquiry_code` |
| 表示 | 状況(主バッジ) | バッジ(色 + アイコン + テキスト) | 派生値(未解決 / 対応中 / 解決済み / 終了)。**色のみに依存しない**(FR-390) |
| 表示 | 質問文(抜粋) | テキスト表示(テーブル列) | 先頭 60 文字 |
| 表示 | 発生理由(副情報) | サブテキスト(グレー、行下) | `reason_code`(no_match / low_confidence / contradiction / user_not_resolved 等)を補助情報として 1 行下に表示 |
| 表示 | メール登録 | アイコン | 登録済み / 未登録 |
| 表示 | 最終投稿日時 | テキスト表示 | 相対表記(「3 分前」「2 時間前」)+ tooltip で絶対日時 |
| 表示 | 担当管理者ユーザー | テキスト表示 | - |
| 操作 | 詳細 | ボタン(行アクション) / 行クリック | 詳細画面へ |
...
| 表示 | 空状態 | EmptyState | 「未解決質問はありません。ウィジェットを設置済みなら正常な状態です。」+ 「ウィジェット設定を見る」リンク(FR-351) |
| 表示 | ローディング状態 | LoadingSkeleton | テーブル行スケルトン 5 行(FR-380) |
| 表示 | ページング | カーソル方式 | - |

**詳細画面**:

詳細画面は **左 6 割: 質問 + チャット / 右 4 割: 候補 FAQ + 状態管理 + 履歴** の 2 ペイン構成(SP では縦積み + タブ「会話 / 状態」)。

| 区分 | 項目 | 種類 | 概要 |
|---|---|---|---|
| 表示 | パンくず | Breadcrumb | 「ホーム / 未解決質問 / {inquiry_code}」(FR-370) |
| 表示 | ページタイトル | PageHeader | 「未解決質問の対応 — {inquiry_code}」+ 部屋状態バッジ |
| 表示 | 問い合わせ ID | 見出し | `inquiry_code` |
| 表示 | 元質問 | 本文ブロック | 全文 |
| 表示 | 回答不可理由 | バッジ + 補足テキスト | `reason_code`(信頼度値などの補足を併記) |
| 表示 | チャット履歴 | パネル(左ペイン) | SCR-013 のサブ表示(メッセージ履歴 + 返信入力欄) |
| 表示 | 候補 FAQ | リンク一覧(右ペイン) | 関連性の高い既存 FAQ |
| 表示 | 案件状態バッジ | バッジ | `case_status`(4 値、§4.3) |
| 表示 | チャット対応 | バッジ + 日時 | 返信済み(初回返信日時付き) / 未返信 |
| 表示 | 状態履歴(折り畳み) | アコーディオン | 状態変更ログ(FR-077)。デフォルト折り畳み |
...
| 操作 | **その他の操作** ▼(折り畳み) | アコーディオン | 危険・低頻度操作をまとめる(誤クリック防止) |
| 操作 | ┗ **対応不要として終了**(admin のみ) | ボタン(確認ダイアログ) | 終了理由(必須・最大 500 文字)入力 → `case_status=closed`(FR-079 / FR-354) |
| 操作 | 一覧へ戻るリンク | Link | SCR-011 一覧へ |
| 表示 | 終了情報 | テキスト表示 | `case_status=closed` のとき終了日時・操作者・理由を表示 |
| 表示 | 権限不足表示 | PermissionTooltip | chat:respond なしのメンバーは返信欄をグレーアウト + tooltip「チャット対応権限が必要です」(FR-356) |

**主要文言(SCR-011)**:

| 用途 | 文言 |
|---|---|
| クイックフィルタ | 「自分の未対応」「期日切れ」「今日来た」「すべて」 |
| 状況バッジ | 🔴 未対応 / 🟡 対応中 / 🟢 解決済み / ⚫ 終了 / 🟣 FAQ 化済み |
| 「対応不要として終了」確認モーダル タイトル | 「対応不要として終了しますか?」 |
| 同上 本文 | 「終了するとエンドユーザーに通知が送信され、再オープンには運営者への依頼が必要になります。」 |
| 同上 必須入力 | 「終了理由」(最大 500 文字) |
| 同上 Primary | 「対応不要として終了する」 |
| 同上 Secondary | 「キャンセル」 |
| 完了トースト | 「対応不要として終了しました。エンドユーザーへ通知を送信しています。」 |
| 空状態 | 「未解決質問はありません。ウィジェットを設置済みなら正常な状態です。」 |
| 権限不足 | 「閲覧のみ可能(返信権限がありません)。担当者へ権限付与をご依頼ください。」 |

#### 5.4.6 SCR-012 FAQ 管理(一覧 / 編集 / インポート / 下書き生成)

> **詳細設計での画面分割**: 詳細設計 §6.10「SCR-012 FAQ 管理一覧」/ §6.11「SCR-012 FAQ 編集 / AI 下書き生成」の 2 サブ画面に分割して実装する(URL ルーティング `/faqs` と `/faqs/:id/edit` で完全分離)。本書は SCR-012 を 1 ブロックで定義する。

**ページヘッダ(参照: FR-350)**:
- 一覧: タイトル「FAQ」/ 説明文「ウィジェットで自動回答される FAQ を管理します」
- 編集: タイトル「FAQ 編集」または「新規 FAQ 作成」/ 説明文(編集): 「{FAQ ID} の内容を編集します。30 秒ごとに自動保存されます」

**一覧画面**:

| 区分 | 項目 | 種類 | 概要 |
|---|---|---|---|
| 表示 | 状態サマリチップ | チップグループ | 「公開中 X」「下書き Y」「非公開 Z」をクリックで状態フィルタ即適用(FR-371) |
| 入力 | キーワード検索 | テキストボックス | 質問文・回答文の D1 FTS5 全文検索(FR-300) |
| 入力 | カテゴリフィルタ | プルダウン | プロジェクト内のカテゴリ |
| 入力 | 並び順 | プルダウン | 関連度 / 更新日時 / 作成日時 |
| 表示 | 適用済フィルタチップ | AppliedFilterChips | 検索条件適用時にチップ表示 + 「すべてクリア」(FR-371) |
| 表示 | 件数表示 | テキスト | 「1-50 / 全 124 件」(FR-358) |
| 表示 | FAQ ID | テキスト表示(テーブル列) | - |
| 表示 | 質問文(抜粋) | テキスト表示 | 先頭 60 文字 |
| 表示 | カテゴリ | テキスト表示 | - |
| 表示 | 状態バッジ | バッジ(色 + アイコン + テキスト) | 状態別。色のみ依存禁止(FR-390) |
| 表示 | 登録元 | テキスト表示 + リンク | 未解決質問起源の場合に表示(BR-016 / FR-048) |
| 表示 | 更新日時 | テキスト表示 | 相対表記 + tooltip で絶対日時 |
| 操作 | + 新規作成 | ボタン(Primary) | 編集モードへ |
| 操作 | 編集 | ボタン(行アクション) / 行クリック | 編集モードへ |
| 操作 | 公開 / 非公開切替 | トグルスイッチ | 状態変更(admin のみ、自動公開禁止 FR-104) |
| 操作 | 削除 | ボタン(行アクション、確認ダイアログ) | 論理削除。管理画面からの復元手段は提供しない |
| 操作 | 一括操作バー | BulkActionBar | 1 件以上選択時に下部固定で表示。「○件選択中: 公開する / 非公開化 / 削除する / クリア」上限 50 件(FR-323 / FR-372) |
| 操作 | CSV インポート | ボタン → モーダル | CSV / JSON インポート(FR-310)。競合処理(スキップ / 上書き / 別件追加)を選択 |
| 操作 | CSV エクスポート | ボタン | CSV / JSON エクスポート(FR-311) |
| 表示 | インポート進捗 | ProgressText + プログレスバー | 100 件超は非同期ジョブ化、24h タイムアウト(FR-381) |
| 表示 | エラーログダウンロード | リンク | 失敗レコードの行番号・理由を含む CSV(FR-310) |
| 表示 | 空状態 | EmptyState | 「FAQ がまだありません。最初の FAQ を作成しましょう。」+ 「+ 新規作成」「CSV をインポート」のダブル CTA(FR-351) |
| 表示 | ローディング状態 | LoadingSkeleton | テーブル行スケルトン 5 行(FR-380) |

**編集モード**:

編集モードは **左 7 割: 質問・回答エディタ / 右 3 割: AI 下書きパネル + メタ情報** の 2 ペイン構成。SP では縦積み + 「AI 下書き生成」は下部アコーディオン。

| 区分 | 項目 | 種類 | 概要 |
|---|---|---|---|
| 表示 | パンくず | Breadcrumb | 「ホーム / FAQ / {FAQ ID} 編集」または「ホーム / FAQ / 新規作成」(FR-370) |
| 表示 | 自動保存インジケータ | AutosaveIndicator | ヘッダ右端に「30 秒前に保存しました」/ 「保存中…」/ 「保存できませんでした」(FR-321) |
| 入力 | 質問文 *(必須) | テキストエリア | 1〜500 文字(FR-046)。下部に文字数カウンタ「142 / 500」 |
| 入力 | 回答文 *(必須) | リッチテキストエリア | 1〜5,000 文字(FR-046)。簡易ツールバー(太字/箇条書き/リンク) |
...
| 操作 | キャンセル | ボタン(Secondary) | 一覧へ戻る。未保存変更があれば UnsavedChangesGuard で警告(FR-373) |
| 操作 | 下書き保存 | ボタン | `status=draft` で保存 |
| 操作 | 削除する | ボタン(Danger / 確認ダイアログ) | 論理削除。再認証は不要(FR-005 対象外)だが、確認ダイアログで「元に戻せません」を明記(FR-354) |
| 操作 | 公開する | ボタン(Primary / 確認ダイアログ) | `status=published` で保存(admin のみ)。再認証不要(FR-005 対象外)。誤公開時は同画面から「非公開化」可能 |
| 操作 | **AI 下書きパネル**(右ペイン)(FR-104) | パネル(折り畳み可) | 元未解決質問を選択 → 「下書きを生成する」→ プレビュー → 「採用する」「やり直す」。生成結果は `draft` 状態で保存(`published` への直接遷移なし、§4.2.1 ガード)。プロンプトテンプレートは運営者管理(FR-342) |
| 表示 | 楽観ロック衝突 | エラーアラート | `faqs.version` 不一致時に「他のユーザーが更新したため保存できませんでした。最新を確認してください」+ 「最新を表示」リンク(FR-320 / FR-353) |

**主要文言(SCR-012)**:

| 用途 | 文言 |
|---|---|
| 公開確認モーダル タイトル | 「FAQ を公開しますか?」 |
| 同上 本文 | 「公開するとウィジェット利用者(エンドユーザー)に表示されます。誤公開時は同画面から「非公開化」できます。」 |
| 同上 Primary | 「公開する」 |
| 削除確認モーダル タイトル | 「FAQ を削除しますか?」 |
| 同上 本文 | 「削除すると元に戻せません(運営者への復元依頼が必要です)。」 |
| 同上 Primary | 「削除する」 |
| AI 下書き 注意文 | 「生成結果は下書きとして保存されます。内容を必ず確認してから公開してください。」 |
| AI 下書き Primary | 「下書きを生成する」/ 採用後「採用する」 |
| 完了トースト(公開) | 「FAQ を公開しました。ウィジェットで利用可能です」 |
| 空状態 | 「FAQ がまだありません。最初の FAQ を作成しましょう。」 |

#### 5.4.7 SCR-013 個別チャット部屋

**ページヘッダ(参照: FR-350)**:
- 管理者側: タイトル「個別チャット — {問い合わせ ID}」/ 説明(状態に応じて変化)「利用者の返答待ち(あと {N} 日で自動クローズ次段階へ)」等
- エンドユーザー側: タイトル「お問い合わせ — {問い合わせ ID}」/ 説明「ご質問にお答えします」

**管理者 / admin 側**(一覧 + 部屋):

部屋画面は **左 6 割: メッセージ履歴 + 入力欄 / 右 4 割: 案件・自動クローズ操作 + 危険操作** の 2 ペイン構成(SP では縦積み + タブ「会話 / 状態」)。

| 区分 | 項目 | 種類 | 概要 |
|---|---|---|---|
| 表示 | パンくず | Breadcrumb | 「ホーム / 個別チャット / {問い合わせ ID}」(FR-370) |
| 表示 | 部屋状態バッジ | バッジ | `room_status`(open / closed)を画面上部に固定 |
| 表示 | **自動クローズ進捗バー** | Timeline + ProgressText | `reminder_state` 6 段階を「3/6 段階 (あと N 日で利用者最終確認)」のように抽象化表示。詳細(各段階の説明・残時間)は折り畳み(FR-381) |
| **一覧画面 (左ペインリストモード)** | | | |
| 入力 | プロジェクトフィルタ | プルダウン | 全体 / プロジェクト別 |
| 入力 | 対応状態フィルタ | チェックボックスグループ | `case_status` |
| 入力 | 部屋状態フィルタ | チェックボックスグループ | `open` / `closed` |
| 表示 | 問い合わせ ID 列 | テキスト表示 | - |
| 表示 | 案件状態列 | バッジ(色 + アイコン + テキスト) | `case_status`(色のみ依存禁止 FR-390) |
| 表示 | 自動クローズ段階列 | バッジ + 残時間 | `reminder_state` の段階番号(3/6 等)と次段階までの残時間 |
| 表示 | 件数表示 | テキスト | 「1-50 / 全 N 件」(FR-358) |
| 表示 | 空状態 | EmptyState | 「個別チャットはまだありません。エンドユーザーからの問い合わせが始まるとここに表示されます」(FR-351) |
| **部屋画面 (左ペイン: 会話)** | | | |
| 表示 | メッセージ履歴 | チャットメッセージリスト | 投稿者種別 / 本文 / 時刻、ページング、`aria-live="polite"` で読み上げ更新 |
| 表示 | **機密情報注意警告帯** | アラート帯(入力欄直上に常時表示) | 「機密情報(電話番号・クレジットカード番号・パスワード等)は入力しないでください」(FR-091)。色 + アイコン + テキストで FR-390 を満たす |
| 入力 | 本文 | テキストエリア | 必須。最大 2,000 文字(FR-090)、エンドユーザーは 10 件/分の投稿頻度制限 |
| 操作 | 送る | ボタン(Primary) | メッセージ保存 → 相手側に通知 Queue 投入 |
| **部屋画面 (右ペイン: 状態・危険操作)** | | | |
| 入力 | 案件状態 | プルダウン | `open` / `resolved` のみ(`closed` は専用ボタン) |
| 操作 | 変更を保存 | ボタン | 案件状態更新 |
...
| 操作 | 管理者ユーザー割当 | プルダウン | - |
| 操作 | **その他の操作 ▼ (危険操作セクション)** | アコーディオン | 以下の危険操作をまとめ、メインエリアからの誤クリック防止 |
| 操作 | ┗ 部屋を閉じる | ボタン(Danger / 確認ダイアログ / 再認証 🛡) | admin のみ。`room_status=closed`(FR-005 対象。`users:manage` 同等の重要操作) |
| 操作 | ┗ **部屋を再オープン**(admin のみ表示、closed 時のみ) | ボタン(確認ダイアログ / 再認証 🛡) | クローズから 30 日以内に限り同 `room_id` を再オープン(FR-088)。監査ログ記録 |
| 表示 | 部屋クローズ後 帯 | アラート帯(画面上部固定) | `room_status=closed` 時「この問い合わせは {終了日} に終了しました。再オープンは運営者へ依頼してください」(FR-351 系列の状態説明) |
...
| 表示 | **機密情報注意警告帯** | アラート帯(入力欄直上に常時表示) | 「機密情報(電話番号・クレジットカード番号・パスワード等)は入力しないでください」(FR-091) |
...
| 表示 | 終了案内 | アラート帯 | `room_status=closed` のとき投稿フォームを非表示、「この問い合わせは {終了日} に終了しました。新しい問い合わせを送信するには {project_name} のサポート窓口へお問い合わせください」 |
...
| 用途 | 文言 |
|---|---|
| 機密情報注意 | 「機密情報(電話番号・クレジットカード番号・パスワード等)は入力しないでください」 |
| 自動クローズ進捗バー | 「自動クローズまで: 残 N 日(現在 X/6 段階)」+ 「詳しい段階を表示 ▼」 |
| 部屋クローズ確認モーダル タイトル | 「この問い合わせを終了しますか?」 |
| 同上 本文 | 「終了するとエンドユーザーに通知され、再オープンは運営者への依頼が必要になります。」 |
| 同上 Primary | 「終了する」(再認証必須 🛡) |
| 部屋再オープン確認モーダル タイトル | 「この問い合わせを再オープンしますか?」 |
| 同上 本文 | 「再オープンするとエンドユーザーへ通知され、自動クローズタイマーが再開します。」 |
| 同上 Primary | 「再オープンする」(再認証必須 🛡) |
| 完了トースト(終了) | 「問い合わせを終了しました。エンドユーザーへ通知を送信中です」 |
| 部屋クローズ後表示 | 「この問い合わせは {終了日} に終了しました。再オープンが必要な場合は運営者へご依頼ください」 |
| 「利用者待ち」判定 | ボタン: 「利用者待ち」(tooltip: 「利用者の返答待ちと判定し、自動クローズタイマーを進めます」) |

#### 5.4.8 SCR-014 ウィジェット設定

**ページヘッダ(参照: FR-350)**:
- タイトル「ウィジェット設定」
- 説明文「サイトに埋め込むウィジェットの公開キー・見た目・配置を設定します」

**画面構成**: 左 50% に設定パネル、右 50% に **プレビュー(デバイス切替可)** を固定配置。SP では縦積み + プレビューは折り畳み。

| 区分 | 項目 | 種類 | 概要 |
|---|---|---|---|
| 表示 | パンくず | Breadcrumb | 「ホーム / ウィジェット設定」(FR-370) |
| **(1) 公開キー管理セクション** | | | |
| 表示 | ウィジェット公開キー | テキスト表示 + コピーボタン | 形式 `pk_live_<32-char base62>`(FR-194)。参照のみ。コピー成功時に **緑チェックアイコン** + トースト「公開キーをコピーしました」 |
| 表示 | キー有効期限残日数 | カウントダウン表示 | 残時間に応じて色変化(残 30 日以上: 通常 / 残 7-30 日: 黄 / 残 7 日以内: 赤)。色 + 数値 + テキスト 3 重識別(FR-390) |
| 入力 | 公開キー有効期限 | プルダウン | 7 日 / 30 日 / 90 日 / 180 日 / 1 年、無期限不可、デフォルト 1 年(FR-194) |
| 操作 | **有効期限を 1 年延長する** | ボタン(Secondary) | 期限の残日数を基に「現在の残日数 + 1 年」を新有効期限として設定。1 クリックで延長(ローテーション不要) |
| 表示 | 旧キー使用中バッジ | バッジ + 注意文 | ローテーション猶予 30 日中に旧キー使用検知時に表示。「旧公開キーがまだ使われています。{N} 件のアクセスを検知(最終 {timestamp})」 |
| 操作 | 公開キーを再発行(ローテーション) | ボタン(Danger / 確認ダイアログ + 再認証 🛡) | 新キー発行と同時に旧キーは 30 日猶予で失効予告状態へ(FR-193)。確認モーダルで「新キーをサイトに反映するまで旧キーは 30 日間有効です」と明記 |
| **(2) 見た目セクション** | | | |
...
| **(3) プレビュー(右ペイン固定)** | | | |
| 表示 | プレビュー | プレビューウィンドウ(右ペイン 50%) | 設定変更時にリアルタイム反映 |
| 入力 | プレビューデバイス切替 | タブ(PC / Tablet / SP) | プレビューサイズを 3 段階で切替(360px / 768px / 1200px) |
| 表示 | 「Powered by」表示 | 注釈 | 本サービス運営者ロゴを必須表示(FR-152) |
| **(4) 埋め込みコードセクション** | | | |
| 表示 | 埋め込みコード | コードブロック + コピーボタン | コピー用に表示。コピー成功時に緑チェックアイコン + トースト「埋め込みコードをコピーしました」 |
| 操作 | 設定の保存 | ボタン(Primary、ヘッダー右に固定) | 設定更新 + KV キャッシュ無効化 |
| 表示 | 完了トースト | トースト | 「ウィジェット設定を保存しました。サイト側に反映されるまで最大 60 秒かかります」 |

**主要文言(SCR-014)**:

| 用途 | 文言 |
|---|---|
| 公開キーコピー成功 | 「公開キーをコピーしました」 |
| 埋め込みコードコピー成功 | 「埋め込みコードをコピーしました」 |
| キー有効期限 残 7 日以内 | 「⚠ 公開キーの有効期限が残り {N} 日です。有効期限を延長するか、再発行してください」 |
| 旧キー使用警告 | 「旧公開キーがまだ使われています。{N} 件のアクセスを検知(最終 {timestamp})」 |
| ローテーション確認モーダル タイトル | 「公開キーを再発行しますか?」 |
| 同上 本文 | 「新しいキーが発行されます。旧キーは 30 日間有効ですが、サイト側で新キーへの差し替えを早めに行ってください」 |
| 同上 Primary | 「公開キーを再発行する」(再認証 🛡) |
| 完了トースト(保存) | 「ウィジェット設定を保存しました。サイト側に反映されるまで最大 60 秒かかります」 |

#### 5.4.9 SCR-015 利用状況・課金ダッシュボード

**ページヘッダ(参照: FR-350)**:
- タイトル「利用状況」
- 説明文「お使いの量と料金、契約状況を確認できます」

| 区分 | 項目 | 種類 | 概要 |
|---|---|---|---|
| 入力 | 期間選択 | プルダウン + 日付ピッカー | 当月 / 前月 / 任意期間 |
| 表示 | **TrialBanner(常時表示)** | バナー | トライアル中は常時表示(残日数バッジ)。残 3 日以内は赤強調 + 「支払い方法を登録」CTA |
| 表示 | サスペンション中の表示 | アラート | `accounts.contract_status=suspended`(オーナー行) のとき「現在ご利用いただけません」+ 支払方法更新導線(§6.2.7) |
| 表示 | **サマリカードグリッド** | SummaryCard × 6(PC 6 列 / Tablet 3 列 / SP 2 列) | (1)質問数 (2)解決数 (3)未解決数 (4)FAQ 件数 (5)チャット部屋数 (6)AI 利用コスト。各カード下に小型 UsageBar + 状態ラベル併記 |
| 表示 | UsageBar 状態ラベル | テキスト + 色 | 〜60%: 表示なし / 61〜80%: 「⚠ もうすぐ予算到達(残 N 件)」(黄) / 81〜100%: 「🔴 月次予算に到達」(赤) / 101〜125%: 「🚨 月次予算超過(超過分は次月請求)」(濃赤)。色 + アイコン + テキストの三重識別(FR-390) |
| 表示 | `metering_billable=false` 件数 | カード(サマリと別行) | 失敗時の課金対象外件数(要件 §8.11.1) |
| 表示 | **料金プランカード** | カード | 質問数: ¥0〜10,000 件/月 (無料枠) + ¥0.5/件 (超過分)、FAQ 件数: 8,000 件まで(警告) / 12,000 件まで(拒否)、チャット部屋: 200 件/月まで。文章ではなくレイアウト化(§8.11.1) |
| 表示 | 月次予算上限 | テキスト表示 | 管理者ユーザー設定値(FR-121) |
| 操作 | 予算上限を変更する | ボタン(再認証必須 🛡、FR-005) | 設定変更画面へ |
| 表示 | 請求状態 | バッジ(色 + アイコン + テキスト) | `active` / `trialing` / `past_due` / `canceled` / `suspended` |
| 表示 | 警告アラート(画面上部) | アラートバナー | 上限接近(80%)/ 超過(100%)/ 追加制限(125%)時の三段階表示(FR-122)。具体的アクション提示(「予算上限を変更する」リンク併記) |
| 表示 | 請求履歴 | テーブル(折り畳み可) | 月別履歴、月毎の請求書 PDF リンク |
| 操作 | **最新の請求書 PDF を開く** | ボタン(画面上部、目立つ位置) | 最新月の請求書 PDF を別タブ表示(R2 署名 URL、有効期限 5 分) |
| 操作 | 支払い方法を変更する | リンク | Stripe Customer Portal へ遷移 |
| 操作 | しきい値・予算アラート設定変更 | ボタン → 設定変更画面 | 再認証必須(FR-005) |
| 操作 | 契約を解約する | ボタン(Danger、画面最下部、確認ダイアログ + 再認証) | 解約フローへ。誤クリック防止のため画面最下部の「契約管理」セクション(折り畳み)に配置 |
| 表示 | 空状態(請求履歴) | EmptyState | 「請求履歴はまだありません。トライアル終了後に最初の請求が発生します。」(FR-351) |

**主要文言(SCR-015)**:

| 用途 | 文言 |
|---|---|
| TrialBanner(残 4 日以上) | 「🎁 無料トライアル 残 N 日 / 30 日 — トライアル中は全機能が無料です」(青背景) |
| TrialBanner(残 3 日以内) | 「⚠ 無料トライアル 残 N 日 — 支払い方法を登録しないと自動停止します」(赤背景) |
| UsageBar 80% 警告 | 「⚠ もうすぐ予算到達(残 N 件で月次予算上限)」 |
| UsageBar 100% 到達 | 「🔴 月次予算に到達しました」 |
| UsageBar 125% 超過 | 「🚨 月次予算超過(超過分は次月請求)」 |
| 解約確認モーダル タイトル | 「契約を解約しますか?」 |
| 同上 本文 | 「{cutoffDate} までサービスをご利用いただけます。データのエクスポートは退会画面から実施してください。」 |
| 同上 Primary | 「契約を解約する」(再認証必須 🛡) |

#### 5.4.10 SCR-016 設定(退会・課金画面入口)

**ページヘッダ(参照: FR-350)**:
- タイトル「設定」
- 説明文「契約・データ保持・アクセス制限などの設定を変更します」

**画面構成**: 4 セクションに分割。誤操作防止のため危険操作セクションは画面最下部に分離:
1. 連絡先 / アカウント情報
2. データ保持(質問ログ / チャット履歴 / 通知ログ)
3. アクセス制限(IP 許可リスト)
4. **契約管理(危険操作・最下部に分離)**

| 区分 | 項目 | 種類 | 概要 |
|---|---|---|---|
| 表示 | パンくず | Breadcrumb | 「ホーム / 設定」(FR-370) |
| **(1) 連絡先 / アカウント情報** | | | |
| 表示 | 契約情報 | テキスト表示(編集不可) | オーナーアカウントの基本情報 |
| 入力 | 連絡先メール | メールアドレスボックス | 編集可能 |
| **(2) データ保持** | | | |
| 入力 | 質問ログ保持日数 | 数値入力 / プルダウン + helper | 初期値(365 日)以下のみ設定可。引き上げ不可(NFR-702、サーバ側で検証)。helper: 「設定日数を超えた質問ログは自動削除されます。引き下げのみ可能で、後から引き上げはできません」 |
| 入力 | チャット履歴保持日数 | 数値入力 / プルダウン + helper | 初期値以下のみ。helper: 「設定日数を超えたチャット履歴は自動削除されます」 |
...
| 操作 | **現在の IP を追加する** | ボタン(Secondary) | 操作中のクライアント IP を `<your-ip>/32`(IPv4)または `/128`(IPv6)として末尾に追加。CIDR 知識不要で導入のハードルを下げる |
| 表示 | 現在の IP 表示 | helper | 「あなたが現在使用している IP: {currentIp}({location_hint})」を常時表示(誤って自分を遮断するのを防止) |
| 表示 | 自己ロックアウト警告 | アラート(変更検知時) | 入力中の IP リストに「現在の IP」が含まれていない場合「⚠ あなたの現在の IP がリストに含まれていません。保存するとアクセスできなくなります」を即時表示 |
| 操作 | 設定を保存 | ボタン(Primary、ヘッダー右に固定) | 各項目を更新。IP 許可リスト変更時は確認ダイアログ + 再認証(FR-005 セキュリティ重要操作扱い) |
| **(4) 契約管理(最下部に分離)** | | | |
| 表示 | 退会案内 | 注意文 | 「退会するとサービスがご利用いただけなくなります。データのエクスポートは退会画面から実施できます」 |
| 操作 | 退会を申請する | ボタン(Danger、最下部) | → SCR-024 退会フローへ。リンクとボタンの中間で目立たせない |
...
| 用途 | 文言 |
|---|---|
| 質問ログ保持日数 helper | 「設定日数を超えた質問ログは自動削除されます。引き下げのみ可能で、後から引き上げはできません」 |
| IP 許可リスト プレースホルダ | 「例: 203.0.113.0/24(社内ネットワーク)、2001:db8::/32(IPv6 範囲)」 |
| IP 許可リスト 現在 IP 表示 | 「あなたが現在使用している IP: {currentIp}」 |
| 自己ロックアウト警告 | 「⚠ あなたの現在の IP がリストに含まれていません。保存するとアクセスできなくなります」 |
| IP 許可リスト変更確認モーダル タイトル | 「IP 許可リストを保存しますか?」 |
| 同上 本文 | 「保存後、IP 許可リスト外からはアクセスできなくなります。現在の IP({currentIp})がリストに含まれていることを必ず確認してください」 |
| 同上 Primary | 「IP 許可リストを保存する」(再認証 🛡) |
| 完了トースト(保存) | 「設定を保存しました」 |
...
| 区分 | 項目 | 種類 | 概要 |
|---|---|---|---|
| 表示 | **オーナー要約バナー(折り畳み可)** | 情報パネル | 通常表示は「オーナー: {表示名} ({メールアドレス}) [詳細 ▼]」のみ 1 行。展開時に「オーナーは契約 1 つにつき 1 人で、課金・退会・規約再同意を専有します。MVP では降格・停止・削除・譲渡はできません」を表示(FR-015a / FR-333) |
| 表示 | 権限フラグの説明セクション | 折りたたみパネル | 5 種フラグそれぞれの「できること」一覧と「ダッシュボードはどの権限も不要で利用可」を明示。常時折り畳み |
| 入力 | 検索 | テキストボックス | 表示名・メール部分一致 |
| 入力 | 状態フィルタ | プルダウン | すべて / 有効 / 停止中 / 招待中 |
| 表示 | 件数表示 | テキスト | 「1-50 / 全 N 件」(FR-358) |
| 表示 | 利用者(オーナー / メンバー)表示名 | テキスト表示(テーブル列) | 自分の行は「{表示名}(あなた)」と末尾に注釈 |
| 表示 | メールアドレス | テキスト表示(テーブル列) | - |
| 表示 | ロール | バッジ(色 + アイコン + テキスト) | `オーナー`(青・👑)または `メンバー`(灰)。色のみ依存禁止(FR-390) |
| 表示 | 状態 | バッジ(色 + アイコン + テキスト) | 「⏳ 招待中」「🟢 有効」「⚫ 停止中」 |
| 表示 | **権限フラグアイコン群** | アイコンバッジ × 5(色 + 形 + tooltip) | 📝 FAQ 管理 / 💬 チャット対応 / 👥 ユーザー管理 / ⚙ プロジェクト設定 / 📋 ログ参照。付与時はカラーアイコン、未付与は淡色グレー。各アイコン hover で `PermissionMessages.*` の説明を tooltip 表示。オーナー行は「👑 全権限(オーナー)」バッジ 1 つに集約 |
| 表示 | 割当プロジェクト | バッジ群 | 割当されたプロジェクト名を列挙。3 件超は `+N 件` で折り畳み。割当 0 は「ダッシュボードのみ」バッジ(灰)、オーナー行は「全プロジェクト」バッジ |
| 表示 | 最終ログイン日時 | テキスト表示 | 相対表記「3 時間前」+ tooltip で絶対日時 |
| 表示 | **招待中行強調** | 行背景色 | `pending_activation` 行は黄色背景。バッジ「⏳ 招待中(残 N 日)」、残 1 日以下は赤背景 + 「🔴 期限切れ間近(残 N 日)」(FR-016 / FR-020) |
| 操作 | + メンバーを招待 | ボタン(Primary) | SCR-017-M1 メンバー招待 / 編集モーダルを「招待」モードで開く |
| 操作 | 編集 | ボタン(行アクション) | メンバー行のみ。SCR-017-M1 を「編集」モードで開く。オーナー行は操作列を非表示(FR-333) |
| 操作 | 削除する | ボタン(Danger / 行アクション / 確認ダイアログ + 再認証 🛡) | メンバー行のみ。`accounts.deleted_at` を設定し論理削除する。`pending_activation` 行では招待取消として動作する。オーナー行は操作列を非表示(FR-333 / FR-005) |
| 表示 | 空状態 | EmptyState | 「メンバーがまだ招待されていません。メンバーを招待するとあなたと一緒に運用できます」+ 「+ メンバーを招待」CTA(FR-351) |
| 表示 | 権限不足ガード | PermissionTooltip | `users:manage` 権限なしのメンバーが URL 直アクセスした場合、403 → ダッシュボードへリダイレクト + トーストで「ユーザー管理権限が必要です」(FR-356) |

**主要文言(SCR-017)**:

| 用途 | 文言 |
|---|---|
| オーナーバナー(折り畳み時) | 「オーナー: {表示名} ({メールアドレス}) [詳細 ▼]」 |
| オーナーバナー(展開時) | 「オーナーは契約 1 つにつき 1 人で、課金・退会・規約再同意を専有します。MVP では降格・停止・削除・譲渡はできません」 |
| 権限フラグアイコン tooltip(FAQ) | 「FAQ の登録・編集・削除・公開ができます」 |
| 権限フラグアイコン tooltip(チャット) | 「未解決質問への返信・対応ができます」 |
| 権限フラグアイコン tooltip(ユーザー) | 「メンバーの招待・権限変更・停止ができます」 |
| 権限フラグアイコン tooltip(プロジェクト) | 「プロジェクト作成・編集・ウィジェット設定ができます」 |
| 権限フラグアイコン tooltip(ログ) | 「監査ログを閲覧できます(返信権はありません)」 |
| 削除確認モーダル タイトル | 「メンバーを削除しますか?」 |
| 同上 本文 | 「削除すると元に戻せません。アクセス権が即時失われ、再招待が必要になります。」 |
| 同上 Primary | 「削除する」(再認証必須 🛡) |
| 完了トースト(削除) | 「メンバーを削除しました」 |
| 完了トースト(招待取消) | 「招待を取り消しました。リンクは無効になりました」 |

#### 5.4.11a SCR-017-M1 メンバー招待 / 編集モーダル

SCR-017 の「+ メンバーを招待」または行アクション「編集」から開く全画面割込みモーダル。招待 / 編集の双方を同一画面で扱い、モード切替で見出し・背景色・CTA・初期値を 3 重に分岐する。本モーダルは独立した画面 ID を持ち、サイドメニューには表示しない。SP ではフルスクリーン表示 + Primary CTA 下部固定。

| 区分 | 項目 | 種類 | 概要 |
|---|---|---|---|
| 表示 | モード見出し | 見出し | 招待モード「メンバーを招待」、編集モード「メンバー編集 — {表示名}」 |
| 表示 | **自己編集警告帯**(編集モードかつ自分自身編集時のみ、画面上部固定) | アラート帯 | 「あなた自身を編集中: 権限・状態は変更できません」(FR-334) |
...
| 入力 | **権限テンプレ(プリセット)** | ラジオボタングループ | 「FAQ 担当者」「問合せ対応者」「プロジェクト管理者」「ログ参照のみ」「カスタム」の 5 択。プリセットを選択するとメンバー権限フラグが自動セット。「カスタム」選択時のみ下のチェックボックス群を表示。フロント側プリセット定義のみで、API は個別フラグの組合せを送信(MVP は要件レベルのロール概念を持たない FR-336 と整合) |
| 入力 | メンバー権限フラグ(カスタム時のみ展開) | チェックボックス × 5(アイコン付き) | `📝 FAQ 管理` / `💬 個別チャット対応` / `👥 ユーザー管理` / `⚙ プロジェクト設定` / `📋 ログ参照`(FR-015b)。各項目右に「この権限でできること」のヒントを併記。編集モードでは差分プレビュー(追加 / 剥奪)を併記 |
| 入力 | 割当プロジェクト | チェックボックス群 | プロジェクト一覧から 0 個以上を選択(FR-015d / FR-018c)。操作者自身が割当を持つ(またはオーナーである)プロジェクトのみ選択可 |
| 表示 | **割当 0 件選択時の即時警告** | アラート(チェックボックス群直下) | 「⚠ プロジェクト未選択の場合、このメンバーはダッシュボードのみ利用できます」を選択 0 件時に即時表示 |
| 表示 | 招待状態 | バッジ | 編集モード時のみ。「⏳ 招待中(残 N 日)」「🟢 有効」「⚫ 停止中」の現値表示(FR-016 / FR-020) |
| 表示 | **変更内容プレビューパネル** | パネル(モーダル下部、編集モード時のみ) | 差分を「変更前 → 変更後」形式で表示し、追加・剥奪が分かりやすい形にまとめる(例: 「💬 チャット対応 を剥奪」「📝 FAQ 管理 を追加」) |
| 操作 | 招待メールを送信 | ボタン(Primary、再認証必須 🛡) | 招待モードでのみ表示。`access_tokens.purpose='activation'`(7 日)を発行し招待メール送信(FR-015 / FR-016 / FR-020) |
| 操作 | 変更を保存 | ボタン(Primary、再認証必須 🛡) | 編集モードでのみ表示。表示名・通知設定・権限フラグ差分・割当プロジェクト差分を一括保存(FR-018 / FR-018a / FR-018c) |
| 操作 | 招待を再送 | ボタン(再認証必須 🛡) | 編集モードかつ `pending_activation` 行のみ。旧リンク失効・新リンク発行(FR-016b) |
| 操作 | 招待を取り消す | ボタン(Danger、確認ダイアログ、再認証 🛡) | 編集モードかつ `pending_activation` 行のみ。`access_tokens.purpose='activation'` 失効 + `accounts.status='disabled'` |
| 操作 | メンバーを停止 | ボタン(Danger、確認ダイアログ、再認証 🛡) | 編集モードかつ `active` 行のみ。`accounts.status='disabled'` 更新 + 全セッション失効(FR-019 / FR-333) |
| 操作 | 停止を解除 | ボタン(確認ダイアログ、再認証 🛡) | 編集モードかつ `disabled` 行のみ。`accounts.status='active'` 更新 |
| 操作 | 強制ログアウト | ボタン(Danger、確認ダイアログ、再認証 🛡) | 編集モードかつ `active` 行のみ。対象メンバーの全セッションを失効する(UC-052)。オーナー行では非表示(FR-333) |
| 操作 | キャンセル | ボタン(Secondary) | 変更を破棄してモーダルを閉じる。未保存変更があれば UnsavedChangesGuard で確認(FR-373) |
| ガード | 自己権限剥奪 | (UI 制御) | 自分自身の `users:manage` チェックを外す操作はモーダル上で非活性化 + tooltip「自分の権限は剥奪できません(他のユーザー管理権限保持者に依頼してください)」(FR-018a / FR-334 / FR-356) |
| ガード | 自己状態変更 | (UI 制御) | 自分自身を編集モードで開いた場合、「停止」「強制ログアウト」「招待取消」は非活性化 + 自己編集警告帯を表示(FR-334) |

**呼出元**: SCR-017 行アクション「編集」/「+ メンバーを招待」ボタン。URL は `/members` のままで、UI 上はモーダルオーバーレイとして提示する。

**権限テンプレ プリセット定義(参照: §5.3.3 文言の共通基準)**:
...
| 用途 | 文言 |
|---|---|
| 自己編集警告帯 | 「あなた自身を編集中: 権限・状態は変更できません」 |
| 権限テンプレ説明 | 「よく使う組み合わせから選べます。後から個別に調整できます。」 |
| 割当 0 件警告 | 「⚠ プロジェクト未選択の場合、このメンバーはダッシュボードのみ利用できます」 |
| 招待送信完了トースト | 「招待メールを送信しました。有効期限は 7 日間です」 |
| 招待再送完了トースト | 「招待メールを再送しました(旧リンクは失効しました)」 |
| 招待取消確認 タイトル | 「招待を取り消しますか?」 |
| 同上 本文 | 「取り消すと招待リンクが失効します。再招待には新しいリンクを発行する必要があります。」 |
| 同上 Primary | 「招待を取り消す」(再認証 🛡) |
| メンバー停止確認 タイトル | 「{name} を停止しますか?」 |
| 同上 本文 | 「停止中はログインできません。停止解除はいつでも可能です。」 |
| 同上 Primary | 「メンバーを停止」(再認証 🛡) |
| 強制ログアウト確認 タイトル | 「強制ログアウトしますか?」 |
| 同上 本文 | 「現在のセッションをすべて終了します。次のログイン時に再認証が必要です。」 |
| 同上 Primary | 「強制ログアウト」(再認証 🛡) |
| 自己権限剥奪ガード tooltip | 「自分の権限は剥奪できません(他のユーザー管理権限保持者に依頼してください)」 |

#### 5.4.12 SCR-018 プライバシーポリシー / 利用規約閲覧

**ページヘッダ(参照: FR-350)**:
- タイトル「プライバシーポリシー / 利用規約」
- 説明文「最新の利用規約とプライバシーポリシー、および過去の改訂履歴を確認できます」

**画面構成**: 上部に同意状態バッジ + 最新版見出し、下に **全文(目次付き)**、最下部に過去バージョン一覧(折り畳み)。

| 区分 | 項目 | 種類 | 概要 |
|---|---|---|---|
| 表示 | パンくず | Breadcrumb | 「ホーム / プライバシーポリシー・利用規約」(FR-370) |
| 表示 | **同意状態バッジ**(上部固定) | バッジ(色 + アイコン + テキスト) | 「✓ 同意済み({同意日})」(緑)/ 「⚠ 未同意」(赤、SCR-025 へのリンク併記) |
| 表示 | 現行版見出し | h2 | 「利用規約 vX.X(発効 {YYYY-MM-DD})」 |
| 表示 | 目次(全文の章構成) | ナビゲーション(右ペイン固定 or 上部、SP では折り畳み) | 全文の見出しをクリックで該当章へジャンプ。長文閲覧の補助 |
| 表示 | 全文 | リッチテキスト表示(sanitize 済) | 利用規約 / プライバシーポリシー |
| 表示 | **過去バージョン一覧**(折り畳み) | アコーディオン + 表 | 履歴を畳んで表示。展開時にバージョン番号 + 発効日 + 「全文を見る」リンク |
| 表示 | バージョン比較リンク | リンク | 過去バージョンと最新版の差分を別タブで表示(diff 形式、参考表示) |
...
|---|---|
| 同意済みバッジ | 「✓ 同意済み({同意日})」 |
| 未同意バッジ | 「⚠ 未同意 — 同意手続きを行ってください」+ 「同意手続きへ」リンク(SCR-025 へ) |
| 過去バージョン リンク | 「過去のバージョン履歴を表示」(クリックで折り畳み展開) |
| バージョン比較リンク | 「v{old} と v{new} の差分を見る」 |

#### 5.4.13 SCR-021 お知らせ一覧

**ページヘッダ(参照: FR-350)**:
- タイトル「お知らせ」
- 説明文「サービス運営者からの通知を確認します(未読 {N} / 全 {M})」

| 区分 | 項目 | 種類 | 概要 |
|---|---|---|---|
| 表示 | パンくず | Breadcrumb | 「ホーム / お知らせ」(FR-370) |
| 表示 | クイックフィルタチップ | QuickFilterChips | 「未読のみ」(**デフォルト選択**)「重要のみ」「課金」「お知らせ」「システム」「すべて」(FR-371) |
| 表示 | 適用済フィルタチップ | AppliedFilterChips | 適用条件 + 「すべてクリア」(FR-371) |
| 入力 | 詳細フィルタ(折り畳み) | 折り畳みパネル | 期間 / キーワード検索 |
| 表示 | 件数表示 | テキスト | 「1-50 / 全 24 件(未読 3 件)」(FR-358) |
| 表示 | **未読行の背景強調** | 行背景色 | 未読は薄い水色背景 + 行頭ドット(✓ 視認性向上) |
| 表示 | 未読インジケータ | 行頭ドット | アクセシビリティ用 `aria-label="未読"` |
| 表示 | 種別バッジ | バッジ(色 + アイコン + テキスト) | AnnouncementCategoryBadge(FR-390) |
| 表示 | 重要度 | アイコン + テキスト | `critical` 🔴+「重要」/ `high` 🟡+「重要」/ `normal` 無印 / `low` 淡色(FR-181 / FR-390) |
| 表示 | タイトル | テキスト表示 | 行クリックで SCR-022 へ。未読タイトルは bold |
| 表示 | 配信日時 | テキスト表示 | 相対表記「3 時間前」+ tooltip で絶対日時 |
| 操作 | 詳細表示 | 行クリック | SCR-022 へ。同時に該当行を既読化 |
| 表示 | **BulkActionBar(複数選択時のみ)** | 画面下部固定 | 1 件以上選択時に表示。「○件選択中: 既読化する / 選択を解除」最大 100 件(FR-323 / FR-372) |
| 操作 | 一括既読化 | ボタン(BulkActionBar 内) | 選択した行を既読化 |
| 操作 | **表示中の未読をすべて既読化** | ボタン(画面右上) | 現在のフィルタで表示中の未読のみを既読化。「{N}件を既読化しますか?」確認ダイアログ → 「{N}件を既読化する」(全件無条件既読ではなく **表示中のみ** に変更) |
| 操作 | すべての未読を既読化 | リンク(画面右上、副次的) | 念のため残すが、文言を「フィルタ無視で全未読を既読化」と明示。確認ダイアログ |
| 表示 | ページング | カーソル方式 | - |
| 表示 | 空状態 | EmptyState | 「お知らせはまだありません」/ 未読のみフィルタで 0 件時「未読のお知らせはありません」(FR-351) |
| 表示 | ローディング状態 | LoadingSkeleton | テーブル行スケルトン 5 行(FR-380) |
...
| 用途 | 文言 |
|---|---|
| クイックフィルタ | 「未読のみ」「重要のみ」「課金」「お知らせ」「システム」「すべて」 |
| 表示中の未読既読化ボタン | 「表示中の {N} 件を既読化」 |
| 全未読既読化リンク | 「すべての未読({M} 件)を既読化」 |
| 全既読化確認モーダル タイトル | 「{N} 件のお知らせを既読化しますか?」 |
| 同上 本文 | 「既読化したお知らせも、お知らせ一覧から引き続き閲覧できます」 |
| 同上 Primary | 「{N} 件を既読化する」 |
| 完了トースト | 「{N} 件のお知らせを既読化しました」 |
| 空状態(未読のみ) | 「未読のお知らせはありません」 |
| 空状態(全て) | 「お知らせはまだありません」 |
...
| 区分 | 項目 | 種類 | 概要 |
|---|---|---|---|
| 表示 | パンくず | Breadcrumb | 「ホーム / お知らせ / {タイトル(短縮)}」(FR-370) |
| 表示 | **メタ情報バー**(タイトル直下) | バッジ群 | 種別バッジ + 重要度バッジ + 配信日時 + 既読化日時(あれば) |
| 表示 | 種別バッジ | AnnouncementCategoryBadge(色 + アイコン + テキスト) | billing / announcement / system(FR-490) |
| 表示 | 重要度バッジ | ImportanceIndicator | `critical`(🔴)/ `high`(🟡)/ `normal`(無印)/ `low`(淡色)(FR-181 / FR-490) |
| 表示 | タイトル | h1 | - |
| 表示 | 配信日時 | テキスト表示 | 絶対日時 + 相対日時(「3 時間前」) |
| 表示 | 既読化日時 | テキスト表示 | 既読の場合のみ |
| 表示 | 本文 | リッチテキスト表示(sanitize 済 HTML) | §10.5.1 二重サニタイズ、許可タグ・属性ホワイトリスト |
| 表示 | **関連リンク**(本文末尾) | リンク一覧(目立つカード形式) | 種別に応じた遷移先を明示。`billing` → 「利用状況ダッシュボードへ」、`announcement` → 外部告知 URL、`system` → 「関連設定画面へ」 |
| 操作 | 自動既読化 | 自動 + 完了トースト | 詳細画面表示時に自動既読化(初回のみ。`read_at` がある場合は更新しない)。完了時に画面下部にトースト「このお知らせを既読化しました」を 4 秒表示(自動既読化の事実を可視化) |
| 操作 | 一覧へ戻る | リンク(画面下部 + パンくず) | SCR-021 へ |
| 操作 | 前のお知らせ / 次のお知らせ | リンク | 一覧の並び順に基づく前後のお知らせへ移動 |

**主要文言(SCR-022)**:

| 用途 | 文言 |
|---|---|
| 自動既読化トースト | 「このお知らせを既読化しました」 |
| 関連リンク(billing) | 「利用状況ダッシュボードを見る →」 |
| 関連リンク(announcement) | 「詳しくはこちら →」+ tooltip で URL ドメイン表示(信頼性確保) |
| 関連リンク(system) | 「関連設定画面を開く →」 |
| 前後ナビ | 「← 前のお知らせ」「次のお知らせ →」 |

#### 5.4.15 SCR-023 メール確認

**ページヘッダ(参照: FR-350)**:
- 送信済み状態: タイトル「メールを送信しました」/ 説明「{メールアドレス} に確認メールを送信しました。メールに記載のリンクから登録を完了してください」
- 確認成功状態: タイトル「メールアドレスの確認が完了しました」/ 説明「ログインしてサービスをご利用ください」
- 確認失敗状態: タイトル「確認リンクが無効です」/ 説明「リンクが期限切れか、すでに使用されています」

| 区分 | 項目 | 種類 | 概要 |
|---|---|---|---|
| 表示 | 状態タイムライン(参考表示) | Timeline | 「① 新規登録 → ② 確認メールのリンクをクリック → ③ ログイン」を縦タイムラインで表示。現在ステップは ② |
| 表示 | 送信先メールアドレス | テキスト表示(強調) | 確認メール送信先のメールアドレス |
| 表示 | 案内文 | テキスト表示 | 「確認リンクの有効期限は 24 時間です」+ 「メールが届かない場合は迷惑メールフォルダもご確認ください」 |
| 操作 | **メールを再送する** | ボタン(Secondary、レート制限カウントダウン併記) | 確認メール再送(レート制限 5 分以内 1 回まで)。残時間表示「再送する(あと {N} 分 {S} 秒)」、5 分以内は disabled |
| 操作 | メールアドレスを変更する | リンク(Secondary) | 入力間違いの場合に新規登録画面(SCR-002)へ戻るリンク |
| 表示 | リンククリック後の確認成功 | 完了画面 | アイコン ✅ + 「メールアドレスの確認が完了しました」+ 「ログインする」CTA |
| 表示 | リンククリック後の確認失敗(期限切れ・使用済み) | エラー画面 + 復旧 CTA | 「確認リンクが期限切れ、または使用済みです(有効期限 24 時間)」+ 「新規登録からやり直す」CTA(SCR-002 へ)+ 「メールアドレスを変更して再登録する」リンク |
...
| 用途 | 文言 |
|---|---|
| 送信済み画面 タイトル | 「メールを送信しました」 |
| 送信済み画面 本文 | 「{メールアドレス} に確認メールを送信しました。メールに記載のリンクから登録を完了してください(有効期限 24 時間)」 |
| 再送残時間中 | 「メールを再送する(あと {N} 分 {S} 秒)」 |
| 再送可能時 | 「メールを再送する」 |
| 確認成功画面 タイトル | 「メールアドレスの確認が完了しました」 |
| 確認成功画面 Primary | 「ログインする」 |
| 確認失敗画面 タイトル | 「確認リンクが無効です」 |
| 確認失敗画面 本文 | 「リンクが期限切れか、すでに使用されています(有効期限 24 時間)。メールアドレスをご確認のうえ、新規登録からやり直してください」 |
| 確認失敗画面 Primary | 「新規登録からやり直す」 |
| 再送上限超過 | 「メール再送の上限に達しました。{N} 分後にもう一度お試しください」 |

#### 5.4.16 SCR-024 退会申請

**ページヘッダ(参照: FR-350)**:
- タイトル「退会申請」
- 説明文「退会するとサービスがご利用いただけなくなります」

オーナー専有機能(FR-333)。メンバーはサイドメニューから到達不可で、URL 直アクセス時は 403。

| 区分 | 項目 | 種類 | 概要 |
|---|---|---|---|
| 表示 | **退会フロー タイムライン** | Timeline | 3 ステップ可視化:(1)申請(今日) → (2){当月末日}: サービス停止 → (3){当月末日 + 30 日}: データ完全削除(復元不可)。現在ステップは `aria-current="step"` で強調 |
| 表示 | 退会案内 | テキスト表示 | 月末締めで停止、データエクスポート猶予 30 日(要件 §14.7.2)。タイムラインの下に補足 |
| 表示 | データエクスポート 強調セクション | パネル(目立つ位置・Primary 並みボタン) | 「退会前にデータをエクスポート(推奨)」見出し + 「退会後 30 日間はエクスポート可能ですが、退会前に行うことを強く推奨します」+ 大きな CTA「📦 すべてのデータをエクスポートする」 |
| 表示 | 課金影響 | パネル | 「当月分の従量課金は通常通り請求されます。次月以降の請求は発生しません」 |
| 表示 | メンバーへの影響 | パネル | 「全メンバーのアクセス権が当月末で失効します。招待中のメンバーは自動的にキャンセルされます」 |
| 入力 | 退会理由(任意) | テキストエリア | 任意。最大 500 文字、文字数カウンタ「0 / 500」 |
| 操作 | キャンセル | ボタン(Secondary) | SCR-016 へ戻る |
| 操作 | 退会を申請する | ボタン(Danger / 確認ダイアログ + 再認証必須 🛡) | `accounts.contract_status=deleted_pending`(オーナー行)、ウィジェット停止 |
...
| 用途 | 文言 |
|---|---|
| タイムライン ステップ 1 | 「① 申請(今日)」 / サブテキスト「当月末まで通常通り利用可能」 |
| タイムライン ステップ 2 | 「② {cutoffDate} 末日: サービス停止」 / サブテキスト「データエクスポート猶予 30 日」 |
| タイムライン ステップ 3 | 「③ {deletionDate}: すべてのデータを完全削除(復元不可)」 |
| データエクスポート CTA | 「📦 すべてのデータをエクスポートする」 |
| 退会理由 placeholder | 「サービス改善のため、よろしければご記入ください…」 |
| 退会確認モーダル(`ConfirmDialogs.withdraw`) タイトル | 「本当に退会しますか?」 |
| 同上 本文 | 「{cutoffDate} にサービスが停止し、{deletionDate} にすべてのデータが完全削除されます。この操作は取り消せません。」 |
| 同上 Primary | 「退会を申請する」(再認証必須 🛡) |
| 完了メッセージ | 「退会申請を受け付けました。{cutoffDate} までは通常通りご利用いただけます」 |

#### 5.4.17 SCR-025 規約再同意割込み

**モーダルヘッダ(参照: FR-350)**:
- タイトル「【重要】利用規約改定のお知らせ」
- 説明文「契約継続のため、改定内容をご確認のうえご同意ください」

**画面構成**: 全画面モーダル割込み(SP / PC ともフルスクリーン)。**3 段 timeline で機能制限の段階的影響を可視化**。

| 区分 | 項目 | 種類 | 概要 |
|---|---|---|---|
| 表示 | 改定内容 | リッチテキスト表示(sanitize 済) | 旧 / 新の差分または要約。長文の場合は折り畳み可 |
| 表示 | 発効日 | テキスト表示(強調) | カレンダーアイコン + 日付 |
| 表示 | 同意期限 | カウントダウン + テキスト表示 | 発効日 + 14 日(要件 §14.7.3)。残時間に応じて緑 → 黄 → 赤 |
| 表示 | **影響タイムライン**(3 段 timeline) | Timeline | 段階的影響を可視化。色 + アイコン + テキストで FR-390 を満たす:<br>① **発効日まで**: 通常通り全機能利用可<br>② **発効日 〜 同意期限**: 同意 / 不同意の選択を促す表示(機能制限なし)<br>③ **同意期限超過後**: 課金画面操作 + 新規プロジェクト作成が不可、FAQ 編集とウィジェット稼働は継続 |
| 入力 | 規約同意 | チェックボックス | 必須。「{利用規約バージョン} に同意します」+ 「全文を読む」リンク(SCR-018 を別タブで開く) |
| 入力 | プライバシーポリシー同意 | チェックボックス | 必須。同様に全文リンク併記 |
| 操作 | 同意して続行する | ボタン(Primary) | 両チェック充足時のみ活性化。同意記録 → 元画面へ復帰 |
| 操作 | 同意しない(退会手続きへ) | リンク(Danger 文字色、Secondary 並みの目立たなさ) | 退会フロー誘導(SCR-024 へ)。確認ダイアログで「同意しない場合は退会手続きへ進みます。退会手続き画面でも変更可能です」 |
| 表示 | 機能制限ガード案内 | アラート | 期限超過時「同意期限({期限日})を過ぎたため、課金画面操作と新規プロジェクト作成は同意するまで制限されます。FAQ 編集とウィジェット稼働は継続します」(要件 §14.7.3) |
| 表示 | 段階的実施案内 | helper テキスト | 「契約単位で発効日が分散されます。他契約の同意状況によって発効日が異なる場合があります」 |
...
|---|---|
| タイトル | 「【重要】利用規約改定のお知らせ」 |
| Timeline ステップ ① | 「① 現在: 通常通り全機能をご利用いただけます」 |
| Timeline ステップ ② | 「② {発効日} 以降: 同意 / 不同意の選択をお願いします」 |
| Timeline ステップ ③ | 「③ {同意期限}以降: 同意するまで一部機能を制限します」 |
| 機能制限内容(③ 詳細) | 「制限される機能: 課金画面操作、新規プロジェクト作成」/ 「継続される機能: FAQ 編集、ウィジェット稼働、お知らせ閲覧」 |
| 規約同意チェック | 「{利用規約 vX.X} に同意します [全文を読む]」 |
| プライバシー同意チェック | 「{プライバシーポリシー vX.X} に同意します [全文を読む]」 |
| Primary ボタン | 「同意して続行する」 |
| 不同意リンク | 「同意しない(退会手続きへ)」(赤文字、リンク程度の目立たなさ) |
| 不同意確認モーダル タイトル | 「退会手続きへ進みますか?」 |
| 同上 本文 | 「同意しない場合、退会手続き画面へ移動します。退会手続き画面でも改めて同意を選択できます」 |
| 同上 Primary | 「退会手続きへ進む」 |
| 同上 Secondary | 「規約画面に戻る」 |
| 期限切れ警告(③ 状態) | 「同意期限({期限日})を過ぎたため、課金画面操作と新規プロジェクト作成は同意するまで制限されます」 |
| 完了トースト | 「利用規約に同意しました。引き続きサービスをご利用いただけます」 |

#### 5.4.18 SCR-027 エンドユーザー再入室画面

**ページヘッダ(参照: FR-350)**:
- 有効状態: タイトル「お問い合わせを再開」/ 説明「以前のお問い合わせ内容を確認・続きを送信できます」
- 期限間近: タイトル「お問い合わせを再開(期限間近)」/ 説明「リンクの有効期限が残り {N} 日です。期限切れ後は新しいリンクをメールで再送できます」
- 期限切れ: タイトル「リンクの有効期限が切れています」/ 説明「メールアドレスを入力して新しいリンクを受け取ってください」
...
| 表示 | トークン有効期限 | テキスト表示 + アイコン | 「あと {N} 日 {H} 時間」+ カラーグラデーション(残 14 日以上: 緑 / 残 3-14 日: 黄 / 残 3 日以内: 赤)。色 + 数値 + アイコンの三重識別(FR-390) |
| 表示 | 期限切れ・無効エラー | アラート(画面上部固定) + 復旧 CTA | 「リンクの有効期限が切れました(または無効です)。下記から新しいリンクをメールで受け取ってください」 |
| 入力 | メールアドレス(期限切れ時のみ) | メールアドレスボックス | 必須。元の問い合わせと同じアドレスで本人確認 |
| 操作 | **新しいリンクをメールで送る**(期限切れ時のみ) | ボタン(Primary、ProgressText 付き) | レート制限 5 分以内 1 回まで。送信中は「メールを送信しています…」、完了時はトースト |
| 操作 | **続きから再開する**(有効時) | ボタン(Primary、大きく目立つ) | トークン検証 → SCR-013 個別チャット部屋へ遷移、`chat_rooms.room_status=open` を確認 |
...
|---|---|
| 有効状態 タイトル | 「お問い合わせを再開」 |
| 有効状態 Primary | 「続きから再開する」 |
| 残時間表示(残 14 日以上) | 「✓ あと {N} 日 {H} 時間 ご利用いただけます」 |
| 残時間表示(残 3-14 日) | 「⏳ あと {N} 日 {H} 時間 ご利用いただけます(期限間近)」 |
| 残時間表示(残 3 日以内) | 「⚠ 残り {N} 日です。お早めにご確認ください」 |
| 期限切れ タイトル | 「リンクの有効期限が切れています」 |
| 期限切れ 本文 | 「以前のお問い合わせを継続するには、新しいリンクをメールで再送できます」 |
| 再送 Primary | 「新しいリンクをメールで送る」 |
| 再送中 ProgressText | 「メールを送信しています…」 |
| 再送完了画面 タイトル | 「メールを送信しました」 |
| 再送完了画面 本文 | 「{メールアドレス} に新しいリンクをメールで送信しました(有効期限 30 日)。メールに記載のリンクから再開してください」 |
| 再送上限超過 | 「メール再送の上限に達しました。{N} 分後にもう一度お試しください」 |

...
export const SuccessMessages = {
  ...
  projectCreated: 'プロジェクトを作成しました。ウィジェット設定を確認してください',
  projectUpdated: '変更を保存しました',
  projectDeleted: 'プロジェクトを削除しました',
  contactEmailVerificationSent: '連絡先メールに確認メールを送信しました。リンクをクリックして確認を完了してください',
  contactEmailVerified: '連絡先メールを確認しました',
...
画面項目（表示・入力・操作・主要制約）はメイン基本設計 §5.4.3 SCR-003 を正本とする。本節では、当該画面の実装に関する Zod スキーマ・正規表現の実装値・エラーコード・呼出 API・ハンドラ実装方針および UX 補強(ステップタイムライン・強度メーター・再送 CTA)を記載する。3 ステップ構成（メール入力 → 受信メールクリック → 新パスワード設定）の実装フローを以下に示す。
...
送信後はクライアント側で **段階 1 送信済み中間表示** に遷移し、再送残時間カウントダウンを開始(レート制限 5 分)。
...
function PasswordStrengthMeter({ password }: { password: string }) {
  const result = evaluatePasswordStrength(password); // { score: 0-4, missingTypes: ['upper','lower','digit','symbol'], lengthShort: 5 }
  const labels = ['極弱', '弱', '中', '強', '極強'];
  return (
    <div className="strength-meter" aria-live="polite">
      <div className={`bar level-${result.score}`} aria-hidden="true" />
      <span>{labels[result.score]}</span>
      {(result.missingTypes.length > 0 || result.lengthShort > 0) && (
        <ul className="hints">
          {result.lengthShort > 0 && <li>あと {result.lengthShort} 文字必要です</li>}
          {result.missingTypes.length > 0 && <li>あと {result.missingTypes.length} 種類の文字種が必要です(英大文字 / 英小文字 / 数字 / 記号)</li>}
        </ul>
      )}
    </div>
  );
...
スコアは zxcvbn ベース、ただし FR-006 強度要件(12 文字以上 + 3 種類以上)に達するまで「強」以上の表示を許容しない。

#### 6.6.4 ステップタイムライン

```tsx
const PASSWORD_RESET_STEPS = (currentStep: 1 | 2 | 3): Step[] => [
  { id: 's1', label: 'メールアドレスを入力',     state: currentStep > 1 ? 'done' : 'current' },
  { id: 's2', label: '受信メールのリンクをクリック', state: currentStep === 2 ? 'current' : currentStep > 2 ? 'done' : 'future' },
  { id: 's3', label: '新しいパスワードを設定',   state: currentStep === 3 ? 'current' : 'future' },
];
```

段階 1 入力時は `currentStep=1`、送信後の中間表示は `currentStep=2`、段階 2 入力時は `currentStep=3` を渡す。

#### 6.6.5 エラー表示

| エラーコード | 表示文言 |
|---|---|
| `TOKEN_INVALID` | 「再設定リンクが無効です。新しいリンクを再送してください」+ 「再送する」CTA(`/auth/forgot` へ) |
| `TOKEN_EXPIRED` | 「再設定リンクの有効期限が切れています(有効期限 1 時間)。新しいリンクを再送してください」+ 「再送する」CTA |
| `TOKEN_REUSED` | 「このリンクは既に使用されています。新しいリンクを再送してください」+ 「再送する」CTA |

エラー帯は画面上部に固定し、`<Alert variant="danger" role="alert">` で再送 CTA を直結配置(FR-353)。

#### 6.6.6 関連参照

> FR-004 / FR-006 / FR-350 / FR-353 / NFR-309
...
- 「+ 新規プロジェクトを作成」/「編集」のクリックは SCR-010-M1 を新規作成 / 編集モードで開く（クライアント側ルーティング)。サーバ呼出は SCR-010-M1 の保存時に行う。
- 「削除する」のクリックは `ConfirmDialogs.deleteProject`（§6.2.8.3）+ 再認証ののち、`DELETE /api/v1/projects/{id}` を呼ぶ。プロジェクト削除は配下の FAQ・チャットを連鎖削除する高影響操作のため再認証必須（FR-005 重要操作の扱いとして本書で明示）。

#### 6.7.3 レイアウト方針(PC / SP)

- **PC(≥ 1024px)**: テーブル形式（ID / 名前 / 許可ドメイン / 連絡先メール / 更新日時 / 操作)。許可ドメインは行内で最大 3 件まで表示し、4 件目以降は `<details>` で折り畳む。
- **SP(< 768px)**: カード形式に切替。プロジェクト名を見出しとし、許可ドメインはサマリ表示、連絡先メール状態をバッジで強調。

#### 6.7.4 関連参照

> FR-030, FR-032, FR-033, FR-034, FR-035 / FR-350 / FR-351 / FR-354 / FR-358 / FR-371 / FR-390 / BR-002 / AC-009
...
const FormMeta = {
  create: {
    title: '新規プロジェクトを作成',
    description: 'ウィジェットを埋め込むサイトのドメインと連絡先メールを設定します',
    primaryLabel: 'プロジェクトを作成',
    successToast: SuccessMessages.projectCreated,
  },
  edit: {
    title: (name: string) => `プロジェクト設定 — ${name}`,
    description: 'プロジェクトの設定を変更します',
    primaryLabel: '変更を保存',
    successToast: SuccessMessages.projectUpdated,
  },
} as const;
```

`SuccessMessages.projectCreated` / `projectUpdated` は §6.2.8.5 で追加(下記)。

#### 6.7a.4 連絡先メール再送 UI

```tsx
function ContactEmailResendButton({ project, onResend }: { project: Project; onResend: () => Promise<void> }) {
  const cooldown = useResendCooldown(project.contactEmailResendAvailableAt); // 60 秒
  const disabled = cooldown.remainingMs > 0;
  return (
    <button type="button" disabled={disabled} onClick={onResend}>
      {disabled
        ? `確認メールを再送する(${Math.ceil(cooldown.remainingMs / 1000)} 秒後から可能)`
        : '確認メールを再送する'}
    </button>
  );
}
...
レート制限超過(1 時間 5 回)時はサーバが 429 を返し、UI は「再送の上限に達しました。1 時間後にもう一度お試しください」を表示。

#### 6.7a.5 関連参照

> FR-030, FR-031, FR-033, FR-033a, FR-033c / FR-350 / FR-353 / FR-373 / BR-002 / AC-009
...
- 「公開キーを再発行する」操作は再認証必須（FR-005）。新キー発行と同時に旧キーは 30 日猶予で失効予告状態へ（FR-193 / NFR-322）。
- 確認モーダルは独自仕様で、本文に「旧キーは 30 日間有効です。サイト側で新キーへの差し替えを早めに行ってください」を必須表示。
- API: `POST /api/v1/projects/{id}/widget-key/rotate`

#### 6.14.3 公開キー有効期限の 1 クリック延長

```ts
// POST /api/v1/projects/{id}/widget-key/extend
// body: { extendDays: 365 }
// サーバ処理:
// 1. 現在の publicKeyExpiresAt を取得
// 2. publicKeyExpiresAt + extendDays(最大 365 日まで)を新有効期限として更新
// 3. キー文字列自体は変更しない(ローテーションとは別操作)
// 4. 監査ログ `widget.public_key.extend` を記録
```

クライアント側は `<button>` 1 つで操作可能(再認証不要、ボタン操作のみ)。延長後の新有効期限を即時表示し、トーストで「公開キーの有効期限を延長しました(新有効期限: {YYYY-MM-DD})」を表示。

#### 6.14.4 プレビュー実装(デバイス切替)

```tsx
type PreviewDevice = 'pc' | 'tablet' | 'sp';
const DEVICE_WIDTHS: Record<PreviewDevice, number> = { pc: 1200, tablet: 768, sp: 360 };

function WidgetPreview({ settings, device }: { settings: WidgetSettings; device: PreviewDevice }) {
  return (
    <div className="widget-preview" style={{ width: DEVICE_WIDTHS[device] }}>
      <iframe
        srcDoc={generatePreviewHtml(settings)}
        title={`${device.toUpperCase()} 表示プレビュー`}
        aria-label={`${device.toUpperCase()} サイズでのウィジェットプレビュー`}
      />
    </div>
  );
}
...
プレビューは右ペイン 50%(PC)、SP では `<details>` で折り畳み。
...
成功時は緑チェック + トーストの 2 重フィードバック(視覚 + 通知)で確実性を高める。
...
- `POST /api/v1/projects/{id}/widget-key/extend`(有効期限の 1 クリック延長)

#### 6.14.7 関連参照

> FR-150〜156 / FR-193 / FR-194 / FR-350 / FR-390 / BR-002 / AC-015
...
- `GET /api/v1/me/current-ip`(現在の IP を返す。「現在の IP を追加する」ボタンと自己ロックアウト警告用)
- `PATCH /api/v1/me/ip-allowlist`(再認証必須)
- `POST /api/v1/data/export`
- `GET /api/v1/data/export/{jobId}`

#### 6.16.3 「現在の IP を追加する」ボタン実装

```tsx
function AddCurrentIpButton({ onAdd }: { onAdd: (cidr: string) => void }) {
  const { data: currentIp } = useSWR<{ ipv4?: string; ipv6?: string }>('/api/v1/me/current-ip');
  if (!currentIp) return null;
  const cidr = currentIp.ipv6 ? `${currentIp.ipv6}/128` : `${currentIp.ipv4}/32`;
  return (
    <button type="button" onClick={() => onAdd(cidr)}>
      現在の IP を追加する({cidr})
    </button>
  );
}
...
クライアントの IP は `cf-connecting-ip` ヘッダから取得し、IPv4 なら `/32`、IPv6 なら `/128` のホスト単位 CIDR でリストに追加。CIDR 知識不要で導入可能。

#### 6.16.4 自己ロックアウト警告

IP 許可リスト変更時、`currentIp` が新しいリストに含まれていない場合は保存前に警告:

```tsx
function checkSelfLockout(entries: { cidr: string }[], currentIp: string): boolean {
  return entries.some(e => cidrContains(e.cidr, currentIp));
}

// 入力中: 含まれない場合 <Alert variant="warning"> で常時表示
// 保存時: ConfirmDialog で「現在の IP がリストに含まれていません。保存しますか?」
```

確認モーダル本文に `{currentIp}` を差し込み、ユーザーが今のアクセス元を確認したうえで判断できるようにする(FR-454)。

#### 6.16.5 関連参照

> FR-009 / FR-125 / FR-162 / FR-167 / FR-179 / FR-330 / FR-350 / FR-353 / FR-354 / AC-016
...
- `GET /api/v1/me/announcements?category=...&unread_only=...&quickFilter=...&cursor=...`
- `GET /api/v1/me/announcements/counts`(クイックフィルタ各チップの件数、5 秒 TTL)
- `POST /api/v1/me/announcements/bulk-read`(選択した複数件の既読化、最大 100 件、`{ ids: string[] }`)
- `POST /api/v1/me/announcements/read-filtered`(現フィルタの未読を既読化、最大 100 件、§7.12.4 / FR-323、監査ログ `inbox.read.bulk` を記録)
- `POST /api/v1/me/announcements/read-all`(全未読を既読化、ユーザーの明示確認後のみ、最大 100 件単位で分割実行)

#### 6.19.2 クイックフィルタ実装

```ts
type QuickFilter = 'unread' | 'important' | 'billing' | 'announcement' | 'system' | 'all';

function quickFilterToQuery(qf: QuickFilter): Partial<AnnouncementQuery> {
  switch (qf) {
    case 'unread':       return { unread_only: true };
    case 'important':    return { severity: ['critical', 'high'] };
...
デフォルトは `unread`(URL クエリで状態保持)。空状態の文言もフィルタ状態に応じて切り替える(`EmptyStates.announcements` を `unread` バリアント込みで拡張)。

#### 6.19.3 未読行強調の CSS

```css
.announcement-row[data-unread="true"] {
  background-color: rgba(54, 102, 204, 0.05); /* 薄い水色 */
  font-weight: 600;
}
.announcement-row[data-unread="true"] .title {
  font-weight: 700;
}
```

色のみに依存しないよう、`<span class="unread-dot" aria-label="未読">●</span>` を行頭に併記(FR-390)。

#### 6.19.4 関連参照

> FR-180 / FR-181 / FR-183 / FR-323 / FR-350 / FR-351 / FR-358 / FR-371 / FR-372 / FR-390 / BR-023 / BR-026
...
- `GET /api/v1/me/email-verification/{token}`(リンクから検証)
- `POST /api/v1/me/email-verification/resend`(再送、レート制限 5 分)

#### 6.21.2 画面状態の分岐

クライアントは URL から到達した時点で次の 3 状態のいずれかをレンダリングする:

```tsx
type VerifyState =
  | { kind: 'pending'; email: string; resendAvailableAt: Date }     // 送信済み
  | { kind: 'success'; email: string }                                // 確認完了
  | { kind: 'error'; reason: 'expired' | 'used' | 'invalid' };       // 失敗
```

- `pending`: `/verify-email` パスへ直接アクセス時(トークンなし)、または `?status=sent` クエリ付き
- `success`: `GET /api/v1/me/email-verification/{token}` が 200 を返却
- `error`: 同 API が 410(`TOKEN_EXPIRED` / `TOKEN_USED`) または 404(`TOKEN_INVALID`)を返却

#### 6.21.3 再送残時間カウントダウン

```tsx
function ResendEmailButton({ availableAt, onResend }: { availableAt: Date; onResend: () => Promise<void> }) {
  const remaining = useRemainingSeconds(availableAt);
  const disabled = remaining > 0;
  return (
    <button type="button" disabled={disabled} onClick={onResend}>
      {disabled
        ? `メールを再送する(あと ${Math.floor(remaining / 60)} 分 ${remaining % 60} 秒)`
        : 'メールを再送する'}
    </button>
  );
}
...
レート制限超過は 429 で `RESEND_RATE_LIMITED` を返却し、UI は `ErrorMessages.rateLimited` を表示。

#### 6.21.4 エラー表示と復旧導線

| 状態 | 文言 | 復旧 CTA |
|---|---|---|
| `TOKEN_EXPIRED` | 「確認リンクが期限切れです(有効期限 24 時間)。新規登録からやり直してください」 | 「新規登録からやり直す」(SCR-002 へ) |
| `TOKEN_USED` | 「この確認リンクは既に使用されています。すでに登録が完了している可能性があります」 | 「ログインする」(SCR-001 へ) |
| `TOKEN_INVALID` | 「確認リンクが無効です。新規登録からやり直してください」 | 「新規登録からやり直す」(SCR-002 へ) |

エラー画面は `<Alert variant="danger" role="alert">` で原因 + 復旧 CTA を直結配置(FR-353)。

#### 6.21.5 関連参照

> FR-003 / FR-350 / FR-351 / FR-353 / FR-373
...
API: `POST /api/v1/me/terms/agree { termsVersion, privacyPolicyVersion }`

両バージョンを同時に同意することで、規約とプライバシーポリシーの整合性を担保。同意後はモーダルを閉じて元の操作を再開。同意期限(発効日 + 14 日)超過時は機能制限ガードを発動し、課金画面操作と新規プロジェクト作成を停止する(要件 §14.7.3)。

#### 6.23.3 影響タイムライン実装

`<Timeline />`(§6.2.7.5)で 3 段階を可視化:

```tsx
function ConsentImpactTimeline({ effectiveAt, deadlineAt, now }: {
  effectiveAt: Date; deadlineAt: Date; now: Date;
}) {
  const currentStep =
    now < effectiveAt   ? 1 :
    now < deadlineAt    ? 2 :
                          3;
  const steps: Step[] = [
    { id: 'pre',   label: `① 現在: 通常通り全機能をご利用いただけます`,                          date: formatDate(now),         state: currentStep === 1 ? 'current' : 'done' },
    { id: 'grace', label: `② ${formatDate(effectiveAt)} 以降: 同意 / 不同意の選択をお願いします`, date: formatDate(effectiveAt), state: currentStep === 2 ? 'current' : currentStep > 2 ? 'done' : 'future' },
    { id: 'after', label: `③ ${formatDate(deadlineAt)} 以降: 同意するまで一部機能を制限します`,    date: formatDate(deadlineAt),  state: currentStep === 3 ? 'current' : 'future' },
  ];
...
同意期限超過後、以下のミドルウェアで制限を発動:

```ts
function requireConsentForRestrictedOps(c: Context, next: () => Promise<Response>) {
  const restrictedOps = ['POST /api/v1/projects', 'PATCH /api/v1/billing', 'POST /api/v1/billing/payment-method'];
  if (!restrictedOps.includes(`${c.req.method} ${c.req.path}`)) return next();
  const consent = c.get('consent');
  if (consent.requiresReAgreement && consent.deadlinePassed) {
    return c.json({ error: 'CONSENT_REQUIRED', message: '同意期限を過ぎたため、この操作は実行できません。先に利用規約への同意を完了してください' }, 403);
  }
  return next();
}
```

UI 側は SCR-015 / SCR-010 で 403 受信時に「先に利用規約に同意してください」と SCR-025 へ誘導するモーダルを表示。

#### 6.23.5 不同意フロー

「同意しない」リンク押下 → 確認モーダル(本文「同意しない場合、退会手続き画面へ移動します。退会手続き画面でも改めて同意を選択できます」)→ SCR-024 へ遷移。

#### 6.23.6 関連参照

> FR-011 / FR-164 / FR-350 / FR-354 / FR-390 / AC-027

### 6.25 SCR-027 お問い合わせ再開画面

画面項目（表示・入力・操作・主要制約）はメイン基本設計 §5.4.18 SCR-027 を正本とする。本節では、当該画面の実装に関する Zod スキーマ・正規表現の実装値・エラーコード・呼出 API・ハンドラ実装方針のみを記載する。
...
- `POST /widget/v1/reentry { token }`(トークン検証 + 再入室)
- `POST /widget/v1/reentry/resend { email }`(期限切れ時のメール再送、レート制限 5 分)
...
```tsx
type RemainingState = 'safe' | 'warning' | 'urgent' | 'expired';
function remainingState(expiresAt: Date, now: Date): RemainingState {
...
```tsx
const REMAINING_MESSAGES: Record<RemainingState, (n: number, h: number) => string> = {
  safe:    (n, h) => `✓ あと ${n} 日 ${h} 時間 ご利用いただけます`,
  warning: (n, h) => `⏳ あと ${n} 日 ${h} 時間 ご利用いただけます(期限間近)`,
  urgent:  (n, _) => `⚠ 残り ${n} 日です。お早めにご確認ください`,
...
```tsx
function ResendForm({ inquiryHint, onResend }: { inquiryHint?: string; onResend: (email: string) => Promise<void> }) {
  ...
  return (
    <form onSubmit={async (e) => {
      e.preventDefault();
      setSubmitting(true);
      try { await onResend(email); } finally { setSubmitting(false); }
    }}>
      {inquiryHint && <p>以前のお問い合わせ時に使用したメールアドレスを入力してください(参考: {inquiryHint})</p>}
      <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="例: example@your-domain.com" />
      <button type="submit" disabled={submitting}>
        {submitting ? 'メールを送信しています…' : '新しいリンクをメールで送る'}
      </button>
    </form>
  );
}
...
サーバ側ガード: 入力メールアドレスと `inquiries.email_hash` が一致する場合のみ再送(列挙攻撃対策で、不一致でも 202 を返す)。

#### 6.25.4 関連参照

> FR-083 / FR-084 / FR-174 / FR-350 / FR-353 / FR-390 / AC-011 / AC-024
